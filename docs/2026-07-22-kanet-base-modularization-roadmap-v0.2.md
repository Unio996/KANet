# KANet 底座模块化路线图 v0.3（钉死候选稿 · Owner 磨合过目五条全落实）

> **Status**: CURRENT（2026-07-22 · Bettor 主编 · 整合 KANet-UI/NWT/J1/J2 四方对抗意见，频道裁决 #v6ij51 在案；v0.2.1 增量：D2 按 35 命令分类表实证结果由双轨升三轨，四方全数确认；v0.3 增量：Owner 磨合过目裁决五条全落实——两处 stale 文本清除、三钉(b)执行主语钉死、drain 超期终态化兜底、类 C 存量补审批次组挂账、类 B 权限摸底即刻启动令）
...
- 与公测运营/ZK 主线火力配比：**批次总量量化**——Owner 磨合粗算 M0(1)+M1(3)+M2(4-5)+M3a(3-5)+M3b(1-2)+M3c(1)+M4(2-3)+M5(1) ≈ 13-18 批；加上 Owner 同轮点出的 D2-C 存量补审（2-4 批）后 Bettor 修正总量为 **15-22 批 ≈ 8-22 个红队日**，与 ZK 主线和公测运营共享同一 NWT 瓶颈。默认配比（Owner 已认可）：ZK(J2) 与 M3a(J1) 并行不抢占；M0-M2 走非钱路带宽。裁决依据是这个总量，不是"六个里程碑"的印象。

## 5. 待办与交 Owner 事项

| 项 | 状态 |
|---|---|
| 35（原估 34）命令分类表（J2 主核 + KANet-UI 复核） | ✅ 完成（A6+B9+C20=35，四方确认，已入 D2 三轨定稿） |
| 类 B 现状调用权限摸底（9 条盲签命令的调用点/权限链取证） | **即刻启动（Owner 令：只读取证不违钉死令；这 9 条是现在活着的风险）**——J2 主查 + NWT 出"什么算显式权限控制"判据并复核 |
| 类 B caller 白名单方案设计 | D2 设计稿硬前置，按稿内节奏（钉死后） |
