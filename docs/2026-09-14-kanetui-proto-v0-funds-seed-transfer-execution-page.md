...
4. **执行（v0.5：改 3 笔精确面值，非 4 笔各 0.5）**：relay 签名输入面值上限 1.0 KAS；J2 真实编译+mass 精算三个 kind 的最小面值后（账本 1402），依次转 3 笔，**每笔都等上一笔 landed 再发下一笔**（NO-TX-NO-STATE，避免同 relay 并发 pending）：
   `POST /api/relay/27e2b2ac-8c8e-4a3d-874e-c57ebd239757/transfer`，header `X-KANet-Admin-Secret: <ADMIN_SECRET_FUNDS值>`，body `{"to":"kaspa:qpq956nh8ud2z85mg0elg6ju3pufukknx2f7vnqpwjjsznzvwj4a2mtqc2anq","amount":"<面值>"}`，依次：
...
5. **验收**：目标地址链上应见 **3 个独立 UTXO**（0.5 / 0.5 / 0.95，非合并）；源余额减 1.95 KAS + 3 笔手续费；三个 `txId` 均 landed（Bettor 独立链上核）；结果落 COORD-LEDGER。
