...
- **证据核（2026-09-05T11:18Z）**：跑 J2 `kasia-console/scratch/_j2_p2_explain_schema_only.mjs`——P2-1 A′ / P2-3 / P2-4 / P2-5 计划与设计一致；🔴 **P2-2 AFTER 计划 = BEFORE**（无 `MATERIALIZE m`），根因脚本 :62 `WITH m AS (` 漏 `MATERIALIZED` ⇒ CTE 被展平；我同 DDL 补关键字实测 = 设计计划（MATERIALIZE m → deadline 索引 → SCAN m → idx_pool_sides_market 点查），但 **`ORDER BY stake_amount` 的 TEMP B-TREE 仍在**（排过滤后的 side 集）——设计文须补这句；脚本修后重跑再进 C 包 Owner 材料。P2-5：pool_bettor_sides 无 bettor_pk 前导索引 ⇒ 新索引不冗余；查询对形，AFTER 无 TEMP B-TREE。
- **A 包审 + C3 复核（2026-09-05T11:23Z）**：`scratch/_j2_p2A_patch_2026-09-05T11-20Z.diff`（sha `a433fb3f…3588`，4 文件 +152/−2）**GREEN-conditional**——单源 `ZK_READY_EXPR`/DDL/字面量查询、`_filterZkCandidateRows` 一字不动、v200 两索引 boot 内建记账、V3 EXPLAIN 断言/V4 无 TEMP B-TREE ✓；🔴 必改：`PHASE2_SHADOW_EVERY` 默认 10 ⇒ 三 ZK tick 合计约每 100 s 跑一次旧 LIKE 全扫（18 s 同步）= 修后窗自污染 ⇒ 默认 0、影子窗内 ≥100 + 向量"默认不调 legacy"。C3 我 cp 副本 readonly 复核：事件 124/124 合法、distinct 116、**大小写不一致 0**、新 JOIN = 旧 LIKE（我 95=95 用 claim-auto 完整谓词；J2 145=145 谓词集不同，已要求钉 §4c SQL 原文）。P2-2 脚本补 `MATERIALIZED` 重跑 = 设计计划，证据闭合。
- **C3 口径闭合（2026-09-05T11:24Z）**：145 = 命中 (market_id, bettor_pk) 对的全部 side（不带 `claim_txid IS NULL`，含 50 个已 claim）；95 = claim-auto 完整谓词的真实候选集（去 redeem 过滤或去 markets JOIN 仍 95）。两口径 old = new 均成立；§4c 应两数并列标谓词。
- **A v2 + B 审（2026-09-05T11:33Z）：两包 GREEN**。A v2 `scratch/_j2_p2A_v2_patch_2026-09-05T11-29Z.diff`（sha `d17c79cd…e62b38`）= v1 + `resolveShadowEvery`/`shadowDue`（默认 0 关、≤0 永假）+ V6 向量，其余一字不动。B `scratch/_j2_p2B_patch_2026-09-05T11-29Z.diff`（sha `ebd49f0f…5828b`）：谓词 `metadata IS NULL OR metadata='' OR (json_valid AND COALESCE(json_extract('$.zk_continuation'),0) IN (0,''))`——我 17 类 metadata 形差分（NULL/''/空白/坏 JSON/无键/null/false/0/''/true/对象/空对象/数组/字符串/顶层字符串/顶层数组/1）与旧 JS 候选集逐类相同、零不一致；`CROSS JOIN` 钉 `SCAN ps → SEARCH pm(id=?)` 复现；新路径保留 `readPayoutShardV2AttestedState` try/catch（上下文行 :37）；`marketMetaById` 只对候选补取；影子同默认 0。生效随 console 自然重启（v200 boot 内建 ≤3 s）。
- **落地核（2026-09-05T11:37Z）**：A v2 = **06f486f3**（= d17c79cd 行集合，4 文件 +175/−2）、B = **f5a78cd3**（= ebd49f0f 行集合，3 文件 +135/−7）、设计 v0.2.3 = 31162ce1；GREEN-final 交 Bettor 推；生效随 console 自然重启（v200 建索引 ≤3 s）。**D 脚本** `kasia-console/scratch/_j2_p2D_analyze_small_tables.mjs` GREEN-conditional：范围对（5 具名小表、无 kaspa_tx_log、无 optimize、默认 readonly dry-run、前后 EXPLAIN JSON 标 CHANGED、--rollback DROP sqlite_stat1）；条件 = `busy_timeout=10000` + 验收看 console 自己重启后的 sql.* 行（缓存 statement 保留旧计划）+ CHANGED 变差即回滚。
- **D 脚本终审（2026-09-05T11:38Z）：GREEN**——`busy_timeout=10000`（:32）、缓存计划头注 + --apply 末提示、WORSE = after 相对 before `^SCAN `/`TEMP B-TREE` 行数增加（:50）⇒ rc=2 提示 --rollback、JSON sha256 打印；活库 dry-run + 临时库 apply/rollback 路径 J2 已跑，WORSE 分支纯函数读码即可。等 Bettor 低负荷窗；真验收 = 下次 console 自然重启后 sql.* 行。
- **D 执行异常归因（2026-09-05T11:42Z · ledger 903）**：Bettor 11:38Z --apply 5 表 ANALYZE 3.2 s → WORSE ×2（P2-2 `SCAN pm`、P2-4 `SCAN s`）→ --rollback → 新连接 dry-run P2-2 ≠ 最初基线。**根因 = 回滚不完整**：better-sqlite3 编译 `ENABLE_STAT4`（compile_options 实读），`ANALYZE` 同时写 `sqlite_stat1` + `sqlite_stat4`；--rollback 只 DROP stat1，活库 **stat4 仍 402 行（五表）**，规划器不依赖 stat1 照读 stat4 ⇒ P2-2 走 `idx_pool_markets_deadline` 市场先行。内存库复现：ANALYZE→两表，DROP stat1→stat4 留。**live 影响**：DROP 改 schema cookie ⇒ console 5392 新 prepare 已在 stat4 下规划（非等重启）。建议 Bettor 立即 `DROP TABLE IF EXISTS sqlite_stat4`（+stat2/3）恢复基线并新连接核。裁定同意 **D 不采**（WORSE 在 stat1+stat4 下、stat4-only 又一套形，统计态不稳且未审）；P2-2/P2-4 真修 = C 包确定性改写。D 脚本须改：rollback 删 stat1–4、三模式列四表状态。
- **D 事件闭合（2026-09-05T11:43Z）**：Bettor 11:42:16Z `DROP TABLE IF EXISTS sqlite_stat4/3/2/1`（f0988440）；我 11:43:08Z readonly 复核：`sqlite_stat*` 全无，新连接 dry-run 7 条 EXPLAIN 与 11:38:39Z 基线 JSON **7/7 逐条相同**（回滚后证据 `scratch/_j2_p2D_explain_dryrun_2026-09-05T11-43-09-125Z.json`）。D 不采；C 包照旧。
- **C 包审（2026-09-05T13:24Z · Owner C GO ledger 905）：C1 GREEN、C2 GREEN**。C1 `scratch/_j2_p2C1_p22_patch_2026-09-05T13-20Z.diff`（sha `21754a41…70ea7`，5 文件 +292/−2）：settler 主路 SQL 体一字未动（只动 prepare 包装两行）；共用 `phase2-shadow.mjs`（集合差、events 一行、双层 try/catch、不回流）；影子 legacy 去 LIMIT vs MATERIALIZED 新查询无 LIMIT；空 unfixable `''` 占位与 live :460 同；**§1a** `refund_attempted_at < ?`（绑 `datetime('now','-1 hour')` TEXT）我内存库证：列 `TIMESTAMP`（NUMERIC 亲和），空格形/T 形/整数 epoch 三种存值下与内联 `datetime()` 逐行同判，`julianday` 对整数得 NULL ⇒ 翻转（J2 避开正确）；副产物：T 形值永远过不了冷却（'T'>' '），活库 0 行。C2 `scratch/_j2_p2C2_p24_patch_2026-09-05T13-14Z.diff`（sha `ffba89e4…328eb`，3 文件 +145/−15）：旧 SQL 13 行逐行搬入模块、主路照旧、`runLegacy` 复用主路结果、反转驱动只影子、两条已知语义差各造一行断言 onlyLegacy 形。默认 `PHASE2_SHADOW_EVERY=0`；影子窗 ≥100，一周零 mismatch 后主路切换另审。备注：两条语句 cutoff 相隔毫秒，秒界行可差一行、LOUD 即自愈。
- **C 包落地核（2026-09-05T13:26Z）**：C1 = **8c7d41f1**（= 21754a41 行集合）、C2 = **50ac7f25**（= ffba89e4 行集合）、设计 v0.2.4 = f26ed46d；测试留证 4/0、8/0、6/0；GREEN-final 交 Bettor 推。影子默认关；影子窗 ≥100，一周 `phase2_shadow_mismatch` 事件 = 0 后主路切换另审。
- **E 笔 + 13:56Z 重启核（2026-09-05T14:00Z）**：E（启动打一行 `[phase2-shadow] PHASE2_SHADOW_EVERY=N`，`scratch/_j2_p2E_announce_patch_2026-09-05T13-58Z.diff` sha `f3b11769…df234`，5 文件 +25/−3）GREEN——模块级一次守卫、同一 resolver、三消费方 import 后各调一次、纯日志；落地 **a17128c2** = 补丁行集合，GREEN-final。计划内重启 13:56Z（console 19184，kanet.env `PHASE2_SHADOW_EVERY=100`）我亲核：v199 记账通过、**v200 两索引 38 ms / 42 ms**、migrations complete、heartbeat 行在、启动段 sql.* 4 行（pair boot 6.5 s 一次性 / migrate 1.8 s、0.6 s / fee-emit 1.1 s）、broker-intake 0、③ skip 18。影子首条正面证据 = ③ 放行后 `[phase2-shadow] … equal` 行；一周 `phase2_shadow_mismatch` = 0 后主路切换另审。

## §14 ③ 放行后首个有信息量窗（seg1 2026-09-06T20:08:19→20:25:12Z · 17 min · NWT 独立算 vs J2 页交叉核）

- **数据源**：`logs/console.log.prev-20260906T202535Z` 行 344779（首个 `resume: node synced`）→ EOF（最后 diag `at=20:25:12.900Z`）。门状态：全段开，仅 zk.* 四站 20:10:2x→20:10:49Z 翻动一次（kaspad 第 7 轮头部相位）。
- **lag 事件（gap 口径）**：n=13，gap ≥4 s **6**（lag 口径 5），≥10 s **4**（14671@20:09:48.7 / 13423@20:11:43.5 / 13996@20:15:44.4 / 14006@20:20:57.3），max 14.7 s，Σgap 84.5 s（J2 Σlag≥4 s 56 s）。修前 09-05 窗：≥4 s 119 / 1,663 s（46%）。
- **sql.* ≥200 ms 按 src（7 行 Σ10.7 s）**：`phase2-claim-queries.mjs:46` 3×1,966（rows 95）· `preprune-capture-worker.mjs:68` 1×3,459 · `pool-market-settler.js:510` 2×750 · `phase2-handoff-candidates.mjs:21` 1×429。**归零且有阳性对照**：`zk-autonomy-ticks.mjs:45/:239`（三 ZK tick 各 33 次）、`api/pool.js:3300`、`broker-intake-watcher.js:717`、`pair-ingestor.mjs:63` ⇒ Phase-1 ①② + Phase-2 A/B/P2-5 在门开 17 min 内成立。C（P2-2/P2-4）按设计仍主路旧查询，量级同修前。
- **wrapped ≥3 s**：`preprune-capture-worker.tick` 73,217 / 184,442 / 181,380 ms；`zk.judgeProposeAutonomousTick` 842,869 ms（异步含 await，不归因）；pool/settle/prediction-voter/bshard-voter 各一次 3.3–4.8 s（放行首 tick）。
- **④ 对齐率**：四次 ≥10 s 停顿 ±0.5 s 内无任何 sql.* 或同步站起点 ⇒ 0/4；只能按 **containment** 判：全部落在 preprune worker 三次 tick 内（20:09:12–20:10:26 / 20:11:13–20:14:17 / 20:14:17–20:17:18）。停顿期间日志静默 ⇒ 未包同步段。处置 = Bettor 943（env 开关）→ P2-6 6a/6b（e5578a23）；因果"关 worker ⇒ 停顿消失"**本窗证不了**（seg2 门关）。
- **用户面**：`Console unreachable (5 failures)` 32 行全在 20:11（J2 计 2,938 = 含 1–4 failures 全部 ingest 失败行）；`http.slow send-command` 303/187/89/74/73 s（status 200，type 未核，可能是 400 s walk 路径）。
...
- **§14 勘误（2026-09-06T21:44Z）**："用户面" 一条把 20:11 的 relay `Console unreachable` 当作停顿的证据——**撤**。21:40:18Z 同样一波（15 条 5-failures + 122 条 `aborted due to timeout` + 同秒 `Console recovered`）发生时 console **无任何 lag 行、无 ≥1 s 步、无重启**（pid 40064 自 20:41:14Z 未变）；两次都紧随 relay 的 `[rpc] block #N` 里程碑（IBD 体相位 block-added 以 ~30 blk/s 推给 ~40 个 relay 子进程，各自 POST ingest ⇒ 扇入超时）。⇒ relay 失败风暴 = **IBD 期 relay 订阅扇入的独立签名**，与 console 停顿正交（20:11 两者同时发生；21:40 只有前者）。同时 kaspad RPC 在体相位变慢（`utxo-fetch-probe` 1 ms→1,189 ms；worker 门读 469→1,256 ms）。只报观测；ingest 幂等/重试路径未核。
