...
## 6. 09-05 01:xxZ 追加：重启 vs 断连重试的边际成本实测（S2 依据）+ D-a 上界算术
| 事件 | 起 | `syncing ahead` | headers 100% | 缺体扫描 | 首块体 | 总 |
|---|---|---|---|---|---|---|
| 08-28 进程重启 | 13:18:38 | 13:19:57（+79 s） | 15:16:08（1.18 M 头·1 h 56 m） | 15:16:10 → 15:24:30（8 min） | 15:24:30 | 2 h 06 m |
| 09-04 进程内重试 | 03:26:33 error | 03:28:24（`IBD started` +38 s） | 06:24:07（1.60 M 头·2 h 56 m） | 8 min | 06:32:10 | 3 h 06 m |
- ⇒ 重启只比断连重试多 **~80 s 启动**（含 utxoindex resync）；header 相位与 8 min 缺体扫描两者都赔。**"与下一次自然断连合并"成立，动手窗 = 断连后整个 header 相位（2–3 h），不是 60 s**；需要的是预授权 + 预置脚本，不是有人守着。
...
### 11a. 勘误（NWT · 2026-09-04T21:38Z）：§11 第 5 条"每块 ~1000 次读的串行链"撤回；③ 的两态解释成立；裁决改为"块率由对端请求延迟锁定"
- **撤回依据（我自己的数据）**：§11 逐秒 40 s 与 21:36:53Z 起再采 25 s，IO Read 每秒 10–15k **平坦、与到货团无关**；IO Write 只在每团处理那 1 s 尖到 800–1400/s，其余秒 0–2。块处理在到货秒内完成、读却整秒平坦 ⇒ 平坦读不是块处理，第 5 条推论不成立。
- **Bettor 归因（我接受，标"零态未亲见"）**：getMetrics 100 ms 轮询（`scratch/_bettor_p2p_bytes_timeline.mjs`、`_bettor_borsh_vs_bodies.mjs`）：每团 78–91 块对应 27–43 MB borsh 出流（≈32 个 relay 的 blockAdded 全块订阅）+ 团后 100–120 KB/s 入流轮询；风暴在时无零秒、不在时有零秒（Bettor 21:07Z 见零秒；我 21:17Z / 21:36Z 两次都没见）。我核：kaspad 17210 端口 **36 条 established borsh 连接**；P2P **入站 0**、出站 1（136.243.93.17）⇒ 没有在给别的 peer 供块，读负载只可能来自 RPC 服务。
- **决定性量（Bettor 采，我复核其口径）**：我方 RequestIbdBlocks = 3407 B tx 尖峰（99×32 B+头，吻合），在上一团结束后 0.2/0.9/0.2/1.6/5.6 s 发出；对端请求→首字节 6.6/4.2/5.5/4.7/5.9 s ⇒ 周期 75–85% 在对端。**§11 判据"两模型在到货间隔上不可分"被这条分开**——分的量是请求发出时刻，不是空档 CPU；我 §11 把"空档 CPU 是否为 0"当唯一判别量，是我的框架错。
- **裁决口径（同意 Bettor 修正）**：② 不落、③ 不落；块率由对端请求延迟锁定；P1 撤（去 cache 只加本机成本）、B′ 撤（无热线程，亲和无靶）。§11 第 6 条"③ 两条件形式满足"随基线口径修正后不再是矛盾：③ 的前提是"本机 I/O 受限"，而读负载已归 RPC 服务。
- **D-b（请求流水线深度 2）预审两点**：(a) 源码前提：`dequeue_with_timeout!` = 纯 `recv()`（common.rs:184-192），route 只按 payload 类型分发、**不按 response_id 匹配**（IbdBlock 响应带 response_id，客户端不看），顺序安全完全靠对端 `HandleIbdBlockRequests` 单循环顺序服务（v7/request_ibd_blocks.rs）——前提成立，措辞应为"客户端忽略 id、靠对端顺序"而非"无 request_id"。(b) **收益前提**：4.2–6.6 s 是对端**空闲等待**（队列/调度，可与下一请求重叠）还是**逐请求串行忙时**（每请求一次锁等/一次固定成本，排队也省不掉）——n=5 的"不随团大小变"两者都符合；"静默后成团"（非逐块滴流）偏向前者但不排除后者。**唯一判别 = 试验本身**（重编译 + 重启 ≈ 30 min IBD 成本：~20 min header 重议 + ~8 min 缺体扫描），设计稿须把"第二请求首字节紧接第一请求末字节 vs 再等 4–6 s"写成验收判据，并写回退（去补丁重启）。

## 12. P2 缓存回退预案（NWT · 2026-09-06T01:50Z · 触发线 28.5 GB · Bettor ledger 929）
- **增长源判定：不是块缓存**。consensus-006/LOG 头：单 column family；`write_buffer_size=16 MB × max_write_buffer_number=6` ⇒ memtable ≤ 96 MB；块缓存硬顶 8192 MB（`cache_index_and_filter_blocks=1`、`pin_l0_filter_and_index_blocks_in_cache=1`，索引/过滤块计入）——按 1.3 万读/s × 4–16 KB，body 相位开始后几分钟即满、之后不增。01:49Z WS 25.74 GB / Private 27.67 GB ⇒ **非缓存部分 ≈ 17.7 GB 且在长**（08:36Z 20.1 → 25.7）= kaspad 自身 LRU 缓存（`--ram-scale=3.0` ×3）+ 18,444 句柄的表缓存元数据；body 续上 15 min 抬 2.8 GB 为 header 相位换出后**回填**。
- **三案**（各一次重启，改 watchdog :47 参数串 + 段 3 换起流程，exe 不变）：(a) `--rocksdb-cache-size=4096`（补丁下限）WS 一次性 −4 GB → ~21.7，读 syscall ~13k→16–18k/s、物理读 0.6k→~1.5k IOPS（估）、块率不变（对端锁定）、CPU/块 +10–20%；(b) 去 flag（≈无块缓存）WS −8 GB → ~17.7，读 ~20k+/s、物理 2–3k IOPS、CPU/块 +20–30%、块率不变；(c) `--ram-scale=1.0` 对准增长源，非缓存部分粗估 ÷2–3 ⇒ WS ~14–16 GB 且包络变平（估），body 块率不变，**header 相位可能变慢**（每断连 30–40 min 重议用到这些缓存）。
- **建议**：28.5 先 (a)；包络仍升再 (c)。**时机**：紧接自恢复周期之后重启（tip 新鲜，header 重议只剩几分钟）⇒ 成本 ~15 min 而非 45；断连 ~每 8–9 h 一次。触发前不动。
- **补充（2026-09-06T02:26Z）**：02:24–02:26Z 对齐采样 kaspad CPU 6.7–8.3%（≈0.07 核）、IO Read 260–300/s，而 09-05 21:xxZ 同进程 ~1 核 / 1.3 万读/s；borsh 客户端仍 35、console 子进程 35、块速 27–28 blk/s 不变 ⇒ 本机负载 ÷10 块率不动，再证对端锁定；昨日 0.8 核是 relay 侧**活动**非连接数（③/④ 压掉 console 侧轮询后消失，只报不判）。⇒ 上文 (a)/(b) 的读回升 / CPU 估值是**高负载态上界**，当前态回退代价更小。WS 02:21Z 26.60 → 02:24Z 26.61，回填斜率收尾。
