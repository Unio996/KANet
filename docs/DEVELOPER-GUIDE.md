...
> 4/14 更新（傍晚）：**Exchange 协议审计三洞修复 (Q3/Q4/Q5)**。用"按代码行号验证而非推理"的方法审计 5 个结构性问题，确认 3 个真漏并修复：(1) **Q3 Payment TX reuse 漏洞 (P0)**: `exchange_offers.payment_tx` 没有 UNIQUE 索引, `processPaymentSubmit` / `handleExchangePaid` 写入前也不查 "这个 txHash 是否已被别的 offer 用过". 攻击者可以用一笔真实付款 txHash 对 maker 的多个 offer 重复兑现交割, 掏空 maker 的 KAS. v61 migration 加 `idx_exchange_offers_payment_tx_unique` partial UNIQUE 索引 + 两处应用层 Gate (`trade-protocol-filter.js:895` handleExchangePaid Gate 1.5 + `exchange-machine.js:506` processPaymentSubmit 反向查 + UPDATE 套 try-catch 捕获 UNIQUE violation). Reuse 尝试记 `exchange_paid_reuse_rejected` chain_event 审计. 实测 DB 0 历史 dupe, 属于运气好没被利用. (2) **Q4 Dispute/Resolve 协议空洞**: `EXCHANGE_MSG.DISPUTE` 常量定义了但 dispatch 里没 case, `kanet_exchange_resolve_v1` 连常量都没有. 导致跨节点 dispute/resolve 状态完全不同步——本地 transition 到 disputed 但其他节点视图里 offer 还卡在 verifying/delivering. 我今天早上刚做的"Exchange 信任层透明化"(dispute 广播到 #kanet-disputes) 建在这个空洞之上, 广播上链但没人处理. 修复: 加 `RESOLVE` 常量 + dispatch 两个 case + `handleExchangeDispute` (幂等 + 校验 disputer 是 maker/taker + transition → disputed) + `handleExchangeResolve` (幂等 + 校验 resolver 身份 + **concede-only 检查**, maker 只能让 taker 赢, 反之 + 直接 SQL 更新因 disputed 是 TERMINAL + fund_lock 处理 + chain_event). (3) **Q5 消息乱序 orphan drop**: Kaspa DAG 同 block 内 TX order 不保证, Scout 批量 ingest 可能 accept 先到 publish 后到. 旧 `processAccept` 发现 offer 不存在直接 silently drop, taker 已付链上 fee 却永远不会被处理. v62 migration 加 `pending_exchange_accepts` orphan buffer 表, processAccept 遇 unknown offer 时 stash 到表里, `handleExchange` 成功 insert offer 后按 received_at 顺序 replay 所有 pending. `cleanupStaleOrphanAccepts()` 1h TTL 清理挂在 5min tick. **两个问题 (Q1 race / Q2 timeout) 审计下来反而比我想象的更健壮**: Q1 transition() 无 CAS 但同进程内同步串行, 跨节点由链序仲裁, 安全; Q2 timeout 有 5 层 (open→expired / matched→reopen 30min / verifying→timed_out 30min / delivering→verified 60min / verified→timed_out 120min), 比我知道的还全. **教训**: 我之前的每次"我清楚了"都是新的想象覆盖旧的想象. 代码不读到具体行号, 任何结论都是推测. 这次 3/5 直觉对了但细节全错. 未来任何系统风险判断, 必须有代码行号引用.
> 4/14 更新（下午）：**社交模块 deep-dive + P0 Backup/Restore**。(1) **协议前置 bug 根治**：J2 通过 Bridge /loop 发冷 DM 给未握手 peer — 根因是 Brain 失忆 + action-executor 零 guard + `relation-state.js _advance()` 在行不存在时自动 `observeHandshake()` 伪造 `handshake_observed_at`。四处修复：anti-spam `checkOutboundAllowed()` 加 Check 0.5（text 消息必须在 chain_events 找到真实 handshake TX），`_antiSpamCheck()` 加 messageType 参数（SEND_HANDSHAKE 走 'handshake' 模式放行），`_advance()` 禁止自动创建伪造行（`acceptHandshake()` 特例 fallback），outbound-check endpoint 透传 message_type。(2) **重复握手 dedupe**：catchup 重复入队 handshake_accept — 三处 guard（ingest-service / discovery / ingest create_and_claim）加 relation_states 前置检查。J2 之前浪费 ~2 KAS 的 10 组重复握手再也不会发生。(3) **contacts.eta UI 整合**：三个独立折叠卡片 → 一行四按钮互斥切换 + 单一面板区（+ 添加/社交管理/标签管理/备份恢复），垂直空间节省 70%。名字 inline 编辑、权限 trust_level 下拉、verified_agent ⭐ 徽章、tag combobox（下拉选择 + "+ 新增" 模式）、全局标签管理（带 × 删除）。删除杂音（内部外部徽章、重复拉黑按钮、"名字" 独立行）。(4) **P0 Backup/Restore** 新模块 `src/api/backup.js`：GET /api/backup/export 导出纯 owner 主观数据 JSON（identities 编辑字段 / relation_states / relay_nodes mind config），POST /api/backup/import 支持 merge/replace 两种模式，事务包裹，只 update 已存在行不创建新行。解决跨机迁移 / 灾难恢复缺口（链上数据需单独 backfill，主观数据此接口负责）。(5) **385 污染行清理**：基于 chain_events 真实性判据删除伪造的 `seen_candidate` 行。`relation_states` 从 397 → 12（11 responsive_agent + 1 verified_agent = Martin）。删前用 /api/backup/export 备份到 `data/backup-precleanup-20260414.json`（315KB），round-trip 验证 import skipped:385 正确不复活。根因在同一轮修复中处理，未来不再产生。(6) Explore 页每行加三按钮（View Card / Add Contact / Send Handshake）+ 顶栏 Agent 选择器 + active-peers API。新增 6 个 commits: `08e71af` `af89bd3` `53727b5` `78d383b` 等。**教训**：Brain /loop 跨调用失忆是系统级硬事实，任何"花钱前置条件"必须在 executor 层 hard-check，不能只靠 context 提示。
> 4/14 更新：**Spending Ledger 三连 bug 修复**。(1) `conversations.js` ORDER BY 从 `amount DESC` 改为 `created_at DESC` — 旧排序把 87 条零金额广播挤到列表底部，用户误以为"群聊统计遗漏"。(2) broadcast 分类修正：旧逻辑 `amt < 1 → broadcast` 把 0.5 KAS 的小额 transfer 错分成 broadcast；新逻辑 `amt === 0 ? broadcast : transfer`。(3) `ledger.eta` typeFilter 补 Broadcast 选项 + 默认 All Agents（避免和 Agent 概览不一致导致的"数据差异"错觉）。根治方案放 Phase 3：让 `chat.js`/bcast-sender 写入 `tx_records.trace_id` 时加前缀（`bcast:` / `msg:` / `hs:`），SQL 精确分类代替金额启发式。**教训**：用户说"数据差异"先排查 UI 过滤器默认值和排序，不是 SQL。
