...
> **Status**: **DEPLOYED & CLOSED**(v1.0 · 2026-07-21 · P2 第一批 §3.1-3.5 全案红队闭环)。落地历程: §3.5 折入 J2 发现+草案(#ue9cp6.1)→ Bettor 方向审 GREEN-with-3-notes(#uegipr)→ NWT 正式红队 GREEN-with-2-MUST-FIX → 三态设计经 Bettor/NWT/J2 交叉论证定案(#uesq36)→ §3.1/§3.2/§3.3/§3.4 落码(`d829e8fe`)→ NWT diff 审抓阻塞级 bug(`decodeV1State` marker 位置错)→ 修复+KANet-UI 真实生产行验证(`ced75f31`)→ NWT 独立发现 v189 backfill 缺 dry-run 门禁 + Bettor 升格 MUST→ `K18_BACKFILL_CONFIRMED` 环境闸落码(`09f911da`)→ NWT 全批**最终 GREEN**→ KANet-UI 生产库 backfill dry-run(721 行: v1_committee=623/v2_zk=20/unknown=78,78=refunded 63+pruned_expired_waived 15,零活跃态)→ Bettor 定 DoD-4 判据口径("零在途盘"非"unknown=0")+ 三方(J1/NWT/KANet-UI)会签通过→ Bettor 批装载窗→ KANet-UI 执行装载(`K18_BACKFILL_CONFIRMED=1` 写入 kanet.env, 健康检查全绿)→ backfill 实跑结果与 dry-run **精确吻合零偏差**→ J2/NWT 装载后活代码复跑(§3.5 8 区块 15 断言绿,STOP-threading self-heal 生产环境确认)→ **NWT 宣布 P2 第一批红队审闭环**。详细时间线+每步 commit/证据见 §6。**批2(gate 接入实际调用点+高频零子进程性能实测+classifier 家族/模板相等维度拆分改良)另立设计卡,NWT 待命**;DoD-5(pruned_expired_waived 15 + A0 不符 6 的正式归因)保持独立开放项,不因本批装载而结清。
...
**处置**:①`decodeV1State` marker 检查改为 `buf[1]`(`ced75f31`)。②测试 fixture 改造:`buildFakeV1RedeemHex` 的 `buf[0]` 显式设为跟 marker 值不同的 `0x6b`(KANet-UI 实测观察值),证明修复后确实不再依赖 offset 0 的值;新增 2 条回归 case——「buf[0]!=0x08 但 buf[1]==0x08 依然通过」+「buf[1] 损坏才真的 FAIL」,直接堵死同类循环论证复发。③`_j1tn_k18_v1_offset_hexdump.mjs` v2 改造:除了原有的人眼 hex 切片预览,新增直接调用 `probeStructuralSignature`/`classifyPayoutShardFamily` 对真实生产行跑一遍并打印结构化判定结果——不再要求人工数 offset 验证,脚本自己给结论。**KANet-UI 用 4 个样本(ozzeu/pb73v/f08c4/vmhud)重跑,`probeStructuralSignature(row,'v1_committee')` 全部返回 `{ok:true}` —— marker@1 + predicateCommit@518/poolMerkleRoot@1002 结构签名全部跟真实字节吻合,NWT 复核修复本身 GREEN。**

**2026-07-21 追加二(NWT 独立发现,backfill dry-run 门禁盲区,Bettor 升格 MUST)**:NWT 发现 v189 的 backfill UPDATE 是 `runMigrations()` 无条件执行的真实 migration,不是 dry-run——任何原因触发的 console 重启都会真的跑一次 backfill,没有代码层面的开关卡住它等 K-18 §5 DoD-0"dry-run 报告人工过一遍才能真正执行"这道闸先发生,跟本卡自己写的 DoD-0 铁律自相矛盾。Bettor 用当天 07:35 一次计划外重启(RPC 劣化触发,与装载计划无关)的真实先例把这条从"可选跟进"升格为"装载前 MUST"。**处置(`09f911da`)**:`ALTER TABLE` 保持无条件(schema 变更本身无害),classify+UPDATE 的真实 backfill 循环收进 `K18_BACKFILL_CONFIRMED === '1'` 才执行,不设就只 log 待处理行数+跳过,不阻断启动。本机验证三态(全新库/有 unknown 行不设 flag/设 flag 后真执行)全部符合预期。NWT diff 审确认干净,批次收口。

**2026-07-21 追加三(生产 backfill dry-run + 装载 + 收尾)**:KANet-UI 用 GREEN 版模块代码在生产库跑只读 dry-run(脚本 `kasia-console/scripts/_kanetui_k18_family_backfill_dryrun.mjs`,已入库):全量 **721 行** payout_shards,family 分布 **v1_committee=623 / v2_zk=20 / unknown=78**。unknown 78 行精确拆解为 **refunded=63 + pruned_expired_waived=15**(63+15=78,零第三类无法解释的行),detail 统一签名"V1 结构签名符但 recompile byte-compare 不等(closed=0)"。

DoD-4 判据口径三方拍定(Bettor 定调):实质是**"unknown 行中在途/活跃盘必须=0"**,不是"unknown 字面=0"——NWT 独立核实 `bshard-settle-daemon.mjs:558-569` 的 `selectRipeMarkets` WHERE 子句,daemon 只捡 `pending_bettors`/`verifying`/`settled_partial_claims` 三种状态,`refunded`/`pruned_expired_waived` 均不在其中;KANet-UI 逐行显式断言 78 行 status 分布精确 = `refunded 63 + pruned_expired_waived 15`,零条其它状态。J1 域内判定:①这两类都是 daemon 不处理的终态桶,非在途盘误伤 ②covenant_family 当前唯一消费者(`assertZkNativeImmutable`)zero 活调用点,gate 也还没接线,unknown 不影响任何现网行为 ③但这不等于 DoD 项 5(§0 两条遗留线索正式归因)已结清,继续作为独立开放项。三方(J1/NWT/KANet-UI)会签通过,Bettor 批准装载。

**另一个质量发现(Bettor 提出,不阻塞本批,列入批2)**:`classifyPayoutShardFamily` 把"家族身份"(`probeStructuralSignature` 管的结构签名,V1 vs V2)和"创世模板字节相等"(recompile byte-compare,更严格的子集条件)两个维度捆在一起才判定 `v1_committee`——63 条 refunded 行结构签名早就符合 V1(家族身份本身没有疑问),只是因为 recompile 对 `closed:0` 模板不等(refund-close 之后 `closed`/`payoutRoot`/可能 `w0..w16` 都已偏离 genesis 模板)才落进 `unknown`。J1 拍板:选(a)接受现标签,63 行事后 UPDATE 修正,classifier 拆分(家族身份靠结构签名单独判定 / 模板相等挪去 §3.3(c) coherence 概念不倒灌回 family)立批2 独立卡,不在装载窗仓促二次改动同一文件(当天已经在这个文件踩过一次 marker off-by-one,避免重复同类风险)。

**装载执行**(KANet-UI):窗前 checklist 全过(git 树干净 / `K18_BACKFILL_CONFIRMED=1` 写入 kanet.env / tick 间隙判据留足安全窗口)。装载内容 = §3.5 + 批1 四 commit(`d829e8fe`/`ced75f31`/`09f911da` + KANet-UI 自己的 dry-run 脚本 commit,至 `a6d7f876`/`05575a08`)。装载后:健康检查全绿(RPC 零错误、零 uncaught、tick 节奏正常、console 健康);backfill 实跑结果与 dry-run 预测**精确吻合,零偏差**;J2 装载后活代码复跑 §3.5(`bshard-verify-claim-landed-amount.test.mjs`,部署后 HEAD)8 区块 15 断言全绿,case 7(NWT MUST-FIX②)确认 STOP-threading self-heal 路径在生产环境同样成立。**NWT 宣布 P2 第一批(§3.1-3.5)红队审闭环。**

**批2 已开工(Bettor 派工,2026-07-21)**:范围 = ①gate 接入实际调用点(调用点分级:高频 `ensurePayoutShard`/`V2` 三步/低频 `consolidateAndBuildPsState` 四步)②高频路径零子进程性能实测 ③`classifyPayoutShardFamily` 家族身份/模板相等两维度拆分改良。照旧设计先行→NWT 红队→落码,另立设计卡,不在本卡继续追加。
