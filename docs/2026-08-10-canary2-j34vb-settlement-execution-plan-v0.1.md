...
### ✅ 判别窗已走完(2026-08-10 14:04Z)· 结论 = **甲2/乙 支**

**两个 tick 全部落在回填(13:54:39)之后,读数一致**:
```
tick 1  [13:57:16] zkJudgeProposeTick_propose market=93-j34vb: unreachable
        errMsg 全文 {"marketId":"…j34vb","message":"unreachable"}   bshard_close_request_v2: 仍不存在
tick 2  [14:04:03] 同上, 逐字相同
        zk_judge_propose_last_attempt_at 两次都在推进 ⇒ tick 确实跑了, 不是"没跑所以没报错"
```
⇒ **甲1 被排除**:回填后错误**既没消失也没变形态**。
⇒ **进程级 wasm 毒化坐实** ⇒ 交 @KANet-UI 按预授权开重启窗。

### 🔴 这条结论【不是】"回填没用",两件事必须分开记

| 事实 | 状态 |
|---|---|
| 回填**解开了 K-18 coherence gate step(d)** | ✅ **已达成**(S4 输出 `回填并通过 gate`;目标盘已不再 divergent;S5 落值核实全过) |
| 回填**没有**解开 propose 的 `unreachable` | ✅ 已证(本窗两 tick) |
| ⇒ 合起来 | **堵点【至少两层】。回填拆掉了它该拆的那一层,而它前面还有一层 wasm trap。** |

🔨 **给下一个人的判据**:`unreachable` 发生在 step(d) 之前或与它无关 —— 因为 **gate 已经过了,失败点没变**。
⇒ **别把"回填后仍不结算"读成"回填是错的方向"**;也**别**在重启修好 trap 之后,又把结算成功**全部**归给重启(那 12 个还没回填的盘会告诉你答案:它们即使 trap 好了,仍会撞 step(d))。
🔵 **⇒ 其余 11 盘 / 93 盘积压的处置顺序由此明确**: **两层都要拆**,且回填这一层与重启那一层**互不替代**。
