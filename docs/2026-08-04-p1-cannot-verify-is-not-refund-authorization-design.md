...
>
> ---
> ### 🔵 v0.5 变更块(2026-08-06 · J2 · @Bettor 07:33/07:35 批 · **两件, 都不动任何论点**)
>
> **【一】§5 要求形态修订 —— 标记:【要求形态修订 · 命题一字未动】**
> - 🔴 **§5 的原文一字不删, 原样留在下方**;本块只在其上追加"**该形态在本仓不成立, 换什么、为什么**"。
> - **命题不动**:「验证中断 ⇒ 市场不得自动进入退款广播路径」**逐字保留**, 终态断言 `unresolved_needs_authorization` **逐字保留**。
> - **只改观察方式**: §5 原文要「mock `dispatchRefund` 断言调用次数 = 0」。**2026-08-06 实核: 在本仓做不成**——
>   `dispatchRefund` 虽 export(`pool-market-settler.js:2724`), 但 settler 内部两个调用点(`:1241`/`:1384`)调的是**模块内本地绑定**;
>   ESM 导出是**只读 live binding**, 外部改不动、内部也不看外部 ⇒ 只剩 loader hook, 而那与本仓测试框架的 allowlist 设计意图正面冲突
>   (`test-framework/lib/runner.mjs:737` 逐字:「万能口等于给测试框架开一条绕过一切声明式约束的路」)。
>   🔵 **§5 那句写于本仓有 `call_module_export` 之前, 没人验过它在本仓成不成立。今天验了。**
> - **提议的替代**: 驱动**真实** `poolSettlerTick`(`:546` 已 export)+ 断言**落库痕迹为零**。
> - 🔴 **等价与否【不由本稿裁, 归 @NWT】**(@Bettor 2026-08-06 明纠作者越界): (140) 写死 ⑤ 完成判据 = 预注册原文,
>   **改它的形态只有「显式修订 + 独立审」一条路, 不许由执行者自评等价而静默替换。** 本块就是那个"显式修订", 独立审待 NWT。
> - **全案见** `docs/2026-08-06-precond5-verification-interrupt-no-autorefund-test-design-v0.1.md`(含六臂断言表、
>   两条**做不成的臂**的理由、阳性对照**弱一格**的如实标注、以及 allowlist 增项申请)。
>
> **【二】§10.7 第 1 条坐标勘误 —— 标记:【坐标勘误 · 零论点变更】**
> - §10.7 第 1 条与 `pool-market-settler.js:1410` 注释都写 **`:248-268`**。**2026-08-06 现读: 那段今天是 `freezeAwaitingAuthorization` 的定义(`:256`)**;
>   真正的 bettor 退款扫描闸在 **`:488-493`**(`legacyRefundBuilderTick` 内)。
> - 🔴 **为什么单独标出来**: 这条坐标是**验收清单自己用来指认被验收物的坐标** —— 按它逐条核, 会核到一个与被验收物无关的地方,
>   而**核完的读数看起来完全正常**。(在册同族: 坐标存在 ≠ 断言对。)
> - ✅ **NWT 2026-08-04 的 GREEN 作用域不因本勘误漂移**: 改的是**指路的手指**, 不是**被指的东西**。
> ---
>
...
> 🔵 **【要求形态修订 · 命题一字未动】(v0.5 变更块, 2026-08-06)** —— **本节原文以下一字未删, 照读**。
> 唯一被修订的是**观察方式**: 下面第二个 bullet 写的「mock `dispatchRefund` … 调用次数 = 0」**在本仓结构性做不成**(ESM 只读 live binding + 内部本地绑定调用),
> 提议改为「驱动真实 `poolSettlerTick` + 断言落库痕迹为零」。**命题与终态断言逐字保留。**
> 🔴 **等价与否归 @NWT 判, 不由作者自评。** 理由、六臂断言表、两条做不成的臂、阳性对照弱一格的标注,
> 全在 `docs/2026-08-06-precond5-verification-interrupt-no-autorefund-test-design-v0.1.md`。
