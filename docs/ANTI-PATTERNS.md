...
## 规则 70 —— 「执行」≠「产生可见效果」：说一段代码"从未执行"，要先证的是【它没进过那个分支】，不是【它没留下痕迹】

**触发（J2，2026-08-07，`G4-SETTLER-CHAIN-REDERIVE-BRANCH` 拆弹线）**：一段 `WHERE from_address = ?` 查询恒不匹配（`from_address` 全 NULL），据此判定该分支"从未执行"，据此论证移除它是零行为变化。**移除本身仍然对**——但"从未执行"这句话本身是**字面假**：`if (...) { ...; continue }` 这个分支**每个 tick 都真的进去过、真的跑过查询**，只是查询恒空、没有下游可观测效果。执行发生了；只是它什么都没改变。

**为什么这个区分要紧**：如果拿"从未执行"当验收判据，下一次有人往同一段代码里加一条**有副作用**的语句（哪怕逻辑完全不变），这句判据依然读作"成立"（因为分支确实"没执行过危险动作"这件事本身没变），而实际风险已经从"分支进不去"变成"分支进去了但过去没人加过会动手的代码"——**判据的措辞没有区分这两种状态，而它们的风险完全不同**。正确的判据是分两句：①**这个分支被进入过 N 次**（tick 计数/日志行，能证明"执行"）；②**这些进入里有零次产生下游可观测效果**（查询恒空/写入恒零，能证明"无效果"）。混成一句"从未执行"会把两句里较弱的那句（②）读成较强的那句（①+②）。

**铁律**：
- 描述一段代码的历史行为时，**"执行次数"与"可见效果次数"是两个独立的量，分开报**，不要用后者去描述前者（"从未产生效果" ≠ "从未执行"）。
...
- 🔵 与在册「不干活换来的安全不可验证，风险搬到了激活那一刻」（`safe-by-inert`）是同一张卡的**镜像**：`safe-by-inert` 说的是"没做事所以看起来安全，但没法验证它真做对了没有"；本条说的是"**做了事（执行了）但没留下痕迹，所以被误读成没做事**"——前者的坑是"安全" 被高估的置信度，后者的坑是"从未执行"这个描述本身就不准。两者常在同一张卡上先后出现（先怀疑"没干活"是不是真安全，再发现"没干活"这个前提本身站不住）。

## 规则 70 —— 「它从未执行过」与「它没留下痕迹」是两个命题：分支进去了、查空了、`continue` 了,**那也是一次执行**

**触发（J2 2026-08-07 · Codex `a19087c7` 外审抓,NWT 复核认）**:移除 settler 那段 cross-node chain re-derive 分支时,我给的结论是「**该分支在生产中从未执行过**」,依据是三组读数——当前零可选市场 / `from_address` 全库无非空行 / cross-node 市场零终态写痕。
...
**铁律**:
- **写「从未执行 / 从未跑过 / 没被调用」之前,先问: 我证的是【控制流没进去】,还是【它没留下我能看见的东西】?** 后者是绝大多数情况,而它**弱得多**。
...
🔵 **与规则 65 那条「safe by inert」(不干活换来的安全不可验证,风险搬到激活那一刻)并排,而方向相反**:
- **safe-by-inert**:看起来安全,其实危险(风险在未来激活);
- **本条**:说成"没执行",其实执行了,只是没留痕(**错在对过去的描述**)。
⇒ **两条共用一个底:「没有可见效果」既不能证明"没运行过",也不能证明"将来不会运行"。**
