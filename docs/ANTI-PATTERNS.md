...
## 规则 67 —— 服务端会改写你发的消息（trim + 5000 字符截断）：发送器自证若做"回读逐字节比对"，判"没落地"会判假，而错误动作是真花费（J1 2026-08-03 实烧 9 笔）

**触发**：J1 用写文件方式生成频道播报正文，文件末尾多了一个换行；他的发送脚本自证靠"回读后跟本地构造的字节逐一比对"。`kasia-console/src/api/chat.js:250` 与 `:264` 上 `/api/chat/send` 对 `message` 做 `.trim()`（掐头也掐尾，不只是掐尾）——链上落地的字节因此**必然**跟他本地构造的原始字节不同。逐字节比对判"不相等"⇒ 判"没发出去"⇒ 自动重发；而**实际上每一次都已经 HTTP 200 + `ok` + `txId`，是真花费、真落地**。这个失败形态最毒的地方：**验证"没通过"和"真没发出去"在他那侧读数完全相同**，脚本因此在无人为干预下连续重发 9 次（间隔约 45-53 秒/次），直到 `TaskStop`（先杀了外壳没杀住底下的 bash 子进程，见下条残留问题）+ 操作员在物理层 kill 掉那个进程才停。

**同一函数（`_computeChainContent`，镜像 `kasia-relay/src/relay.mjs:28` 的 `capMessage`）还有第二种改写**：广播正文超过 `MAX_BROADCAST_CHARS=5000` 会被截断到 5000 字符再上链——逐字节比对对长消息同样会假阴。

**铁律**：
- **发送器自证的锚必须是 `txId`（POST 响应里拿到的那个），不是消息内容本身**——`txId` 不受服务端任何内容改写（trim / 截断 / sanitize 替换字符）影响，回读时按 `tx_hash`/`txId` 或消息里附的唯一 nonce/tag（子串匹配,不是全文逐字节比对）去核实,已经是本仓多数发送器(`_bettor_send.cjs`/`_j2_send.cjs`)在用的正确形态。
- **如果一定要比内容,双方都先 `.trim()` 再比,且要意识到超 5000 字符的部分永远比不上**(截断是设计行为,不是要修的 bug)。
- 🔴 **最重要的一条,独立于用什么锚**：**一旦已经拿到 `200 + ok + txId`，后续任何形式的"验证不过"都只能导向"停手、报人、留证据"，绝不导向"重发"**——那个时间点上钱已经真花了/消息已经真落地并绑定了那个 `txId`，重发不是补救，是**确定的重复花费**。判别式：写重试逻辑前问自己"如果这次重试的理由只是'我这边验证不到,不是服务端说失败',我还要不要发第二笔真钱"——答案永远是不要。
- **本机 `_*_send.cjs` 审计结论**（KANet-UI 2026-08-04,只审计不代改,发送器是各自身份的钱路口）：`_j2_send.cjs`/`_bettor_send.cjs` 用 nonce/tag 子串回读,天然不吃 trim 的亏；`_kanetui_send.cjs`/`_nwt_send.cjs` 不做任何回读(无重发逻辑,自然也不吃这个坑,但也没有落地校验)。`_bettor_send.cjs` 有一个独立于 trim 的残余风险：验证不过时**有界重试 3 次**,拥堵时 40 条回读窗口可能把已落地的 tag 滚出窗外造成假阴 ⇒ 最多 2 次有界重复花费(Bettor 本人已自查自认,计划改锚 txId)。J1 本人已在自己机器上自修(`scratch/j1-send-one.sh`,gitignored,只影响他自己)。

## 规则 68 —— 「我找到了 N 处」的可信度取决于用的是【枚举方法】还是【我恰好读到的地方】：两者产出的数字长得完全一样，而只有前者能回答"还有没有第 N+1 处"

**触发（J2 2026-08-06 实撞，同一天内自己更正自己）**：P1 钱路改造把一批路径从 `dispatchRefund` 改成了 `freezeAwaitingAuthorization`，而描述这些路径的注释/日志没跟着改。我在频道报「**四处**文案 + **四处**计数器」。数字具体、位置带 `file:line`、读起来完全像一份清单。

**它是怎么被戳穿的**：我在核**另一件事**时顺手撞到第 6 处（`pool-market-settler.js:987`）。改用系统枚举后实数是 **11 处文案 + 5 处计数器**。

**根因**：我那份不是枚举出来的，是**我恰好读过的地方的集合**。而这两种做法的产物**在形式上无法区分** —— 都是一串 `file:line`，都带证据，都能复核**已列出的那些**。**没有任何东西会提示"这份是不完整的"**，因为不完整性恰恰表现为"你没看见的那些不在里面"。

**铁律**：
- **报"共 N 处"之前，先写下你用的枚举谓词**，并且这个谓词必须是**机械可复跑**的（一条 grep / 一段脚本），不能是"我把相关的地方看了一遍"。
- **谓词要按【效果】而不是按【我记得的关键词】**：本例正确的谓词不是"搜 `dispatchRefund`"，而是"**对每一个 `freezeAwaitingAuthorization` 落点，回查它上游的描述**" —— 前者会漏掉措辞不同的（如 `:1408` 写的是 `force cancel + maker refund`，压根没有 `dispatchRefund` 这个词）。**两个方向都跑，取并集。**
- **报数时把谓词一起报**，让读的人能判断"这个 N 是在什么范围内的 N"。只报数字 = 让人以为它是全集。
- 🔵 与规则 64（修并行实现之一必须枚举全部拷贝）、规则 58（黑名单天生不完备）同族：**三条都是"我以为我覆盖了，而覆盖面从来没有被机械确定过"**。

## 规则 69 —— 改一条路径的**行为**时，描述这条路径的**观察量**必须同批改：否则下一个人拿旧观察量去核新行为，会得到"没生效"的结论，而他没做错任何事

**触发（J1 首报一处 / J2 系统枚举出 16 处，2026-08-06）**：P1 卡把「验证不成 ⇒ 自动退款」改成「验证不成 ⇒ 冻结待授权」，**行为改对了**。但：
- 11 处注释与日志仍写 `→ dispatchRefund` / `force cancel + maker refund`，其中 `:1297` 逐字写着"**退所有 bettor + maker**"，而那条路今天**一分钱不退**；
- 5 处 `refund++` 紧跟在 `freeze` 之后 ⇒ tick 汇总行打的 `refund=N` **把冻结盘计成退款盘**。

**为什么这不是"文案小事"**：**这些正是任何人用来判断"那个修法到底有没有生效"的观察量。** 代码做对了，而它的每一个自述都在说它做了改造前那件危险事。它同时污染两侧：
- **人**：读日志的人得到相反的结论；
- **测试**：`poolSettlerTick` 返回的 `refund` 本该是最便宜的断言轴，而"冻结"与"退款"在这个数上**读数相同** ⇒ 该轴对前置⑤ 那条用例**不可用**，用例被迫绕开它去读 DB 痕迹。**一个被污染的观察量会让本该便宜的验证变贵，甚至变得做不成。**

**铁律**：
- **行为改动的 diff 里必须包含它的观察面**：注释 / 日志字符串 / 计数器名与归属 / 返回对象里的字段。**"零行为改动"的文案 diff 与行为 diff 可以分两段审，但不能只做后者。**
- **计数器出问题时，优先【新增一个正确的量】而不是【删掉错的那次自增】** —— 删掉会让这类处置在汇总里**完全消失**，从"计错"变成"看不见"，更难发现。
- 🔴 **推论（验收面）**：**凡拿某个计数器/日志行当验收判据之前，必须先证它此刻仍在数它名字所说的那件事。** 本条就是现成反例：名字叫 `refund`，数的是 `refund + freeze`。
- 🔵 与在册「导出量在动 ≠ 被它导出的那件事在变」是同一族的**反方向**：平时是"日志说成功而其实失败"，本例是"**日志说发生了危险行为，而其实发生的是安全行为**"。**两个方向都指向同一句：日志不是代码做了什么的证据。**

## 规则 70 —— 「它从未执行过」与「它没留下痕迹」是两个命题：分支进去了、查空了、`continue` 了,**那也是一次执行**

**触发（J2 2026-08-07 · Codex `a19087c7` 外审抓,NWT 复核认；2026-08-07 KANet-UI 去重合并两段重复条目，本条为定稿）**:移除 settler 那段 cross-node chain re-derive 分支时,我给的结论是「**该分支在生产中从未执行过**」,依据是三组读数——当前零可选市场 / `from_address` 全库无非空行 / cross-node 市场零终态写痕。

**错在哪(不是取证不足,是命题写错)**:那段代码**每个 tick 都在跑** —— 它进入 `try`、执行 `SELECT`、拿到空结果、`continue`。**「从未执行」字面即假。** 我实际证到的只是「**它没有产生可见效果**」。

**为什么这个区分要紧**：如果拿"从未执行"当验收判据，下一次有人往同一段代码里加一条**有副作用**的语句（哪怕逻辑完全不变），这句判据依然读作"成立"（因为分支确实"没执行过危险动作"这件事本身没变），而实际风险已经从"分支进不去"变成"分支进去了但过去没人加过会动手的代码"——**判据的措辞没有区分这两种状态，而它们的风险完全不同**。

**Codex 另外三条(都成立,但性质是"证据不够强",与上面那条不同级)**:
- **现态查询 ≠ 历史执行台账** —— 查的是"现在长什么样",不是"历史上发生过什么";
- **留存表未证 append-only、也未证与全节点一致** —— 本机 `kaspa_tx_log` 未必是当时的全量;
- **"记录字段缺席"不等于"写入动作从未发生"**。

**铁律**:
- **写「从未执行 / 从未跑过 / 没被调用」之前,先问: 我证的是【控制流没进去】,还是【它没留下我能看见的东西】?** 后者是绝大多数情况,而它**弱得多**。正确的判据要分两句：①**这个分支被进入过 N 次**（tick 计数/日志行，能证明"执行"）；②**这些进入里有零次产生下游可观测效果**（查询恒空/写入恒零，能证明"无效果"）。混成一句"从未执行"会把较弱的②读成较强的①+②。
- **可证的措辞长这样(照抄本例的降级原文)**:「**被检本机留存数据集上 X 无非空行,故该查询无可匹配行;留存 Y 集无 Z 写痕;分支历史调用本身、其他节点与已不留存数据上的行为,双向皆未证。**」
- 判定某段代码"安全/无害"时，问清楚安全性来自哪一层：**代码没被进入**（结构性不可达）比**代码被进入但没数据可动**（数据依赖性沉默）更强——后者只要上游一个字段被填就会武装，前者需要改这段代码本身。两种"安全"的加固方式不同，混着报会让读的人误判需要盯防的是哪一层。
- 🔴 **而降级措辞【不等于】削弱处置**:本例移除的理由是**不安全的推断链是代码实况**,与历史上有没有击发无关。**别因为证据被降级就把该拆的雷留下 —— 那是把"我说不满"错读成"事情没那么严重"。**

🔵 **与「不干活换来的安全不可验证，风险搬到了激活那一刻」（`safe-by-inert`，本仓仅存于 memory，此文件内暂无独立编号，勿标错号）并排，而方向相反**：
- **safe-by-inert**：看起来安全，其实危险（风险在未来激活）；
- **本条**：说成"没执行"，其实执行了，只是没留痕（**错在对过去的描述**）。
⇒ **两条共用一个底：「没有可见效果」既不能证明"没运行过"，也不能证明"将来不会运行"。** 两者常在同一张卡上先后出现（先怀疑"没干活"是不是真安全，再发现"没干活"这个前提本身站不住）。

## 规则 71 —— 「operator 手工脚本」入库后就是一件现成工具：审查它的人先 grep 它有没有被 route/cron 引用（2026-08-27 · §6-1 挑战签发 · NWT 裁）

- 背景：`kasia-console/scripts/u1-issue-challenge.mjs` 是 §6-1 Track-A 的**手工**挑战签发（(527)(528) 裁：自动签发口 = 北极星功能、部署闸绑 §10）。它入库 = 可发现 = 把"手写 INSERT"变成现成工具；不跨信任边界，但**降低本地 actor 门槛**。
...
- 🔴 **判据(承重)**: **探测器自身故障 ≠ 被测对象故障**。`*-load-fail` / 依赖缺失 / 探针自身崩(exit code 表示"探针没做出关于节点的判断")这一类, watchdog **必须 fail-safe: 不计入死亡判据、不触发重启**(重启治不了探针坏, 只会空拉/连锁)。只有"探针成功探测到节点问题"(连不上/超时/身份不符)才是节点侧信号。
- 🔴 **fix-up(NWT NOTE·防与 daa=0 自相矛盾)**: 上一句**不含"数据空(daa=0)"**——`daa=0` 在 **IBD 期是合法 SYNCING**(节点正常在同步), **不是**节点侧故障信号; 把它当"数据空⇒可判死/重启"正是 VB-9 三态要禁的 8/23 型误判。**"数据空(daa 非正数)"只在【已同步(isSynced=true)】后才算节点侧信号**(那时 daa 该有值却没有=真数据坏, probe code 3 收窄义)。⚠ **本规则(探针自坏)与 VB-9(daa=0 误判)是两个不同根因**: 根因一 = 探针**加载不到**(kaspa-wasm junction/Redirection Guard, code 6, 本规则治); 根因二 = 探针**加载正常但 daa=0 被当死**(code 3, VB-9 三态治)。两者都真、互不替代。
...
## 规则 79 —— SQLite 里 ISO `'…T…Z'` 存值与 `datetime('now', …)` 空格形做裸字符串比较：**语法合法、逻辑恒错**，`node --check` 与任何"跑通了"都看不见（2026-08-29 · 两周内咬两次：`aba0b94c` hold-monitor 两条 + intake 预筛一条，修在 `c1a35749` · lint `R-SQL-TIME-STRINGCMP` `70f0db11` 守 · NWT 提议）

- **实录**: `created_at` 存 `2026-08-29T07:35:12.345Z`（JS `toISOString()`），SQL 写 `created_at > datetime('now', '-30 minutes')`；`datetime()` 返回 `2026-08-29 07:05:12`（**空格**）。SQLite 对 TEXT 做字节比较：同日期前缀下 `'T'`(0x54) > `' '`(0x20) ⇒ **存值恒大于任何 datetime() 值** ⇒ "30 分钟内"对全表恒真、"超过 60 分钟"恒假。hold-monitor 的 `stuck`/`intent_stale` 因此永远 0（看起来"很健康"），intake 预筛把所有有 intent 的 offer 永远排除（看起来"没重复退款"）——**两个假绿灯**。测试也没抓到：向量写在同一形下自洽。
- 🔴 **判据**: 列名带 `_at`/`_ts` 且另一侧出现 `datetime(`/`date(`/空格形 `strftime('%…%d %H…')` 的 `< > <= >= <> BETWEEN` ⇒ 一律不写。**静态看不出某列到底存哪种形**（`DEFAULT (datetime('now'))` 的表存空格形，那样比较其实对），所以规则判的是"形不安全"不是"错"——新写一律用对两种存值都对的形。
- 🔨 **怎么写才对（两法，按路径热度选）**:
  1. **`julianday()` 两侧**（默认，钱路/分钟级 tick）：`julianday(created_at) > julianday('now', '-30 minutes')`——`julianday` 对 T 形与空格形都能 parse，是唯一对两种存值都对的写法。**代价**：包了列 ⇒ 该列索引废（全表扫）；60 min tick、行数 < 1e5 的钱路无妨（NWT perf 注）。
  2. **canonical epoch-int 列 + 索引**（热路径/大表）：存 `created_ms INTEGER`（`Date.now()`），比较 `created_ms > ?` 参数由 JS 算（`Date.now() - 30*60e3`）；整数比较走索引，且规则不 flag。新表优先这法；老表加列要走 migrate + DATABASE.md。
...
- **更要命的一层（2026-08-29 追加实核）**: 这个机制**在 8/22 就被正确诊断、并当天上了缓解措施** —— `--ram-scale=0.4`（Owner 批），机制原文写在 `scripts/kaspad-watchdog.ps1:22-26` 注释里：「默认按大内存假设分配缓存 → 处理 trusted blocks 时全机 swap → 节点卡到回不了 P2P ping → 三个 peer 同时判它死并断开 → IBD 整轮作废重来」。**但措施没奏效**：生效后仍有 **64 次**重启（8/23 : 25、8/24 : 6、8/25 : 2、8/26 : 21、8/27 : 10），循环又跑了 5 天没人发现。⇒ **这 9 天真正塌掉的环节不是"没诊断出来"，而是"没人验证补丁管不管用"。** 本规则的价值因此从"帮你发现问题"升级为"帮你验证修复"——后者才是缺口所在。
...
- 🔨 **上了缓解措施之后，必须用同一个计数器验证它奏效**。「改完就当好了」= 把假设当结论。本例的缓解措施**在配置上完全正确**（kaspad 官方对 4–8 GB 机器就是建议 `0.3–0.5`），但**对这台机器的实际内存缺口仍然不够** —— **配置正确 ≠ 问题解决**，而没有计数器时这两者在观感上完全一样。
- 🔨 **怎么写才对**:
...
- 📎 **附注（对偶·存量巡检，J1 2026-08-29 19:45Z 自报 · Bettor r32 记）**：本条讲**怎么设计不会瞎的监控**，没讲**怎么发现已经瞎掉的存量监控**——J1 立本条当天，younio 上就有他自己 8/22 留下的 `channel-monitor.mjs`（PID 10640）**空转 7 天**：进程活 ✓ / 11 MB ✓ / 无报错 ✓，而 `ssh -L 3201` 隧道早断、`monitor-lastseen-younio.json` 七天零更新、`curl 127.0.0.1:3201` 拒连。**存量巡检 = 枚举"我起过的所有长期进程"，逐个问它【最后一次产出真实数据】是什么时候**（**硬判据** = 端口/隧道是否仍在 + 实测读一次；**状态文件 mtime 仅作佐证**，见下条修订），不是问它活不活。同日反向同病：Bettor 把活着的 Monitor 误判为死（`TaskList` 不列 Monitor）——两边都没走到"最后一次真实产出"这一步。见 `docs/iteration/j1-inbox/2026-08-29T19-45Z-*`。
- 📎 **附注修订（mtime 降为佐证，J1 2026-08-30 00:24Z 报 · Bettor r33 裁）**：上面三判据里的 **mtime 不能单独承重**，两个方向都不成立。
  1. **取法决定准不准**：mtime/size 必须用 `Get-Item <完整路径>` 取（bash 同类 = `stat -c %Y <路径>`）；**`Get-ChildItem` / `dir` 读的是缓存目录项**，对**被进程持开写入**的文件会失真。双机同日实测：da9 `logs/console.log` 目录项落后 **87,787 字节 / 3 分 20 秒**；younio 同名文件目录项报 `08-26 02:32 / 543.8 KB`，而同一时刻开句柄真实长度 **26,757,915 B**、末行时间戳 `2026-08-30T00:16:34` —— **落后 4 天 / 26 MB，而进程完全健康**。同一次列目录里另外三个文件（`console.err.log` / `console.log.prev` / `heartbeat`）逐字相符，**只有被持开的那个是陈的**。⇒ **同一判据在不同时刻的误差跨 3 个数量级**，落后量不可预测。
  2. **mtime 陈 ≠ 进程死**：僵尸 `channel-monitor` 那次 mtime 判对是**碰巧**——那个状态文件没有进程持开。承重的一直是"实测读一次"：J1 查 younio console 时正是靠 `HTTP 200 / 45,459 字节` + heartbeat 新鲜判它活着，只看前两条会把一个健康进程报成"死了四天"。
  3. **mtime 新 ≠ 数据有效**：只说明有人写过，不说明写进去的是有用的东西（规则 80 正文同义）。
  4. **推论——凡以「日志在不在滚」判【冻结】的告警都受此影响**：如 coord(728) 的 `wasmBytes ≥ 4000 且 10 min 不变`（wasm `memory.grow` 失败后数值冻结 = 实例毒化签名），若"不变"经由目录项的 size/mtime 得出，**陈目录项既能伪造冻结、也能掩盖真冻结**，必须读**行内内容**。Bettor r33 已逐条实核自己四件仪器（`ready_watch.sh` 取行内 `wasmBytes=` / KANet-UI D 行取 `_ibd_monitor.log` 末行 ISO 时间戳 / `hb_guard.sh` 走 `netstat`+`curl` 不读 mtime / supervisor `heartbeat_fresh()` 用 `stat -c %Y` 按路径取且心跳文件无进程持开）**均不受影响**。
  见 `docs/iteration/j1-inbox/2026-08-30T00-24Z-j1-dirent-stale-*` 与 `2026-08-30T00-30Z-bettor-r33-*`。

## 规则 81 —— `git worktree remove` / 任何递归删除**顺着 junction/symlink 进目标**：删 worktree 删掉了 live 主树的 `node_modules`（2026-08-29 · J2 自伤）
> 📌 **状态注记（2026-09-13 · NWT · 出处 COORD-LEDGER (1021)(1022)）**：本条 2026-09-13 **原样复现**——NWT 为红队 diff 审建的隔离 worktree 同样把 `kasia-console/node_modules` junction 到 live 树，`git worktree remove --force` 报同一句 `Invalid argument`，穿透删了 live 树 85 个顶层包（同 @scope 前缀止于被占用文件的签名）。**不是这条规则没用，是它只写在文档里、没有机制挡**：下方"机制"一节点名的 `scripts/check-worktree-junctions.mjs` 在事故发生时**已经存在一年多**，但它是个独立检查脚本，删 worktree 前"记得手动跑它"这一步全靠人记忆——这次和 08-29 那次一样，没人记得跑。🔨 **真正的缺口**：需要一个把检查**接线**进删除动作本身的 wrapper（如 `scripts/safe-worktree-remove.mjs`：先跑检查、有链自动拆或拒绝执行，才允许 `git worktree remove` 通过），不能停留在"有一份独立脚本、靠自觉调用"——这与本文件通则里"机制存在但没接线"是同一个病（同 test-only export guard 那次教训）。触发形不止 `git worktree remove` 一种：PowerShell `Remove-Item -Recurse` 对同类 junction 有相同的穿透风险（未在本次亲验，但物理机制相同，按"两种触发形都算"处置，不要只防 git 那一条命令）。
> 📌 **状态注记（2026-09-14 · KANet-UI · Bettor(1161)(1162) 派工 · 第三次复现 + 一次自伤工具事故的教训）**：本条 2026-09-14 **第三次复现**，这次的具体死法跟前两次（都是 PowerShell junction）不同——根因是 **NWT 用 bash `ln -s` 建的 symlink**，清理侧 worktree 时未先摘链就 `git worktree remove --force`，同一条 `remove_dir_recursively` 不认 reparse point 的病，穿透删了主检出 `kasia-console/node_modules` 下**全部 8 个 scoped(`@*`) 包**（`@fastify/formbody|static|view`、`@noble/hashes`、`@polymarket/clob-client-v2`、`@solana/spl-token|web3.js`、`@stoqey/ib`）以及 `shared/vendor/kaspa-wasm` 整个目录（7 个文件，二级穿透——见下方 (d)）。恢复用 `npm install`（禁 `npm ci`）+ `Remove-Item`/`git checkout` 逐一补，主网 console(PID 12404) 全程未重启，恢复前后各留 `npm ls --depth=0`/`node_modules` 计数对账（193→225，UNMET 8→0，`git status --short` 的 `D` 计数 0），细节见 COORD-LEDGER (1161)(1162)（KANet-UI 恢复执行记录）。**四条补充纪律（Bettor 1162 原话逐条落）**：
> - **(a) 摘链与递归删除必须是两个显式分开的步骤，第一步验证成功才做第二步**——不能把"扫描完发现没链"直接当作"可以放行删除"的同一次调用里的下一行代码（见下方本条状态注记末尾对 `safe-worktree-remove.mjs` 那次事故的复盘，这正是那次犯的错）。
> - **(b) 只用 PowerShell 建链、只用 PowerShell/`cmd`（`Remove-Item -Force` 或 `rmdir`，都是非递归的单文件/单 reparse point 操作）摘链，禁止 bash `ln -s` 建任何指向 worktree 外部的链接**——本次事故的链本身就是 bash 建的，Windows 上 bash 的 `ln -s` 对目录建出来的东西在底层可能不是标准 NTFS junction（依赖 Git for Windows 的符号链接权限/模拟层），行为比 PowerShell `New-Item -ItemType Junction` 建的更难预测、更容易被后续工具（含 git 自己）错误识别，规则 81 前两次事故的链都是 PowerShell junction 尚且穿透，bash 符号链接没有理由更安全。
> - **(c) 任何递归删除（`git worktree remove`、`Remove-Item -Recurse`、`rm -rf`、`git clean -x` 等）动手前，必须先对目标树跑一遍 `node scripts/check-worktree-junctions.mjs`，确认它报"该树内 0 条 reparse point"（含 internal 和 →LIVE 两类都要 0，不是只看 →LIVE）才能继续**——"记得手动跑"这条纪律 2026-08-29/2026-09-13 两次都没人记得，第三次不能继续指望"这次会记得"，本条把它写成硬性前置动作，且明确覆盖到不止 `git worktree remove` 一种触发命令。
> - **(d) `kasia-console/node_modules/kaspa-wasm → ../../shared/vendor/kaspa-wasm` 这条二级跳转是本仓一个既有的、设计内的脆弱点，不是本次新引入的**——`kasia-console`/`kasia-relay`/`kaspa-scout` 三处的 `node_modules/kaspa-wasm` 全部 junction 到共享的 `shared/vendor/kaspa-wasm`（见本条 2026-08-29 原文"📎 附注"段），任何对 `node_modules` 的递归删除动作（含"看起来只是清一个包"的操作）会顺着这条链**二级穿透**进 `shared/vendor`，本次事故实测坐实（该目录 7 个文件、含一个 11MB 的 `.wasm` 二进制全部被删，后由 Bettor 从 git 索引恢复，未走 npm——这个目录本来就不受 npm 管，丢了没有"重装"这条路，只能靠 git 恢复，风险比 npm 包更高一级）。任何要对 `node_modules` 做整删（`npm ci`/`rm -rf node_modules`/`Remove-Item -Recurse node_modules`）的操作，必须先确认这条链本身有没有被摘除，不能假设"我只是在弄 npm 包，跟 shared/vendor 没关系"。
> - **(e) 侧 worktree 一律不得以任何形式链接主检出的活体 `node_modules`**——需要依赖就在该 worktree 内**独立** `npm install`，不图省事 junction 到主树省安装时间。这不是本次的新教训，是 2026-08-29 那次事故的原教训（该条正文"🔨 怎么做才对"第 4 点已写过"侧 worktree 要跑测试，各自 `npm install --prefer-offline` 独立装，不要 junction 到 live 树的任何目录"），**本次(2026-09-14)在同一条纪律上再犯一次**——KANet-UI 自己为 `scratch/_kanetui_wt_hotwallet` 建的 `kasia-console/node_modules` junction 正指向主检出的活体 `node_modules`（本人全程如实报备过这条链的存在，链本身未被本次事故触发穿透，但它是同一类risk，不该继续留着）。该链计划在下次需要跑测试前按 (a)(b) 两步摘除（`Remove-Item -Force` 摘链 → `Test-Path` 验证已不存在），随后在该 worktree 内独立 `npm install`；当前不紧急处理，先记账。
>
> 🔨 **机制评估：把 `check-worktree-junctions.mjs` 做成 pre-remove wrapper（只写设计，本次不落码，Bettor 1162 派工）**——本条 2026-09-13 状态注记已经点名这个缺口、给了 `scripts/safe-worktree-remove.mjs` 这个名字建议；**KANet-UI 2026-09-13 当天确实尝试落码了一版，而且在这一版自己身上又踩了一次同族事故**，这段历史必须先说清楚，新设计才不会重蹈：
> - **上一版的真实缺陷**（文件现移至 `scratch/_kanetui_safe_worktree_remove_PARKED_2026-09-13.mjs`，已作废、禁止运行）：文档头写"无 `--detach`（默认，只读，不改动任何东西）"，但代码里 `if (!bad.length) { runGitWorktreeRemove(); process.exit(0); }`——**"扫描到 0 条跨树链"这个分支无条件直接执行了真实的 `git worktree remove --force`**，跟文档承诺的"只读扫描"直接矛盾，且这个分支根本不受 `--detach` 参数控制。KANet-UI 当时用一个**真实在用**的 worktree（`scratch/_j2_wt_a_patch`，J2 的工作目录）测试"只读扫描"，因为这个缺陷被无预警地真删了——commit 本身安全（git 历史没丢），但 worktree 工作目录本身丢了，J2 需要另外核实有没有未导出内容因此丢失。Bettor 当场止损裁定：暂停任务，把文件挪到 `scratch/` 并加"禁止运行"头，不 commit，原计划搁置。
> - **这次事故直接印证了新设计必须满足的底线**（不是本页凭空加的谨慎，是上一次真实数据）：
>   1. **扫描（只读）与删除（有副作用）必须是两个不同的、显式的子命令或调用**，不能是同一次调用里"看情况走哪个分支"——0 条链和"发现链但用户没确认"这两种情况在**任何路径下**都不能落到会真的执行删除的代码分支，"0 条链 = 天然安全可以直接删"这个直觉本身就是上一版的死因，改成"0 条链"也需要一个独立的、显式的第二步才能真的删。
>   2. **摘链（若发现链）本身要用 (a)(b) 两条纪律**——非递归 `rmdir`/`Remove-Item -Force`，每条摘完立即回读目标目录条目数与摘链前对比，不一致立即 LOUD 中止（上一版这部分逻辑是对的，没有在这次事故里出问题，予以保留）。
>   3. **任何新工具的第一次验证，只准对着专门为此造的一次性假 worktree 跑，不准对着任何真实在用的 worktree 跑**——上一版的第二个错误（也是更根本的那个）是测试方法论本身：不是"代码有 bug"这一件事导致了事故，是"拿真实目标去试一个从没跑过的新工具"这个操作习惯导致了本可以被 0 成本规避的损失。造假 worktree 的成本（`git worktree add scratch/_test_disposable_wt <任意分支>`，测完 `git worktree remove` 掉，反正是刚造的空壳）远低于真删一个真实在用的树。
>   4. **`internal`（目标在待删 worktree 自己树内）与 `→LIVE`/`external`（目标在树外）两类链的处置强度不同**：`internal` 链本身不构成"删了别人东西"的风险（目标本来就要跟着这个 worktree 一起删），可以在确认过后自动摘除；但 `→LIVE`/`external` 链哪怕已经验证"摘链不影响目标内容"，仍然代表"这个 worktree 曾经依赖外部某处的资源"，自动摘除前应该额外打印一行清晰的"即将摘除一条指向 [target] 的外部链接"提示，不应该在同一次无人值守的自动化流程里悄悄拆掉——摘错一条正被另一个会话依赖的 →LIVE 链虽然不会像本次一样丢数据，但会让那个会话的环境无预警地坏掉，同样值得一条明确提示而非静默处理。
> - 上述四点是**设计要求**，不是代码——本次派工明确"只写方案不写代码"，落码另派、且落码后的验证必须走上面第③点（假 worktree 先测），不得复用第一版的验证方式。
...
- 🔨 **怎么做才对**：
  ...
  4. **侧 worktree 要跑测试，各自 `npm install --prefer-offline` 独立装**，不要 junction 到 live 树的任何目录（`node_modules` / `data` / `logs` 同理）。"把 worktree 放主树外 + `NODE_PATH`"不是出路——**ESM 不认 `NODE_PATH`**，只剩独立装。
  5. **Windows 下的具体死法要认得**：`git worktree remove` 撞被占用文件（`.node` 被在跑进程锁住）报 EINVAL 中止 ⇒ 人手 `rm -rf` 收尾——**git 那一半已经穿过 junction 删了目标**，`rm -rf` 只是把剩下的壳清掉，让现场看起来"干净"。**含 junction/symlink 的 worktree 禁 `git worktree remove` / `rm -rf`，先 `rmdir` 拆链。**
...
## 规则 83 —— 备份/导出/数据库快照落在任何 git 工作树内，即便 untracked 且 `git check-ignore` 无命中，也是一枚"下一次 `git add -A` 就会引爆"的地雷（2026-09-14 · KANet-UI 自伤 · Bettor 1240 独立核实抓到）

- ❌ **错误**：D-019 主网部署前，按 Bettor 要求给 `console.mainnet.db`（含全部 relay 的加密助记词密文）做重启前备份，图省事直接写进了本次证据目录 `docs/provenance/2026-09-14-kanetui-d019-pin-deploy/`——一个 git 工作树内的路径。备份本身逻辑对（用 `better-sqlite3` 的 WAL-aware `.backup()`，sha256 前后核对一致），**唯独落盘位置错了**。
- 💥 **后果（未遂，Bettor 复核时抓到）**：这个 2MB 文件是 untracked，`git check-ignore` 对它**无命中**（不匹配 `.gitignore` 里任何现有模式——它不叫 `*.db`/不在被忽略的 `data/` 目录下，文件名是这次现造的 `console.mainnet.db.pre-d019-backup`）。这意味着：**下一次任何人在这棵共享检出上跑一次 `git add -A`（这个仓库里发生过很多次，含本 session 自己），这份含全部主网账号加密助记词的数据库快照会被直接 add 进下一次 commit，一旦 push 就上了 GitHub**——不是"理论风险"，是"下一次不设防的常规操作就会触发"。
- 🧠 **为什么会犯**：(1) 图路径方便——证据目录本来就要写 provenance README，备份文件"顺手"放同一个目录，心理上没有把"证据文本"和"含密钥材料的二进制"当成两类完全不同风险等级的东西区分对待；(2) 只核对了"文件本身会不会被这次 commit 带上"（我确实检查过 `git status --short` 只显示 untracked，没有 `git add` 它），**没有核对"这个位置本身安不安全"**——untracked 不是安全，untracked 只是这一次没被带上，跟这个位置**未来任何一次** `git add -A` 会不会带上它是两个问题；(3) `.gitignore` 覆盖的是**已知模式**（`*.db`、`data/`、`scratch/`……），一个现造的新文件名天然不在任何已知模式里，"靠 gitignore 兜底"这个假设本身对**新文件名**不成立。
- 🔨 **怎么做才对**：
  1. **任何备份/导出/数据库快照/密钥材料相关文件，落盘前先问一句"这个绝对路径在不在任何 git 工作树内"**——不是问"会不会被这次 commit 带上"，是问"这个位置本身会不会被未来任意一次 `git add`/`git add -A` 带上"。在 git 工作树内 = 危险位置，不管当前是不是 untracked、是不是被 gitignore 命中。
  2. **本项目约定的安全落盘位置 = 不在任何 git 工作树内**（如 `C:\KANet-backups\`，不存在则先建；🔴 **`.gitignore` 覆盖不构成豁免，不要用"反正被 ignore 了"当安全判据**——ignore 规则本身可以被改、被删，忽略的文件仍然可以被 `git add -f` 强制加入，"这个路径当前被 gitignore 命中"只是描述现状，不是任何形式的保证）。判定一个候选位置是否在 git 工作树内，用 `git rev-parse --is-inside-work-tree`（在候选目录里跑，返回 `true` 就不能用）——**光看"这不是 `D:\kanet-tn12` 底下"不够**，本条 2026-09-14 当天就在这条上栽了一次二次跟头：把文件挪到了 `C:\KANet\backups\`，以为挪出了 `D:\kanet-tn12` 这棵树就安全了，没独立核实 `C:\KANet` 自己也是一棵挂着真实 GitHub `origin` 远端且当前分支正在 track 它的 git 工作树——第二个候选位置同样不合格，直到搬到 `C:\KANet-backups\`（`C:\` 盘根本身确认不是任何 git 树）才真正解决。
  3. **写完之后立即用 `Move-Item`（非跨设备时保留同一份 inode，非"删了重写"）挪到安全位置，原地重新算一次 hash 核对搬移没有损坏文件**——不要假设"反正是 untracked，之后再挪也行"，之后 = 可能来不及。
