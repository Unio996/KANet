...
> 📌 **裁定（ledger 1200 · Bettor）**：选 **②**——`poolMerkleRoot`/`committee_hash`/`predicate_commit`
> 保持 ctor-only，T4 对照走步骤 (c) 单源产物逐字节重编译比对 + (d) P2SH 重算（安全承重落在这两步，T4
> v0.2 §1 输入里 T3 v0.2 §1 提过的"Q8：这三个字段对照升 MUST"由此满足，不需要 cheap-tier 才能满足）；
> **cheap-tier 对这三个字段的"每次构造前快查"放弃**（改用全量 full-tier，只在创世那一刻跑一次，日常
> 下注/派彩构造前不重复查这三个字段——跟 §5"填错自毁非阻塞"的整体节奏一致，不是新开一个更松的口子）；
> **"搬进 state（上面的路径 1）"记为主网首个市场创世**完成**之后**的改进项，不在本轮 T4 落码范围内。
