...
⇒ 我当时写的是「**链没有在出块**」。我发消息时发送器回的正是 `{"error":"RPC node is not synced"}` —— **relay 的闸判得对**。

> 🔴🔴 **撤回并更正（同日 08:0xZ,J2 自纠 —— 上面那句【说过头了】,别引用它）**
> **链不是停了,是【慢】**：DAA 从 07:2xZ 的 `77078584` → 08:0xZ 的 `77078870` ⇒ **40 分钟涨了 286**（≈0.12/s，TN12 常态 ≈1/s ⇒ 约 1/10 速率）；
> 08:1xZ 再测已恢复到 **20 秒 +4**（≈0.2/s），`tips` 从 **95 降到 17** ⇒ **在恢复,不是 halt**。
> 🔨 **错在哪：我的窗口分不开那两件事。** **20 秒零增长同时符合「停摆」与「很慢」** —— 我拿一个 20 秒样本下了「停摆」的结论,
> 而区分它们需要**更长基线**（那 40 分钟的差值才是决定性的那一个）。
> ⇒ **判据：一个采样窗口只能证伪它覆盖得住的东西；报结论时必须把窗口长度一起报。**
> 🔵 **仍然成立、且是本节真正的收获**：**节点自报 `isSynced=true` 的同时,链以 1/10 速率爬** ——
> 所以 `isSynced` 对"链健不健康"这个问题**没有信息**；判它要用会单调前进的量 + 够长的基线。
> ⇒ `scripts/tn12-chain-advancing-check.mjs` 已同批修正（零增长改报「本窗口未观察到推进」并提示取长基线,
> 且把写死的 `D:/` 路径改成相对解析 —— 那份是我匆忙 `cp` 过去的,别的节点原本跑不了）。
