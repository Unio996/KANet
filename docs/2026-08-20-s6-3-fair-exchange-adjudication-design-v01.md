...
### §17.20 Codex proactive-unsynced v0.12 verdict = HOLD（2026-08-21·giveup 修法无效·同 Shape-A 塌第二次·+sweep+oauth_cid provenance）
Codex 主动审（bridge 无增量但 bshard-m3-deploy 从 v0.9 fbc11da 进到 v0.12 34629969 ahead 8）：v0.10/11/12 改进真但未 closed。三项：
- 🔴🔴 **MUST-FIX 1（giveup 修法【无效】·同一病第二次）**：v0.11 `T_giveup_LOCKED_F >= T_cutoff_LOCKED_R` **不能**造"reveal 窗关后才 giveup"——SilverScript 不能 enforce reveal 上界，`T_cutoff_LOCKED_R` 只是 LOCKED_R **refund 支的下界、不 disable reveal 支**。过 cutoff 后 LOCKED_R 若没被 refund，reveal 仍结构 valid ⇒ 首动方阈值后仍有 race-依赖选择（有利 reveal / 不利 giveup）= **free-option 没关。lower-bound on giveup 造不出 upper-bound on reveal**。🔴 **这是 Shape-A 塌（§17.17）同一病第二次**：两次都把 `T_cutoff_LOCKED_R` 当 reveal-deadline、实为 refund-floor。J2 提 giveup 修、Bettor 采纳路由、NWT/J1 都过、Codex proactive 逮。**全队反复把 `T_cutoff_LOCKED_R` 读成 reveal-ceiling、它只是 refund-floor。** 修法：free-option 降为 **liveness/race 假设**（reactive/refund 侧须在 race 经济可利用前 LAND/CONFIRM）或**结构 state-transition 让一支真 disable 另一支**；别再用 lower-bound ordering 声称关。矩阵 Fb 排序格改标（今把 lower-bound ordering 当成 disable 对面支）。
- 🔴 **MUST-FIX 2（normative-body 全 sweep·= Bettor ① 项）**：CURRENT v0.12 仍含 Shape-A 正文：§2 `refund:` / §2.5 表 `current_daa >= T_refund_LOCKED_F` + "花 O ⟺ 领 LOCKED_F" / §2.6 旧 COUPLED 行 + 旧静态 ordering / "latest O creation <= T_cutoff_LOCKED_R"（已否决的推理）。在 CURRENT artifact 里、未标 historical、矛盾 §4。Codex 强调**全 sweep 非再局部打补丁**：每 live 段一套拓扑（`LOCKED_F: transition/giveup` · `O_AUTHORIZED: reactive-claim/recovery` · `O: reciprocal`），历史 Shape-A 段显式标 non-normative，phantom current_daa 不留在 current 表。
- 🔴 **MUST-SPECIFY 3（新·oauth_cid provenance buildability 门）**：活先例（covenant 续继）= 续链 output 带一个由 input 授权的 `CovenantBinding`、跨后继保留 input 的 `psCovId`。Shape-B spec 说消费 LOCKED_F(`locked_f_cid`)造 O_AUTHORIZED(`oauth_cid` 另一身份)，但**没给 `oauth_cid` 的 durable 派生/绑定规则、也没证这身份转移被 deployed Toccata 接受**。`OpOutputCovenantId==oauth_cid` 只查结果身份、非证合法创建。冻一个确切机制对 deployed 证：若 O_AUTHORIZED 是 LOCKED_F 续继 ⇒ oauth_cid 是否=续继身份 + 给 CovenantBinding 构造；若新身份 ⇒ genesis/授权机制 + 双方怎么在 reveal 前 bake（reveal tx/outpoint 那时不存在）。
- 派：J1 v0.13 = 三项（giveup 真修=liveness/race 或结构 disable · normative-body 全 sweep · oauth_cid provenance 对 deployed 证, 先裁可建）· J2 矩阵 Fb 格改标 + 据真修重建 · NWT 红队。别急送 Codex。
- 🔨 Bettor 认：我采纳 giveup 修法路由时没核它是否也犯 Shape-A 同病（lower-bound ordering ≠ upper-bound disable）；全队第二次栽在"T_cutoff 是 refund-floor 非 reveal-ceiling"。入 memory 防第三次。

### §17.21 🏆 Codex v0.15 verdict = CONDITIONAL DESIGN-LAYER CLOSE（2026-08-22·同链 C4-FINALITY Shape-B 设计层条件闭合·里程碑）
Codex 正式复审 v0.15（RESPONSE-MSG266）:**同链 C4-FINALITY Shape-B DESIGN-SPEC: CONDITIONALLY CLOSED**（条件于冻结的 §1.5 假设）。Codex 独立重读 v0.15 规范正文 + J2 重建矩阵（不依赖 grep 输出）:branch set 已 Shape-B 内部一致（`LOCKED_R{reveal-transfer,terminal-refund}` · `C{reveal-continuation,terminal-refund}` · `LOCKED_F{transition→O_AUTHORIZED,giveup}` · `O_AUTHORIZED{reactive-claim,recovery}` · `O{reciprocal-claim,recovery}`），旧 Shape-A 承重 claim 不再扛规范证。
**设计层闭的 5 条**: ① reveal 侧盗窃缝——领 LOCKED_R 支自身要求同笔消费 unique C + exact LOCKED_F、造真 O、造 exact O_AUTHORIZED 续继 ⇒ 排除"付首动方却留旧本金" ② 反应方反向焊——O 与 O_AUTHORIZED 互 co-input + baked recipient/value ③ 实际-reveal 锚——recovery 锚 reveal-created 对象的 consensus-visible DAA、非不可 enforce 上界（晚 reveal 平移窗非缩窗）④ 同链 reorg 耦合——反应方消费 reveal 后代、reveal 消失则后代不能独立存活 ⑤ P-SAFE-1 本地正事实。
🔴 **精确 scope（Codex 强调·不准读大）**:
- no-theft = **条件于 active-timely 反应方 confirm + frozen entropy/secrecy/finality 假设**、**非无条件 fair exchange**。
- free-option（`Fg` cutoff 后残留）= **OPEN 经济/ordering/liveness 残留、非 principal-theft 缝**（v0.15 措辞已对；**禁**升级成"结构消除"或"无条件"）。
- 矩阵 = coverage aid **非证明**（pairwise ≠ ≥3 支组合；每 WELD/EXCL/COUPLED 标签待 `.sil` 存在后由交易级/变异负测杀）。
