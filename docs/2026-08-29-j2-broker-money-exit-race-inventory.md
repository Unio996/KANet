...
## §11 v0.5 · (c) `_autoSettleAsset` 的 reopen→再付敞口（机械链，供 NWT 定级）+ P7-bis 落码指针
- **`_autoSettleAsset`**（`tpf:2833`）：taker 侧对 `verification='kaspa_tx'`（及任何 `isSupported(want_asset, want_chain)` 的资产）在 accept 后**自动把 want 资产（KAS/USDT/USDC）付给 maker**——它是 taker 的"付款"（exchange 语义），**不是** maker 的交割（交割 = `_verifyAndComplete` `exchange-machine.js:975`，写 `delivery_tx`）。触发：E 路 `tpf:2075-2077 setImmediate(_autoSettleAsset)`（`handleExchangeAccept`，`localRelay && !is_dex_broker`）/ H 路 `api/exchange.js:545-552 _autoSendKas`；两路共用 §9.2 的 `processAccept` 闸 ⇒ 同一 accept 一次。送后写 **`payment_tx`**（`tpf:_autoSettleAsset` 尾 `UPDATE exchange_offers SET payment_tx = ? WHERE id = ?`，与 `_autoPayExchange :2745` 同列）→ 广播 `paid_v1` → `processPaymentSubmit`。
- **敞口形（与 P7-bis 同构）**：KAS/资产已发出、`payment_tx` 已写，但 `paid_v1` 广播失败/迟到 ⇒ offer 仍 `matched` ⇒ 30 min 超时 ⇒ **两处 reopen 之一**清 `payment_tx`（`exchange-machine.js:730-737` 本地 tick / `tpf handleExchangeTimeout` 对端 timeout_v1）⇒ 再 accept ⇒ `_autoSettleAsset` 再付一次 ⇒ **同 offer 两笔 KAS/资产出**。触发条件同 P7-bis 三件（付了 / paid 没落 / 超时），路径可达性同：本地非 broker agent 经 UI/API 或链上 accept；retail broker 路 `is_dex_broker=1` 豁免 E 路。**maker 交割（`delivery_tx`）不在 reopen 窗内**（两处 reopen 都 `WHERE protocol_status='matched'`，交割发生在 verifying/delivering 之后）⇒ "re-deliver"不成立，成立的是 **"re-pay"**（taker 侧）。
- **结论**：敞口 = P7-bis 同一条（列同、reopen 点同），**已由同一道门覆盖**（`guardReopenIfSettled` 判 `payment_tx OR delivery_tx`）；定级建议与 P7-bis 同 **P1**（非 retail 可达），不单列 P0。
- **P7-bis 落码指针**：batch-2 `coord/broker-money-path-2`（sha 见 Bettor 回执）：`exchange-machine.guardReopenIfSettled` + `checkMatchedTimeout` 门在 `timeout_v1` 广播之前 + `tpf.handleExchangeTimeout` 门 + UPDATE `AND protocol_status='matched'` CAS；向量 `exchange-machine.reopen-guard.test.mjs` 6/6（真 schema，tpf 路经 `onBroadcastWritten` 真派发）；突变：去 `delivery_tx` 半边 ⇒ V3 红；去 tpf 门 ⇒ V5/V6 红。
