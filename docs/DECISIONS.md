...
3. **部署**：随**维护窗** relay 重启生效（`docs/2026-08-28-postsync-maintenance-window-runbook-v0.4.md` §red-line-7-observe），窗在本机 READY + J2 T+125 证据之后；IBD 期不动任何进程。
   ...
   > 📌 **维护窗批次清单（2026-08-29 06:0x Z · Bettor · 出处 COORD-LEDGER (708)(709) · 全部 NWT GREEN · 落地 = READY 后 Owner 批的一次窗）**：① 红线 7 observe（本条）；② `57fde30f` console 启动期 fail-fast（**已因 05:14Z supervisor 重启风暴提前上线**，NWT 事后 GREEN，不回退）；③ **supervisor v0.1.3**（`docs/2026-08-29-j2-supervisor-lifetime-storm-guard-and-boot-grace-design.md` + provenance `docs/provenance/2026-08-29-supervisor-v01/`：write-ahead 状态文件 / lifetime 风暴保护 / PID-aware boot grace 300 s + CreationDate 防复用 / `index.js` boot-marker + 心跳 gate on `__booted`；selftest 48/48 J2 5× + Bettor 3× + NWT 5×；**apply 用 MANIFEST sha256 钉死的候选全脚本，不 `git apply` 带 scratch 前缀的 diff**）；④ runbook v0.5.1（`//F //T` + stop 后 0 relay 双判据验证闸）；⑤ llama loopback 验收（`a5-verify.ps1`）+ 防火墙 llama 规则 Disable；⑥ watchdog v0.4 enable（D-013 §3 五前置）。J1 角色 B 提权执行暂停期间，提权步骤由 Bettor 令下逐条指派。
