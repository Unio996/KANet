...
> 🟢 **节点健康三层厘清(COORD-LEDGER (467)-(469))**: ①**共识层健康**(DAA 严格单调, J1 笔记本 + console 节点隧道读两台皆进)②**RPC isSynced 层闪烁**(同一 console 节点: KANet-UI 本机读 false+stall vs J1 隧道 same-period all-true ⇒ 两 host 读一节点得反结果 = **测量层差异非节点降级**; isSynced=false=RPC 层 slow-production 间歇非共识降级; 已持续~2 天=非新降级)③**注册层不依赖节点**。
