...
### D-034 KANet 核心 = 通用经济分润原语（可编程经济权益），broker 是其第一个应用；三层结构；第一步 = Kaspa 即时分账结算模板；本工作线由 Owner 亲口开闸 (2026-09-27 · Owner 本机终端原话「我们有exchange组件，完全可以把usdt兑换成kaspa（那个组件再发展可以让几乎所有主流币都直接兑换kaspa），这就解决问题了。我自己提供劳动，收kaspa，不存在不合规问题。先干起来！按你思路干！」；前序 Owner 原话「broker是一个通用的经纪人……撮合卖家买家，并赚取佣金」(2026-09-26)、「这个功能继续扩展就越来越像信托」(2026-09-27) · Bettor 起草记账)
1. **定位**：KANet 的核心产品是任何人/应用/行业可直接调用的去中心化分润协议（通用经济分润原语；北极星表述"可编程经济权益"）。broker（撮合买卖、赚佣金）是它最自然的第一个应用。承接 Owner 2026-06-22「分润=社会资源协调，任何行业可用的独立组件」、2026-07-12「佣金自动分发独立抽象、第三方可用」、2026-07-15「每个应用做为 broker 一端」、D-012 四角色。
2. **三层结构，单向依赖**（规则 → 执行 → 接入；下层不认识上层）：
   - **规则层**：复用 `packages/fee-split`（`feeSplit` / `validateFeeRules` / `canonicalizeFeeRules` / `computeFeeRulesCommit`）为唯一分配算法与规则承诺，不重写。
   - **执行层**：按资产选执行方——KAS：Kaspa covenant 结算模板（本决定第一步）；其他币：经 exchange 组件（Maker 角色）兑换为 KAS 后进入 Kaspa 结算（Owner 定：消费者付什么币都行，由 exchange 换成 KAS）。每种执行方式须标明信任边界；跨链兑换在 KAS 侧硬锁落地前不得称"原子结算"。
   - **接入层**：SDK + 标准 JSON 配置 + 配置页（后续步骤）。
3. **结算组织方式**：每笔订单（或每个长期分成周期）一个独立 covenant 地址，规则与资金同在该 UTXO；不设共享大金库。据此落实 D-025 五条（现值即真相 / 地址可推导 / 自证回执 / 一步一花 / 退出写进脚本）及 §6 两条。
4. **只做两种模板**：A 即时分账（先做）；B 条件分账（锁定 → 事实/证明 → 分账，超时退款；事实源与挑战窗按 D-025 §6）。预测市场现有结算路径不迁移。
5. **第一步（开工）**：Kaspa 即时分账模板，SilverScript v1.0.0（D-019 pin），先在 simnet 真共识验证；必测对抗：篡改收款地址 / 篡改比例 / 重复支付 / 服务离线仍可退款 / 小额份额触及 storage mass 上限的处理。流程：J2 设计稿 v0.1（含"查现成"清单）→ NWT 攻击面审 → J2 实现 + simnet → NWT diff 审 → 合入（按 Owner 常设授权；部署另批）。
6. **与 D-033 的关系**：D-033 冻结其余新功能仍有效；本工作线是 Owner 亲口开闸的例外，范围以本条为限。`docs/GOAL.md` 只由 Owner 修改，Bettor 不改。
7. **第一阶段收敛（2026-09-27 Owner 贴入方案，Bettor 并入）**：
   - **一句话目标**：任何人都能在 Kaspa 上创建一份分润协议，任何参与者都无法单方面改变它，任何人都能独立验证并执行它。
   - **必须是链上强制分账**，不是钱包按计算结果自愿付款。"即时" = 不等外部 Oracle/履约判断，不要求锁定与释放同一笔交易（正常路径可两笔）。
   - **边界锁死**：仅 KAS；分润计算复用 `@kanet/fee-split`；SilverScript 固定模板；参与者 = 商家 / broker / 引荐人；无外部 Oracle；超时退款 + 明确手续费规则；验证只靠已知协议地址、当前 UTXO、交易回执；接入 = SDK + JSON 配置 + 最小网页组件。不做长尾金库、多级推广、ZK 批量结算，不引入新代币。
   - **三条验收（完成标准，不是优化项）**：① 规则不可篡改——锁定后任何人不能加佣金、换收款地址、改输出截留；② 资金出口完全自主——broker 与 KANet 服务全部离线时，其他参与者仍能构造合法交易完成分账，或超时后完成退款；③ 第三方独立部署——全新开发者只凭开源组件与公开配置即可创建协议、完成分账并核验，不需要我们的数据库或管理员授权。全部先在测试链执行，再由 NWT 攻击面审。
   - **后续顺序**：② USDT 支付适配器（复用 exchange，独立 Maker 提供 KAS 流动性；跨链锁定/退款/交割保障单独验收）；③ 法币仅 Track B（标准适配接口 + 持牌第三方支付机构验证，不自营法币收付）。
8. **商品佣金计划 + 多渠道归因，灵活性放宽（2026-09-27 · Owner 贴图「一笔交易，多个渠道共同参与」（商家定渠道佣金预算，邮件/视频/网站渠道按有效归因领取，未用佣金事先约定去向）+ 原话「我认为灵活性可以再适当放宽。总之，任何人拿着都可以快速部署，快速使用。」）**：
   - **报价层**：商家签名的 JSON 报价（价格、渠道预算、角色与比例、未用佣金去向、有效期、可选渠道白名单）；**归因链接** = 报价 + 渠道地址，邮件（HTML 卡片+链接）/视频（简介链接）/网站（JS 组件或 iframe）共用同一格式；**开源纯静态结账页**，任何人可托管，订单地址只由报价+链接参数推导，渠道可自行核对归因未被抹掉。未归因份额由 `feeSplit` 事先按报价规则并入指定方后烤入合约，合约不感知归因。
   - **放宽方向（不破 §7 三条验收为界）**：收款方数量与角色名不写死（上限由实测 mass 决定）；收款地址不限 P2PK（任意脚本地址）；`fee-split` 的 provider≥50% / 单角色≤50% 从硬限改为可配置的默认护栏；渠道默认无需注册（身份=地址），白名单为商家可选项。
   - **快速部署**：交付物以"拿来即用"为准——SDK 一个包、一份 JSON 配置、一个静态页即可跑通建报价→分发链接→下单→分账/退款→核验；不需要我们的服务器、数据库或授权。
   - **渠道押金（可选，2026-09-27 · Owner 原话「复制这个并添加新的broke地址用户，自己也需要往合约里面注入1个kas」+ 确认「A，按你建议写进设计」）**：用途 = 加入门槛与防刷（防批量造假地址抢归因），**不是**手续费储备（订单手续费仍由订单资金自负，D-034 §5/§7 不变）。形态：每个渠道/broker 一个押金 covenant 地址，默认 1 KAS，**可退回、只有押金人本人能取回**，任何人可核验其存在；报价里由商家选"要求渠道押金 / 不要求"，**默认不要求**（保持拿来即用）；要求押金时，未持有有效押金的归因链接视为无效归因。罚没条件本版不设（作弊判定需另行设计，未设计前押金只作门槛）。
...
2. **冻结的正当触发只剩基础设施故障**：数据源不可达/题目规格损坏/过截止未判/封盘太晚装不下争议期/系统故障。用户与运营者都没有冻结按钮（现状已如此，保持）。
   > 📌 状态注记（2026-09-22 · Bettor · NWT 设计审 SHOULD · COORD-LEDGER 1634）：单口径设计 v0.2.2 在"系统故障"类下新增两个冻结 reason——`unexpected_verdict_kind`（出现不该存在的第二裁判票，纯 fail-safe）与 `event_identity_mismatch`（判定时抽到的参赛方 ≠ 建题冻结的事件身份，数据源换了事件）；均属基础设施故障延伸，不是"分歧"出口。
3. **执行（设计先行 + D-031 先查已有）**：现有闸 = proto-oracle-spec.mjs 五必填字段校验、判定来源仅 ESPN 结构化算术 + Polymarket 条件、coingecko 无 predicate 不开自动判定、截止时间预留争议期。派 NWT 红队题「用现有建题接口出一道会被冻结的题」，能出即缺口；缺口优先收紧现有校验与来源名单，不另起。排序：F3/F4 之后、simnet 异常臂之前。
4. **退款路（R-a~R-c）继续做完**：它是故障兜底，不因本决定停；但对 Owner 的口径从「异常路自动化」改为「故障兜底自动化」，主线重心前移到出题端闭环。
5. **不影响**：D-029、D-022、D-025 五不变量（本决定与「退出写进脚本」同向）。

   > 📌 状态注记（2026-09-22T10:30:59Z · Owner 本机终端「对齐没问题，D-032 按单口径出设计稿」· Bettor 记账 · COORD-LEDGER 1630）：**方向已拍 = 一题一个裁判（单口径）**。依据 D-031：老系统 voter 本就一市场一源（voter.js:748–784 按 outcome_market_source 二选一），批 B 把 ESPN 谓词与 Polymarket 条件同时挂一题是新造的轮子，也是 NWT 红队（1625）三处缺口的共同根因。设计稿 `docs/2026-09-22-bettor-d032-single-judge-question-closure-design-v0.1.md`（待 NWT 审）；Polymarket-only 口径另票。

### D-031 绝不轻易新造轮子 = KANet 第一设计原则;所有接位文件 + 设计审核第一条;再犯立即出局 (2026-09-20 · Owner 本机终端原话「kanet系统设计原则就是绝不轻易新造轮子…放在所有接位文件第一条,设计审核第一条,谁他妈再犯,马上滚蛋」「这都是我想到的、能记得的,我都不知道kanet你们还漏了多少东西,纯纯的瞎几把乱搞一通」· Bettor 记账)

1. **决定**:动手 / 设计前**必先查**(仓库 grep/find、KB `D:\KANet-Knowledge-Base`、`docs/DECISIONS.md`、既有模块 / 设计文档 / **在跑的系统**)有无现成可复用;有 ⇒ 复用 / 接线 / 迭代,**不重画、不重写、不另起**。**设计与设计审核的第一条 =「这东西是不是已经有了?复用了吗?」** 答不出"查过、确实没有"就打回。违者立即出局。
2. **"重新指向 / 迁移一个已跑通的模块" ≠ "重新设计"**——别把前者做成后者(电报模块教训)。
3. **落地**:CLAUDE.md 顶部(第一原则,置铁律 0 之上)、`docs/DEV-FRAMEWORK.md`(第一原则 + ② 审核第一判据)、5 个角色接位文件(Bettor/J1/J2/KANet-UI/NWT)第一条——均已加。
4. **结构性治本(不止规矩)**:建 **KANet 能力 / 资产总清单**(系统清点已发起,全仓 + 文档 sweep),列已有模块 / 在跑系统及状态 / 接口 / 表 / 资产 / 用户功能,作为"查有没有"的权威对照;根治"Owner 是唯一记得已有东西的安全网"。
5. **触发**:proto-v0 推倒测试网可跑结算重写(Owner 后知);冷存资产不可见(D-028);**测试网上成熟在跑的电报模块**被 Bettor 另起设计稿(已撤)——"重新指向"误当"重新设计"。

### D-030 全开发深度采用 TypeSafe(System One/Jev) advisory 技能；只做参谋、拿不准弃权；绝不进 settle/钱路 (2026-09-20 · Owner 本机终端原话「我希望我们在整个开发过程中，深度用上这个技能」「这个对我们开发 kanet 提高效率效果实在是太重要了」· Bettor 记账+实测)

1. **决定**：TypeSafe 技能（插件 `typesafe-ai`，`用 TypeSafe 技能` 触发）作为 KANet 开发的**常设能力深度采用**。模型（旗舰 Jev）返回类型化判断 + 置信度（Choice 选一个 / Noul 是否成立的概率 / Score 有序程度），代码掌流程，低置信度弃权、升级给人。
2. **红线**：只用在 **advisory 面**（判定赢家的建议 / 可疑市场标记 / NL 命令路由 / 市场描述→结构化参数抽取 / 证据核验升级）；**绝不进 settle / 钱路 / 出口闸**——结算必须确定性 + fail-closed，AI 弃权 = 不给答案，而结算不能没有答案。
3. **数据边界（D-021）**：只把非敏感 / 合成 / 公开的判断输入发给托管服务 `api.typesafe.ai`；`TYPESAFE_API_KEY` 已注入本机 env，**绝不入库 / 日志 / 公开仓**；真实资金规模、内网入口、密钥一律不发。
4. **实测（2026-09-20，合成数据）**：清晰证据 → winner=YES conf 1.0 / resolvable 0.96；模糊证据 → winner=UNRESOLVABLE conf 0.99 / resolvable 0.04（"拿不准就弃权"如设计）。约 500 in / 60 out token/次。
5. **排序**：**不改 D-029**——主网结算（今天）仍第一优先；TypeSafe 作为其后第一个铺开的能力。各处采用前走既有门（设计 → 审 → 实现 → 审 diff → 合），confidence 阈值在真实数据上校准。

### D-029 唯一目标 = 批 9 接线，让主网市场真能结算；其余全砍 (2026-09-20 · Owner 本机终端原话「主网结算能自动跑起来（批 9 接线），让市场真的能结算。该砍全部砍，就搞这个。」· 此前原话「我们必须好好对齐目标」「主要是没有焦点」· Bettor 记账 · COORD-LEDGER 1595–1596)

1. **唯一目标**：批 9 接线跑通——9-2a 出口按结算标签分闸 → 9-2b 驱动分支（指针 → C1 链上事实 → builder → 广播，开关默认关）→ 9-3 `/resolve` → 9-4 隔离 simnet console 端到端四步结算 → 主网 relay 重启 + Owner 批开关。
   > 📌 状态注记（2026-09-22 · Bettor 核 · COORD-LEDGER 1634）：**9-3 `/resolve` 从未实现**——`api/proto.js:311` 仍调必抛的占位 `buildAndBroadcast`（501、不写库）；主网 a59c7b48 的胜方是 1602 所记 Bettor 受控手写。按 D-032（运营者无裁决按钮、判定题无人工出口），**默认不再实现人工 /resolve**，主网新建市场须为单口径判定题；见单口径设计 v0.2.2 §2.7。Owner 若要保留人工裁决路径请否决。
2. **砍掉 / 冻结**：P2 开机自启 runbook、P3 内存哨兵、D-028 冷存可见上线、D-027 ② 握手开关笔二——后两者代码已写完，随批 9 上主网那次 console 重启一并合入生效，此前不审不动。
3. **流程**：每笔一轮红队审只报 MUST，SHOULD 记票；十秒能做的事不出设计稿不送审；回执只给测试末行 / 变异一行 / lint 末行；账本按里程碑记。
4. **电报机器人**：维持 D-024 暂停；其代码不调用 proto v0 接口、默认连已下线的 :3200，接主网只能回话绑地址，不能下注 / 建钱包 / 收结算通知 / 看到活市场；批 9 结算跑通后再立"bot 接 proto v0 API"项。
5. **不影响**：D-026 / D-027 ① 已落地项。
...
2. **五条不变量（J1 归纳，Owner 认可）**：
   - **现值即真相**：任一时刻全部事实（第几期、已付、剩余、条款承诺、收款人）都在当前那一个 UTXO 的 state 里，不靠回放历史；Kaspa 会剪枝，靠历史的设计迟早要靠归档节点或索引器。验收：只用当前 UTXO 一次查询即可回答全部状态问题。
   - **地址可推导**：金库地址 = f(条款承诺, 版本模板哈希)。验收：给任意第三方一张公开参数表，他不问我们就能算出地址并查到当前状态。
   - **自证回执**：我们 API 每条响应带 covenant id、outpoint 与可重算参数。验收：对方拿任意 Kaspa 节点一次查询可复核（照抄 dotk openapi 标准）。
   - **一步一花**：每期释放必须花上一期 UTXO、产出下一期。验收：重复领取在共识层被拒，无任何对账表。
   - **退出写进脚本**：期满退款、标的失效停止释放、争议窗全是脚本分支，不是运维动作。验收：不存在"需要我们按按钮"的路径；否则我们就是那个索引器。
3. **Bettor 补三条**：
   - **状态要装得下**：state 字段必须在 covenant state 大小与节点 mass 上限内，每期续约交易的 storage / compute 在 simnet 节点实测 < 475,000（沿用结算线的精确断言）——"现值即真相"不能靠把 state 撑大来实现。
   - **最小信任层要隔离**：每期"有效播放量"这个外部事实去不掉最小信任（链保证钱按规则走，保证不了数据是真的）；设计上 attest 只能解锁**当期 ≤ 上限**的额度、不能改收款人、不能改条款——事实源作恶或失联的最大损失 = 当期额度或按退出条款退回，不能是整池。
   - **验证接口写明**：核验一律用我们自己节点的 wRPC（`api.kaspa.org` 不返回 covenant 字段），文档写明用哪个调用、怎么算、怎么比。
4. **口径**：分成执行可验证，播放数据最小信任（KB 既有口径）。
5. **不影响**：主网结算后半程与 `kanet.k` 注册两条线的当前排期；本决定作用于 J1 的金库设计票（(1509)E）。
6. **补两条（2026-09-26 · Owner 本机终端原话「把这两点补进 D-025」· 来源：Bettor 外部类似设计调研，EIP-5732 / UMA Optimistic Oracle · COORD-LEDGER (1669)）**：
   - **承诺绑定提交人**：条款"先承诺后揭示"时，承诺哈希必须把提交方身份（出资方地址或其 covenant id）连同随机盐一起算进去（即 `H(条款 ‖ 盐 ‖ 提交方)`），揭示时由脚本核对揭示者就是承诺里那个提交方；盐不得复用。防他人抄走链上承诺抢先揭示（EIP-5732 明写的坑，dotk 未加盐亦属同类）。验收：第三方拿同一承诺与正确条款/盐、但用自己的身份揭示，共识层被拒。
   - **attest 加挑战窗**：委员会对"当期有效播放量"的 attest 落链后不立即解锁额度，先进入一段挑战期；期内任何人可押保证金对该 attest 发起挑战，被挑战的当期额度冻结走争议分支（争议窗属"退出写进脚本"一条），无人挑战则期满解锁。防委员合谋虚报（UMA Optimistic Oracle / KPI Options 通行做法）。与"最小信任层要隔离"叠加：即便挑战失灵，损失仍封顶当期额度。验收：挑战期内领取当期额度被共识拒；带保证金的挑战能把当期额度转入争议分支。
...
3. **`RootClaim.sil:103 require(payout >= 1000)` ⇒ 直接删除**（Owner「最简洁」）。
   > 📌 状态注记（2026-09-22 · Bettor 核 · COORD-LEDGER 1638）：**本条未执行**——`kasia-console/src/lib/RootClaim.sil:103` 仍是 `require(payout >= 1000)`（文件自 09-15 未动，09-16 起无提交触及），JS 侧 `proto-settlement-inputs.mjs:17` 的 `CLAIM_PAYOUT_MIN = 1000` 守卫与之镜像并 fail-closed（J2 2026-09-22 F1 对抗重跑 FZ7 撞上，600 的池被拒签）。属合约改动（换模板哈希、仅新市场生效、须 simnet 真共识 + NWT 审 + mainnet-sil-set.json 与 provenance 同步），排在 D-032 实现之后单独立票；执行前 min_bet=1 的新市场若总池 < 1000 会锁死，运营须避免。理由：该 1000 是代币化前 sompi 时代遗留字面量，与 API 默认 min_bet = 1 冲突；payout = 0 的 claim 只浪费调用者自己的手续费，不造成他人损失，不值得为此保留一条门槛（同 D-017 铁令：约束先问"防的真实损失是什么"）。与 partial 多赢家自续约偏移修复（同 1469 ctor 烤入手法）**合并一次改动**，仅适用新市场；修后须在 simnet 用多赢家 partial 形状真跑。
...
5. **变更纪律**：换锚点 = 全部 provenance 失效，须 NWT 全量重编对照 + D-019 状态注记；不得静默升级编译器。
   ...
   > 🔴 状态注记（2026-09-14T11:33:07Z · Bettor · NWT 确认 · COORD-LEDGER (1340)(1341)）：**主网集 = 11 个 .sil，不是 10 个**——本条 §1 与 `kasia-console/scripts/mainnet-sil-set.json` 漏了 **dust ticket 合约**（`ShardLeaf_direct.register_append` 经 `ps_tmpl_hash` 铸出、`RootClaim.claim_draw` / `RefundClaim.refund_payout` 以 `ticketInIdx` 读回并消费的 spent-once 票），且清单 `excludedLegacyExamples` 把 `src/lib/PoolSide*.sil` 整族误归旧 rolling 家族排除。真票语义 = `src/lib/PoolSide_v08_shard.sil`（4 字段 + `authorize_spend(sig bettorSig)`），生产 TN12 用 legacy 编译器（非本 pin）算其 ps_tmpl_hash。**警示**：T3 provenance 全套向量用 `PoolSideStub.sil`"替身"（`#[covenant.singleton] passthrough`，只有续约无任何 entry/出口）派生 ps_tmpl_hash——NWT 确认：若该 Stub 模板被烤进主网 genesis，票永远花不掉 ⇒ **赢家永远 claim 不了、资金死锁**，与 (1338) 孤儿筹码同为"合约只有续约没有出口"病灶；且 ps_tmpl_hash 为 ctor 常量，烤错不可回滚。**处置**：J2 迁 `src/lib/sil-v1/PoolSideTicket.sil`（v1.0.0 语法、语义逐字不变）→ 本 pin 编译 → 清单第 11 条并移出 excluded → provenance 补真票消费向量（正 1 负 2）→ NWT 审（连带追 `pool-bshard-market-setup.mjs` 生产调用点 `poolSideSilPath` 指向）；**NWT GREEN 前，任何 genesis 不得烤 ps_tmpl_hash**。换锚点/换编译器纪律（§5）对第 11 个合约同样适用。
   > 🔴 状态注记（2026-09-14T11:44:00Z · Bettor · J2 查证 + NWT 独立核 · COORD-LEDGER (1342)(1343)）：**T3 全集的"真实签名通过"路径离线从未被证明，唯一可证处 = 主网真链。** 事实：cli-debugger `D:/silverscript/debugger/session/src/*.rs`（6 文件）零 checkSig/schnorr 实现（编译器 `compile.rs:3805` emit 真 `OpCheckSig`，缺的是 debugger 执行层的签名数学；是"完全未实现"还是"有引擎但 harness 造不出可验 sighash"未查到底，不影响结论）；T3 全部 checkSig 向量遵循"reaches_sig_gate / placeholder sig"约定、expect 全 fail（provenance 14 文件）。覆盖面（勿只锚定 ticket 一处）：`PoolSideTicket.authorize_spend`、`KanetTokenClaim.spend`、`RootClose.close_commit` 4-of-5（含 §5 单 keypair 模拟委员会）——**三处签名门离线仅证"到达 checkSig 行并按预期失败"**。⇒ **GO-F 金丝雀纪律（最小金额 · Owner 单独批广播 · 逐笔 landed 独立核）覆盖全部签名门，不只 claim 路径；原型首次真实下注 → 结算 → claim 即 T3 真签名路径首次实证，不得当普通功能测试。** 仓库内不存在其他能做真实签名校验的离线工具（NWT 核：无 regtest 节点 / kaspa-wasm 验签工具）。
   > 🔴 状态注记（2026-09-14T16:33:21Z · Bettor · J2 实测 + NWT 独立复现 · COORD-LEDGER (1390)-(1392)）：**"T3 已验证 ShardLeaf_direct + KTT 代币化"的范围须收窄**：T3 provenance `ShardLeaf_direct.reference.ctor.json` 中 `token_tmpl_hash`/`market_id`/`ps_tmpl_hash`/`shard_pool_id`/`rootclose_tmpl_hash`/`rootclose_init_payoutRoot` 六个 byte[32] **全为零占位**，T1 provenance 的 KTT `market_tmpl_suffix` 为 5 字节任意 marker——两合约**从未真实交叉编译对接**；既有向量只证"给定固定值时 entry 比对逻辑跑得通"，**不证"这些值能否被真实、自洽、无环地生产出来"**。实测：ShardLeaf_direct 非-State ctor 字段（除 market_id）均烤进模板 ⇒ 整段模板逐市场不同，"模板 hash 为协议常量"前提不成立；KTT 文件头 A″"市场模板稳定 ⇒ 单向无环"系纸面论证。**当前判断**：KTT `ownerIsMarketInput` 为尾切片等值（`:58`），ShardLeaf_direct 存在与所有 ctor 取值无关的固定 132 字节尾 ⇒ 若该尾在 silverc v1.0.0 可编出的 covenant 集合中唯一，则无环且仍为编译器强制；**唯一性未证前，主网集不得创世**（market_genesis 暂停）；若不唯一 ⇒ 合约层手术（四字段进 State 或 KTT 侧改）→ NWT 红队 → Owner 批。本条与 §5 换锚点纪律同级。
   > 🔴🔴 状态注记（2026-09-14T16:45:21Z · Bettor · NWT 构造性证明 · COORD-LEDGER (1395)）：**尾唯一性问题被更根本的发现取代——KTT (b-in) 模板锁可伪造**：`KanetTestToken.sil:34` 的 `want = next_states[j].owner` 由花费者自填 ⇒ `OpInputCovenantId(idx)==want` 为自指重言式；`okTail` 尾匹配对 fork `ShardLeaf_direct.sil` 单行削弱（去 solvency / 去 ZERO32 守卫）原样编译**不设防**（公共后缀 634/403 字节）⇒ **T1 v0.6 H1(a+b)"接收方必须是 KANet 市场模板"不成立**；(1393) 的"11 合约互不撞尾"只证按规矩写的合约互不撞，不是攻击者面对的约束。**处置**：market_genesis 维持暂停；KTT (b-in) 分支标记"不提供合法性分类保证"；修法（want 绑定不可伪造锚：genesis 承诺的合法市场 id 集合写入代币 State 或全模板 hash 匹配替代尾匹配）= 合约层变更 → Bettor 方案 → NWT 红队 → **Owner 批** → 全部向量重跑。严重度作用域（真实市场持币能否被拉走）待 NWT 再核后定。D-018 §6"仅对自发免费无限铸造测试币成立"的边界在此再次生效：本缺陷在零价值代币 + 单操作员下无经济损失，但任何第二方参与前必修。
   > 🟢 状态注记（2026-09-14T16:50:13Z · Bettor · NWT 追证 · COORD-LEDGER (1396)）：**严重度收窄定案**：真实市场（ShardLeaf_direct）已持有的 KTT **攻击者拉不走**——`register_append` 输出 owner 硬编码自身，`convert_to_rootclose` 目的地由 `rootclose_tmpl_hash` 全模板哈希 pin 钉死为真 RootClose 编译产物；NWT 自纠：`OpOutputCovenantId` 只是簿记形式校验（rusty-kaspa `covenant_id.rs`），真守卫从来是模板哈希 pin。(b-in) 缺陷 = "不提供接收方是否合法 KANet 市场的分类担保"（钓鱼类：代币可被存进自建假市场；D-017 零价值免费铸下无经济损失）。**金丝雀（仅 genesis + register_append）在单操作员 + 零价值代币 + 无第二方下资金安全风险不因此上升**；`RootClose.convert_to_claim`/`convert_to_refundclaim`/`PayoutShard.absorb` 同款追踪 = resolve/claim 实现前阻塞项。(b-in) 修法（want 绑真锚）仍按铁律 0 走，**任何第二方参与前必修**。金丝雀先行与否 = Owner 拍。
   > 📌 状态注记（2026-09-14T16:54:10Z · Bettor · NWT GREEN · COORD-LEDGER (1398)）：**修法方向定案（实现仍须 Owner 批）**：v0 = **②** `market_tmpl_hash` 从 KTT ctor 挪进 State（genesis 填）、(b-in) 改 `readInputStateWithTemplate` 全模板核（消编译期环、收口 tail forge）**+ 链下市场注册表**承担"合法市场"分类（P3）；**③ 全局权威签名**（= 本条 (1043)(1044) 注记曾降为后备的方案 D）保留为备选，若启用须比照 committee 模型 **M-of-N 门限、不单钥**。**硬触发点（可判定，写进 canary 范围文档作自查）**："一旦出现第三方能绕开官方 UI 自己拼交易的能力（公开 SDK / 开源客户端 / 任何直发合约路径），链下注册表当场失效，③ 或等价链上强制从后备变硬阻塞。"合规口径补注：covenant_id genesis 只要求公式算对，任何脚本（含空壳）都能取得非 ZERO32 id ⇒ D-017 §2 "只许 covenant 持有"由协议层免费成立，与 (b-in) 判据形式无关。三文件（RootClose 4 entry / PayoutShard 5 entry）同款追踪 GREEN，resolve/claim 实现前的该阻塞项关闭。
   > 🔴 更正注记（2026-09-14T17:16:24Z · Bettor · NWT 构造攻击 · COORD-LEDGER (1401)）：**上条"v0 = ②"须更正**——NWT 用与市场无关、仅 State 形状相同的 Decoy 合约在 cli-debugger 上打穿 ②：`readInputStateWithTemplate` 的 prefixLen/suffixLen/expectedHash 在 ② 里全由 spender 自选，**② 只解决编译期环，防冒充维度较旧设计更松**，"锁修复/更强得多"措辞作废。防冒充在 v0 仍 100% 由链下注册表承担（前提与触发点不变）。**候选 ④（依赖反转）评估中**：KTT ctor 烤该市场全模板 hash（常量），ShardLeaf_direct 的 token_tmpl_hash 挪进 State——若 NWT 真对抗通过，可结构性关闭 decoy 与 fork 而无需注册表或 ③。新票 T-SHARDLEAF-STRAY-TOKEN-DOS（`scanOwnedTokenInputs` 无 stray 检测，未实测）；builder 硬要求：市场代币输入只按记录的 outpoint 选取、永不按 owner 扫描。
...
2. **何时 / 为何**：2026-09-13 V-T-6 根因定案（COORD-LEDGER 1117）：silverc `binding=cov` 框架不能内省非己 covenant 输出（工具链层，非协议层），代币合约无法自证"接收方是合法市场/领取 covenant"。退路①（1117）把接收侧证明挪到 market/claim 手写 entry，代币 transfer 删 b-out，代币层只剩 `>=`。
   > 🔴 **状态注记（2026-09-15 · J2 · 账本 1417 常设令复核准备上报上游时 Bettor 审出反证 · 账本 1418）**：**V-T-6 不存在，是原调查的向量构造错误，不是编译器/工具链限制**。原 4 探针（`ProbeBoutCov`/2/3/4）的失败交易只有 1 个输出、且该输出属于外部 covenant，而 `next_states.length` 却声明为 1——`binding=cov` 生成的 wrapper 在跑用户函数体前会先做"本 covenant 组续约输出数 == `next_states.length`"这条基数检查（真实报错里 `__cov_out_count = 0` 正对应"组内续约输出数=0"，且失败位置在 `1:1`/wrapper 层，不在探针自己那行 `require`）——一直没被读出来，误判成"`OpOutputCovenantId` 在 `binding=cov` 里对外部输出内省失效"。**决定性反证**（D-019 pin 真实编译+跑通，中性重写 `Repro.sil`/`ReproNoCheck.sil`）：把交易改成 2 个输出（输出 0 = 本组真实续约、状态与 `next_states[0]` 一致；输出 1 = 外部 covenant），组内续约输出数与 `next_states.length` 对齐后，`binding=cov` 函数体内 `require(OpOutputCovenantId(1) == next_states[0].owner)` **正确通过基数检查**，且对**外部**输出的 `OpOutputCovenantId` 读值完全正确——`want` 值对时 PASS，`want` 值错时精确在该 `require` 行 FAIL（`__cov_out_count = 1`，不再是 0），去掉该 `require` 的对照合约证明这个交易形状本身合法（不是判据空转）。**不影响本条已生效的降级**——H1(b)/b-out 本身已被 Owner 更晚的裁定（D-017 注记，账本 1408）独立撤销（"代币就是代币"，KanetTokenClaim.sil 同病同治删除同族检查），`sum_in>=sum_out` 与市场侧 `scanOwnedTokenInputs` 双边对账的论证不依赖 V-T-6 是否成立，**不需要回退任何已落码内容**——本条只更正"为什么"这一段历史记录的准确性。`docs/provenance/2026-09-13-j2-t1-token-sil-implementation/KNOWN-GAP-V-T-6.md`、`docs/2026-09-13-nwt-redteam-j2-v-t-6-root-cause-v0.3-review-v0.1.md` 同步补状态注记；`docs/upstream/2026-09-15-silverscript-issue-vt6-final.md` 撤稿，不上报上游。
3. **谁批**：NWT 先裁"放宽 = MUST-FIX，非开放问题"并给出 token#2 静默销毁攻击（6da618e8，1119）；J2 路由稿 v0.2 154855de 选方案 (1) 市场侧遍历（1120）；NWT 机制 PASS、等价性裁条件式三前提（4b39c801，1121）；Codex 判"分布式 >= + A 类扫描 == 精确守恒 NOT-ESTABLISHED"并指出入口替换缺口（桥 c67b229c，1123）；**Owner 2026-09-14 终端确认方向，并加三条硬要求（§5）**。
4. **谁在上层补回**：市场/领取合约每一个入口——A 类 `require(scanOwnedTokenInputs() == 本入口实际处理量)`，B 类 `noTokenInput`（owner 无关、更严）；遍历界 = 协议 `max_tx_inputs` 1000 或先拒超界（1122）；加 §5 (a) 输出侧对账。两半合一才等价旧精确守恒 ⇒ **T1 v0.6 与 T3 v0.3 必须同一次合入**（拆开合入 = 合入一个已知漏洞窗口）。
5. **Owner 三条硬要求（2026-09-14 终端原话要点）**：
   - (a) **对账必须双边**——输入侧全数只保证"没有市场代币被悄悄吞掉"，不保证"输出侧去向合法"。每个入口同时断言：本 tx 中所有市场脚本锁定的代币输入 = 本入口声明要处理的那一笔；输出侧市场代币总量 = 输入总量 − 合法支付。探针向量须覆盖"输入对上了、输出把币转去陌生地址"，缺则补。
   - (b) 本条必须记在 DECISIONS——**以后换币或上生产资产，第一个翻出来审的就是这一行**。
   - (c) **市场合约与币种解耦**——"人人可 mint、供应无限压死价格"只是本轮经济无害的论证，市场合约设计不得依赖该前提；它验的是"归我的代币进出对账"，与币值无关；将来换币 = 配置变更而非合约重写。
6. **边界**：本降级的经济无害性**仅对自发免费无限铸造测试币成立**。任何换币、任何带市场价格的资产、任何生产资产上链前，本条必须重审，默认回到精确等式或等价的双边强制对账。部署/激活仍需 Owner 单独审批（铁律 0）。
...
5. **另案不挡路**：da9 2026-09-08 22:29Z → 09-13 07:17Z 停机原因（KANet-UI 查事件日志）；J1 稿 §8 v1.0.0 `COMPILER_VERSION` 仍 "0.1.0" ⇒ pragma 保持 `^0.1.0`，J2 迁移计划 §3 反过来；J2 批 A/B/C 离线编译要在 v1.0.0 重跑。
   > 📌 状态注记（2026-09-13T10:16:43Z · Bettor · 出处 NWT `docs/2026-09-13-nwt-redteam-j1-kcc20-testtoken-v0.1.md`（045bf608）+ COORD-LEDGER (1008)）：NWT 红队 verdict 采纳——**总判可作批 T 输入，带 3 条 MUST-FIX 门槛**：① (a) 只挡裸地址、挡不住自建 covenant 场外流转（KCC-0020 规范 covenant-id/v1 的 owner-authentication = Empty）⇒ §5 合规措辞收窄为"不能持有在任何裸地址"，**(b) 模板前缀锁由"可选升级"改为关闭已知缺口的必要项**；② §3.5 三入口的态切换必须是**编译期常量 / 新部署**，不得是运行时状态标志；③ `borrow_scheme` 部署时钉死 0x00 disabled 并测负向量（KCC-0020 §5 borrowed receive 旁路）。另 §6-4"名下"绑定 HOLD 未设计（批 T 稿必写）。§9 两处 PUSH-BACK（缺 drain 在飞交易步骤；"随时重同步"改条件句）**进 KANet-UI runbook**。§7 五拍：①③⑤ PASS，② PASS，④ 加条件 = 三 MUST-FIX 满足后批 T 才可进实现。§8 v1.0.0 断言 NWT 隔离核全部成立。
   > 📌 状态注记（2026-09-13T10:24:14Z · Bettor · 出处 KANet-UI 4e9f2390 + COORD-LEDGER (1011)）：**Owner 四拍中的「波 0 GO / 第二台机」已由本条裁定 9（主网节点跑 da9 本机）解决 = 不起第二台机**，`docs/2026-09-13-kanetui-wave0-second-machine-skeleton-v0.1.md` 核心前提作废（页首已注）；**S-2「TN12 第二前向节点」（ledger 998）随 TN12 退役关闭**（Bettor 裁，Owner 可否决）。剩余 Owner 待拍：G-1 GO · 合约迁移波 3 前置 · 主网密钥/资金上限 · 本条 §3 的 GO-1。
   > 📌 状态注记（2026-09-13T11:40:24Z · Bettor · 出处 J2 P8 6d268f79 + NWT c5c88415 + COORD-LEDGER (1032)(1033)）：**主网合约集瘦身 v2 采纳**——剔除 fold-tree 家族（FoldNode / FoldNode_sealonly / PoolLeaf / PoolShard_fold / PoolLeaf_nofold_probe），主网集 = ≈7（ShardLeaf / ShardLeaf_direct / PayoutShard / PayoutShardV2 / RootClose / RootClaim / CloseZkV2）+ 测试币新 2；理由 = 该路线 2026-06-20 已因 template-match 可被伪造 PayoutShard 击穿而架构性废弃（`ShardLeaf.sil:11`），console 侧 builder 全仓零 live 调用者。seal_to_root 脚本层双记缺口真但不可达，**不开事故账**；C1 角色 1 修法只留设计。候选：relay `unlockBshardFold/unlockBshardSeal` 死分支清理排 TN12 收尾。
   > 📌 状态注记（2026-09-13T12:43:43Z · Bettor · 出处 NWT 29959d41 + J2 T1 v0.4 0f49a912 + COORD-LEDGER (1043)(1044)）：**测试币合约（T1）落码方向 = A″**——代币 ctor 只烤市场模板 hash + 领取模板 hash（两者稳定，P10/P11 实证），per-instance 值（代币 hash、oracle 公钥、winner_pubkey、market_cov_id…）全放状态字段只做等值比较，领取的 market_cov_id 由市场入口用自己 OpInputCovenantId 写入；依赖图单向无环。**A′（独立注册 covenant R）排除**：covenant 创世输出的状态内容不受共识检查（covenant_id 本身会验、伪造不了，但 bound/market_cov_id/模板 hash 字段任意可填），任何人可铸"已绑定"的 R′ ⇒ 双 R 攻击；KanetMarketRegistry.sil 与 P10 存档为排除方案。**D（烤发行方公钥 + 链下签名模板证明）降为后备**（把允许集裁决权搬到一把链下私钥，与结构性强制取向相反）。可再造面如实记：攻击者可用市场模板铸 market′，但须走真实 T3 业务门禁才有意义，模板级允许集固有性质非漏洞。
   > 📌 状态注记（2026-09-13T13:26:21Z · Bettor · 出处 NWT 2f9377e7 + 4a8d3d43 · Codex 8968452d/8118732e · COORD-LEDGER (1045)–(1048)）：**(c) NO-TX / NO-STATE-CHANGE 修复补丁完整（GREEN）**，在侧分支 `coord/j2-c-no-tx-landed`（5 笔 88557a04…1d4b7fd2，基 8a44568a）：submit_intents 幂等意图（INSERT 先于 IPC）+ relay 两阶段 prepared→submitted + 同字节重播（finalize() 重算 id）+ escrow/payout 五站点 + prediction-payout-gate（落 REORG_SAFE_MIN_DEPTH 才 completed/'paid'）+ **escrow_landed_at 硬消费门**（NULL ⇒ 无接受/匹配/结算/价值移动/声誉终态，门在 exchange-machine.transition 单一所有权点）+ F3 对账器 v1 + covenant 往返超集测试。NWT 亲跑 7 测试文件全绿。**合入主线 = 钱路，待 Owner 批**；落地窗与 (a)(b) 同（波 0/1）。顺带修两既有缺陷：settler SELECT 从不选 delivering；transition 被拦返回原 offer 不抛照样派彩。I5 双付面由此关闭（设计+代码），运行期验收待集成。
   > 📌 状态注记（2026-09-13T13:46:12Z · Bettor · 出处 NWT 89eadff4 + J2 T1 v0.5 57e4c344 / T2 7d1f47cd·127c3aee / ctor 盘点 a6bd91fb · COORD-LEDGER (1049)–(1053)）：**T1 测试币合约设计 v0.5（A″）NWT GREEN**（三 MUST 独立重算兑现：市场两组 ctor template_hash 同 a5c45a36…、领取三组同 209abf73…；Q7 winner_pk = byte[32]）；**T2 市场/领取骨架方向 GREEN**（Q1 = 领取合约自核代币输出；B 类不在场证明 = witness 供前后缀 + 状态 token_tmpl_hash 内建 blake3 现场验，P13 实证）；**Q8 裁决**：判据 = 结构性用途（循环界/切片）才强制 ctor；`gateTmplHash`（CloseZkV2 ZK 电路锚）**MUST 留 ctor**（搬状态 = 创世状态不受检的洞开在全 ZK 路径最要命字段上）；其余 6 个模板 hash 留 ctor（全网固定，家族链单向不成环）；`poolMerkleRoot` / `committee_hash` 可搬但 T4 对照升 MUST；`market_id` 删；predicate_commit baked-use 行删；seal_count/min_bet 交 T3。**Bettor 拍 T4 对照检查范围 = 市场创世全部状态字段**（单源产物逐字节对照 + P2SH 重算 + market_genesis intent + landed 硬门），非仅 token 三参。实现仍 = 钱路/合约，Owner 批。
   > 🔴 状态注记（2026-09-13T14:09:05Z · Bettor · Owner 终端直令原话「直接把所有tn12涉及东西马上，全部冻结。完全不要再动了，全部转向主网，不要在提及任何tn12」· COORD-LEDGER (1061)）：**旧测试网一切事项冻结**——退役 runbook 执行、其 GO-1 前置（开机触发源）、挖矿残余进程、历史卡单核实、收尾候选清单、并存期资源观察，全部 SHELVED，不再派工、不再向 Owner 提请、不再在频道/ledger 讨论；现有进程原样不动。**团队唯一主线 = 主网**：① J1 起主网只读节点（执行页 876cc412）；② console/relay 主网化（(a)(b)(c) 侧分支合入 + 主网 env）；③ 代币合约 T 系列实现。本条取代 D-017 §3 的执行门顺序与 1056 的"并存"表述。
   > 🔴 澄清注记（2026-09-13T14:44:41Z · Bettor · Owner 终端原话摘要「（挖矿进程）、禁用两个开机触发源、停 kaspad——这些都还没做。这个应该尽快完成啊」· COORD-LEDGER (1080)）：**冻结 = 旧网的开发/审核/讨论停止；旧网栈的停栈本身（runbook v0.1.3 §2.1–§2.4）按 Owner GO-1 由 J1（提权 SSH）尽快执行完毕**：停 console 6716 → 挖矿 13788/31636 → watchdog 18576/24220 → 优雅停 kaspad 16644 → 禁用两个 SYSTEM 计划任务 + Startup .lnk。**不删数据目录（GO-4 另批）**。停栈后旧频道 API 消失，协调改走本机 SendMessage + inbox 文件。J1 14:16Z"console 被别的会话拉起"归因错误：:3200 监听者 PID 6716 自 09:46Z 未变，kanet-stop.sh 未杀到 Session 0 进程。
   > 🟢 **旧网栈停栈完成（2026-09-13T14:54:55Z · J1 提权执行 · Bettor 地面核 · COORD-LEDGER (1086)）**：console / 两 watchdog / kaspad（//F 兜底）/ 挖矿桥 / mining-watchdog 全停；:3200/:5555/:17210 空闲；Startup .lnk 改 .disabled；SYSTEM 计划任务 `KANet-KaspadWatchdog` / `KANet-TN12-BootSequence` / `KANet-Console-Supervisor`（第三个为 J1 今日误加）全部 Disabled；90 s 观察窗稳定。**console 反复复活根因 = 第三个 console-supervisor 循环 PID 21948（`console-supervisor.sh _run`，调 `kanet-start-headless.sh`）**，非任何 agent。数据目录 158 GB 未删（GO-4 另批）。本机仅余主网节点 28244。旧频道 API 永久下线，协调 = 本机 SendMessage + inbox 文件。教训：停栈须提权会话；`kanet-stop.sh` 对 Session 0 进程报假成功。
   > 🟢 状态注记（2026-09-13T15:11:57Z · Bettor · NWT 463ba696 · COORD-LEDGER (1092)）：**T4 市场创世 console 侧设计 v0.2 NWT PASS** ⇒ T1–T4 设计层全部收口。落码硬前提（(c) 已合入主线）已满足；T4 落码前须先做 GO-C（主网 console 起服务）。非阻塞记档：T4 §2.1.1 对 gateTmplHash 的"四段拼接"描述只对 closeZkTmplAnchor 成立（CloseZkV2.sil:48 为两段），下次订正。
   > 🟢🟢 **GO-C 完成（2026-09-13T15:40:30Z · Bettor · COORD-LEDGER (1099)）：主网 console 实例已起并通过六项验收**——PID 20212 :3202，基于主线 16e56349，全新库 `data/console.mainnet.db` 迁移至 v203 完成（118 表；`PRAGMA user_version` 非本仓判据，migrate.js 不用它），`[rpc-health] using local node: ws://127.0.0.1:17110` 且零 configured/discovery 回退行（strict LOCAL_ONLY 正向臂），`[broker] disabled (BROKER_ENABLED!=1)`，relay_nodes 0 行无 relay 起，**I4 负向量实测**（合成 testnet-12 行走 startRelay() ⇒ network_mismatch 拒起，测后清理），存活 >5 min。GO-D（正式接入频道/对外）前两项须清：① bsc-watcher 仍无条件启动、tick 内动态 import broker-state-authority 每次 FATAL 被 catch（broker 门漏 start 层，J2 broker-optional-2）；② env 补 ORACLE_SILENT_TIMEOUT_MIN=1440（代码自带主网钢线）。GO-E（各 agent 主网 relay + 真 KAS）仍独立待 Owner。
   > 🟢 状态注记（2026-09-13T15:44:50Z · Bettor · NWT 3e6ae96a · COORD-LEDGER (1102)）：**Codex 3ce6513a F2-R 残余双付窗 MUST-FIX 已合入主线 31ca29ba**——inputs_spent 三缺态改终态 AMBIGUOUS/HOLD（kaspa_tx_log 命中 prepared_txid 才 submitted），不 abandon 不重建；NWT 核三个入口（重入 / F3 对账器 / resumeStaleIntents）均摸不到 AMBIGUOUS，Codex 六步负向量亲跑绿，v204 两态核。已知设计代价：watched 集外收款 ⇒ 人工清单偏长（安全方向）。主网价值路径运行期验收仍 HOLD（Codex 边界）。
   > 🟢 状态注记（2026-09-13T16:00:28Z · Bettor · COORD-LEDGER (1105)）：**GO-D 干净稳态达成**——主网 console 重启为 PID 12404 :3202（主线 924096ce 含 (a)(b)(c) + fresh-db + broker 门 ×2 + F2-R），复用 v203 库并幂等接上 v204；`[broker] disabled`、`[rpc-health] using local node: ws://127.0.0.1:17110` 零回退、migrations complete、**stderr 65 s 零 FATAL**（余 4 行皆 fail-closed 预期）、ORACLE_SILENT_TIMEOUT_MIN=1440 生效、I4 负向量重跑一致、存活 >5 min；Bettor 地面复核一致。GO-D 的"正式接入频道/对外"部分待 GO-E（relay 身份 + 真 KAS，Owner 独立批点）。
   > 🟢 状态注记（2026-09-13T16:20:43Z · Bettor · NWT 75bfe476 · COORD-LEDGER (1115)）：**T1 测试币合约实现合入主线 e1894b64**（`kasia-console/src/lib/sil-v1/KanetTestToken.sil`，A″ v0.5；v1.0.0 编译 4 entry 3863 B；15 向量 14/15，provenance 29 文件；NWT 从 3ed9733 独立重建编译器产物逐字节相同、向量与探针独立复跑一致；OpOutputCovenantId 语义三方互证 = 纯回显，genesis 诚实性由 consensus 脚本外预检）。**离线实现，不部署不上链**。**硬性依赖（NWT 红队纪律，Bettor 采纳）**：V-T-6（b-out = 代币进入全新领取 covenant 的唯一路径）收口或 T2/T3 改路由之前，T2/T3 不得按当前 T1 形态落码；退路 = ② 真 kaspa-wasm 复现定位工具链层 / ① 接收方证明责任挪到市场合约（H5 原则）。
   > 🟢 **Owner 放行（2026-09-13T14:20:24Z · 终端原话「放行！」· COORD-LEDGER (1068)）**：① 三侧分支 `coord/j2-a-local-only-strict` 6dec9379 → `coord/j2-b-network-single-source` 346adbce → `coord/j2-c-no-tx-landed` 1d4b7fd2 **批准合入主线**（合并预演 68e766c3 仅 1 处相邻 import 冲突；NWT 三份 GREEN；Codex CODE-VERIFIED）；② **主网 console/relay 起服务窗口批准**（方案 b1a32c54，NWT 审后 KANet-UI 执行）；③ **代币合约实现放行**（T1 KanetTestToken.sil → T3 四文件按 v0.2 不变量，侧分支 + 向量 + NWT diff 审）。执行序 = 合入 → console 主网化 → 合约实现。仍不授权任何真实资金移动；主网带值波按 D-017 四波。
   > 🟢 状态注记（2026-09-13T14:35:18Z · Bettor · COORD-LEDGER (1077)）：**(a)(b)(c) 已合入主线 = merge commit 91b2ac6c**（合并分支 coord/mainline-abc-merge 27041390，NWT 验证 518a11f5：三笔 merge 与预检一致、第 4 笔只动测试不削断言、亲跑 10/10 + 全仓 lint 仅 chains.js:31-32 历史债）。主线现含 strict LOCAL_ONLY、网络单一源 helper、submit_intents/escrow_landed_at/F3 对账器。运行期验收（主网 env + 本机 RPC fail-closed）随 GO-C/GO-D 做。

   > 🟢 状态注记（2026-09-13T17:47:24Z · Bettor · COORD-LEDGER (1129)）：**GO-E 设计完成**——清单 v0.5（9b300cbf）NWT GREEN（364bfce3）：验证身份 = ephemeral manual relay 五条硬门 + 九步实测流程 + Owner 备份确认关卡 + 删行走经审查脚本不开 HTTP 端点。执行仍 HOLD：等 Owner 三选一（导入既有零引用小额身份 / 新建 1 个 / 先不动）；导入候选须过 NWT 四条前提，源码具名常量地址一律排除；20,301 KAS 地址钥匙绝不进 relay 进程。

   > 🟢 状态注记（2026-09-13T18:02:19Z · Bettor · COORD-LEDGER (1133)）：**T 系列设计层定案**——T1 v0.6 .sil（2a5c1eb0）、四处 draw-down MUST-FIX（f2fce916）、T3 v0.3 稿（949dfc96）NWT 全 GREEN（a036641c），含 Owner 2026-09-14 三条（双边对账 V-outbind、币解耦逐行、D-018 引用）。落码序：§7 v1.0.0 语法迁移（硬性前提）→ 19 入口逐条 → NWT 逐条核 → T1+T3 同一次合入。部署/激活仍需 Owner 单独审批。

   > 🔴 **Owner 裁定（2026-09-15 · Owner 本机终端原话「无限供给！人人都可以mint!何罪之有？」「代币就是代币，为什么有这样的缺陷？」· Bettor 记账 · COORD-LEDGER (1407)(1408)）：撤销 H1(b)"接收方必须是 KANet 市场模板"——测试币回到上游 silverscript v1.0.0 `kcc20.sil` 参考形态（代币只管持有者在场 + 守恒；不核收方代码）**。本条 §2「只许 `owner_scheme==0x04` covenant 持有」**保留**（结构性：owner_scheme 钉 covenant-id + 花费时持有者 covenant 在场），但**不再引申为"只许 KANet 市场模板持有"**。由此：上方 09-13T10:16 注记①"(b) 模板前缀锁 = 关闭已知缺口的必要项"**作废**；09-13T12:43 注记 A″"代币 ctor 只烤市场模板 hash"中**代币烤市场模板的部分作废**（市场侧烤代币模板 hash 保留 = 单向无环）；当日被拒的**方案 C 改采纳**（`docs/2026-09-13-j2-t1-kanet-test-token-kcc20-contract-design-v0.1.md` §… 方案 C）。**T-KTT-BIN-TEMPLATE-LOCK-FIX 以"删除 (b-in)"结案**（待 NWT 核市场侧闭合），候选 ②/④ 及 J2 设计 v0.1/v0.2 全部作废归档；"逐市场发币"产品问题消失（一种通用测试币、赢币可复投、钱包单余额）。合规措辞 = NWT 09-13 已定的窄句（"不能持有在任何裸地址"；包装 covenant 场外流转对免费无限铸造的测试币无经济意义，不再作为合约层目标）。**金丝雀走 A（先改合约再跑）**：改动 = 删行 + 重编 + 重跑向量，代价一天内，Bettor 拍、Owner 可否决。执行门不变：J2 设计 v0.3 → NWT 红队 → 落码 → NWT 逐字节复编 → 合入。
   > 🟢 状态注记（2026-09-13T19:15:08Z · Bettor · COORD-LEDGER (1159)）：**主网账号迁移前置——热钱包硬上限代码线全部 NWT GREEN**（侧分支 coord/kanetui-hotwallet-caps 六笔 a65c28dc→01a0f136：startRelay() 准入门三条检查 + 互斥锁；驻留期监控 relay-hotwallet-monitor.js 含全局 precheck fail-closed；M0a considered amendment #6/#7）。迁移 runbook v0.4、2-1 规格 v0.2、GO-E v0.6 均定稿。**合入与主网 console 重启部署待 Owner 确认**：per-relay 800 KAS / 热钱包总额 1000 KAS / 冷清单 = Trader-B + MarketMaker-A；导入顺序 stress → ≤21.5 → NWT；GO-E 身份取自迁入小额账号。Codex 对 45594804 的 HOLD 待其复核 01a0f136 解除。

   > 🟢 **Owner 拍板（2026-09-13T19:54:31Z · 终端原话「所有；已知账户都要导入！我们今后还有很多事情要做。」· COORD-LEDGER (1168)）**：主网账号迁移范围 = 已知 19 个主网账号**全部导入** console（含 Trader-B 20,301.72 / MarketMaker-A 1,004.996），不设"不导入"冷清单。Bettor 裁定分两层：导入 = 行进库，全部执行；relay 运行/私钥驻留仍受 2-1 准入门（per-relay 800 / 总额 1000 维持，Owner 未另调）与驻留监控约束，两大额账号为"已导入、relay 不启动"档，调高上限须 Owner 明确一句话并知悉全部 ≈21,890 KAS 转为热钱包敞口。导入顺序 stress → ≤21.5 → NWT → 两大额。GO-E 身份取迁入小额账号。

   > 🟢 状态注记（2026-09-13T20:18:49Z · Bettor · COORD-LEDGER (1175)）：**热钱包硬上限线已合入主线 60b2f026**（8 笔 NWT GREEN，Codex HOLD 已解）。下一步 = KANet-UI 按执行页 v0.3 重启主网 console 部署准入门 + 驻留监控（env per-relay 800 / 总额 1000 / 冷清单 Trader-B + MarketMaker-A），随后按迁移 runbook v0.6 分四批导入 19 个账号（Owner 1168），每批经 GO-E 九步与 Owner 备份确认关卡。
   ...
   > 🔴 tripwire（2026-09-13T22:40:48Z · Bettor · NWT 确认 · COORD-LEDGER (1203)）：T4 创世对照对 poolMerkleRoot / committee_hash / predicate_commit 采用 (c) 全字节重编译对照 + (d) P2SH 重算路径，其安全论证的**唯一前提**是这三个字段在全部 T3 合约中永远只作 ctor 常量、**没有任何赋值语句**（当前源码零赋值）。任何后续改动若给它们加入 state 写入路径，该论证立刻失效且不会自动报错 ⇒ 必须触发 NWT 复核；T4 v0.4 加硬性注释与 lint 钩子。另：predicate_commit 目前仅为 baked 防篡改占位（blake2b(x)!=x），真实 oracle 判定绑定尚未接线，MUST 覆盖的是结构完整性，不是业务裁决门。

   > 🟢 **状态注记（2026-09-14T02:19:25Z · Bettor · COORD-LEDGER (1214)）：T 系列已合入主线 merge 4b48393a**——KanetTestToken v0.7、KanetTokenClaim、8 个主网集市场合约（PayoutShard / PayoutShardV2 / RootClaim / RootClose / ShardLeaf / ShardLeaf_direct / CloseZkV2 / RefundClaim）代币化落码，每笔 NWT GREEN，Codex T3 集成 HOLD 于 18c000b0 解除（d7b31325），合入范围仅 10 个 .sil + 文档/证据，主线 lint 0 errors。**合入不等于部署**：代币部署、市场创世（T4 对照工具尚待落码）、激活、GO-F 金丝雀广播均需 Owner 单独批。D-018 守恒降级状态由 PENDING 转为"已随 T1 v0.7 + T3 合入落地，边界不变"。
   > 📌 状态注记（2026-09-14T08:04:23Z · Bettor · 出处 J1 inbox `2026-09-14T08-05Z-j1-DONE-da9-outage-rootcause-softfix-applied-owner-order.md` + J1 分支 5616ca9d §9 + COORD-LEDGER (1296)）：**第 5 条"另案" da9 09-08→09-13 停机根因已查明**（J1 三源实核）：Windows 自动更新 09-08 22:29Z 重启 × 重启后无人登录 × Tailscale 未开 unattended（每次开机 NeedsLogin→stopped）；机器与外网全程正常，排除 Modern Standby / S3 / 服务崩溃 / NordVPN。**软修复已按 Owner 直令施加**（J1 经 SSH，可逆、不碰密码；Bettor 本机只读复核一致）：`tailscale set --unattended=true`（ForceDaemon=true）、`HKLM\...\WindowsUpdate\AU` AUOptions=2 + NoAutoRebootWithLoggedOnUsers=1（Win11 Pro 完整尊重）；自动登录未动。**生效判据未完成**：须无人登录状态重启 da9 一次、Tailscale 自行 Running——重启会停主网 console（PID 18320 / 16 relay / ≈43.9 KAS 驻留）与全部智能体会话且无自启任务，**归 Owner 定运维窗口**，同窗决定"KANet 改为不登录也运行的计划任务 / 是否自动登录"。补丁改维护窗手动装。本条不写"已修"。
...
2. **裁 A′**：不换编译器（A 的真实代价 = `d25bd34..db9e1ba` 45 提交语言/API 漂移：`byte[](x,n)` 两参已不存在 ⇒ `CloseZkV2.sil:45` 不编、template hash→blake3、dispatch tag 改签——会打碎 D-015 的 ZK 形；**编译器钉版约束**）；不走 B（参照输入 DAA 是创建时非当前）/ C（PMT 域窗界矿工可挪、全套时序重推、违 v1.3 全 DAA 总裁）。源内 `int E = OpTxInputDaaScore(selfInIdx) + n_recovery_delay_daa; require(E >= 0); require(E < 500000000000); require(tx.time >= E);` ≡ 上游 `tx.daa` lowering（`OP_WITHIN [0,5e11)` 逐字等价）。
3. **两条 v0.15 未写的构造侧硬前置（真 MUST-FIX，builder 侧代码另报备→NWT→落）**：(a) recovery tx **`lock_time = E`（DAA 数）非 0**——现 `kasia-relay/src/lib/p2sh.mjs` 各 builder 一律 `lockTime: 0n` ⇒ CLTV `E > 0` 必拒 ⇒ recovery 路**当前不可构造**；多输入时 **`lock_time = max(E_i)`**（对较早输入保守 over-delay，多等不少等）；(b) 被锁输入 **`sequence ≠ MAX`**（否则 `:83-88` 绕过终局 ⇒ DAA 延迟失效）。
4. **状态**：Shape-B 设计层 = "recovery lock 在 8065184 可表达（magnitude-determined CLTV）；真 MUST-FIX = 构造侧 `lock_time=E` + `sequence≠MAX` + A′ 源域守卫"（**不**按 Codex 措辞"REOPENED on 域混"）；**gate (a) 仍 OPEN** 待 READY 后真链向量 N6（`lock_time=E−1` ⇒ `UnsatisfiedLockTime`）/ N7（`5e11+t` ⇒ `mismatched locktime types`）/ N8（tip DAA ≤ E ⇒ 未终局拒）/ P（界后落地）+ 探针 v0.3 `recovery_daa` 入口。探针 S4 断言改"恰 1 字节 = LSB、全落 9 字节区"（Codex ③ 采）。
5. **派工**：J1 落 v0.15 八处措辞 + A′ 守卫 + 两硬前置 + `max(E_i)`（主责文，NWT 审）；J2 报备 builder `lockTime`/`sequence` 参数 + 探针 v0.3；NWT 经桥回 Codex（附四坐标 + 两 builder 前置，供其复核 refute）。
   > 📌 状态注记（2026-08-29 07:2x Z · Bettor · 出处 Codex 桥 `9eab914a` `RESPONSE-20260829-UNSYNCED-S63-RECOVERY-APRIME-CODEX-REVIEW.md`）：**Codex 接受 refute**（自纠 14c81c1c 把上游 #214 分域过度套到 8065184；`tx.time` 在 pinned 编译器是 misleading 关键字非共识域类型）⇒ **A′ 设计层 SOUND、Shape-B same-chain 恢复时序 RE-CLOSED AT DESIGN LAYER（CONDITIONALLY CLOSED 恢复）**；gate (a) 仍 OPEN 待部署路径 N6/N7/N8/P + same-cid readback（通用节点拒 = inconclusive）。措辞更正：A′ 与上游 `tx.daa` **非 byte-for-byte 同 lowering**，正确说法 = "same fail-closed domain predicate / same CLTV semantics"（v0.16 与 J2 稿"逐字等价"改）。🔴 **新 MUST-FIX（helper 生产化前）**：`cltv-locktime.mjs` `cltvSequence()` 不拒 `> MAX_TX_IN_SEQUENCE_NUM`（`2^64` 能过）⇒ 须 `0 <= seq < MAX` + 向量 `MAX−1` PASS / `MAX` / `MAX+1` / 超大 BigInt REJECT；恢复配置须 `n_recovery_delay_daa > 0`（`lock_time=0` = 共识已终局无延迟，funds-safety 不得静默零延迟）。不影响 A′ 证明（gate (a) 路径用 sequence 0）。已派 J2 fix-up。
   > 📌 状态注记（2026-08-30 · Bettor · 出处账本 (741)(742) + `docs/2026-08-30-j2-silverscript-v1-migration-plan-v0.1.md`）：上游 silverscript **v1-rc1**（2026-08-30，正式版 ≈9/6）经 J2 隔离试编核实 **不带 OP_PICK `pick_from_depth` off-by-one**（`#178 compile.rs refactor` 消灭旧路径），⇒ 本机修复 8065184 **对 v1 不需重港**，只对仍用 0.1.0 的上游者成立；但 rc1 codegen 全变（dispatch tag 序言/状态越界检查/alt-stack）⇒ 42 合约字节码与 P2SH **全换**、产物/ctor JSON 格式变、`tx.time` 只收 `temporal`、`ScriptPubKeyP2PK` 返回 `byte[36]`。**pinned `silverc-zk-8065184` 与 §6-3 A′ 设计仍原地有效**（已部署字节码不受编译器版本影响）；迁移 = 独立项目（批 A–D，Owner 定优先级），与 D-005 慎重铁律同。
   > 📌 状态注记（2026-08-29 09:2x Z · Bettor · 出处 Codex 桥 `418fffbd` `RESPONSE-20260829-UNSYNCED-S63-RECOVERY-CONFIG-AUTHORITY-FOLLOWUP-CODEX-REVIEW.md`）：**MUST-FIX A（raw.entry 选 ABI 入口）/ B（raw.max 抬 sane-max）= CLOSED at code/test layer**（`232294d7`：`loadRecoveryConfig` 对自有 `entry`/`max` 一律 `CLTV_CONFIG_OVERRIDE_FORBIDDEN`，entry 只取代码钉死 `RECOVERY_DAA_ENTRY`、max 只取 `DELAY_SANE_MAX_DAA`；非 vacuous 向量 `{n:5e8,max:1e9}` 拒）。🔴 **新增 wiring-time 要求（非重开 A/B）**：`_loadRecoveryConfigWithMaxForTests(raw,max)` 从生产模块导出，"test-only" 是命名约定不是访问边界 ⇒ **接线前须机械 import-surface guard**（helper 移出生产模块作用域，或 wiring acceptance test 对任何生产路径 import/re-export/call 它即红；不得只靠"`loadRecoveryConfig` 自身不调它"那条测试）。builder 仍 HOLD/unwired；A′ 条件接受；gate (a) OPEN 待 TN12 真链（same-cid successor readback / 后继花落链深度 / provenance+CLTV 边界负向量）。本审不授权任何 build/接线/部署/签名/广播/DB 变更。Bettor 拟 batch-2 收口后派 J2 加源级 guard test + lint `R-TESTONLY-EXPORT-IN-PROD`（形待 NWT）。
   > 📌 状态注记（2026-08-29 10:1x Z · Bettor · 出处 Codex 桥 `5725b96e` `RESPONSE-20260829-MSG286-TESTONLY-WIRING-GUARD-CODEX-REVIEW.md`）：J2 `coord/j2-testonly-guard` `e9796810`（两 `_*ForTests` helper 移出生产模块到 `*.testonly.mjs`，生产零 test-only 导出；NWT GREEN，突变 FORBIDDEN 11/3、BRAND 12/2）⇒ **Codex：两 helper move-out = ACCEPTED/CLOSED at current code surface**。🔴 **新 MUST-FIX（接线前）TG-1 + TG-2**：TG-1 = lint 豁免生产文件里 `export … _xxxForTests` 定义行（未来可重引入且被故意放过；G0 只扫两文件）⇒ 生产上下文定义/引用一律 ERROR、全仓扫；TG-2 = 规则只扫符号名，生产 namespace import / 动态 `import()` / `require` / re-export `*.testonly.*`、`__testonly__/` 按模块路径绕过 ⇒ 路径轴 ERROR 不看符号；阳性对照 ≥4（生产定义红 / `import * as` 红 / 动态 import 红 / test-context 绿）。已派 J2 叠 e9796810 修。builder HOLD、A′ 不变、gate (a) OPEN。
   > 📌 状态注记（2026-08-29 12:1x Z · Bettor · Codex `2ce3f1a9` + NWT 终审 + 桥 MSG-288 `7c0177b4`）：Codex 回 MSG-287 再抓 **TG-3**（lint 无参默认 targets 不含 `kasia-relay/src` ⇒ "全仓"不实；仓级事实：relay 从未进默认 walk，且无参全仓 lint 本就红 7 条——pre-commit 只 lint staged）与 **TG-4**（路径轴漏 ESM 副作用 import `import './x.testonly.mjs'`）。J2 v4–v8 → guard 终头 **`0db4df77`**（NWT GREEN）：默认 walk = `git ls-files` 五目录含 relay、`.mts/.cts`、副作用 import 形、G2/G3 仓级对照、4 个零引用死脚本 `git rm`；无参存量 7→3（`chains.js:31/:32` 随 `coord/j2-chains-explorer` `7f307bf3`、`exchange-machine:1098` 随 batch-2 自消 ⇒ 全合后 0）；284 个 tracked `scripts/_*` 归档提案（`docs/2026-08-29-j2-tracked-underscore-scripts-archive-proposal.md`）待 Owner。等 Codex 回 MSG-288；两分支合并待其回执（均非钱路）。builder HOLD、gate (a) OPEN 不变。
   > 📌 状态注记（2026-08-29 12:2x Z · Bettor · Codex `382c9cab` `RESPONSE-20260829-MSG288-TESTONLY-GUARD-V4-CODEX-REVIEW.md`）：**TG-3 / TG-4 / `.mts/.cts` = CLOSED**；E1/E2 = DOCUMENTED / ACCEPTED AS NON-LOAD-BEARING（BRAND 私有 WeakSet 出处仍是权威边界，lint = belt-and-suspenders）；chains `7f307bf3` 与声明一致（mainnet 逐字、testnet null、call-time 选网）；**"接线前 test-only guard 要求" = CLOSED at code/test layer**。builder HOLD/unwired、gate (a) OPEN 不变（待 TN12 same-cid successor readback / 后继花落链深度 / CLTV+provenance 证据）。Bettor：guard 分支合入主线（lint + relay 未接线 lib + tests，非钱路）；chains-explorer 合并随 Owner 批（operator 面链接行为变化）。
   > 📌 状态注记（2026-08-29 10:5x Z · Bettor · NWT 终审 + 桥 MSG-287 `ea3b5cd7`）：TG-1/TG-2 修 = `coord/j2-testonly-guard` 头 **`f0f39c48`**（NWT GREEN，8 探针）：符号轴生产上下文 `_*ForTests` 定义/引用一律 ERROR + 全仓 G0；路径轴 `from`/`export … from`/`require(`/动态 `import(` 字面目标 `*.testonly.*`、`__testonly__/` ⇒ ERROR；**轴 3** prod→任何 test-context 路径（`.test.`/`.fixture.`/`test-framework/`）⇒ ERROR（关掉 fixture-relaunder；全仓 875+31 文件 0 误伤）；符号轴剥单行字面量/行尾注释（真引用仍逮）。🔴 **known-escape（regex 逮不到，已写进规则 `:1864-1870`）**：E1 计算路径（拼接/变量）、E2 间接 loader（`createRequire` 别名 / `import.meta.resolve`）；NWT 实测确认。🔵 **安全边界 = BRAND**（testonly cfg 无模块私有 WeakSet 品牌 ⇒ `planRecoveryDaa` 拒），lint 只是 belt-and-suspenders ⇒ 逃逸是 lint 完整性缺口非 authority-bypass。等 Codex 回 MSG-287；builder HOLD、gate (a) OPEN 不变。
   > 📌 状态注记（2026-08-29 08:2x Z · Bettor · 出处 Codex 桥 `2b8ebe2a` `RESPONSE-20260829-UNSYNCED-S63-RECOVERY-BUILDER-FOLLOWUP-CODEX-REVIEW.md`）：Codex **接受四项闭合**——sequence u64 上界（`cltvSequence` `0<=s<MAX`）CLOSED；零延迟 CLOSED（`E==0` 拒 + allow-zero test-only + `loadRecoveryConfig` 强制 `assertPositiveDelay` + 私有 WeakSet brand，去 `BRAND.has` 的突变对照 = load-bearing 证据）；A′ 字节证 = durable pre-runtime PASS（探针 v0.3 两机字节相同 + 独立解码）；措辞 ACCEPTED。**两条新 MUST-FIX（`recovery-lock-builder.mjs` 仍 HOLD/未接线）**：A `loadRecoveryConfig` 不得收 `raw.entry`（入口身份属编译 ABI/接线非运营参数；代码/ABI 钉 + 负向量 `entry:<另一合法整数>` 不改 spend branch；真 ABI 替换探针 ABI 时重钉常量与证据）；B 不得收 `raw.max`（sane-max 1e7 是策略，配置不得自抬；test-only helper；负向量 `{n:1e7,max:1e7+1}` 拒；gate (d) 生产带检留在 ctor/接线边界）。gate (a) 仍 OPEN 待 TN12 部署路径证据。J2 fix-up → NWT 审 → Codex 回。
...
5. 与 D-001/铁律 0.5 一致：ZK committed、rolling 死路不变；本条只定"ZK 走哪一形"，不新开方向。
   > 📌 状态注记（2026-08-29 06:1x Z · Bettor · 出处 J1 P2 `docs/iteration/j1-inbox/2026-08-29T02-50Z-j1-P2-chainlayer-recoordinated-7b1e18cc.md` · Bettor 亲核 `git show 7b1e18cc:`）：**风险面补记**——live 链层自述 `crypto/txscript/src/zk_precompiles/groth16/mod.rs:19` `*NOTE: Experimental code; not yet fully audited for mainnet use.* TODO(covpp-mainnet)`；`lib.rs:61-63` `MAX_SCRIPTS_SIZE / MAX_SCRIPT_ELEMENT_SIZE / MAX_OPS_PER_SCRIPT` 三个 1M 限额均标 `TODO(covpp-mainnet)`（pre-fork 限额待恢复）。⇒ 本条"已建成、生产 armed"指 **TN12**；主网化前 Groth16 precompile 审计状态与限额激活逻辑是**必须计入**的依赖，不改本条裁定。另：P1 younio 复算 gateTmplHash `4ec7ca3d…` 一致，但 J1 如实标注 = **拷贝 da9 WASM 计算，非独立构建**（younio 无 Rust/RISC0 工具链，`imageId_younio = n/a`）；真独立 derive 待 guest `Cargo.lock` 入库 + 第二机独立构建——记为 D-001 债，不在 younio 装工具链（内存病 + 角色 B 暂停）。J1 另独立检验设计断言"suffix/gateTmplHash 只绑 imageId、不随 journalHash 变"成立（同 imageId 两份不同 journal 的 receipt ⇒ 同 suffix 指纹 `365d4c8e…`）。
   > 📌 状态注记（2026-08-29 08:0x Z · Bettor · 出处 J2 `docs/2026-08-29-j2-zk-guest-imageid-reproducibility-plan.md` §6 + provenance `docs/provenance/2026-08-29-zk-guest-imageid/`）：**D-001 债（imageId 跨机可复现）第一手正向证据**——da9 WSL 副本树全新重编（guest rustc `1.94.1-dev (06e01cb0d)`，139 crates 1 m 04 s）⇒ `PAYOUT_ID == c9918501…` 且 `payout.bin` 与 2026-07-12 产物**逐字节相同**（sha `885c6fca…`）；guest 工具链 = `~/.risc0/toolchains/v1.94.1`（J1 r17 "无处钉"不成立，rzup 只是管理器）；锚 = risc0 v1.94.1 + Cargo.lock（68822fff）+ risc0-build 3.0.5 + `guest_source` 内容锚（`TOOLCHAIN.lock.json`）；`scripts/verify-image-id.sh` 零构建/`--build` 两模式只读不改。**裁：不钉 host `rust-toolchain.toml`**（host 对 guest 字节不承重；钉 `1.96.1` 需装独立 toolchain 且 live `zk-prove-worker` 直接读 live 树 toml ⇒ prove 全挂/自动下载风险）。剩：younio 跨机独立 derive（READY 后；报值、mismatch 即交付、绝不 copy）；docker 确定性构建 = 未来改 guest 的路（改 guest = 新 covenant = D-005）。
...
3. **部署**：随**维护窗** relay 重启生效（`docs/2026-08-28-postsync-maintenance-window-runbook-v0.4.md` §red-line-7-observe），窗在本机 READY + J2 T+125 证据之后；IBD 期不动任何进程。
   ...
   > 📌 **维护窗批次清单（2026-08-29 06:0x Z · Bettor · 出处 COORD-LEDGER (708)(709) · 全部 NWT GREEN · 落地 = READY 后 Owner 批的一次窗）**：① 红线 7 observe（本条）；② `57fde30f` console 启动期 fail-fast（**已因 05:14Z supervisor 重启风暴提前上线**，NWT 事后 GREEN，不回退）；③ **supervisor v0.1.x**（`docs/2026-08-29-j2-supervisor-lifetime-storm-guard-and-boot-grace-design.md` + provenance `docs/provenance/2026-08-29-supervisor-v01/`：write-ahead 状态文件 / lifetime 风暴保护 / PID-aware boot grace 300 s + CreationDate 防复用 / `index.js` boot-marker + 心跳 gate on `__booted`；v0.1.3 selftest 48/48 J2 5× + Bettor 3× + NWT 5× **但 NWT 对抗审撤回最终 GREEN**：HUNG_BOOT 判定未查 `last_boot_ok_ts` ⇒ 长命 console 瞬时阻塞会 1 次即判死（比旧 3-fail 更凶）⇒ **v0.1.4 MUST-FIX**（was-ALIVE ⇒ soft 3-fail；never-alive/DEAD ⇒ 立即）+ Bettor 裁心跳陈阈 10→20 s；**v0.1.4 = provenance `dab53cbc`（selftest 64/0：J2 5× + Bettor 3× + NWT 5×，sha 10/10）NWT 最终 GREEN（06:1x Z），runbook v0.5.3 `3dad6037` re-pin，`3b10b479`(v0.1.3) 禁部署**；**apply 用 MANIFEST sha256 钉死的候选全脚本，不 `git apply` 带 scratch 前缀的 diff**）；④ runbook v0.5.1（`//F //T` + stop 后 0 relay 双判据验证闸）；⑤ llama loopback 验收（`a5-verify.ps1`）+ 防火墙 llama 规则 Disable；⑥ watchdog v0.4 enable（D-013 §3 五前置）。J1 角色 B 提权执行暂停期间，提权步骤由 Bettor 令下逐条指派。
   > 📌 追加（2026-08-29 08:2x Z · Bettor · 出处 NWT `docs/2026-08-29-nwt-node-exe-firewall-surface-scope.md` + COORD-LEDGER (716)）：⑤ 扩展为 **node.exe 入站面收窄**——(a) agent-adapter `listen` 缺 host 参数 ⇒ 全接口（"守卫在一个服务漏了同胞"型）⇒ 修 = 侧分支 `coord/adapter-loopback` 6c21bb12（`ADAPTER_HOST` 默认 `127.0.0.1`，NWT 审中），部署 = adapter 重启随窗；(b) 防火墙 `node.exe` 应用级 Allow → 只放必需端口的 port-rule（backstop，改前 `Export-NetFirewallRule`）；(c) 外部 onboarding 网关非 loopback 是否该此刻对外可达 = **Owner 确认**（是 ⇒ port-rule 只曝该端口；否 ⇒ 撤 env 默认不启动）；(d) zk-prove-server tailnet 接口保持。明细不进公开 git（本机 `scratch/node-firewall-surface-detail.md`）。角色 B 已恢复"令下执行"（r18），提权步骤仍由 Bettor 逐条下令。
...
1. **§10 跨节点 pubkey 身份 v1 = GO（开工）**。依据 = 设计 `docs/2026-08-19-s10-pubkey-identity-design.md`@847bcf22（Codex MSG-250 五项 CLOSED，register-only）+ 实施计划 `docs/2026-08-27-j2-s10-impl-plan-v0.1.md` v0.2 = 70761d33（NWT 深审 PASS-WITH-NOTES e128a735）。**范围铁律**：v1 只做 `operation="register"`；rotate/revoke/旧记录迁移不含；不做自动签发口；**不部署对外、不动 Owner 实例 §0 墙（D-012 §0 Track A 七铁律）**；不改 A2 六字段语义（S10 独立信封）。流程：J2 落码（pathspec commit 不推）→ 每 commit NWT diff 审 → Bettor 推 → Codex 桥审 → **上线 = D-005 独立迁移由 Owner 另拍**。
   > 📌 状态注记（2026-08-27 · J2 · 切片计划 `docs/2026-08-27-j2-s10-commit-slice-plan-v0.1.md` v0.2 16ecff6d）：五片已落码——**C1 `99da28cd`**（S10 信封 + 纯验证器 + 向量 29/29；NWT GREEN，golden generator = J1 独立实现 ⇒ 真独立 oracle）、**C2 `71230846`**（v198 `u1_relay_identity`：PK = `relay_pubkey_xonly` 小写 64-hex CHECK，无 `local_relay_id`/无 relay_id 回退索引，④ 7/7；NWT GREEN）——两片已推（HEAD 02982868）；**C3 `d03f7365`**（A2 接线 fail-closed：缺 `s10` ⇒ `RELAY_NOT_OWNED`；验签公钥取 payload、DB 地址活算只做绑定；`localNetwork` 取本地配置不回落 payload；事务内重做绑定 + INSERT；46/46，mutants 27 detect/0 MISSED/0 INERT/0 BROKEN/5 UNREACHABLE（既有）；两处有意偏离切片计划交 NWT 判）+ **C3-fw `591e0e27`**（`cases/identity/` 两条后部署负向）+ **C4 `e75dad8b`**（builder 密钥区签 S10 + 离线自证 4/4；e2e 补 s10 + ⑦ 无 s10 live 负测臂；runbook v0.4）+ **C5**（本注记 + ANTI-PATTERNS 规则 73）——待 NWT 逐审、Bettor 推、Codex 桥审。**⑦ 语义（NWT ① 一致）**：v1 关掉的是**跨节点/远程**抢注（远程签不出本机 relay-B 地址那把钥）；**同机 loopback 抢注仍在**（同 (528)），不宣称关掉；跨节点身份表 = `u1_relay_identity`；rotate/revoke 仍 OPEN。🔴 **代码入库 ≠ live**：live `kasia-console/data/console.db` 2026-08-27 实核（Bettor 直读, (698)）**有 v197、只缺 v198**（console 未重启跑迁移）；§6-1 E2E 与后部署验收都在 D-005 之后。（J2 首报"既无 v198 也无 v197"**为误**：从仓根跑脚本时 `client.js` 默认路径随 cwd 漂到 `D:/kanet-tn12/data/console.db` 杂库——ANTI-PATTERNS 规则 74。）**Codex MSG-285 回（bridge `5bf01a28`）= GREEN-at-code-layer（register-only）；LIVE 仍 HOLD**：L1–L5 PASS、两处偏离 ACCEPTED、⑦ 措辞 ACCEPTED 仅带既有限定（不证同机 principal、不闭 loopback/文件系统、不含 rotate/revoke/legacy 继承）；一条 SHOULD-FIX（v198 `network` 表级闭枚举，C6 落）；**live = D-005 迁移 + Codex GREEN-at-live 八臂（runbook v0.5 §4）**。
...
3. **watchdog SYNCING 三态落码计划 v0.3（48d025f6）= 批**。按计划：三态（ALIVE/SYNCING/DEAD）替代"daa=0 判死"；刹车 N=5/T=5 min/冷却 30 min（沿 `scripts/kanet-console-supervisor.sh:30-32`）不得被恢复动作自身 reset；(PID,CreationDate) 元组判自重启；显式退出码 7/8/9；VA-1…9 向量。**启用条件不变**：本机节点 READY（`isSynced ∧ daa > 80,095,687`）+ VA 全过 + NWT diff GREEN；IBD 期只落码不启用（`KANet-KaspadWatchdog` 任务保持 Disabled）。
   > 📌 状态注记（2026-08-27 · Bettor · 出处 COORD-LEDGER (697)）：码已落 = probe `e8375cd7`+fix-up `02982868`、watchdog+VA `90d7443a`（NWT GREEN-WITH-NOTES，VA 17/17 NWT 本机复现）。**启用前置闸追加三条（NWT 红队，enable 闸非 push 闸）**：① 自/外判别 + 刹车-keep 决策目前是 while 循环内联，VA-5/VA-8 单测够不到 ⇒ **抽成纯函数**（平行 `Get-Verdict`）使其可单测；② L1 no-process override 依赖 CIM：`$cur=null` 可能是 WMI 故障非真无进程，且 probe 用 `tasklist`、watchdog 用 `Get-CimInstance` 两套会分歧 ⇒ CIM-empty 须与 probe 结果对质或 CIM 重试后再 override；③ 刹车 `$script:restartAttempts` 为内存态，watchdog 自身重启即清零（由 supervisor 对 watchdog 的刹车兜底）⇒ 记明。启用 = READY + VA 全过（含 ①） + ②③ 处置 + NWT diff GREEN + Bettor 令。
   > 📌 追加（2026-08-28 · Bettor · 出处 COORD-LEDGER (701)）：enable 闸 ①②③ 已落（3dfac191 + b817e47c，NWT GREEN）。enable-time 集成测（`kaspad-watchdog-enable-va.test.ps1`，KANet-UI 施工）引入 `KASPAD_WATCHDOG_TESTMODE=1` 单一总开关门控的 4 个 test-only 元素（进程名 / probe mock / **spawn 覆盖** / MAX_TICKS）。因 spawn 覆盖抬高了 TESTMODE 的影响面（生产误开 = spawn 哑进程 = kaspad 永不重启），**启用前置断言追加第 ④ 条：`KASPAD_WATCHDOG_TESTMODE` 在生产启动环境与任务 XML 中 unset**；watchdog 启动检出 TESTMODE=1 须 LOUD。
...
> 📌 **⑦ 状态注记（2026-08-27 · J2 · §10 v1 C3 `d03f7365`, 待 NWT 审/未推/未部署 · 不改上方原话）**: §10 v1 起注册要求 relay **地址钥**签的 `s10` 信封（`RELAY_NOT_OWNED` 已是 `REG_REJECT` 白名单新值; 绑定 = `XOnlyPublicKey.fromAddress(relay_nodes.address)` 活算 === payload 钥, 验签公钥仍取 payload）⇒ **⑦ 的跨节点/远程抢注关掉**（远程签不出本机 relay-B 地址钥）; **同机 loopback 抢注仍在 (528) 内**, 不宣称关掉; 跨节点身份表 = `u1_relay_identity`（v198, PK = pubkey, 无 relay_id 回退列/索引, ANTI-PATTERNS 规则 73）; rotate/revoke 仍 OPEN（设计 §7）。上方"要 relay 本人 IPC 签挑战"那半句 = 今已由 builder（密钥区同叶钥）/ e2e（relay `ecdsa_sign`）两条路各实现一次。**部署与否以 D-013 §1 状态注记为准。**
