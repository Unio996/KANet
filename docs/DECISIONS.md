...
3. **执行（设计先行 + D-031 先查已有）**：现有闸 =  五必填字段校验、判定来源仅 ESPN 结构化算术 + Polymarket 条件、coingecko 无 predicate 不开自动判定、截止时间预留争议期。派 NWT 红队题「用现有建题接口出一道会被冻结的题」，能出即缺口；缺口优先收紧现有校验与来源名单，不另起。排序：F3/F4 之后、simnet 异常臂之前。
