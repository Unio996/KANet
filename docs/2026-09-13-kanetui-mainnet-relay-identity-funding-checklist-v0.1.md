...
> 📌 **状态注记（2026-09-14 · KANet-UI · Owner 1168 ④ + Bettor 1168-补）**：上面这段与下方 §2 描述的"新建 1 个验证身份"这条路径**已被 Owner 的账号迁移决定绕过**——Owner 1168 拍板"所有已知账户都要导入"后，波 0/1 的验证身份改为**复用本次迁移进来的一个已有小额账号**（`Bettor` 1.59303211 KAS 或 `Trader-A` 7.45579730 KAS 二选一，见迁移 runbook `docs/2026-09-14-kanetui-mainnet-account-migration-runbook-v0.1.md` §8），不再走本页 §2"`POST /relays/generate-mnemonic` 全新生成"这条流程。下方 §2-§5 描述的九步生命周期（验证→停→等复活窗→Owner 备份确认→DELETE）**本身仍然适用**，只是流程起点从"新生成一行"换成"迁移已经导入的这一行"——这条不改变本节"是否需要建身份"的技术结论（本节结论仍然成立：不需要新建，波 0/1 用不到"新建"这个动作本身，无论走哪条来源都一样）。**顺带记一条 Rule 1 零引用核查结果（Bettor 1168-补，跟本页§5"零引用"判据同一条纪律）**：21 个有资金地址里 `Trader-B` 命中源码具名常量（同 GO-E 系列此前点名过的 `TRADER_B_ADDR`/`BROKER_KASPA` 一档），其余 20 个（含 `Bettor`/`Trader-A` 这两个候选）0 命中——`Trader-B` 不适合作为本页任何用途的验证身份候选，不止因为它归冷（迁移 runbook §6.1，relay 不会被拉起），也因为它本身撞了这条独立的零引用判据，两条理由分开成立、不是同一件事。
