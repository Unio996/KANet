...
- **不批准"设计稿通过=可以合入"**：T1 v0.6 + T3 联合合入前，我要求三件事逐条兑现且我逐条复核：(a) 7 个/约15条
  ...
  > 📌 **状态注记（2026-09-14 · NWT · Bettor 1195/1195-补 · 不改原文）**：本条(c)"钉死为不小于协议实际
  > 最大输入数"这句字面读=`max_ins_scan`须`≥max_tx_inputs(1122裁1000)`——T3 v0.3落码实际采用的是
  > `require(tx.inputs.length<=MAX_INS_SCAN)`先拒超界交易再遍历(`MAX_INS_SCAN=8`)这个**等价安全的替代
  > 形式**，不是字面的"≥1000"。这条替代形式我在`a036641c`已经明确核过并独立验证：取值依据`max_tx_inputs
  > =1000`（1122自己钉的数）引用无误，"先拒超界+界与循环深度共享同一常量"这个等价形跟1122-补的裁决一致，
  > 独立编译+跑通`MarketScanProbe3.sil`全部9条边界向量（4 bound + 5 其它，与`run.log`一致）。**结论：
  > `MAX_INS_SCAN=8`在安全性质上成立，本条(c)的字面表述滞后于后续裁决，不是安全结论被推翻**——本条原文
  > 不改，之后引用(c)时按这条注记理解，不要只读字面"≥1000"就判T3落码违反本条。
