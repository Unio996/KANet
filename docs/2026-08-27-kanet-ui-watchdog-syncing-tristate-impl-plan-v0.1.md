...
> **v0.3(收尾·呈 Owner 批版本·应 NWT GREEN)**: A 自重启判别改 **(PID, CreationDate) 元组精确匹配**(去 15s slack OR, PID 复用伪造不了 CreationDate); B 刹车 alert **真响**(醒目 marker + 频道可用则发, 非埋一行); C 状态文件持续不可读 ⇒ LOUD 告警 + **操作员诊断后手动删 `kaspad-watchdog-state.json` 清刹车(非自动)**; D 基线引用改全路径 `scripts/kanet-console-supervisor.sh:30-32`。
...
- **写死定义(建议值+依据)**: **N=5 次 restart / T=5 min 窗** ⇒ 停拉 + 升 `STALLED-escalate`(交操作员告警, 非重启); 触发后 **30 min cooldown**(cooldown 内即便真死也只告警不拉)。依据 = 照 `scripts/kanet-console-supervisor.sh:30 RESTART_WINDOW_SEC=300 / :31 RESTART_MAX_IN_WINDOW=5 / :32 COOL_DOWN_SEC=1800`(storm/cooldown 逻辑 :146-156, 实测基线值, 同族机制复用不另拍)。env 可覆盖 `KASPAD_WATCHDOG_MAX_RESTARTS`(默认 5)/ `_RESTART_WINDOW_SEC`(300)/ `_COOLDOWN_SEC`(1800)。
...
- 🔴 **自重启 vs 外部重启判别(MUST③)**: watchdog **记录自己上次 Start-Process 的 {ts, spawnedPid}**(状态文件 `kaspad-watchdog-state.json`)。下 tick 见 kaspad (PID, CreationDate) 变, 判别用 **(PID, CreationDate) 元组精确匹配**(v0.3·去 slack): 新 (PID==自己 lastSpawnPid **且** CreationDate==自己 lastSpawnCreationDate)⇒ **自重启** ⇒ **不 reset 刹车计数**(这次拉起本就计在刹车里, 清了=自清=无限拉); 否则(元组不吻合)⇒ **外部重启**(别人/J1 起的)⇒ failCount=0 **且刹车计数可 reset**; 判别失败(状态读不到)⇒ **fail-closed: 当自重启, 不 reset 刹车**。🔵 **为什么去 15s slack**: CreationDate 是内核记的进程创建时刻, **PID 复用伪造不了它** ⇒ 元组精确等值即可, 不需时间近似窗(近似窗反而可能把碰巧同刻的外部重启误判成自重启)。
...
+             # B(v0.3) alert 真响, 不埋一行:
+             $marker = "!!!!! kaspad CRASH-LOOP DETECTED !!!!! restarts>=MAX in window (or cooldown) -> STALLED-escalate, NO Start-Process, OPERATOR ACTION NEEDED"
+             Log $marker; Write-Warning $marker                          # 醒目 marker + WARN 流
+             Try-BroadcastChannel $marker                                # 频道可用则发(同 watchdog 现有告警通道, 不可用则吞掉不阻塞)
+             # (若接 owner-bot/事件日志: 亦可 eventcreate Error 级, 同探针稿 §0.5 不依赖 console 通道)
+         } else {
...
+             $spawnedCd = (Get-CimInstance Win32_Process -Filter "ProcessId=$($proc.Id)").CreationDate  # v0.3 记 CreationDate
+             Save-WatchdogState @{ lastSpawnPid=$proc.Id; lastSpawnCreationDate=$spawnedCd }  # MUST3 自重启判别(元组精确)
+             $failCount = 0
+         }
+     }
+ }
...
+     $mine = ($cur.ProcessId -eq $st.lastSpawnPid -and $cur.CreationDate -eq $st.lastSpawnCreationDate)   # v0.3: (PID,CreationDate) 元组精确, 无 slack
...
```
- **无 slack 常量**(v0.3 去掉 SPAWN_MATCH_SLACK): 元组精确等值。状态文件 `D:\kaspa-tn12-data\kaspad-watchdog-state.json`(env `KASPAD_WATCHDOG_STATE`), 记 `lastSpawnPid` + `lastSpawnCreationDate`(spawn 后 Get-CimInstance 取新进程 CreationDate)。
- 🔴 **fail-closed**: 判别失败(状态文件缺/读错)⇒ **当自重启**(不清刹车)——宁可多等操作员, 不可自清致 8/23 连锁。
- 🔴 **C(v0.3) 状态文件持续不可读**: 单次读错=fail-closed(上)+一行日志; 但**连续 K tick(建议 3)读/写失败** ⇒ **LOUD 告警**(同 B 的 marker+频道, 因为刹车已无记忆=可能连锁而无人知)。**清刹车不自动**: 文档写明**操作员诊断后手动删 `kaspad-watchdog-state.json` 清刹车/复位状态**——不让 watchdog 自己在读不到时重建/清零(那等于自清)。
