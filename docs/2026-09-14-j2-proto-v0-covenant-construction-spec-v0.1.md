...
| `token_tmpl_hash` | `byte[32]` | BACKEND-DERIVED：`KanetTestToken` 编译产物的模板 hash——**协议常量**，与选中哪个代币定义无关（所有 KTT 实例共用同一份编译模板；代币定义只影响 DB 展示元数据），跟 `ps_tmpl_hash` 一样只需要算一次、可复用于所有市场。📌 **修订（2026-09-14，见 `docs/provenance/2026-09-14-j2-template-tail-anchor-uniqueness/`）**：这句"协议常量"曾经是**未经验证的假设**——实测证明若 `KanetTestToken` 的 `market_tmpl_suffix`（该 ctor 依赖的输入）取整个 `ShardLeaf_direct` 的 `extractTemplateArtifact` suffix，会随 `shard_pool_id`/`min_bet`/`rootclose_tmpl_hash` 等逐市场字段变化，`token_tmpl_hash` 也就跟着不是协议常量。**修正结论**：`market_tmpl_suffix` 改取 `ShardLeaf_direct` 编译产物里一段真实存在、唯一、与全部非-State ctor 字段无关的 132 字节稳定尾部（provenance 已证唯一性+真实端到端向量），这样算出的 `token_tmpl_hash` 才真的是协议常量——按这条修正后的机制计算即可，不是走原文字面的"整段 suffix"。 |
...
- **T-PROTO-TAIL-OPCODE-DISASM**（新票，2026-09-14，见 `docs/provenance/2026-09-14-j2-template-tail-anchor-uniqueness/`）：`ShardLeaf_direct` 编译产物末尾那段 132 字节稳定尾部（拟作 `market_tmpl_suffix` 的真实值）没有做逐 opcode 反汇编——机器上没有现成的 silverscript 字节码反汇编工具，provenance 只用间接证据（源码自带的 `OWN_PREFIX_LEN=1`/`OWN_STATE_LEN=36` 常量注释与实测 state_layout 吻合、11+1 合约唯一性扫描本身反证"不是编译器通用 epilogue"）支持"这是合约特定逻辑不是死代码尾巴"的判断，不是逐位确认它对应哪几条 require/opcode。**须 NWT 用 silverscript 源码 opcode 表手工比对确认后，`market_tmpl_suffix` 才能正式定案写入 `proto-v0-template-anchors.json`**，在此之前只是"证据支持、未逐位证实"的状态。
- **T-PROTO-TEMPLATE-CONST-ASSUMPTION-CORRECTED**（记录一次已修正的架构假设错误，2026-09-14，见上方 provenance）：本文档 §2/§4 曾把 `market_tmpl_suffix`/`token_tmpl_hash`/`claim_tmpl_hash`/`refundclaim_tmpl_hash` 都描述成"编译一次、全市场复用的协议常量"——这个说法从未被实测验证，实测证明其中 3 个（`market_tmpl_suffix`/`token_tmpl_hash`，以及大概率 `claim_tmpl_hash`/`refundclaim_tmpl_hash`，因为 `RootClaim`/`RefundClaim` 的 ctor 同样带 `shard_pool_id` 这个逐市场字段）如果按"整段 `extractTemplateArtifact` suffix"来算，实际是逐市场不同的值，不是协议常量——真正验证过的、无条件成立的协议常量只有 `PoolSideTicket` 的 `ps_tmpl_hash`（该合约没有任何 ctor-only 字段）。`market_tmpl_suffix` 的修正方案（取 132 字节稳定尾部）已验证可行，但 `claim_tmpl_hash`/`refundclaim_tmpl_hash` 是否有类似的"稳定尾部"救得回来、还是必须挪进 market_genesis 按市场现算——**尚未对 `RootClaim`/`RefundClaim` 做类似 `05-stable-tail-search.mjs` 的稳定尾部搜索**，是下一步待办，不在本次 provenance 范围内。
