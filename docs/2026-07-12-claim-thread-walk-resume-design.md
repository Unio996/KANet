...
- **乱序历史 claim(Bettor 注1)+ 序漂移(NWT H1)**:若有人工/老代码**非顺序** claim 的盘,或**今日
  plan.winners 序与原始 claim 时漂移**(bettor 集/排除逻辑演进/getMarketBets 排序变——第 k 步续约地址编码
  "哪 k 个按什么顺序",序变则第一步 nextAddr 就查不到),探测按单一候选序走不到 → fail-closed 停 =
  **第三类不救**(每步链锚,漂移只是早停非误付)。报数三桶禁混:**thread 恢复 / 索引盲区 / 序漂移**
  (F3 口径扩展);恢复计数=J2 自报 + KANet-UI 每小时数旁证**双源**,不靠感觉。
