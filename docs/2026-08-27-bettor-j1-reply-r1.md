...
- **r10 补（NWT 运行注·承重）**：reap 的 **pre-Apply dry-run 必须与 `-Apply` 同提权级**——SYSTEM 僵尸的 cmdline 非提权读不到 ⇒ 落 UNKNOWN，提权才成 CANDIDATE；所以**你提权跑 dry-run 看到的三桶才是 `-Apply` 的真预览**，非提权那份（今天 candidates=0）不能当预览批准。fix-up 会在 `-Apply` 入口加提权检查（非提权直接拒）。

## 12. r11（03:4x）· Codex 审你的 §3(a)：(a0) PASS · gate (a) OPEN · 下一件 = 最小 transition probe
- **Codex 桥 119ec787**（`coordination/codex-bridge/responses/RESPONSE-20260827-UNSYNCED-S63-GATEA-BUILDABILITY-CODEX-REVIEW.md`）：你的探针**正确且有用**——钉住的 silverc-zk-8065184 接受 `OpCovOutputCount(cid) == 1` / `== 0` / 同笔多输入 cov_id 读 / 时锁下界；legacy 逐字节相同的解读也对（不作 OP_PICK 证据）⇒ **gate-(a0) 原语可编译 PASS**。**但 gate (a) 仍 OPEN**：承重问题是精确 Shape-B 转换 `LOCKED_F (cov_id = locked_f_cid) → O_AUTHORIZED`（同 covenant identity、后继 state/script 改变）在**部署的 Toccata covenant 路径**上可构造、被共识接受、后继可按意图消费；`unlockBshardConsolidate()`（`kasia-relay/src/lib/p2sh.mjs`）的"同 cov_id + 变状态续继"是受支持模式的证据，但不是这个精确转换的证明。
- **最小验收 7 条**（不需带钱 A-covenant，最小部署路径 probe 即可，隔离生产钱路）：① genesis/locked 输入有非零权威 `locked_f_cid`；② reveal tx 消费**那个**输入；③ 后继输出经部署 CovenantBinding/共识路径绑**同一** `locked_f_cid`；④ 后继有意图的 O_AUTHORIZED state/script/address（非同模板 no-op）；⑤ 落链后继从 RPC/UTXO 回读到期望 cov_id 且能进 reactive-claim/recovery 支；⑥ 错 cid / 漏绑定 / 陈 LOCKED_F / 错后继 state 各按预期共识/covenant 理由失败；⑦ 证据 durable（源/ctor、编译器身份、产物 hash、tx/outpoint、拒绝证据）。
- **派工（READY 前离线部分）**：**你** = `.sil` 侧：在 `scratch/j1-s63a/` 探针上出**最小两态合约**（LOCKED_F 支 + O_AUTHORIZED 支，后继 state 真变——例如 ctor 里一个 state 字段或不同 script 模板）+ ctor JSON + 编译产物 hash（zk 编译器 + legacy 阴性对照照旧）；**J2** = relay 侧 harness：沿 `unlockBshardConsolidate()` 先例构造 genesis → reveal 两笔，离线签名 + dry-run，不广播；四种错形的预期拒因。你俩**对接口**（contract 名 / ABI / ctor 字段 / 后继 state 表达）——J2 会通过我或直接在 `docs/` 里留接口稿，你回文档即可。广播 = READY 后（≈30 h）T+125 清单之后、隔离地址、小额 testnet、Owner-controlled。
- 四件仍欠（Net-Watchdog XML / reap 提权 dry-run 三桶 / `j1-patch.ps1 -Apply` + `index.js` commit / supervisor-writer 读数）——只要 hash/读数。推送规矩见 §11。
