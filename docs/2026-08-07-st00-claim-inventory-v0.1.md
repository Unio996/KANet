...
> **v0.2-a 措辞降级(2026-08-07 11:1xZ · Codex 3486cb17 审后)**: 🔴 **本文件全部资金/计数数字(171,227 / 81,665 / 48% / 52% / 701 / 21+6 / 36,012 / 26 / 6,360 / 1,341 / 99.5% / 40-40 / 14-14 / 2,863 …)一律为 `OBSERVED · NOT-YET-INDEPENDENTLY-REPRODUCIBLE`**——它们是本机运行时观察;证据包已落并过审——**J2 主包 `docs/2026-08-07-st00-exposure-evidence.md`(`712b9ad7`,Codex 八项形态,NWT 抽跑三条 SQL 逐字对上)+ J1 节点侧互补包(`72729b90`,补跨节点重跑不出的本机读数)**,两份合起来才是完整可复现面。⇒ 现级别 = **OBSERVED·REPRODUCIBLE-VIA-EVIDENCE-PACKAGE**(可照证据包 rerun 命令复现),但仍**非 VERIFIED 制度性主张**(身份锚有两处如实缺口: `PRAGMA user_version=0` 无 schema 版本、kaspad 横幅未证当前进程所打;覆盖限=本机留存 DB 非全 L1)。引用数字必带证据包指针。唯一被 Codex 升级的是 **#5 V1 PayoutShard liveness 失败 = CONFIRMED-AT-CODE-LEVEL**(逐行读 `PayoutShard.sil`:cancel_attest 要 validSigs>=4、`closed 0→2`、零 tx.time/timelock/无许可逃生;作用域**仅 V1 PayoutShard 一族,禁外推**)。
