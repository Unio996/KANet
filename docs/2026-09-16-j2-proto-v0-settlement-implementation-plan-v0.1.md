...
**v0.11更新（2026-09-19，批8 ticket_reclaim；Bettor 放行，账本1524 裁定②改形状）**：① **批8 `buildTicketReclaimTxJson`（`PoolSideTicket.authorize_spend`，输家 ticket 自我回收）已落码，离线通过（`proto-claim-draw.test.mjs` 47/47，十个变异对照全红）并已过 simnet（新市场 chain4 完整 1–9 步，首次上链即 ACCEPT；断言信号 == 节点 mempool == 区块记录：storage 260 / compute 15,282；fee 2,292,300；证据 `docs/provenance/2026-09-19-j2-ticket-reclaim-v2-simnet/`；n=1）**。**最终形状（Bettor 1524 裁定②，不动 relay）**：[输家 ticket（普通 P2SH 无 covenant）, relay fee 输入] → [bettor 指定的 34 字节 P2PK（ticket 全额 20,000,000，无默认值）, 找零回 relay]；ticket 由 builder 现签、fee 输入由 relay 签（`signInputIndices=[1]`）；leftover = fee 输入面值（ticket 20M 进 20M 出恰好抵消，同其他 builder）；C1 角色表 `ticket_reclaim:[ticket]`（fee 输入不在任何 C1 表内，同其他 builder），`inputHasCovenant` 钉为 `[false,false]`。早先单输入"builder 已完整签名"形状（提交 `5e62e24f`，simnet ACCEPT，证据 `docs/provenance/2026-09-19-j2-ticket-reclaim-simnet/`）作废——relay `covenant_broadcast` 拒空 `signInputIndices`。② **fee 不复用 `computeRequiredFeeSompiOrThrow`**：selectChangeShape 的"所需费"换成精确 mass（100×max(storage,compute,transient)，节点最低费=100×compute，实测 2 输入形状 1,528,200）×3/2；`feeProfile.ticket_reclaim.cap` 为 1.0 KAS 占位待 NWT 推数（旧 2,000,000 随单输入形状作废）。🔴 **订正**：设计文档 §0.14b 的"本地 814"不要再引（同形状本地 wasm 实为 3,087，1491 的构造未追）。③ **可回收闸**（先于解密/签名）：只有市场已结算（closed=1）且这张票是输家（side≠winningSide）才构造，`marketState` 必须来自链上已验证状态。④ 🔴 **批9 接线阻塞项（比"空 signInputIndices"更深，未修，待 Bettor 裁定）：relay 的 fee 估算器**。relay `computeRequiredFeeSompi` 用 kaspa-wasm 本地 mass×100，`validateNetLoss`/`validateImpliedMinerFee` 的天花板=2×该估算；wasm 对 v1 covenant 交易漏计 computeBudget（每输入 70×100=7,000 mass）。ticket_reclaim 是 compute 占优形状（storage≈0）：relay 估算 141,000 ⇒ 天花板 282,000，而节点最低费 1,528,200 ⇒ **任何 fee 都无法同时满足 relay 与节点**（simnet 实证：fee=282,000 被节点拒 "under the required amount of 1528200 for compute mass 15282"；测试 ㉖ 表征此死结，relay 修好会变红）。其他 builder 恰因 storage 占优、本地 wasm 高估而不受影响。⇒ "relay 侧无需改动"对 ticket_reclaim **不成立**：无论单输入/带 fee 输入形状，走 relay 都需修 relay 的 fee 估算器（钱路，须批准）或让 bettor 自己用钱包直接广播回收（不经 relay）。⑤ `feeProfile.withdraw.cap` 由占位 1.0 KAS 换为 55,000,000（NWT，账本1523，失效条件：固定布局 [KTC, held, fee]）。
...
9. **驱动接线**（§4，`proto-driver.mjs`扩展，含启动日志）——等六个builder都各自simnet验证过
   之后再接线。**relay侧无需新增命令**（§5，v0.2删除此前的relay命令批次）；🔴 v0.11 订正：对 ticket_reclaim 这一步"无需改动"**不成立**——relay 的 fee 估算器（本地 wasm mass×100，天花板=2×）对 compute 占优形状过低，需修 relay 或让 bettor 自行广播回收，详见 v0.11 ④。
