...
**Bettor 裁定记录（2026-09-19）**：M-1（85 / 92 / 80、严重回落 88）、M-2（60 s）、M-3、M-4、M-5（并入下一次本来要做的 console 重启）**全部采纳**；§2.7 只写键名、不动主网 env 正确。流程：NWT 红队审（设计先行第②步）通过后，才等 Bettor 派落码。M-6（谁读 `events`）仍开放。下表保留首版默认值以便对照。
