> **Status**: SUPERSEDED-BY-REMOVAL — Owner 裁定(账本 1408, `docs/DECISIONS.md` D-017 注记)撤销 H1(b)（KTT 花费侧"接收方必须是市场模板"这条检查本身），"代币就是代币"，不需要任何市场识别机制——②/④两个候选连同它们要解决的"怎么安全识别市场"这个问题一起作废，不是选型输给了对方。见 `docs/2026-09-14-j2-ktt-bin-template-lock-fix-design-v0.3.md`（方案 C：直接删掉整个 H1(b) 机制，不是修它）。本文档④部分的真实编译验证过程仍是有效的技术记录（证明了(b)"编译期环消除"这个子问题确实修得干净），保留供参考，不代表方案本身仍在用。
