# §6-3 gate (d) · `k_max` 绝对成本表 · 方法稿 v0.6（脚本入库 durable · 法 3 + gate_input 语义 · 设备算力 A/B/C 档 · 现网算力栏等同步 · 零落码）

> **v0.6（NWT 小注，cosmetic，脚本/向量不变）**：本成本表用 **raw 法 3**（`法3_raw = 块数/W × work_per_block`）作 `H_net` 估；(23) 入场地板用 **`法3_raw/(1+口径/W)`**（口径 = 132 s 戳偏）打折后的**保守下界**——两处法 3 **不同值**，勿混：成本表要的是"攻击者要追平多少"（不打折、偏高 = 攻击看着更贵，用于 CAPEX 对照），入场闸要的是"网络至少有多少"（打折、偏低 = fail-closed 向）。
