> **Status**: CURRENT — Bettor 接位文档（2026-09-19 写，取代 `docs/2026-06-15-Bettor-tn-handoff.md` 里所有 TN12 时代的状态描述；那份文档的"工具调用必须是真 invocation"纪律仍然有效，其余已过期）。**接位者先全读本文件，再读 `docs/DECISIONS.md` 的 D-017 至 D-022，再读 `docs/iteration/COORD-LEDGER.md` 最后 40 条。** D-021：本文件不写密钥值、真实余额、地址、内网入口。
