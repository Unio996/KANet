...
3. **（待批·不改则如此）** `kasia-console/src/ui/exchange.eta:1353` 直接显示 `o.payment_tx` 列值：batch-2 `6554b8d9` 起 auto-pay 转账前会先写 `PENDING:<offer8>:<uuid8>` 标记，成功即换真 hash；**只有转账失败/结果不明的 offer** 会留着标记 ⇒ operator UI 那一格会显示裸串 `PENDING:xxxxxxxx:xxxxxxxx`（可辨识，非用户 DM）。改成「处理中」之类文案 = 用户面 = Owner 域，本批**不改**；Owner 若要改，一行 `o.payment_tx.startsWith('PENDING:') ? '处理中' : o.payment_tx`。 **自查补充（J2 8/29）**：同文件 `:533-537` 详情面板用 `getPaymentTx()` 建 `KANet.explorerTxUrl(chain, tx)` 链接 ⇒ 标记留着时会渲染一条指向 `…/PENDING:…` 的**死链**（operator 面，点了 404，不花钱不误状态）；**更正（同日）**：`:1357-1362` 的 `getExplorerUrl(o)` 同样用 `getPaymentTx` 拼 bscscan/etherscan 链接 ⇒ **两处**会建死链（`:535` 与 `getExplorerUrl` 的调用点），不是一处；同一判前缀修法覆盖两处。修法同一行判 `startsWith('PENDING:')` 时不建链接——仍属 UI 文案面，随第 3 处一并待 Owner。
