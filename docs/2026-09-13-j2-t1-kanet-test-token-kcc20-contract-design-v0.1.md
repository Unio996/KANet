...
- **T1 落码前的唯一结构性未决（v0.4）**：A′ 按裁定写进本稿，但 A′ 的洞我不能当没看见；**请 NWT 判 A′ 的洞是否成立、A″ 是否采纳**（若 A″ ⇒ T2/T3 设计的硬约束 = "per-instance 值只能进状态"，见 §5）。本稿不擅自把 A″ 当成裁定。
...
- **v0.4 · A″ 下的 T2/T3 硬约束（若采纳）**："per-instance 值只能进状态、只做等值比较"——市场：`token_tmpl_hash`(byte[32]) / oracle 公钥（固定个数的 `byte[32]`/`byte[33]` 字段，**不能** `byte[]`）/ 问题 hash / 截止（int）；领取：`winner_pubkey`（byte[32]，H4 从 ctor 改状态）/ `market_cov_id` / `token_tmpl_hash`。市场 ctor 只留结构常量（`max_ins/max_outs`、领取模板 hash）。T3 现无稿，此约束零成本进；若 T3 已有 per-instance ctor 的设计习惯（bshard 家族 `ShardLeaf` 烤市场 id 是这种），要在 T3 稿里显式改。
...
6. **Q6（v0.4 新 · 结构性）A′ 的洞**：§4 "双 R 攻击"是否成立（关键前提 = 创世输出脚本不在创世 tx 执行，R′ 可带任意状态出生）；成立则 **A″**（注册状态并回市场/领取本体）是否采纳、还是 **D**（发行方签名的模板证明）。这是 T1 落码前唯一未决；(b) 的其余部分（P9 四向量）不受影响。
...
- **v0.4 未核**："创世不跑脚本"是 Kaspa 共识对 P2SH 输出的一般事实（输出脚本只在被花时执行）；covenant genesis 的特殊之处只在 cov id 由授权输入 outpoint + 有序输出派生（`consensus/core/src/tx.rs:377`）。我没找到任何"创世状态被验证"的路径，但**没找到 ≠ 没有**——请 NWT 从 `7b1e18cc` 的 covenant 验证代码正面确认一次。
