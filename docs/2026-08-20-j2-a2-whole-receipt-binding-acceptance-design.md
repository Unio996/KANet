...
### 🔴 lineage 唯一性 —— 两格（Codex MSG-260 MUST-FIX 2；**我核漏的那一格**）

`OpCovOutputCount(cid) >= 1` **只挡住"零/非法 cov_id"，挡不住"多个续链 output"**
⇒ 一次 reveal 可产出**多个**合法-lineage `O` ⇒ **违反"唯一 capability"**。
正确形态：**`== 1`**（reveal 恰一续继）+ **每条 terminal/refund/cancel 支的续链 output 数 `== 0`**。

⇒ **必测两格**：
1. 把 `== 1` 改回 **`>= 1`** ⇒ **验收必须挂**；
2. 让某条 **terminal 支产出一个续链 output** ⇒ **验收必须挂**。

🔨 **而我在这一格上犯的错值得写下来**：
我核过 `>= 1` 那句**在不在**（在，且有活先例、有前人红队注释），**并把它当作"绑定成立"的证据点出来**。
**我核的是【存在】，没核【强度】。**

> **从先例照搬一条 guard，会一并搬来它当初被校准的【强度】——而那个强度是为【别的需求】定的。**

先例（`ShardLeaf`）允许多个续继是**合理的**；而 capability `C` 要求**唯一**。
同一句 `>= 1`，在前者是正确的，在后者是洞。
（同族：**首个看起来像闸的检查，很少是最后一道**。）

### 🔴 原子性 —— 一格（NWT 17:28 逮的缝；**我那个修法自己的前提**）

「造 O 是领钱的前提」这句话，**只有在【同一笔交易】里才成立**。
若「花 LOCKED 领本金」与「消费 C 造 O」可以是**两笔独立交易**（witness 条件相同、揭示同一个 `s`），
首动方就能**只广播领本金那一笔** —— 拿到钱、`s` 也公开了、而 `O` 永不出现
⇒ **本该被 enforce-O-creation 堵住的 griefing 原样重现**。

⇒ **必测格**：**把 LOCKED-transfer 与 C-consume/O-create 拆成两笔交易分别提交 ⇒ 领本金那笔必须被拒。**

🔨 **这一格特别容易漏，因为缺失的约束【不在任何一行代码里】**：
两处 require 清单各自看都对，**分岔藏在"它们之间没有任何一句话"**。
⇒ 变异测试在这里的形态不是"改一行"，而是**"拆开提交"** —— 属于**交易级**变异，不是**语句级**。

> **判据：「生产 X 是领钱的前提」= 必须同一笔交易。**
> **分成两笔，它就退化成两个各自独立、可分别选择的动作 —— 而攻击者只会选对自己有利的那一个。**

（同族：安全性质只能来自**唯一路径**。这次路径不是被写成 OR 而分岔，是被**拆成两步**而分岔。）
