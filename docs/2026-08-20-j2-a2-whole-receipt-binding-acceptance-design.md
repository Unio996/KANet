...
### 🔴 cutoff 排序 —— 一格（NWT 17:41 ②；**配置级变异**）

构造引入了一对必须满足相对顺序的常量（`T_cutoff_LOCKED <= C_terminal_refund_cutoff`）。

🔴 **把它只写成 deploy-time 检查清单是不够的** —— 那和「记得禁了」是同一个失败形态：
**没有任何一格会在配反的时候红**。

⇒ **必测格（照本仓既有的"有牙"判据）**：
1. **按正确顺序**部署 ⇒ 攻击 case **必须 BUST**；
2. **把顺序配反**部署 ⇒ **同一个攻击 case 必须 LAND**。

🔨 第 2 条是关键：**它证明这道排序真的在承重**。
若配反了攻击**依然 BUST**，说明拦住攻击的是**别的东西**，这条排序是**装饰**——
而装饰会在下一次重构里被"顺手简化掉"，且没有任何测试会响。
（本仓既有判据：**攻击 case 修后 BUST ∧ 修前同 case LAND**，后半句才证明测试有牙。）

🟡 NWT ① 的提醒一并钉：这对新引入的比较值**必须显式标注单位来源**（DAA-score），
不靠"总裁定应该覆盖了"——今天这类单位 footgun 出现过太多次。

---

## 🔨 族 B 的**三个层级**（这次凑齐了，写下来防后人只做第一层）

| 层级 | 变异什么 | 逮的是哪类洞 | 本卡实例 |
|---|---|---|---|
| **语句级** | 删/改**某一行** require | **写下来的东西不够紧** | 删 merkle 留诱饵；`==1` 放松成 `>=1` |
| **交易级** | 不改代码，**改怎么提交** | **该写的约束根本没写** | 拆成两笔 tx 提交 |
| **配置级** | 不改代码，**改烤进去的常量关系** | **靠"记得配对"的义务** | cutoff 顺序配反 |

🔴 **只做语句级的验收，对后两层【结构性失明】**：
你把每一行都删一遍试，它们也不会红 —— 因为**要测的东西不在任何一行里**。
