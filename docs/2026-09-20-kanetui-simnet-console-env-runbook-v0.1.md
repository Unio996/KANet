...
> **Status**: CURRENT ｜ 依据：批 9 设计 v0.2 §9-4（全新 DB + 全新 env 副本，不碰主网库/主网 env）。首跑已通过（2026-09-19 20:30Z，D-026 树）；起 simnet 节点前先读提交内存（≥80% 不起，起后 ≥82% 立即 `down -WithNode`）。
...
**已知限制**：console 会写 Tree 里的 `logs/`、`data/`（所以必须用 worktree，不用主网检出）；PROTO_RELAY_ID 是 import 时读取，故加它后必须 `restart`；首跑实测：console 起后 stderr 有一行 `[proto] PROTO_RELAY_ID health check failed`——预期（fail-closed：未配 PROTO_RELAY_ID 时 proto 路由不注册，配好并 `restart` 后才有）；simnet 节点 `synced=false`（无 peer，`--enable-unsynced-mining` 靠自己挖），驱动若依赖 isSynced 需 J2 注意。
