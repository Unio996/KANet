# console supervisor · lifetime 风暴保护 + PID-aware boot grace · 设计草案 v0.1.3

> **Status**: DRAFT v0.1.3 · **NWT 设计层 GREEN（4 审点全核，2026-08-29）** · v0.1.3 = 验收 harness 去时序化（生产候选不动；provenance `3b10b479`，5/5 = 48/0 留档；等 NWT 再跑 5 次给最终 GREEN） · v0.1.2 = NWT 两条非阻塞注折入：① `console_state` 的 `rc` 取法（候选本就是 `local rc=0; … || rc=$?`，手写草案是旧形）；② **采** CreationDate 防 PID 复用（§3 行"PID 复用"改写；selftest 47/47） · J2 2026-08-29 · Bettor 派（今晨 05:14Z supervisor 重启风暴两条设计缺口 (a)(b)）· **不动生产代码；候选脚本 + 机器 diff + offline 验收 38/38 在 `docs/provenance/2026-08-29-supervisor-v01/`（不 apply）** · NWT 审 → 与 57fde30f 同批进维护窗（Owner 批：live 进程守护逻辑）。优先级：低于 READY 派单、高于 C3。
