...
> - **真 MUST-FIX（构造侧，Codex 那条掩盖了它们）**：① recovery tx 须 `lock_time = E`（DAA 数）**非 0**（现 `p2sh.mjs` 全 `lockTime:0n` ⇒ CLTV 因 `E>0` 拒 ⇒ recovery UNBUILDABLE）/ 多输入 `lock_time = max(E_i)`；② 被锁输入 **`0 <= sequence < MAX`**（Codex `9eab914a` MUST-FIX：helper `cltvSequence` 须拒 `>MAX` 不只 `==MAX`——NWT `b6a1651a` GREEN 漏了上界，认）+ 配置 **`n_recovery_delay_daa > 0`**（`lock_time=0`=无延迟，不得静默）；③ 源域守卫 A′ `require(E>=0); require(E<5e11); require(tx.time>=E)` = **same fail-closed domain predicate 同上游 `tx.daa` lowering 的 `WITHIN[0,5e11)`（【语义等价、非 byte-for-byte】——`WITHIN` 一 opcode vs `require` 多 opcode；Codex `9eab914a` 措辞纠，v0.16/稿"逐字等价"同改）**。均 builder 侧代码（另报备）。
>
> 🔵 **Codex re-close（`9eab914a` `RESPONSE-…-APRIME-CODEX-REVIEW`，2026-08-29）**：Codex **自纠 `14c81c1c`**（把上游分域过度套到 8065184）、**accept NWT/J2 的 refute = materially correct**；判 **A′ 设计层 SOUND**、**Shape-B same-chain 恢复 = CONDITIONALLY CLOSED（恢复时序原语 DESIGN-LAYER SOUND WITH A′）**、**gate (a) OPEN** 待 **N6/N7/N8/P + same-cid readback** 真链。⇒ 本页 D-016 状态 = Codex 复核确认，不因域混 REOPEN。
