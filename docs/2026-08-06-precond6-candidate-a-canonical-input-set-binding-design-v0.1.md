> **Status**: DRAFT v0.2.1 · **NWT 红队 §9-bis 三项指名任务已答复(2026-08-16)**——verdict `docs/2026-08-16-NWT-redteam-precond6-canonical-input-set-v0.2.1.md`:R-1.3 传递绑定攻击未破/R-2b"提出-比对"架构合法但覆盖面结论同意作者自纠(不升格不降级)/7-9 bets_excluded 授权判归另一张卡。**整体前置⑥ 状态不变,仍待 Bettor 裁 R-3.4 才具备可实现状态,NWT 不越权推进。** 原文未改。🔴 **v0.2.1 = 我自己推出 v0.2 后自查发现的三条更正(见文末「v0.2.1 自纠块」): R-2b 机制破两处 · `oracle_fee_bps` 同样没有链上权威 ⇒ R-3 范围扩大到整个费率层 · R-3 一句支撑推理错但结论更硬。** 引用本稿任何关于 `redeem_ctor` / 费率权威的结论前, **先读文末自纠块**。
...
>    🔵 **免编译器的替代路子我试过了, 在我这份数据上结构性跑不通 —— 写下来省得你重做**:
>    思路是「同一个 `spine_p2sh` 的 commingled 组里, 成员若在某 ctor 列上取值不同 ⇒ 该列必然不在 redeem」。
>    **但本机库 1317 盘【全是 post-fix v0.7】、`spine_p2sh` 两两互异、commingled 组 = 0** `[CONFIRMED·DB实读]` ——
>    post-fix v0.7 烤了 `market_id`(`pool-commingle-detect.mjs:5`), **唯一性由构造保证**, 探针因此永远读不出信号。
>    ⇒ 🔴 **能跑这个测的数据集是【pre-fix v0.7 的那 49 组 commingled】**(v0.1 §2.1 引自
>    `docs/2026-08-03-pbs8-2-candidate-b-implementation-design.md:376`, **那是 J2 那台的读数, 不在我库里**)。
>    **谁手上有那批盘, 谁不用编译器就能出这个结论。**
>    ⚠ 该探针只有**阳性**方向成立:组内某列取值不同 ⇒ 证明它**不在** redeem;组内全同 ⇒ **对该列无话可说**, 不等于它在。
