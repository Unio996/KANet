...
| 🔴 **`exchange-machine.js:1000-1066` auto-deliver KAS/EVM → `delivery_tx` / `retail_dex_orders.deliver_tx_hash`** | **(ii) `transfer` / `sendKas` 从 maker relay 钱包** | 有 `:877 transition('delivering')`，但 **无任何 `delivery_tx` 已存在的检查**（grep `.delivery_tx`/`IS NOT NULL` 0 命中） | **不需要掉电就能双付，但机制首版写错、15:2xZ 更正**：`paid` 回放路被 `trade-protocol-filter.js:2451` `if (offer.payment_tx) skip` 挡住（payment_tx 在 paid 核验时已写）⇒ **自动**双发路 = 与 P1-B 同形的**同 tick 超时重试**（:1000-1016 attempt 循环 ×3、`sendCommandAsync` 默认 30s 超时、迟到 txId 被丢、relay 侧 tx 照落）；**人工**双发路 = :650-657 60 min 把 delivering 回退 verified（注释原话"KAS may have been sent"）后，`api/trading.js:2454-2468` 操作员「Send KAS」端点从 verified `force` 进 delivering 再发，**无任何 delivery_tx / 链上已付检查**；回退本身把"KAS 已发"的事实藏掉。触发条件（delivered_v1 广播 5 次全败）恰是频道 send 500 那种 | **P1（设计级·自动路=超时重试·人工路=无检查的重试按钮）** |
...
1. **P1-A（设计级·不需掉电·机制已更正）**：exchange auto-deliver 的钱包双发路两条——自动 = 同 tick 超时重试（与 P1-B (a) 同形）；人工 = 60 min 回退到 verified 后操作员「Send KAS」端点（`api/trading.js:2454-2468`）无检查再发。`paid` 回放路不成立（`:2451` payment_tx 守卫）。修法方向（I1 另稿，沿 I2 四手段）：任何 transfer 前先对账（`delivery_tx`/`events.kas_delivery`/relay 幂等键）；超时不盲重试；回退不得清"KAS 已发"事实；**操作员端点同样过对账闸**；delivered 广播失败 ⇒ 只重播广播不重发币。
