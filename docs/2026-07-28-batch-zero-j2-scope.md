...
> 🔴 **② 已被 J1 核实,而结论与我的读法【相反】(2026-07-28 06:26,J1 实打运行层)**:
> ```
> ✅ 那条路径【确实去问了】—— rpc-listener.mjs:182-196 会 fetch 那个覆盖端点
>    (CONSOLE_URL 在运行层由 relay-manager.js:72 逐个 relay 注入, kanet.env 里根本没这一行
>     ⇒ 只查文件层会得出【相反】的结论)
> 🔴 真实情况是: 它问了, 而【被问的那道闸几乎总是答 false】——
>    J1 实测: 覆盖表 7,601 行里 94.6% 是 start_daa==end_daa 的单点行;
>    区间并集总长 10,321 DAA vs 索引跨度 7,523,034 DAA ⇒ 覆盖率 0.137%;
>    均匀取 20,000 个 deadline ⇒ covered=true 只有 30 个 = 0.150%
>    🔵 阳性对照(拿索引里第 4,000 个块自己的 daa 查 ⇒ covered=true)⇒ 查询没坏, 坏的是数据形状
> ```
> 🔨 **⇒ 这同时解掉了「3mzoh 是反例」这个问题本身: 它根本不是反例。**
> 覆盖闸对 ~99.85% 的 DAA 都答 false ⇒ **deadline 在索引跨度内却仍落回 backward walk 是常态,不是异常。**
> 🔵 **⇒ 我那句「像是没去问」错在哪值得记**:我看到"索引里有这个 DAA"就推断"闸应该会命中",
> 而**索引里有 ≠ 覆盖闸认它** —— 中间隔着一张我当时不知道存在的表(`spc_daa_index_coverage`)。
> **我把一个我没读过的中间层假设成了透明的。**
