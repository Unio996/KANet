...
### kaspa_tx_log_coverage（v217，L2 期 1 保守 coverage 账；编号前移自原 coord/broker-money-path 分支的 v199，见 migrate.js v217 头注）
...
### broker_refund_intents（v217，退款 write-ahead 意图账；编号前移自原 coord/broker-money-path 分支的 v199，见 migrate.js v217 头注）
...
**写入方**：`broker-state-authority.js advanceToRefunded` Phase 1（同 `transaction()`）+ Phase 2（wiring 阶段 2；本表 v217（前移自原 v199） 时刻零写入方）
