...
### proto_token_defs / proto_markets / proto_bets / proto_bet_intents / proto_claims（v206, 全新建, 0 条）
...
**已知限制（不得漂成"已处理"，任何引用本条目须原样带走）**：① T-PROTO-BETTORPK-BINDING——`register_append` 的 `bettorPk` witness 参数与被消费的代币输入之间无签名绑定，v0 单操作员场景接受，**任何第二方参与前必须先修**；② T-ORPHAN-CHIP-RECOVERY-ENTRY——见上 `proto_bets` 说明，合约层缺口非本轮范围。KAS 侧资金/签名来源经统一接口 `protoFundSource.buildAndBroadcast(txShape)` 抽象（设计稿 §6，具体实现 (B′)/(B)/(C) 待 Owner 定，规则打架已升级——NWT 判 (B′) 违反范围稿 §3 字面"不得暴露/复用 relay 那一套"，Bettor 判 (B) 违反"Console 不碰链/Relay 唯一链上出口"架构铁律）。
