...
**字段（节选，完整见 `migrate.js` v62 建表 + 后续 `ALTER TABLE`）**：id (PK，市场标识，形如 `ext-pool-v07-<ts>-<slug>` 或分片 id `<logical>-s<N>`), maker_relay_id, spine_p2sh/spine_lock_tx（PoolSpine covenant 锚点）, deadline/deadline_daa, protocol_version（v0.5/v0.6/v0.7）, protocol_status（pending_bettors → collecting_sigs → verifying → settling/refunding → completed/refunded/shard_internal 等，见各服务的状态机)，maker_stake_amount/broker_fee_pct/oracle_bond_amount/miner_fee, outcome_*（预言机源绑定), settle_txid/refund_txid, metadata（JSON，`settle_evidence`/`phase2_outputs`/`fee_rules` 等结算期写回都堆在这一列——见下方陷阱), sides_merkle_root/pool_merkle_root, fee_rules（v184, write-once trigger）, root_tmpl_hash（v197, write-once trigger；CP4 §4 建市身份锚）。
...
**当前最新版本：v196（2026-08-12 u1_identity_registration 建表）**
...
- **v197 (2026-08-13 CP4 §4 身份锚 provenance)**: `pool_markets.root_tmpl_hash` 新列（TEXT，64 hex = 建市那次构造烤进 PoolLeaf ctor 的 `blake2b(PoolRoot prefix‖suffix)`；设计 `docs/2026-08-13-j2-cp4-identity-anchor-typed-source-design.md`）。**write-once**：`trg_pool_markets_root_tmpl_hash_write_once` trigger（语义同 fee_rules——已有值禁改写/清空 = RAISE(ABORT)，NULL→值允许一次，等值 UPDATE 放行）。DDL 单源自 `src/lib/pool-market-anchor.mjs`（列名+trigger 常量，migrate 与 DB 层测试共用，防漂）。写入方：`persistMarketRootAnchor(db, marketId, gen)`（结构绑定校验 `gen.rootTmplHash == gen.leafCtor[8]` 后写；`computeMarketGenesis` 传 `{persistDb, persistMarketId}` 时同一次里调）——⚠ **今天无生产建市事务调此路径**（`computeMarketGenesis` 仅 e2e/probe 脚本调），持久化钩子**接线是 OPEN seam**。读取方：`getMarketRootAnchor(db, marketId)`（`buildRefundCommand` 自持的命名可信 resolver；退款跨边界模板认证的"另一端"锚源）/ 委员侧**不读本列**（同 fee_rules：列不跨节点同步，跨节点/委员消费必走链上携带值）。陷阱：⚠ 老市场 root_tmpl_hash NULL ⇒ 退款 builder fail-closed（不默认不回落）；⚠ 锚只存在于建市那台 console，换机/重建库=锚丢=fail-closed。
