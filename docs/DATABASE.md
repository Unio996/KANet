...
- **v212（2026-09-20，J2 · oracle→winning_side 整合批 A，设计 `docs/2026-09-20-bettor-oracle-to-proto-winning-side-integration-design-v0.1.md` §3.1/§7-A/§10 R1·R4，NWT 复核 GREEN dff2d8ba）——`proto_markets.winning_side` 的 DB 层防御（纯 schema，不含 adapter）**：
  - **新表 `proto_market_verdicts`**（append-only：UPDATE/DELETE 触发器拒）：`id` AUTOINCREMENT、`market_id` FK、`source_kind` ∈ extractor|uma|llm|human、`relay_id`（可空）、`outcome` ∈ {0,1}、`confidence`（可空，0..1）、`evidence_ref`（非空白）、`created_at`。是 winning_side 的可引用判定记录；写入方=（未来的）oracle adapter / 人工入口，**当前无写入方**。
  ...
  - 🟡 **诚实边界**：触发器防应用/运维失误与手写 SQL，**不防能 DROP TRIGGER / 伪造 verdict 行的机器写权**（设计 §10 R1 末句）。已存在的老行（如主网首轮市场 a59c，winning_side 已有值而审计列 NULL）照常运转：值不可再改、不可删、status 照常推进。
...
- **v212 (2026-09-20, J2 · oracle 整合批 A)**: 新表 `proto_market_verdicts`（append-only）+ `proto_markets` 加审计列 3 + 判定题列 5 + winning_side 写一次 / operator 禁写判定题 / verdict 引用 / DELETE 守卫 / 题面不可改 触发器 16 个（详见上文 proto 段 v212 条目）。纯 schema，无写入方、无 adapter。
