...
Bettor 要求把 §1 提到的"`anchor.anchorHex` 跟真实编译产物之间没有交叉校验"这个发现并入本票（最小形式：
铸造时断言真编译产物的模板切分与 anchor 一致），或者我判断该独立就拆开写清依赖。**判断：拆开**——见新票
`docs/2026-09-14-j2-closezkv2-anchor-crosscheck-ticket-v0.1.md`（T-ANCHOR-XCHECK）。

理由：
- 本票（宽度断言）只解决"`attestedAtMs` 这一个字段的编码宽度会不会变"这个**窄**问题——即使这条断言全部
  落地，也只能保证"宽度不会因为数值大小而意外改变"，**不能**保证 anchor 本身跟真实编译产物在其它维度
  上一致（比如 `gateTmplHash`/`tokenTmplHash`/`claimTmplHash`/`marketSuffixHash` 四个字段各自的位置/
  内容是否也在模板切分里保持一致——这几个字段本票完全没碰，因为它们是定长 `byte[32]`，天然没有宽度
  问题，但"位置对不对/切分对不对"这类问题宽度断言测不出来）。
- T-ANCHOR-XCHECK 要解决的是**更宽**的问题——"铸造时的真实编译产物，其模板部分是否真的跟 genesis 时
  算出的 `anchorHex` 一致"，这个校验一旦做了，会**顺带**覆盖本票要解决的宽度问题（宽度不一致会导致
  切分点错位，切分错位会导致模板字节不一致，交叉校验会抓到）——即 T-ANCHOR-XCHECK 是本票问题的一个
  **严格更强**的父问题，但落地位置、涉及的字段范围、需要改的文件都不同，值得独立排期审查。
- **依赖关系**：两票**互不阻塞**，可以按任意顺序落码，也可以合并成一次改动落码——但**如果两者都做**，
  推荐顺序是本票（宽度断言）先落地：它在 anchor 计算的更早、更聚焦的位置捕获问题，报错信息更精确
  （"`attestedAtMs` 这个值编不进 6 字节"比"anchor 跟真实产物不一致，自己去查哪里错了"更好定位）；
  T-ANCHOR-XCHECK 之后落地作为兜底——即使本票的宽度断言因为某种没预见到的原因没抓住问题（比如未来
  `.sil` 结构变了、宽度假设本身失效），T-ANCHOR-XCHECK 仍然是最后一道防线。若只做一个，
  **T-ANCHOR-XCHECK 覆盖面更完整，优先级更高**。
