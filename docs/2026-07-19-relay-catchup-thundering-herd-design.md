...
**决策（不纠结继续深挖，先上已验证有效的防御）**：本设计不再等待"100% 锤定精确倍数缺口"这件事，因为已验证的机制（相位对齐触发 + 排队延迟确实存在且方向正确）本身就足以支撑防御性修复的正当性——**修复②（jitter 打散相位对齐）无论最终缺口由什么补充因素解释，都直接消除触发条件本身，是最高价值杠杆**，先做。若后续复发，`catchUpHistory` 建议加耗时埋点（KANet-UI 提议）以便下次直接拿到 profiling 级证据，而不是靠事后日志时间戳推算。
- handshake / unreplied-messages 两条子查询（`rpc-listener.mjs` 内 catchUpHistory 前两段）未逐一审查是否同样缺索引或存在 per-item 多次打 console 的放大效应（本次 11:29 窗口是否有实际 pending handshake 在处理仍未查证）——留作后续跟进项，不阻塞当前修复上线。
