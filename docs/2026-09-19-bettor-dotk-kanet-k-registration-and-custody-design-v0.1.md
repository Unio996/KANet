...
- **Status**: v0.5 · 2026-09-19 · **NWT 审毕：二进制 GREEN；设计有条件 GREEN（条件 M1–M7，见 §6，Bettor 全部采纳；M7 为 NWT simnet 实测后增补的提交 / 替换机制）**；完整 GREEN = M1–M6 落实 + J1 垫片过审 + 充值前空跑通过 → 才派执行。NWT provenance：`docs/provenance/2026-09-19-nwt-dotk-design-and-binary-review/`（`03a1bb3b`）。
...
- **M7 提交与替换机制（v0.5，NWT simnet 实测订正，`aed2937a`）**：① **链式未确认交易被节点接受**（含 covenant 链：register_append#1 花未确认 market_genesis 输出，两笔间不挖块）⇒ **reveal 在 commit 提交后立即提交，不等确认**为默认流程（J1 空跑在真实形状上复核）；② `submitTransaction` 是 `RbfPolicy::Forbidden`（`flow_context.rs:690`），花同一输入的第二笔不论 fee 一律拒 "already spent"——**SDK 的 submit 做不了替换**；`submitTransactionReplacement`（`Mandatory`，`:715`）更高 feerate 被接受、旧笔移出，相同 / 更低被拒；替换判据 feerate = fee / normalized_max(compute, transient, **storage**)（`tx.rs:662`）⇒ **M3 门面白名单由 4 方法变 5 方法**，加 `submitTransactionReplacement`，同样只放行垫片已批准并签过的 txid；第二份 reveal 的 feerate（contextual mass 基）由垫片预先算成**严格高于**第一份，由垫片而非 SDK 提交；只有"第一份已进 mempool 但迟迟不打包"才替换，第一份被拒 / 丢弃则普通提交第二份；`planActivate` 花同一 deed 输入，同受此约束；③ **M1 抢注面确认为真**：P2P 中继路径是 `RbfPolicy::Allowed`（`txrelay/flow.rs:223`），攻击者不需要我们的 RPC，向对等节点发更高 feerate 的同 gap 输入 commit 即可替换我们未确认的 commit；**缓解**：我们的 commit 自己用高得多的 feerate（抬高替换门槛）并在 commit 后立即提交 reveal——实现杠杆：SDK 定价 = max(市场 feerate × feeMass, relay 下限) × 1.05，封顶 5 KAS，feerate 来自 `getFeeEstimate` ⇒ **门面把 `getFeeEstimate` 乘一个倍数返回即可抬高 commit 的 fee，无需改 SDK**，倍数上限以 SDK 5 KAS 封顶为界；残余风险接受。
