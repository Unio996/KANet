...
- **M6 持有钥匙补三条**：恢复演练比 `(ownerType, owner)` 而非只比地址，并用与 SDK 无关的另一实现（rusty-kaspa CLI 钱包或 KasWare）对同一助记词派生比对；注册后一次不广播的**签名演练**（Owner 终端用恢复出的钥匙对 `planTransfer('kanet.k', …)` 的 request 签名，SDK `applySignatures / verifySignature` 验过，只打印布尔）——`activate` 无签名参数，`ownerKey` 编码有误会让名字永久锁死而地址核对看不出；**Bettor 加约束：演练的 `newOwner` 必须 = 持有者自己（自转），这样签好的交易即使泄露被广播也无害**；生成 / verify / 签名演练在 Owner 本机**物理终端**跑（da9 有 RustDesk），确认无远程会话，事后清屏与滚动缓冲。
