# §6-3 gate (d) · `min_O` / `N_claim` / `N_margin` 具名保守值提案 v0.10（合并版 · 证据层 · 零落码 · 不改 v0.15 正文）
...
- 🔴 **`k_max` 政策形状（v0.8，Codex eb4db39c 建议、Owner 定；本稿只落形状不拍数）**：
  ...
  2. Tier-2 入场额外要求 **具名 pre-entry 窗口内的稳定算力地板**——**v0.9 按 (23) v0.6 `b6dbcfd0` 定语义**：`H_floor_total_lb` = 墙钟窗内 `min(法1 tip-bits 反推, 法2 Δblue_work/时间戳跨度, 法3 块时间戳窗计数×work_per_block)` 的低分位（三闸：样本充分 / p10 / 无深坑 + 波动比纳 Poisson）；**它量的是【总】算力，链上分不出对手份额** ⇒ 诚实地板 `H_floor_honest_lb = H_floor_total_lb × (1 − s_adv_cap)`，其中 **`s_adv_cap`** = **独立论证的总对手份额上界**，三源（Codex f65c1fbe 裁 `max(s_owner, s_max)` 作上界 REJECTED 后）：(i) **`1 − s_self`**（评估方自持算力份额，角色相对，开放网须与 (ii) 合用——对手可加算力使 `s_self` 事后缩水）；(ii) **经济 cap——🔴 v0.10 撤 v0.9 的 `H_adv/(H_total_lb + H_adv)`**（Codex 5d23a4be 判**双计**：`H_total_lb` 是含对手的总网络下界，再加 `H_adv` 进分母 = 对手算两次），**改 (23) v0.7 `590092a6` 冻结的二选一（Owner 定 `H_adv` 语义，公式随之）**：**(a) 总对手式** `H_adv_cap` = 在场 + 保护窗内可动员的总对手算力上界 ⇒ **`s_adv_cap = min(1, H_adv_cap / H_total_lb)`**（上界/下界 = 份额上界，方向对）；**(b) 增量式** `H_adv_add` = 入场后仅注入的增量 ⇒ **`s_adv_cap = (max(0, H_total_lb − H_self) + H_adv_add) / (H_total_lb + H_adv_add)`**（J2 红队后用自知 `H_self`（自家矿机读数）的绝对写法，避免 `s_self`/`T` 错配——该式对 `T` 单调递减 ⇒ `T` 取下界 `H_total_lb` 才保守；`H_self` 取入场时刻、与 `H_total_lb` 同窗）；**两式同时可算 ⇒ 取 max（更严）作 `s_adv_cap`**。🔴 **`H_total_lb` 来源钉死 = (21) v0.5 三法 `min` 且 `decision.gate_input === 'OK'`**（`law1_only` = 陈难度过估 ⇒ 分母偏大 ⇒ `s_adv_cap` 偏小 = 危险向；`FAIL_CLOSED` = 测量不可靠——两态都**不得作分母**）。🔴 **两式答的不是同一问题**：(a) 界的是【份额】→ 喂 `H_floor_honest_lb`；(b) 的 `H_adv_add` 是 pump 的【增量】→ 喂第 3 步的 `k_max`。(iii) 身份路（有归属的矿工集），开放匿名网不可用。约束 **`s_adv_cap ≥ s_visible_max`**（`s_visible_max` = 窗内可见最大单 coinbase-**payload** 矿工份额，(24) v0.4 提取器 `docs/provenance/2026-08-27-smax/`，Sybil 令其偏低 ⇒ 只是下界/告警/cap 可信度校验，**不得当上界用**）；**无可信 `s_adv_cap` ⇒ Tier-2 fail-closed**。🔴 **TN12 结论（(23) v0.6，理由从"匿名给不了"迁移为"零算力一方给不了"）**：我方持全部算力 ⇒ 对方 `s_self≈0` ⇒ 其 (i) 给 cap=1、(ii) 需 Owner 给可信 `H_adv` 且对手可能就是矿工本身 ⇒ fair-exchange **两侧都要 cap 成立** ⇒ **现网 Tier-2 关**（机制输出，非判断）。法 3 估计器偏差按 (23) v0.6：live 口径 `132/W`、回溯 `264/W`（PMT ≈ now−132 s，`consensus/core/src/config/constants.rs:26-30`），`H_floor_total_lb(法3) = 法3_raw/(1 + 口径/W)`；
  3. **由地板 + 预算推 `k_max`（v0.10 按两式分工）**：`B_win(k)` 的 k 是"注入后总 / 注入前总"，只与**增量**有关 ⇒ **`k_max = 1 + H_adv_add / H_total_lb`**（用 (b) 的 `H_adv_add` 与同一 `H_total_lb`；(a) 的"在场对手"已在 `H_total_lb` 里，不进 k）；入场要求 `k_max ≤ k_baked` ⇔ `H_total_lb ≥ H_adv_add/(k_baked − 1) =: H_floor_min`；**同时** `H_floor_honest_lb = H_total_lb × (1 − s_adv_cap)`（`s_adv_cap` 取 (a)/(b) 之 max）须 ≥ 反应方保护所需的诚实地板（(23) §4）；`B_win(k_max)` 由 **durable 仿真源 `8310f390`**（NWT (22) 入库）取；
...
2-bis. 🔴 **残余清单（v0.9 按 Codex f65c1fbe + 6fd55a53 重写为六项，作为 (d) 从 PROVISIONAL 升级的唯一路径）**：
   ...
   3. **Owner 批准的具名 `k_max`，挂在 `H_floor_honest_lb = H_floor_total_lb × (1 − s_adv_cap)` 上**——`s_adv_cap` **谁论证**：(i) `1 − s_self`（评估方自证）/ (ii) 经济 cap（Owner `H_adv`）/ (iii) 身份路（不可用于开放匿名网）；`s_adv_cap ≥ s_visible_max`（(24) v0.4 提取器校验）；无可信 cap ⇒ fail-closed；`B_win(k_max)` 取自 `8310f390`。
