# §6-3 gate (d) · `min_O` / `N_claim` / `N_margin` 具名保守值提案 v0.17（合并版 · 证据层 · 零落码 · 不改 v0.15 正文）

> **v0.17（Codex 283 `1c7188e2`，J2 镜像，只改 §6 step 2 (b) 验收段与 §7 2-bis-3）**：Codex 裁 **D-STAT-1 / D-STAT-2 / D-STAT-3 三条 CLOSED AT DESIGN LAYER**（D-STAT-3 = "the proof obligation is now carried by the theorem, not by brute force"；Codex 独立重读 (23) v0.15 定理与 7b1e18cc 承重路径全一致）。🔴 **Scope 原话（Owner 面须原句）**："**D-STAT-3 closes the work-per-public-arrival cap construction under exact reconstructed public state; it does not eliminate the adversarial-capacity model boundary.**"（中译：闭合的是【精确重建的公开状态下】每个公开到达的 work 上界构造；**不消除**对抗容量的模型边界——`Ncand(SP)` 由估计器可得块构成，可得公共集之外的子块/支撑分支不被定理覆盖，归 `B_adv` 或 fail-closed）。(D) `N_small=12` 写死是 feature，仅作引理/回归机器检验；(E) 仅 smoke，永不引作极值覆盖。**`w_cap` 取数/重建实现 OPEN**——四闸须在真 RPC 路实测（见 §6 (b)）。🔴 **gate (d) 整体仍 OPEN / PROVISIONAL**（Codex §5 原文："Remaining non-D-STAT items (claim-shape evidence, post-sync operating evidence, adversarial budget/cap policy, named conservative constants, etc.) are not closed by this verdict."）。落码：(21) v0.9 `a93f9c5e`（`wcap-window.mjs` durable，`hVisUb` 仍参数注入接线留 v0.10）；取数设计 (24) `3f7ef2c5` + 不变量 `448469b2`。其余逐字不动。

> **v0.16（Codex 282 `713232be` + NWT (23) v0.15 `2f632c91`，J2 镜像，只改 §6 step 2 (b) 的验收段/K_SP 序与 §7 2-bis-3）**：Codex 判 v0.14 机制更正 ACCEPTED、精确窗 ∪ N_C PASS-dir、结构性证书 PASS 设计层、洞归估计器 PASS；**唯一 MUST：候选窗"全部子集穷举、零 skip"在生产规模组合不可执行**（`Σ_{k≤247} C(N,k)`），不可执行的验收契约不能闭 D-STAT-3。闭合走路线 (2)：把 (i)-(v) 升为**正式引理链 L1–L5 ⇒ 支配定理**"∀ 合法公开子块 C（SP 固定、`window(SP)` 精确）`work(bits(C)) ≤ w_child_ub(SP)`，无需枚举"（J2 `scratch/_j2_wcap_lemma_chain.md`，NWT 逐核 @7b1e18cc）；生产验收降为可执行 (A) 重建 + 证书 / (B) `bitsCalc == 收块 bits` 全量 / (C) 断言 + 自喂剔除 + 负向量 / (D) 有界对抗模型 `N_small = 12` 写死（引理链机器检验，非生产验收）/ (E) 可选贪心 smoke（标"证不了极值·非验收"）。`K_SP` 定义补"按 `SortableBlock` 序（blue_work 然后 hash，`ghostdag/ordering.rs:38-42`）取最小 7，边界并列全去"。其余逐字不动。

> **v0.15（Codex 281 `95d7f354` score-domain MUST-FIX + NWT (23) v0.14 `27401ba3`，J2 镜像，只改 §6 step 2 (b) 的覆盖集/硬闸/验收与 §7 2-bis-3）**：Codex 判 v0.13 (iii) `W_C ⊆ A_SP := {bs ≥ bs(SP)+1−26,440}` **未证**——蓝分与 DAA 是两个共识量。J2 核 @7b1e18cc：阈值 `lowest_daa_blue_score = blue_score.max(full)−full` 本身是**蓝分域**（`difficulty.rs:190-197`；`window.rs:304/310-311`），采样节拍才是 DAA 域（`:313-316`）；**真洞在别处**：`window.rs:265-282 try_init_from_cache` 克隆 `window(SP)` 后只 push 子块新 mergeset，**继承样本不按子块阈值重过**、淘汰只按蓝功（`:458-468`）⇒ 旧样本可活远超 26,440 蓝分 ⇒ 蓝分区间覆盖集对继承成员不成立。闭合：覆盖集改 **`window(SP)【精确重建】∪ N_C`**（`N_C ⊆ {SP}∪mergeset(C)`、原生阈值 `bs ≥ bs(SP)+1−26,440`、`|N_C| ≤ 7`，零域换算）；历史闸撤 62,440 数字，改**结构性精确证书**（甲 真 genesis 全史零缺失 / 乙 截断根 R 后堆满 661 ∧ 堆内最小蓝功 > blue_work(R) ∧ 零缺失，证明靠 `blue_work` 沿祖先单调 `ghostdag/protocol.rs:99-104/:161`）；实现验收加候选窗超集穷举 × `calculate_difficulty_bits` 零 skip + bits 自洽；score-domain 洞**不归 B_adv**。重建器 v2 9 向量全过（含 red 密 daa/bs ×2.9 分离、non-DAA 排除、证书三态、负向量真失败）。其余逐字不动。

> **v0.14（Codex 280 `d7fefb58` + NWT (23) v0.13 `9040b8ec`，J2 镜像，只改 §6 step 2 (b) 的 `w_cap_window` 构造/硬闸与 §7 2-bis-3）**：Codex 判 D-STAT-1/2 设计层 CLOSED（Garwood 六向量与 `N_min` 3974/10867/24259 独立复算全对）、**D-STAT-3 OPEN**：观测 `w_max` 是随机量，撑不住 `n_ub` 多补的潜在到达；🔴 **J2 的复合式 `n_eff = Σw/w_max` 被拒（分数、非观测计数、非同证）——本版撤 v0.13 那句"可选更紧同证上界"**。闭合 = Codex form-1 协议推导 `w_cap_window`（J2 推导，NWT 逐核 @7b1e18cc）：DAA **无难度上升 clamp**（`difficulty.rs:243-245`，单窗 `expected/measured` 最大 2,640,000），纯协议常数 cap 不存在；但潜在到达 C 的 bits 由 `window(C) = window(SP) ∪ sampled({SP}∪mergeset(C))` **确定性算出**（`window.rs:138-235`），一块最多进 7 个采样（`mergeset_size_limit = 248`，`bps.rs:75-85`；`/40` 采样）⇒ 对每个候选 SP 的**最紧合法窗不等式** `w_child_ub(SP)`（K_SP / A_SP，μ = 0）取 max。J2 首版层 3 `(1+μ)·max(收块 bits, virtual@t0/@t1)`、μ=1% 被 NWT 逮出**欠界**（按"一步滚 1 采样"算的，实为 7）⇒ 降为诊断。其余逐字不动。

> **v0.13（Codex 279 `b4df8328` 三 D-STAT MUST-FIX + NWT (23) v0.12 `0e123323`，J2 镜像，只改 §6 step 2 (b) 的 `H_vis_ub` 构造/硬闸与 §7 2-bis-3）**：Codex 判 MATERIAL PROGRESS / (d) OPEN——b-self 撤 CLOSED、(a-total)/自持路代数/接收计/单一 `B_adv`/`H_self_lb > H_vis_ub` 闸全 PASS-dir；`W_min = 3600 s` 只是运营最小值、**不足以统计收口**。三条落法：**D-STAT-1** 弃高斯 `n + 3.09√n`（任何 n 都欠覆盖：`P(X ≤ n | λ = n+3.09√n)` n=100 ⇒ 2.9e−3、n=36000 ⇒ 1.07e−3 仍 > 1e−3），`n_ub` 冻结 **Garwood 精确** `½χ²_{0.999}(2n+2)`；实现须过夹逼向量 `n + 3.09√n ≤ impl(n) ≤ (√(L/2) + √(L/2+n))²`（后者 = 泊松下尾 Chernoff 闭式，L = ln 1000，证过的上界；n = 0/10/30/100/1000/36000 ⇒ 精确 6.91/24.13/51.08/134.92/1101.63/36590.19）；**D-STAT-2** 机械样本闸 **`n ≥ N_min = 4,000`**（Garwood + `δ_max = 5%` 反解 3,974，J2 独立复算一致；`n` 为 own-clock 窗内实测新可达块数，不由 BPS × W 推）与 `W_min` 并列；**D-STAT-3** `H_vis_ub = n_ub × w_max / (t1 − t0)`，`w_max` = 窗内观测最大单块 work（禁均值；复合泊松式 `w_max·λ_ub(Σw/w_max)` 为可选更紧同证上界，未采——🔴 v0.14 已撤：Codex 280 拒复合式，且观测 `w_max` 本身非硬 mark 界，改 `w_cap_window`）。`t1` 前本机不可见的 work 不假设为零，归 `B_adv`；`B_adv` 语义按 Codex 收紧为真上界（含窗后段才上线、被窗均值摊薄的公开算力）。其余逐字不动。

> **v0.12（NWT (23) v0.11 `abda09f3`，Bettor 派，单点改）**：§6 step 2 (b) 硬闸的 `W_min` 写死 **3600 s**（NWT 理由：10 BPS 下 n = 36,000、σ ≈ 189.7、单侧 99.9% 上分位仅 +1.6%、`T_prop ≪ W`；600 s 不取）。其余不动。

> **v0.11（Codex c6154e89 裁 278 + NWT (23) v0.9 `851a2913` / v0.10 `ca425495` + J2 红队两 MUST）**：🔴 **撤 v0.10 §6 step 2 的 (b) 增量式 `(max(0, H_total_lb − H_self) + H_adv_add)/(H_total_lb + H_adv_add)`——Codex REJECTED（J2 提、错在 J2）**：`s_adv ≤ 1 − H_self/H_total` 对 `H_total` 单调**增** ⇒ 分母要**上界**；`H_total_lb − H_self` 是非我方算力的**下界**（反例 lb=100/true=200/self=20 ⇒ 算 0.80、真 0.90 ⇒ cap 偏小 = 诚实地板偏大 = 危险向）；v0.10 那句"对 T 单调递减 ⇒ 取下界保守"是对旧比值式算的导数，换绝对式后反号（教训：每个估计输入按最终式的偏导符号定取 lb 还是 ub，换式子重核全部）。**改为 (23) v0.10 §3.5 自持路分解式**：`H_total = H_vis + H_hidden`，估计器只见已发布块 ⇒ 至多给 `H_vis_ub`，`H_hidden` 只能具名 ⇒ **`s_adv_cap(b) = (H_vis_ub − H_self_lb + B_adv)/(H_vis_ub + B_adv)`**；`H_vis_ub` = **法 3′ 本机时钟接收计数**（两次 own-clock 轮询新可达块工作和 ÷ (t1−t0) × 泊松单侧 99.9% 上分位，不看块戳——按戳定窗对上界失效：过去侧唯一规则是 `ts > 该块自身 PMT`（`consensus/src/pipeline/header_processor/post_pow_validation.rs:19-28 check_median_timestamp` @7b1e18cc），陈父块可带旧戳准时到达、真工作却落窗外）；**`B_adv` = Owner 具名的单一可动员对抗预算，默认同时是 (b) 的 `H_hidden_ub` 与 step 3 的 `H_adv_add`**（扣着的可解扣 = 注入 ⇒ add ⊇ hidden；能注入的可提前开机扣块 ⇒ hidden 能力 ≥ add ⇒ 同一预算由对手分配，两约束各取最坏，非双计）；拆开须 Owner 论证 + 机械守卫 `H_hidden_ub ≥ H_adv_add`。(a-total) `min(1, H_adv_cap/H_total_lb)` Codex PASS 不动；两路可算取 max。硬闸：`(t1−t0) < W_min ∨ 未取分位 ∨ H_self_lb > H_vis_ub ∨ 无 B_adv` ⇒ (b) 不出 cap 回 (a-total)；两路皆无 ⇒ 关。**k_max 那条不变**（`k_max = 1 + H_adv_add/H_total_lb`，Codex PASS）。其余（时间戳修、132 s 三段）Codex 同批 PASS 不动。

> **v0.10（Codex 5d23a4be 裁 277 + (23) v0.7 `590092a6` + J2 红队，Bettor 裁"确定部分"先出）**：§6 step 2 **撤**作废双计式 `H_adv/(H_total_lb + H_adv)`；`H_total_lb` 来源钉死 = (21) v0.5 三法 `min` 且 `gate_input === 'OK'`（`law1_only` / `FAIL_CLOSED` 不得作分母）；经济 cap 改 (23) v0.7 冻结二选一——(a) 份额式 `min(1, H_adv_cap/H_total_lb)` → 喂 `H_floor_honest_lb`，(b) 增量式 `(max(0, H_total_lb − H_self) + H_adv_add)/(H_total_lb + H_adv_add)`（🔴 v0.11 已撤：Codex c6154e89 判分母须上界，见 v0.11 注）（自知 `H_self` 绝对写法）→ `H_adv_add` 喂 `k_max = 1 + H_adv_add/H_total_lb`；同可算取 max；明写两式答的不是同一问题；step 5 的 132 s 改**三段**（头段 ≤132 s 硬界 / 残余随 W 衰减 / 检测时延 `132+f×W+T_dwell`）。NWT (23) v0.8 若改语义再补。

> **v0.9（合并版，Bettor 派工；吸收 (23) v0.6 `b6dbcfd0`、Codex f65c1fbe / 6fd55a53、NWT (22) `8310f390`、(21) v0.3 / (24) v0.4 / (27) v0.3）**：① 算力地板语义进 §6——`H_floor_total`（`min(法1,法2,法3)`）/ **`s_visible_max`**（可见集中**下界**，只作告警与 cap 可信度校验）/ **`s_adv_cap`**（独立论证**上界**，三源：`1 − s_self` 角色相对 · 经济 cap `H_adv/(H_total_lb + H_adv)`（🔴 v0.10 已撤：Codex 5d23a4be 判双计） · 身份路）、`s_adv_cap ≥ s_visible_max`、**无可信 cap ⇒ fail-closed**、TN12 结论 = "零算力一方给不了 cap，fair-exchange 两侧都要成立 ⇒ 关"；② 法 3 两口径（live `132/W`、回溯 `264/W`）、掩盖 **≤132 s 硬时限**（戳挖时定死）、检测时延 `132 + f×W + T_dwell` = 在飞暴露窗接 §5 披露；③ 3-C `B_win` 守恒主论证 + **durable 仿真源 `8310f390`**（NWT (22) 入库）引用；④ P3 (b) 推荐与 `min_O` 重定义互引 fee-source v0.3（3-A）；⑤ §7 残余清单按 Codex f65c1fbe + 6fd55a53 重写**六项**；⑥ carry-forward 全并：PMT ≈132 s、coinbase payload 归属、守恒署名 NWT、`config/` 全路径、`s_max → s_visible_max`；⑦ 3-D 总界仍 PROVISIONAL-PLACEHOLDER，因子列表更新。
...
- 🔴 **v0.6 · `B_win` 的有界/无界完全取决于时间戳假设**（J2 红队 NWT (19) `c30bb446`，NWT 仿真 `scratch/_nwt_bwin_sim.mjs` 表值 J2 逐格复现）：
  ...
  - **对抗仿真（NWT sim v0.2，攻击者按 +132 s 上限给未来戳；J2 逐格复现；**durable 源已入库 = NWT (22) `8310f390`**（源 + 参数集 + 源 commit + 期望输出，Codex 数值曲线 "NOT YET independently auditable" 由此闭）**：`B_win_adv(k)`：k=1.5→6,436 / 2→9,792 / 3→14,178 / 5→19,203 / **10→25,279** / 50→36,968 / 100→41,236 / **1000→53,070** / 1e6→75,749；k=10 在 2×/4×/8×/16× 窗 plateau 25,108→25,279→25,279→25,279（收敛，不随时长增）；对抗−诚实 = 25,279 − 23,959 = **1,320**。⇒ **占位 55,200 ⟺ `k_max ≲ 1000`**。
...
| **`N_claim + N_margin`（链上 enforce 的那一个数）** | **123,600（PROVISIONAL-PLACEHOLDER，v0.9 因子：① `B_win` 占位 55,200 ⟺ `k_max ≲ 1000` 未具名，须挂 `H_floor_honest_lb`（须 `s_adv_cap`，现网无可信 cap）；② `N_claim` n=1，§5① ≥30 claim-shape（SENDER_TS 口径）未跑；③ `S_unalloc` 声明值待散度；④ `W_dis` 坏节点数据；⑤ P3 (a)/(b) 未终选；⑥ 工具官方跑（(21)(24)(27) provenance）未产出。仿真源已入库 `8310f390`（不再是因子）。本数只在 Codex 不推荐的 `k_max ≲ 1000` 假设下才有意义）** | **≈ 3 h 26 min（名义）** |
...
- 🔴 **`k_max` 政策形状（v0.8，Codex eb4db39c 建议、Owner 定；本稿只落形状不拍数）**：
  ...
  2. Tier-2 入场额外要求 **具名 pre-entry 窗口内的稳定算力地板**——**v0.9 按 (23) v0.6 `b6dbcfd0` 定语义**：`H_floor_total_lb` = 墙钟窗内 `min(法1 tip-bits 反推, 法2 Δblue_work/时间戳跨度, 法3 块时间戳窗计数×work_per_block)` 的低分位（三闸：样本充分 / p10 / 无深坑 + 波动比纳 Poisson）；**它量的是【总】算力，链上分不出对手份额** ⇒ 诚实地板 `H_floor_honest_lb = H_floor_total_lb × (1 − s_adv_cap)`，其中 **`s_adv_cap`** = **独立论证的总对手份额上界**，三源（Codex f65c1fbe 裁 `max(s_owner, s_max)` 作上界 REJECTED 后）：(i) **`1 − s_self`**（评估方自持算力份额）——**v0.11：并入 (ii)(b) 自持路**，因它需要的是 `H_total` 的**上界**而非 `H_total_lb`（Codex c6154e89）；(ii) **经济 cap——🔴 v0.10 撤 v0.9 的 `H_adv/(H_total_lb + H_adv)`**（Codex 5d23a4be 判**双计**：`H_total_lb` 是含对手的总网络下界，再加 `H_adv` 进分母 = 对手算两次），**改 (23) v0.7 `590092a6` 冻结的二选一（Owner 定 `H_adv` 语义，公式随之）**：**(a) 总对手式** `H_adv_cap` = 在场 + 保护窗内可动员的总对手算力上界 ⇒ **`s_adv_cap = min(1, H_adv_cap / H_total_lb)`**（上界/下界 = 份额上界，方向对）；**(b) 自持路（v0.11 按 (23) v0.10 `ca425495` §3.5；v0.10 增量式已撤，Codex c6154e89 REJECTED）**：拆 `H_total = H_vis + H_hidden` ⇒ **`s_adv_cap = (H_vis_ub − H_self_lb + H_hidden_ub) / (H_vis_ub + H_hidden_ub)`**（对 `H_vis`、`H_hidden` 偏导 `= H_self/D² > 0` ⇒ 各取上界，`H_self` 取下界）；**`H_vis_ub`** = 法 3′ **本机时钟接收计数**：两次 own-clock 轮询 `t0<t1` 各取 tips 集，记 `n` = 两次间新可达块数、`{w_i}` 各块 work、`w_max = max_i w_i` ⇒ **`H_vis_ub = λ_ub(n) × w_cap_window / (t1−t0)`**（(23) v0.13 `9040b8ec` D-STAT-1/3；v0.13 的 `n_ub × w_max` 已撤）：`n_ub` = **Garwood 精确**泊松单侧 99.9% 上限 `½χ²_{0.999}(2n+2)`（高斯 `n + 3.09√n` 任何 n 都欠覆盖，只作展示列；实现须过夹逼向量 `n + 3.09√n ≤ impl ≤ (√(L/2)+√(L/2+n))²`，L = ln 1000），🔴 **`w_cap_window`（承重，(23) v0.13 §3.5(b) 改点 1，层 1 最紧合法窗不等式形，μ = 0）** = 对手在已收块上可造的任一合法子块 C 的 work 上界：C 的 bits 由 `window(C) = window(SP) ∪ sampled({SP}∪mergeset(C))` 确定性算出（`difficulty.rs:216` 只吃窗不吃 C 自身戳；`window.rs:138-235/265-282`；堆按蓝功留 661，`:458-468`），一块最多进 `⌊248/40⌋+1 = 7` 个采样（`bps.rs:40` k=124、`:75-85` limit=248、`window.rs:299-322`）。对固定 SP：(i) `W_C ⊇ K_SP`（`K_SP` = window(SP) 去按 `SortableBlock` 序（blue_work 然后 hash，`ghostdag/ordering.rs:38-42`）最小的 7 个，边界并列全去；L1：每次 `try_push` 至多淘汰当时最小 ⇒ 被淘汰原成员是序的前缀 ≤ 7）【协议】；(ii) `measured(W_C) ≥ m_lb(SP) := max(1, max_ts(K_SP) − min_ts(K_SP))`【代数，戳非单调无妨】；(iii)【v0.15 改，(23) v0.14 §3.5(b)】`W_C = window(SP)【精确重建】∪ N_C`（`window.rs:265-282`：继承样本原样克隆、**不重过阈值**，故不能用蓝分区间刻画；`N_C ⊆ {SP}∪mergeset(C)`，每成员 `bs ≥ lowest_daa_blue_score(C) ≥ bs(SP)+1−26,440`【采样器原生阈值，`difficulty.rs:190-197`】，`|N_C| ≤ 7`）⇒ `avg_target(W_C) ≥ T_lb(SP) := min target over window(SP) ∪ Ncand(SP)`，`Ncand(SP) := {SP} ∪ {已收 b : bs(b) ≥ bs(SP)+1−26,440}`【代数，零域换算】；(iv) `target(C) ≥ T_lb·m_lb/2,640,000`（`difficulty.rs:243-245`，`expected = 100 ms × 40 × 660`）【协议】；(v) 固定难度支（`:220-227`）`bits(C) = bits(SP)` 并入 max ⇒ **`w_child_ub(SP) = max(calc_work(compact_bits(T_lb·m_lb/2,640,000)), calc_work(bits(SP)))`**；**`w_cap_window = max_{SP∈S} w_child_ub(SP)`**，`S := {已收 b : bs(b) ≥ bs_top(t1) − 36,000}`（merge depth，`bps.rs:88-90`）。输入全链上可读（`getBlock` header + verboseData），每块建窗增量 O(1)，不需 mergeset 搜索。🔴 **结构性精确证书（v0.15，替代 62,440 数字闸）**：`SP∈S` 的 `window(SP)` 须精确——(甲) 重建链自真 genesis（无父 ∧ daa=0 ∧ bs=0，`genesis.rs:187`）起且沿途 mergeset 成员零缺失；或 (乙) 自截断根 R 起，某块堆满 661 ∧ 堆内最小蓝功 > blue_work(R) ∧ R 后成员零缺失 ⇒ 精确并向后代继承（证：pre-R 样本 ∈ past(R)，`blue_work` 沿祖先单调不减 `protocol.rs:99-104/:161` ⇒ ≤ blue_work(R) < 堆内最小 ⇒ 真堆里也已淘汰）；否则 `WINDOW_INEXACT` ⇒ 不出 cap。成员缺失 ⇒ 永不精确 ⇒ fail-closed，**不得静默取过大 `T_lb`、不得归 B_adv**（Codex 281：公开 DAG 覆盖洞是估计器正确性问题）。**支配定理（v0.16，(23) v0.15 `2f632c91` §3.5(b)）**：(i)-(v) = 引理 L1（淘汰前缀）/ L2（超集跨度）/ L3（覆盖，零域换算）/ L4（`⌊avg·measured/expected⌋` 单调 + `expected ≤ expected_full` + `min(·,max_target)` + `compact` 单调且 `decode(compact(t)) ≤ t` + `calc_work` 非增 ⇒ 三层舍入全保守向）/ L5（固定支 + 可达性 `|window(C)| ≤ |window(SP)|+7`）⇒ **∀ 合法公开子块 C：`work(bits(C)) ≤ w_child_ub(SP)`，无需枚举**。**实现验收（可执行，v0.16 替代"全部子集穷举零 skip"——Codex 282 判其生产不可执行）**：(A) 确定性重建 + 精确证书（O(N)）；(B) `bitsCalc == 收块 bits` 对**全部**精确窗（镜像与共识同算法的直接验证，O(N)）；(C) 已实现子块断言 + 自喂剔除 + 负向量真失败；(D) 有界穷举对抗模型：合成小 DAG，池 **`N_small = 12` 写死**（≤4096 子集 × bs 范围），覆盖 red 密/non-DAA/挤戳/固定/舍入/最大进样 7，每候选直接跑镜像 `calculate_difficulty_bits`——**引理链的机器检验，非生产验收**；(E) 可选贪心极端 smoke（`Ncand∖past(SP)` 按 target 最小取 ≤7）**标"非验收·证不了极值"**（`measured` 与 `average_target` 由淘汰/新进组合决定，不独立可分）。🔴 **Codex 283 `1c7188e2`（v0.17）：D-STAT-3 CLOSED AT DESIGN LAYER，scope 原句**："D-STAT-3 closes the work-per-public-arrival cap construction under exact reconstructed public state; it does not eliminate the adversarial-capacity model boundary."——中译：闭合的是精确重建的公开状态下每个公开到达的 work 上界构造，不消除对抗容量模型边界（可得公共集之外 ⇒ `B_adv` 或 fail-closed）。**(D) 角色**：写死 `N_small=12` 是 feature——只作引理/回归机器检验、不是生产状态覆盖，防止它长成另一个不可执行的验收契约；**(E) 角色**：只是带标签的 smoke，永不引作极值覆盖（除非另给极值证明）。🔴 **实现四闸（Codex 原文，须实测·未闭；`w_cap` 取数/重建实现 OPEN）**：(1) pagination to sink is complete and deterministic for the required cover；(2) sink-anticone additions do not leave an unreturned mergeset closure hole；(3) missing/pruned/IBD members surface as `INEXACT`, never as a smaller reconstructed window；(4) the `t0/t1` used for `lambda_ub(n)` are the same arrival-clock interval for which `wCapWindow` is certified。落码 (21) v0.9 `a93f9c5e`（`docs/provenance/2026-08-27-kmax/wcap-window.mjs`：重建器 + 证书 + `certificate{kind,R,R_blue_work,heapMin_min,missing}` + `wCapWindow`，13 向量；`hVisUb` 仍参数注入，接线留 v0.10 四闸实测后）；取数设计 (24) `docs/2026-08-27-nwt-s63-wcap-fetch-design-v0.1.md`（`3f7ef2c5` + 取数完整性不变量 `448469b2`）。🔴 **机械健全性断言**：每个已收块 B 必 `calc_work(bits_B) ≤ w_child_ub(SP_B)`，任一不过 ⇒ 实现错 ⇒ fail-closed。**v0.14 fix-up（J2 重建器 V3 逮出，Bettor 升为规格要求）**：核 B 时 `w_child_ub(SP_B)` 的 `Ncand(SP_B)`（v0.15 术语，原 `A_SP`）**必须剔除 B 自身**（B 不在自己的窗里，剔除后仍是超集）——否则 B 的 bits 把 `T_lb` 喂饱，断言永不失败 = 空断言；配套**负向量必须真失败**（把某收块 bits 改成远超界的难度 ⇒ 断言须报 `ASSERTION_FAIL`），负向量不失败 ⇒ 断言实现无效 ⇒ 工具 fail-closed。**模型边界（归 `B_adv`）**：私链父块（窗含未发布祖先）；SP 深于 virtual merge-depth root 的死侧链（`post_pow_validation.rs:79`）；多块连续外推（反事实块再当父压戳推窗）。戳操纵不需假设（`measured` 读自已收戳，挤紧 ⇒ `m_lb` 自动变小 ⇒ cap 自动升）。观测 `w_max` / 层 3 `(1+μ)·max(…)` 只作诊断，不承重（μ=1% 欠界）；**不看块戳**（死带归零、修正因子 1；`t1` 前本机不可见的 work **不假设为零**，归 `B_adv`——Codex 279 边界；窗内才变可达的旧 work 多计 = 对上界保守）——🔴 **按块戳定窗不得作上界**（过去侧唯一规则 `ts > 该块自身 PMT`，`post_pow_validation.rs:19-28` @7b1e18cc，陈父块可带旧戳准时到达 ⇒ 漏计 ⇒ 公开算力被当不存在，且不花预算）；**`H_self_lb`** = 同 `(t1−t0)` 窗自家已发布块工作和（(24) coinbase 归属抽），`min(·, 标称)`，同分母；**`H_hidden_ub = B_adv`**（默认，见下）；🔴 **`B_adv` 语义（Codex 279 条件，须真是上界）** = 保护窗内可缺席于【窗均值可见估计】、后变有效的**全部**对手容量/工作之上界——**含在测量窗后段才上线、被窗均值摊薄的公开算力**（`s_adv` 式里可把它从 `H_vis` 挪到 `H_hidden` 项，形式不变；否则它既不在 `H_vis_ub` 也不在 `B_adv`）。**硬闸（(23) v0.12 双闸）**：`(t1−t0) < W_min`，**`W_min = 3600 s`**（(23) v0.11 `abda09f3`；Codex：运营最小值，不足以统计收口）∨ **`n < N_min = 4,000`**（Garwood + `δ_max = 5%` 反解 3,974 向上取整；`n` 实测、非 BPS × W；δ 只放大上界 = 保守向，故此闸是"假设成立 + 界非空洞"闸；对照 3% ⇒ 10,867、2% ⇒ 24,259）∨ 未用精确上限（近似未独立量化误差）∨ **∃ SP∈S：`window(SP)` 无精确证书（甲/乙）⇒ `WINDOW_INEXACT`**（v0.15 替代"历史深度 < 62,440"数字闸；成员缺失 ⇒ 不精确）∨ **健全性断言有块不过 ∨ (B) 有精确窗 `bitsCalc ≠ 收块 bits` ∨ (D) 有界模型有候选不过**（⇒ fail-closed 而非回 (a)；(D) 失败 = 引理链机器检验失败 = 实现错）∨ `H_self_lb > H_vis_ub`（口径错）∨ 无 `B_adv` ⇒ (b) **不出 cap**，回 (a)。**两式同时可算 ⇒ 取 max（更严）作 `s_adv_cap`**。🔴 **`B_adv` 单一预算（MUST-A）**：`H_hidden_ub`（喂 (b) 份额 cap）与 `H_adv_add`（喂第 3 步 `k_max`）**默认同一个数 `B_adv`**——扣着的算力可解扣 = 一份注入（否则放出扣着的就 `k > k_max`）；能入场后注入的都能提前开机扣块 ⇒ 二者是同一资源在两时刻，由对手分配，两条约束各取自己最坏，非双计（联合场景"扣着 B 再全拿去 pump"仍 `k = 1 + B/H_total_lb`）；**拆开（`H_hidden_ub < H_adv_add`）须 Owner 论证"提前开机扣块不可行"+ 机械守卫 `H_hidden_ub ≥ H_adv_add`**（堵 `H_hidden_ub = 0` 配大 `H_adv_add` 的后门）。Codex "不得混"指两公式**角色**（份额分母 vs pump 增量），不指两个数须不等。🔴 **`H_total_lb` 来源钉死 = (21) v0.5 三法 `min` 且 `decision.gate_input === 'OK'`**（`law1_only` = 陈难度过估 ⇒ 分母偏大 ⇒ `s_adv_cap` 偏小 = 危险向；`FAIL_CLOSED` = 测量不可靠——两态都**不得作分母**）。🔴 **角色分工（v0.11）**：(a)、(b) 都给【份额】cap → 喂 `H_floor_honest_lb`；`B_adv` 同时以 `H_adv_add` 身份喂第 3 步的 `k_max`【增量】——两个公式角色不同、同一个预算数。(iii) 身份路（有归属的矿工集），开放匿名网不可用。约束 **`s_adv_cap ≥ s_visible_max`**（`s_visible_max` = 窗内可见最大单 coinbase-**payload** 矿工份额，(24) v0.4 提取器 `docs/provenance/2026-08-27-smax/`，Sybil 令其偏低 ⇒ 只是下界/告警/cap 可信度校验，**不得当上界用**）；**无可信 `s_adv_cap` ⇒ Tier-2 fail-closed**。🔴 **TN12 结论（(23) v0.6，理由从"匿名给不了"迁移为"零算力一方给不了"）**：我方持全部算力 ⇒ 对方 `s_self≈0` ⇒ 其 (i) 给 cap=1、(ii) 需 Owner 给可信 `H_adv` 且对手可能就是矿工本身 ⇒ fair-exchange **两侧都要 cap 成立** ⇒ **现网 Tier-2 关**（机制输出，非判断）。法 3 估计器偏差按 (23) v0.6：live 口径 `132/W`、回溯 `264/W`（PMT ≈ now−132 s，`consensus/core/src/config/constants.rs:26-30`），`H_floor_total_lb(法3) = 法3_raw/(1 + 口径/W)`（v0.11：这是**下界**口径，按戳漏计只让 lb 更低 = 保守；**上界**不得复用，只能法 3′ 接收计）；
  3. **由地板 + 预算推 `k_max`（v0.10 按两式分工）**：`B_win(k)` 的 k 是"注入后总 / 注入前总"，只与**增量**有关 ⇒ **`k_max = 1 + H_adv_add / H_total_lb`**（`H_adv_add = B_adv`，v0.11 与 (b) 的 `H_hidden_ub` 同一预算；与同一 `H_total_lb`；(a) 的"在场对手"已在 `H_total_lb` 里，不进 k）；入场要求 `k_max ≤ k_baked` ⇔ `H_total_lb ≥ H_adv_add/(k_baked − 1) =: H_floor_min`；**同时** `H_floor_honest_lb = H_total_lb × (1 − s_adv_cap)`（`s_adv_cap` 取 (a)/(b) 之 max）须 ≥ 反应方保护所需的诚实地板（(23) §4）；`B_win(k_max)` 由 **durable 仿真源 `8310f390`**（NWT (22) 入库）取；
  4. **跌破地板 ⇒ Tier-2 fail-closed**，**不静默沿用旧比值**（同 Codex 267 (d) 最后一条 "if the measured environment violates the bound, Tier-2 must fail closed"）；连续再量 + 滞回两阈 + 恢复须全新完整稳定窗（(23) §5）。
  5. 🔴 **撤算力掩盖的三段（v0.10 对齐 (23) v0.7 `590092a6`，Codex 5d23a4be：不许再写"132 s = 所有掩盖影响的硬上限"）**：**① 全掩盖/预戳头段 ≤ 132 s**（对手用未来封 +132 s / 预戳能把法 3 完全掩盖的最长头段墙钟——这一段是硬界，共识戳规则给：时间戳在挖块时定死、撤后改不了旧块）；**② 残余估计器影响随滑动 W 窗衰减**（头段后预戳块老化出窗，法 3 残余偏差随 W 线性衰减，折算 `H_floor_total_lb(法3) = 法3_raw/(1 + 口径/W)`，口径 live 132 / 回溯 264）；**③ 阈值检测时延 = `132 + f×W + T_dwell`**（在声明的出块速率 + Poisson 统计假设下：`W > 132/f_detect` ∧ `W ≥ (1/f)²/R`）。总影响 = ① + ② + ③，真正的守卫是"戳挖时定死 + 未来封"这条机制（给出 ① 的 132 s），不是一个数。⇒ **在飞头寸暴露窗 = `t_w + 132 + f×W + T_dwell`**，须 < 覆盖在飞头寸的 covenant 保护，并**写进反应方入场披露**（`k_baked` / 入场 `H_floor_honest_lb` 采样 / `s_visible_max` 及窗 / `s_adv_cap` 及依据 / `H_adv` / 在飞期不受再量保护）——链上可核部分与 Owner 假设部分分层。
...
1-bis. 🔴 **MUST（v0.6 立、v0.7 改、v0.8 按 Codex 定形、v0.9 挂 `s_adv_cap`）：`M_observe` 的 `B_win` 需【具名 `k_max`，且挂在需独立论证 `s_adv_cap` 的诚实地板上】**——`B_win = f(k)` 有界（Codex ACCEPTED 定性；数值曲线 durable 源 `8310f390` 已入库）。`k_max` **不是**一个可以凭空拍的比值：Codex 原句 *"The ratio is only meaningful relative to a credible pre-attack honest-hash baseline. On a nearly empty network, '1000× current hash' may still be operationally cheap, so the assumption is weak exactly where the protocol would rely on it most."* ⇒ **政策形状见 §6**（Owner 预算 → pre-entry 稳定算力地板 → 推 `k_max` → 跌破 fail-closed）；(21) 稿给绝对成本表作 Owner 输入。
...
2. 三个数全是 PROVISIONAL-PLACEHOLDER：① 缺深确认级落链证据（`N_claim` 证据基近零，3-B 已标）；② 节点尾部是坏节点的数；③ P3 形状未出；④ `B_win` 有曲线（durable 源 `8310f390`）无定值——**`k_max` 未具名，且须挂在需 `s_adv_cap` 的诚实地板上**（v0.9）；⑤ `S_unalloc` 待 §5① 散度替换；⑥ `W_dis` 91 min 是坏节点数据。
2-bis. 🔴 **残余清单（v0.9 按 Codex f65c1fbe + 6fd55a53 重写为六项，作为 (d) 从 PROVISIONAL 升级的唯一路径）**：
   1. **≥30 笔真实 claim-shape、深度合格（≥20）的落链观测，Leg A 用 SENDER_TS 口径**（发送方/harness 发出时绑定 txid 的 submit 时刻；(27) v0.3 `docs/provenance/2026-08-27-claim-depth/`：代理只给 Leg B 确认深度物理，Leg A 的 MEMPOOL_SEEN/PROXY_POLL 不入最终界；relay 7 站点 `[submit]` 日志 hook 待 Owner 批）→ 定 `N_claim = 实测包络 + 具名 S_unalloc`（p100 非未来上界）。
   2. **同步后 `W_dis` 运行包络证据**，每个区间同时记 wall-clock 时长与参考/网络 DAA 推进（§5②；参考节点列只定 `W_dis` + sanity，不定 `B_win`）。
   3. **Owner 批准的具名 `k_max`，挂在 `H_floor_honest_lb = H_floor_total_lb × (1 − s_adv_cap)` 上**——`s_adv_cap` **谁论证（v0.11 按 (23) v0.10 §3.5/§8）**：(a-total) `min(1, H_adv_cap/H_total_lb)`（Owner 具名 `H_adv_cap`）/ (b) 自持路 `(H_vis_ub − H_self_lb + B_adv)/(H_vis_ub + B_adv)`（`H_vis_ub = λ_ub(n)(Garwood 精确) × w_cap_window / (t1−t0)`，`w_cap_window` = 层 1 最紧合法窗不等式 `max_{SP∈S} w_child_ub(SP)` μ=0 + 健全性断言，法 3′ 接收计 + 硬闸 `W_min = 3600 s ∧ N_min = 4,000 ∧ window(SP) 精确证书 ∧ (B)(C)(D) 验收过`，覆盖集 `window(SP)【精确】∪ Ncand(SP)`，支配定理 L1–L5（无需枚举），(23) v0.15 `2f632c91`；**Codex 283 `1c7188e2` D-STAT-1/2/3 CLOSED AT DESIGN LAYER**（scope 限精确重建公开状态、不消除对抗容量模型边界；实现四闸须实测未闭），落码 (21) v0.9 `a93f9c5e`、取数 (24) `3f7ef2c5`+`448469b2`；`B_adv` Owner 具名单一预算，默认 = `H_adv_add`；拆开须论证 + 守卫 `H_hidden_ub ≥ H_adv_add`）/ (iii) 身份路（不可用于开放匿名网）；`s_adv_cap ≥ s_visible_max`（(24) v0.4 提取器校验）；无可信 cap ⇒ fail-closed；`B_win(k_max)` 取自 `8310f390`。
   4. **P3 终选 (a)/(b)**——Codex 推荐 **(b)**；(b) 下 `min_O` 只围绕 O/存储/价值地板重定义、claim 费储备归 claimant/watchtower 就绪度（fee-source v0.3 §6，3-A 互引）。
   5. **以上证据齐后具名最终常量**；**环境违约（跌破地板 / 无可信 cap / 节点不健康 / 落链超界 / 固定难度期）一律 fail-closed**，不静默放宽。
   6. **工具官方跑**：(21) `k_max` 成本表 §4 现网算力（`scratch/_j2_kmax_cost.mjs`，待同法入库）、(24) `s_visible_max`（`docs/provenance/2026-08-27-smax/`）、(27) claim-depth（`docs/provenance/2026-08-27-claim-depth/`）各出带 sha256 的正式 JSON（SYNC-GATE 过后，(17) 清单 ③c/③d/③e），才有数可填。**本稿的贡献是把"该量什么、怎么量、取值规则、保守方向"钉死**，不是把数拍死。
