# §6-3 gate (d) · `min_O` / `N_claim` / `N_margin` 具名保守值提案 v0.9（合并版 · 证据层 · 零落码 · 不改 v0.15 正文）
...
- 🔴 **`k_max` 政策形状（v0.8，Codex eb4db39c 建议、Owner 定；本稿只落形状不拍数）**：
  ...
  2. Tier-2 入场额外要求 **具名 pre-entry 窗口内的稳定算力地板**——**v0.9 按 (23) v0.6 `b6dbcfd0` 定语义**：`H_floor_total_lb` = 墙钟窗内 `min(法1 tip-bits 反推, 法2 Δblue_work/时间戳跨度, 法3 块时间戳窗计数×work_per_block)` 的低分位（三闸：样本充分 / p10 / 无深坑 + 波动比纳 Poisson）；**它量的是【总】算力，链上分不出对手份额** ⇒ 诚实地板 `H_floor_honest_lb = H_floor_total_lb × (1 − s_adv_cap)`，其中 **`s_adv_cap`** = **独立论证的总对手份额上界**，三源（Codex f65c1fbe 裁 `max(s_owner, s_max)` 作上界 REJECTED 后）：(i) **`1 − s_self`**（评估方自持算力份额，角色相对，开放网须与 (ii) 合用——对手可加算力使 `s_self` 事后缩水）；(ii) **经济 cap** `H_adv/(H_total_lb + H_adv)`（Owner 对抗预算，= 本形状第 1 步）；(iii) 身份路（有归属的矿工集），开放匿名网不可用。约束 **`s_adv_cap ≥ s_visible_max`**（`s_visible_max` = 窗内可见最大单 coinbase-**payload** 矿工份额，(24) v0.4 提取器 `docs/provenance/2026-08-27-smax/`，Sybil 令其偏低 ⇒ 只是下界/告警/cap 可信度校验，**不得当上界用**）；**无可信 `s_adv_cap` ⇒ Tier-2 fail-closed**。🔴 **TN12 结论（(23) v0.6，理由从"匿名给不了"迁移为"零算力一方给不了"）**：我方持全部算力 ⇒ 对方 `s_self≈0` ⇒ 其 (i) 给 cap=1、(ii) 需 Owner 给可信 `H_adv` 且对手可能就是矿工本身 ⇒ fair-exchange **两侧都要 cap 成立** ⇒ **现网 Tier-2 关**（机制输出，非判断）。法 3 估计器偏差按 (23) v0.6：live 口径 `132/W`、回溯 `264/W`（PMT ≈ now−132 s，`consensus/core/src/config/constants.rs:26-30`），`H_floor_total_lb(法3) = 法3_raw/(1 + 口径/W)`；
  3. **由地板 + 预算推 `k_max`**：`k_max = 1 + H_adv / H_floor_honest_lb`（k 是"总/原"比），入场要求 `k_max ≤ k_baked` ⇔ `H_floor_honest_lb ≥ H_adv/(k_baked − 1) =: H_floor_min`；`B_win(k_max)` 由 **durable 仿真源 `8310f390`**（NWT (22) 入库）取；
  ...
  5. 🔴 **撤算力掩盖的三层（(23) v0.6，J2 红队后定形）**：掩盖**时限** = **132 s 硬上限**（时间戳在挖块时定死、撤后改不了旧块，要掩盖须撤前预戳到 ≤ `t_w+132`，之后法 3 随窗滑过预戳块线性回落——这是**根守卫**，共识给）；掩盖**幅度** = `132/W`（live）/ `264/W`（回溯），由 W 定尺（`W > 132/f_detect` 且 ≥ Poisson 下界 `(1/f)²/R`）；检测**时延** ≈ `132 + f×W + T_dwell`（W 越大抓得越慢）⇒ **在飞头寸暴露窗 = `t_w + 132 + f×W + T_dwell`**，须 < 覆盖在飞头寸的 covenant 保护，并**写进反应方入场披露**（`k_baked` / 入场 `H_floor_honest_lb` 采样 / `s_visible_max` 及窗 / `s_adv_cap` 及依据 / `H_adv` / 在飞期不受再量保护）——链上可核部分与 Owner 假设部分分层。
