# §6-3 gate (d) · `min_O` / `N_claim` / `N_margin` 具名保守值提案 v0.5（证据层 · 零落码 · 不改 v0.15 正文）
...
- 🔴 **两条风险（v0.4 明写，v0.5 保留）**：(i) **单矿工体制下 k≥2 阶跃易得**——TN12 算力就是一两台矿机，任何人一张卡就能让出块率短时 ≥2×，所以 v0.3 的 "2× 目标 = 封顶" **不成立**；(ii) **150 样本固定难度期**（网络重启 / fork 后 ≈10 min）出块率 ∝ 算力、**无界**——固定 N **盖不住**无界推进 ⇒ 不是调大 N，是 §6 在此期**禁用 Tier-2**（可 enforce 规则见 §6）。
...
- 🔴 **固定难度期禁用（v0.5，可 enforce）**：新网 / BPS-fork 后的头 `MIN_DIFFICULTY_WINDOW_SIZE = 150` 个采样（`config/constants.rs:54`；采样间隔 4 s ⇒ ≈600 s；@10 BPS ≈ 6,000 DAA）内**难度固定、出块率 ∝ 算力无界**——任何固定 N 都盖不住 ⇒ **Tier-2 禁用，不是调大 N**。**规则**：入场前读 `virtualDaaScore`，须满足 `virtualDaaScore − DAA_at_(genesis ∨ 最近 fork activation) ≥ 26,440`（= 一个完整采样窗 661 × 40 DAA，比 150 样本更保守，保证难度已按完整窗调整过），否则拒入场；TN12 的 fork activation 全为 `always()`（`config/params.rs:693-694`）⇒ 现网只需对 `TESTNET12_GENESIS` 起算，**网络重启/重置（TN12 有先例）时这条自动重新生效**。
...
2. 三个数全是 PROVISIONAL-PLACEHOLDER：① 缺深确认级落链证据（`N_claim` 证据基近零，3-B 已标）；② 节点尾部是坏节点的数；③ P3 形状未出；④ **`B_win` 无对抗仿真、无具名信任假设**（v0.5：良性实测不算数）；⑤ `S_unalloc` 待 §5① 散度替换。**`B_win` 的 k（首动方可动员算力比）是一条待 Owner/Codex 给的信任假设**，本稿不拍。**本稿的贡献是把"该量什么、怎么量、取值规则、保守方向"钉死**，不是把数拍死。
