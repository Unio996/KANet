...
- 🔴 **v0.6 · `B_win` 的有界/无界完全取决于时间戳假设**（J2 红队 NWT (19) `c30bb446`，NWT 仿真 `scratch/_nwt_bwin_sim.mjs` 表值 J2 逐格复现）：
  ...
  - **J2 反向核三条攻法，都破不了**：(i) **私挖整窗再一次性释放**：释放时最新戳若 > 接收节点 `unix_now()+132` 被诚实节点拒（未来封是按**接收方**墙钟判，攻击者自己节点不受限但进不了诚实 DAG = pump 没发生）；若等真实时间追上再释放，公共 DAA 速率 = claimed 速率 ≤ 真实 + 132 s 一次性 ⇒ 仍 f(k)。(ii) **控 min_ts**：窗内 min_ts 是最老样本（≈2,641 s 前），新块戳下限 = PMT+1（≈1,320 s 前，`post_pow_validation.rs:23`）> 窗 min ⇒ 压不低 min；`swap_remove(min)` 只影响平均不影响 measured。(iii) **审查信道（戳落后 >660 s ⇒ 全网 `is_nearly_synced=false`）**：与 pump **互斥**——落后 ⇒ measured 变小 ⇒ 难度升 ⇒ 不 pump；pump 时 tip 戳不落后 ⇒ `is_nearly_synced` 仍 true。审查本身无界但 **out-of-model**（bounded-inclusion 之外，Codex 267 (d) 原句 "cannot prove censorship resistance absolutely"）。
