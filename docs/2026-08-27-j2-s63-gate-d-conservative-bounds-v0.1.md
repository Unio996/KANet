# §6-3 gate (d) · `min_O` / `N_claim` / `N_margin` 具名保守值提案 v0.6（证据层 · 零落码 · 不改 v0.15 正文）
...
- 🔴 **v0.6 · `B_win` 的有界/无界完全取决于时间戳假设**（J2 红队 NWT (19) `c30bb446`，NWT 仿真 `scratch/_nwt_bwin_sim.mjs` 表值 J2 逐格复现）：
  ...
  - **首动方 = 矿工（控制时间戳）**：块时间戳只有两条规则——**未来侧** `pre_ghostdag_validation.rs:40-42 timestamp > unix_now() + TIMESTAMP_DEVIATION_TOLERANCE(132 s) ⇒ TimeTooFarIntoTheFuture`；**过去侧** `post_pow_validation.rs:23-24 timestamp <= past_median_time ⇒ TimeTooOld`——**没有任何"不得落后墙钟"的规则**。单矿工按 expected 节奏（每样本 4,000 ms）给戳、真实以 k× 产块 ⇒ `measured == expected` ⇒ `new_target` 恒不变 ⇒ 难度永不升 ⇒ **超量 = (k−1) × 10/s × W_dis，无界**（与固定难度期同式）。代价只是戳逐渐落后墙钟 `(1−1/k)×T`。
  - 🔴 **同一动作还关掉提交口**：戳落后 > 窗/4 ≈ 660 s 时，全网节点 `is_nearly_synced=false`（`protocol/mining/src/rule_engine.rs:131-134 unix_now() < sink_timestamp + expected_difficulty_window_duration/4`）⇒ ① `getServerInfo().isSynced=false`（`rpc/service/src/service.rs:1224 is_sink_recent_and_connected` = `rule_engine.rs:118-119 has_peers ∧ is_nearly_synced`）⇒ 我方 relay 闸 `kasia-relay/src/lib/transaction.mjs:151` 拒 submit；② kaspad **P2P tx relay 停**（`protocol/flows/src/v7/txrelay/flow.rs:119 if !is_nearly_synced { continue }`，注释 "Transaction relay is disabled if the node is out of sync"）⇒ 即使绕过我方闸，claim 也传不出去，只有攻击者（唯一矿工）的 mempool 决定打包。⇒ 反应方与全部 watchtower **同时失能**，且 `W_dis` 由攻击者决定。
  - ⇒ **在声明的威胁模型（first-mover-with-mining，TN12 单矿工）下，`B_win` 没有可仿真的上界；仿真只对"诚实时间戳的算力注入者"成立，而那不是我们声明的对手。** 处置见 §7 ①-bis。
...
| **`N_claim + N_margin`（链上 enforce 的那一个数）** | **123,600（PROVISIONAL-PLACEHOLDER：`B_win` 占位 55,200 未定、`S_unalloc` 声明值、`N_claim` n=1、🔴 v0.6 **`B_win` 在声明威胁模型下不可界**——本数只在 §7 ①-bis (甲) 假设下才有意义）** | **≈ 3 h 26 min（名义）** |
...
1-bis. 🔴🔴 **新 MUST（v0.6，J2 红队 NWT (19) 后 Bettor 裁）：`M_observe` 以 DAA 计在 first-mover-with-mining 威胁模型下【不可界】**——3-C 已证：矿工控制时间戳 ⇒ 难度不响应 ⇒ `B_win = (k−1)×10/s×W_dis` 无界，且同一动作让全网 `isSynced=false` ⇒ `W_dis` 也由攻击者定。**两条出路只列不拍**：
   - **(甲) 加信任假设："首动方不控制多数算力 / 不控制时间戳"**（等价于"存在诚实多数矿工把 PMT 与采样窗拉向真实时间"）。🔴 **如实：TN12 现网单矿工（算力 = 我们自己的一两台矿机），该假设不成立 ⇒ 在 (甲) 下现网【不能开 Tier-2】**；只有多矿工、且首动方算力占比可信 < 50% 的网络才满足。
   - **(乙) `M_observe` 换锚，不以 DAA 计**——例如锚到反应方可独立验证的量（自证时间/外部时钟/多方签名的"活性证明"）或把 claim 窗改为"反应方主动续期"型；这改的是 v0.15 §4-c/§4-e recovery 支的锚 = **架构决，Owner/Codex**，且会与 O-lineage "不靠外部 finality 钟"的初衷冲突（v0.15 §5 @L321），本稿只指出冲突不裁。
   - 两条之外没有第三条："仿真给 k 再上调"在此模型下**不成立**（无界不是大数）。
2. 三个数全是 PROVISIONAL-PLACEHOLDER：① 缺深确认级落链证据（`N_claim` 证据基近零，3-B 已标）；② 节点尾部是坏节点的数；③ P3 形状未出；④ `B_win` 无对抗仿真定值、无具名信任假设（v0.5：良性实测不算数）；⑤ `S_unalloc` 待 §5① 散度替换；⑥ **`B_win` 在声明威胁模型下不可界（v0.6）**。**`B_win` 的 k 只在 (甲) 假设成立后才有意义**，本稿不拍。**本稿的贡献是把"该量什么、怎么量、取值规则、保守方向"钉死**，不是把数拍死。
