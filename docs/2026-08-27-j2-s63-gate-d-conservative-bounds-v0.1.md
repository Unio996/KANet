...
2-bis. 🔴 **残余清单（v0.9 按 Codex f65c1fbe + 6fd55a53 重写为六项，作为 (d) 从 PROVISIONAL 升级的唯一路径）**：
   ...
   3. **Owner 批准的具名 `k_max`，挂在 `H_floor_honest_lb = H_floor_total_lb × (1 − s_adv_cap)` 上**——`s_adv_cap` **谁论证（v0.11 按 (23) v0.10 §3.5/§8）**：(a-total) `min(1, H_adv_cap/H_total_lb)`（Owner 具名 `H_adv_cap`）/ (b) 自持路 `(H_vis_ub − H_self_lb + B_adv)/(H_vis_ub + B_adv)`（`H_vis_ub = n_ub(Garwood 精确) × w_max / (t1−t0)` 法 3′ 接收计 + 双闸 `W_min = 3600 s ∧ N_min = 4,000`，(23) v0.12 `0e123323`；`B_adv` Owner 具名单一预算，默认 = `H_adv_add`；拆开须论证 + 守卫 `H_hidden_ub ≥ H_adv_add`）/ (iii) 身份路（不可用于开放匿名网）；`s_adv_cap ≥ s_visible_max`（(24) v0.4 提取器校验）；无可信 cap ⇒ fail-closed；`B_win(k_max)` 取自 `8310f390`。
