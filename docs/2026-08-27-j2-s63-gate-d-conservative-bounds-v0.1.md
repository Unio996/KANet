...
> **v0.10（Codex 5d23a4be 裁 277 + (23) v0.7 `590092a6` + J2 红队，Bettor 裁"确定部分"先出）**：§6 step 2 **撤**作废双计式 `H_adv/(H_total_lb + H_adv)`；`H_total_lb` 来源钉死 = (21) v0.5 三法 `min` 且 `gate_input === 'OK'`（`law1_only` / `FAIL_CLOSED` 不得作分母）；经济 cap 改 (23) v0.7 冻结二选一——(a) 份额式 `min(1, H_adv_cap/H_total_lb)` → 喂 `H_floor_honest_lb`，(b) 增量式 `(max(0, H_total_lb − H_self) + H_adv_add)/(H_total_lb + H_adv_add)`（自知 `H_self` 绝对写法）→ `H_adv_add` 喂 `k_max = 1 + H_adv_add/H_total_lb`；同可算取 max；明写两式答的不是同一问题；step 5 的 132 s 改**三段**（头段 ≤132 s 硬界 / 残余随 W 衰减 / 检测时延 `132+f×W+T_dwell`）。NWT (23) v0.8 若改语义再补。
