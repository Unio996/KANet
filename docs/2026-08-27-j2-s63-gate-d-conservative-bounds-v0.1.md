# §6-3 gate (d) · `min_O` / `N_claim` / `N_margin` 具名保守值提案 v0.7（证据层 · 零落码 · 不改 v0.15 正文）
...
- 🔴 **v0.6 · `B_win` 的有界/无界完全取决于时间戳假设**（J2 红队 NWT (19) `c30bb446`，NWT 仿真 `scratch/_nwt_bwin_sim.mjs` 表值 J2 逐格复现）：
  ...
  - 🔴 **主论证 = 守恒（Bettor 提法，NWT/J2 各自试破未破）**：**"任一诚实节点收块时 `stamp − 其 unix_now ≤ 132 s`"（`pre_ghostdag_validation.rs:40-42` 按【接收方】墙钟判）⇒ 已发布进诚实 DAG 的链，其采样窗能表示的 stamp-elapsed ≤ 真实 elapsed + 132 s ⇒ 难度算法看到的"时间"至多比真实多 132 s（一次性）⇒ 长期 claimed 速率 = 真实速率 ⇒ 难度把 claimed-time 块率钉回 10/s ⇒ 真实块率 → 10/s。** 于是 DAA-pump 的全部超量 = 一次性 132 s × 10/s（+1,320）+ 难度滞后一窗的瞬态（f(k)，≤ 一窗量级对数缓增）。NWT sim v0.2 的表是这条守恒的**数值佐证**，不是论证本身。且 132 s 预算按接收方时钟判，**不一定吃得满** ⇒ +1,320 是上界侧保守。
  - **J2 反向核三条攻法，都破不了**：(i) **私挖整窗再一次性释放**：释放时最新戳若 > 接收节点 `unix_now()+132` 被诚实节点拒（未来封是按**接收方**墙钟判，攻击者自己节点不受限但进不了诚实 DAG = pump 没发生）；若干等到真实时钟到 newest_stamp 再释放：私挖 `2,640,000/k` ms + 干等 `2,640,000(1−1/k)` ms 内推进 26,440 DAA = **净 10/s 零加速**，可重复但**不叠加** ⇒ 仍 f(k)。(ii) **控 min_ts**：窗内 min_ts 是最老样本（≈2,641 s 前），新块戳下限 = PMT+1（≈1,320 s 前，`post_pow_validation.rs:23`）> 窗 min ⇒ 压不低 min；`swap_remove(min)` 只影响平均不影响 measured。(iii) **审查信道（戳落后 >660 s ⇒ 全网 `is_nearly_synced=false`）**：与 pump **互斥**——落后 ⇒ measured 变小 ⇒ 难度升 ⇒ 不 pump；pump 时 tip 戳不落后 ⇒ `is_nearly_synced` 仍 true。审查本身无界但 **out-of-model**（bounded-inclusion 之外，Codex 267 (d) 原句 "cannot prove censorship resistance absolutely"）。
...
| **`N_claim + N_margin`（链上 enforce 的那一个数）** | **123,600（PROVISIONAL-PLACEHOLDER：`B_win` 占位 55,200 未定、`S_unalloc` 声明值、`N_claim` n=1、v0.7 **`B_win` 有界 = f(k) 但 `k_max` 未具名**——55,200 ⟺ `k_max ≲ 1000`，本数只在该假设下才有意义）** | **≈ 3 h 26 min（名义）** |
...
1-bis. 🔴 **MUST（v0.6 立、v0.7 改）：`M_observe` 的 `B_win` 需【具名 `k_max`】**——v0.6 写"在 first-mover-with-mining 下不可界"**已被 NWT sim v0.2 反驳、J2 反向核后认**（3-C：未来封 +132 s 一次性、过去侧自败、私挖释放/控 min_ts/审查三攻法皆不破界）。现状：`B_win = f(k)` 有界且随 k 单调，NWT 对抗仿真给曲线；**占位 55,200 ⟺ `k_max ≲ 1000`**。
   - **(甲) 降级为：具名信任假设 `k_max`**（"首动方可动员的算力 ≤ k_max × 注入前网络算力"）。🔴 **如实：TN12 现网基线算力极小，k=1000 在绝对量上便宜 ⇒ 这是弱假设**；成立与否 = Owner/Codex 信任决定，本稿不拍；若拍 `k_max`，`B_win` 按 NWT 曲线取值并只准上调。
   - **(乙) 撤回**：既然有界，无须换锚；保留一句提醒——若日后 `k_max` 拍不出可信值，(乙) 才重新上桌，且与 v0.15 §5 @L321 "不靠外部 finality 钟"冲突仍在。
   - **审查信道**（戳落后 >660 s ⇒ 全网停 relay / 我方闸拒 submit）：**无界、但 out-of-model**，与 pump 互斥（3-C），归 bounded-inclusion 假设，Codex 267 (d) 已承认不可绝对证；本稿不用 N 兜它。
2. 三个数全是 PROVISIONAL-PLACEHOLDER：① 缺深确认级落链证据（`N_claim` 证据基近零，3-B 已标）；② 节点尾部是坏节点的数；③ P3 形状未出；④ `B_win` 有曲线无定值——**`k_max` 未具名**（v0.7）；⑤ `S_unalloc` 待 §5① 散度替换；⑥ `W_dis` 91 min 是坏节点数据。**`B_win` 定值 = NWT 曲线在 `k_max` 处取值**，本稿不拍 `k_max`。**本稿的贡献是把"该量什么、怎么量、取值规则、保守方向"钉死**，不是把数拍死。
