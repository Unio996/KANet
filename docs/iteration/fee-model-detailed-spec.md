...
- OPEN 待 Owner 终裁: ①echo 根治时点 (MVP-document vs staged commit-reveal/slash) ②double-compounding 缓解 (a 接受/b cap/c sqrt) ③BPS 值确认 (tn12) ④取整/dust 优先级。
- 终裁后顺序: J2 land base 改 (losing→total) + maker-self-broker + oracle 质押 0.5% 分账 → 链上 Tier4 验 (same-node + **cross-node** J1 节点, [[cross-node-testing-critical-j1-separate-node]]) → 回归。
