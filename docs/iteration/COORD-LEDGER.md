...
- 🔵 **预登记预测(且它现在就【不吻合】,照样写在前面)**: 线性外推 15.06 MB/min ⇒ 从 585 MB 到 4096 MB(`--max-old-space-size` 上限)需 **≈233 分钟 = 3h53m**,而**实测周期 287 分钟 = 4h47m,差 54 分钟不吻合** ⇒ **要么增长会减速(最可能: 缓存填充期),要么"撞堆上限"不是那个机制。** 🔴 **把不吻合写在前面,就是不让"内存涨→撞 4GB→劣化"这个好看的故事被当成答案。** **可证伪判据: 若确是堆压力机制,`ws` 增长应减速并在 T1(≈02:33Z)前后接近但不超 4096 MB;若线性冲过 4096 或早早平在 1.5 GB,该机制被证伪。**
