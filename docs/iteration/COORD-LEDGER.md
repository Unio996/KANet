...
- **ZK 检查点(2026-07-06·team 共识暂停·下次 fresh 秒接)**:
  ...
  - 🔴 **当前硬 blocker = OP_PICK codegen(执行时炸·option A 自修 silverc)**: 完整 2-input CloseZk 覆约广播被 TN12 活链拒 `pick at an invalid location`——**silverc codegen bug 运行时实证**(编译过但字节码 OP_PICK off-by-one·跟 5/11→6/28 pick_from_depth 零差异吻合=没修·活链拒是最强证据)。gate(ZkScriptBuilder)侧 work·**卡在覆约 .sil**。根因已定位(memory `reference-silverc-oppick-offbyone-codegen-bug`·别重复 13 轮 bisect): **silverc StackBindings bookkeeping 在多层嵌套 blake2b/sha256/concat 下深层 bug·pick_from_depth loc==stack_len off-by-one**。**修必须在 emit/codegen 侧(产出正确 OP_PICK index·脚本在【不变的 live VM】跑通)·绝不改 VM 执行语义(硬分叉+D-005)**。这是通往完整 ZK settle 的最后硬骨头·深层编译器 debug·宜 fresh focus。
  - **汇合 = 完整 ZK settle**(真 CloseZk 两-input + 真 guest proof)。**就差 OP_PICK codegen 修好** → 覆约执行通 → 接 J1 真 proof → 完整真 settle。上链走 co-verify 八命门+Owner 批闸。工具链: silverc 隔离·zk-sdk WASM `D:/rusty-kaspa-zksdk-isolated`。
