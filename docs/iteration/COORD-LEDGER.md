...
- **⇒ 根因卡三条线索(归 KANet-UI,排其手上两件之后,设计先行不碰连接层;他已回执并保持队列不插队)**: ①**差分**(同码两机只 :3200 劣化 ⇒ 找两台在连 kaspad 这条路上的差异)②**形状**(累积型 ≈5h,n=2 ⇒ 找随进程寿命单调增长的东西: 连接数/句柄/监听器/缓冲/定时器)③**链条待证**(劣化→判死→自愈→复位)。**搜索空间已很小。**
