...
- **kaspad.exe 在跑**(PID 9084), 但它写的 `logs/kaspad.log` / `kaspad-tn12.out.log` 最后一行停在7月中——现在这个 PID 没往那两份文件写, 无法从旧 log 判断现在同步状态。
- **🔴 独立信源确认未同步**: 本机 relay(KANet-UI-tn, 刚重启)向 kaspad 发广播被拒: `HTTP 500 {"error":"RPC node is not synced"}`——不是频道层的问题, 是 kaspad 自己报 isSynced=false, 与 (260)(261) 的 tips/lag/DAA 停滞读数吻合方向一致。频道广播本条因此走 git-first。
