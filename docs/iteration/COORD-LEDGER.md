...
- 🔴🔴🔴 **J2 从劣化现场日志逐字抓到根因**: `[rpc-health] discover failed: **memory access out of bounds**` —— **28,482 行**(现场副本 `scratch/console-DEGRADED-20260805T1308Z.log`,13.3 MB)。⇒ **kaspa-wasm 的线性内存逼近其自身上限 ⇒ 分配失败 ⇒ 所有走 wasm 的 RPC 调用失败。**
