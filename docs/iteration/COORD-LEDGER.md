...
- 🔴 **P1「验不成≠可以退款」设计 v0.1(`357bd05b`)两处需 Owner 点头(2026-08-04 挂账 by Bettor·出处 J2 §1/§6,设计与红队不被 gate,只 gate 这两处的实装)**: ①**收窄 `:1052` ≈ 把"钱永久卡住"变回常态**——该分支当初正是为解 cross-node quorum 永不可达 ⇒ stake 永锁(ZOMBIE 实证)而加;收窄与 Owner 铁律一致(3000 KAS 先例=宁可永久损失也不退款),**但会产生新一批锁死资金,量级问题该 Owner 再点一次头**,不由设计稿默默替选。②**`:1027` dispute grace 到期自动 refund 是 Owner r518 终裁的终态**,改动其触发语义需 Owner 确认(同函数内 `abstain≥4 ⇒ refund` 为有证据退款、保留=白名单原型;要收的只是"grace 到期这个纯时间条件")。
