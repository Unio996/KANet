...
| （不读）其它 `settled_at` | `kasia-console/src/services/bettor-prediction-settler.js:137` toISOString（写别的表）/ `kasia-console/src/api/kanet-broker.js:227,260,327` **非写点**：read-side `by_market` 投影（`:174` 注释），`r.updated_at` 来自 SELECT 不落表 / `kasia-console/src/api/trading.js:1909` 写 `trade_baselines`（参数）| 别的表 / 不落表 | — | **不在策略内**（(27) 不读；若日后读，先补写点核）| — |
