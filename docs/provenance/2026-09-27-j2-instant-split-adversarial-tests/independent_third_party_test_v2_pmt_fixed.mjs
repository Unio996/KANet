...
async function main() {
  ...
  // 🔴 NWT diff 审 MUST 修复: "已过期"必须用节点真实 pastMedianTime(共识"是否已终结"用的唯一时间源,
  // rusty-kaspa consensus/src/processes/transaction_validator/tx_validation_in_header_context.rs:72-93
  // + pipeline/virtual_processor/processor.rs:1216/1319 逐行核实), 不是 tip 区块时间戳——两者在
  // 出块不规律/曾长时间空闲的网络上可以相差几十分钟(本次 NWT 在共享 simnet 上真实复现过), 用 tip
  // 时间戳算出的"已过期" deadline 可能仍然超前于真实 PMT，导致 refund 被拒 "not finalized"。
  const bdi0 = await rpc.getBlockDagInfo();
  const pmt0 = Number(bdi0.pastMedianTime);
  const deadlineMs = pmt0 - 60_000; // 已过期(相对真实 PMT) -> refund 立即可测(#10); split 无下界随时可测(#11)
