...
// 🔴 2026-09-27 NWT diff 审 MUST 修复后重跑: 改用独立全新 simnet(D-019 同款 2.0.1, 独立端口/目录)，
// 不再用旧共享 simnet(自 09-24 断续挖矿导致 pastMedianTime 严重滞后于墙钟, 是本轮 bug 的根因环境)。
const rpc = new RpcClient({ url: 'ws://127.0.0.1:29817', encoding: Encoding.Borsh, networkId: 'simnet' });
await rpc.connect({});
...
// 🔴 NWT diff 审后修复: deadlineMs/refund 的"到期"判断必须用节点现查 PMT(virtual_past_median_time)，
// 不是 tip 时间戳/Date.now()——否则第二笔(refund)可能先撞上"not finalized"(PMT 未追上 deadline_ms)
// 而不是真正要测的"UTXO 已被花费"，导致测试意图落空却误报 PASS(本条正是 NWT 抓到的那类假阳性同源问题，
// 这里独立修一遍, 与 T10/T15a/T15b 用的是同一个修法)。
await record('T6_double_spend_same_utxo', 'reject', async () => {
  const { cfg } = baseCfg(false);
  const pmt0 = await currentPmt();
  const protocol = createSplitProtocol({ ...cfg, deadlineMs: pmt0 - 60000 });
  const recipientsTotal = cfg.merchant.amountSompi + cfg.broker.amountSompi;
  ...
  await mine(2);
  const pmtNow = await currentPmt(); // 现查, 不用 Date.now()(见上方注释与 SDK buildRefundTx MUST 修复)
  const { tx: refundTx } = buildRefundTx(protocol, utxo, pmtNow); // same outpoint, already spent
  const r2 = await rpc.submitTransaction({ transaction: refundTx, allowOrphan: false }).catch(e => ({ err: e.message }));
  // 断言拒因确实是"已花费"类(UTXO 不存在/找不到该输入的关联条目), 不是"not finalized"(那样会
  // 是同一个假阳性坑, 不算真正测过双花)。
  if (r2.transactionId) return r2; // 若真的通过了, 交给下面统一逻辑标 FAIL(不该发生)
  const isFinalityReason = (r2.err || '').includes('not finalized');
  if (isFinalityReason) return { err: 'WRONG REASON(仍是 PMT 问题非真双花拒绝): ' + r2.err };
  return r2;
});
