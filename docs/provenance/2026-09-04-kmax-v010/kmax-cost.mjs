// (21) v0.10 · k_max 绝对成本表工具 — 入库 durable 版(同 (24)/(27) 法)。只读 RPC。v0.8: 加 D-STAT-1/2 纯函数块(lambdaUb Garwood 上括号 / Chernoff 上轨 / 夹逼+精确向量自检 / nMin / hVisUb 硬闸), 只加不改, main 与三估计器路径未动。
...
// v0.10 接线(只加): 可选 sampledAtMs(wCapWindow 认证时刻) ∉ [t0Ms,t1Ms] ⇒ WINDOW_TIME_MISMATCH((24) v0.2 闸4); 可选 certificate(kind ∉ {GENESIS,TRUNCATION} 或 missing>0) ⇒ NO_W_CAP((24) 闸3); 两参缺省 ⇒ 行为与 v0.8 逐字相同(返回对象不加字段)
export function hVisUb({ n, wCapWindow, t0Ms, t1Ms, alpha = ALPHA, sampledAtMs, certificate }) {
  selfCheck();
  ...
  const wired = sampledAtMs !== undefined || certificate !== undefined;
  if (wired) {
    gate.time_ok = sampledAtMs === undefined ? null : (Number.isFinite(Number(sampledAtMs)) && Number(sampledAtMs) >= Number(t0Ms) && Number(sampledAtMs) <= Number(t1Ms));
    gate.cert_ok = certificate === undefined ? null : (certificate != null && (certificate.kind === 'GENESIS' || certificate.kind === 'TRUNCATION') && !(Number(certificate.missing) > 0));
    if (gate.cert_ok === false) gate.w_cap_ok = false;
  }
  const reasons = [!gate.W_ok && 'W_MIN', !gate.n_ok && 'N_MIN', !gate.w_cap_ok && 'NO_W_CAP', wired && gate.time_ok === false && 'WINDOW_TIME_MISMATCH'].filter(Boolean);
  if (reasons.length) return { H_vis_ub: null, lambda_ub: null, n, W_s, w_cap_window: wCapWindow ?? null, gate, reason: reasons.join('+'), fallback: '(a-total) min(1, H_adv_cap/H_total_lb)', ...(wired ? { sampledAtMs: sampledAtMs ?? null, certificate_kind: certificate?.kind ?? null } : {}) };
  const lam = lambdaUb(n, alpha);
  const base = { H_vis_ub: lam * Number(wCapWindow) / W_s, lambda_ub: +lam.toFixed(6), n, W_s, w_cap_window: Number(wCapWindow), gate, reason: null, fallback: null, note: 'w_cap_window 由参数注入((23) v0.13 层1 重建器待 Codex 281); 单位 = 每块 work(与 calc_work 同), H_vis_ub = work/s' };
  return wired ? { ...base, sampledAtMs: sampledAtMs ?? null, certificate_kind: certificate?.kind ?? null, note: 'w_cap_window 由 (24) fetchCover 透传(certificate + arrivalWindow 同源, (24) v0.2 闸3/闸4); 单位 = 每块 work, H_vis_ub = work/s' } : base;
}
// v0.10 驱动路径草案(默认仍参数注入): 读 (24) fetchCover 输出 JSON(fetch-evidence.json: {wCapWindow, certificate, arrivalWindow{t0Ms,t1Ms}, n_arrivals?, evidence?}) ⇒ 透传 ⇒ hVisUb
//   n_arrivals = own-clock 窗 [t0,t1] 内新可达块数((21) 接收计, 另落); 缺 ⇒ NO_N ⇒ null。四闸(G1–G4)未在真 RPC 路实测前, 此路径只出 PROVISIONAL 数, 不喂 (23) 入场闸。
export function hVisUbFromEvidence(ev, { alpha = ALPHA } = {}) {
  if (!ev || !ev.arrivalWindow) return { H_vis_ub: null, reason: 'NO_EVIDENCE' };
  if (!Number.isInteger(ev.n_arrivals)) return { H_vis_ub: null, reason: 'NO_N', note: 'n_arrivals 须由 (21) 接收计在同一 [t0,t1] 给出' };
  const cert = ev.certificate || null; const sampledAtMs = cert && cert.sampled_at_ms != null ? cert.sampled_at_ms : ev.arrivalWindow.t1Ms;   // 认证时刻缺省 = t1(fetchCover 在 t1 读 sink)
  const g = ev.evidence || null;
  return { ...hVisUb({ n: ev.n_arrivals, wCapWindow: ev.wCapWindow, t0Ms: ev.arrivalWindow.t0Ms, t1Ms: ev.arrivalWindow.t1Ms, alpha, sampledAtMs, certificate: cert }), provisional: true, evidence_gates: g ? { G1: g.G1_pagination_complete_deterministic?.pass ?? null, G2: g.G2_closure?.pass ?? null, G3: g.G3_missing_or_truncated_is_inexact?.pass ?? null, G4: g.G4_same_arrival_window?.pass ?? null, G5: g.G5_bits_selfcheck?.pass ?? null } : null, vantage: g?.vantage ?? null, retries_total: g?.retries_total ?? null };
}
...
async function main() {
  ...
  if (arg('--wcap-evidence', null)) {   // v0.10 驱动路径草案: 不连 RPC, 只读 (24) fetchCover 输出 ⇒ hVisUbFromEvidence(PROVISIONAL, 四闸实测前不喂入场闸)
    const ev = JSON.parse(readFileSync(arg('--wcap-evidence'), 'utf8')); const h = hVisUbFromEvidence(ev); console.log(JSON.stringify({ schema_version: 'kmax-hvis/1', t: new Date().toISOString(), source: arg('--wcap-evidence'), ...h }, null, 1)); return;
  }
