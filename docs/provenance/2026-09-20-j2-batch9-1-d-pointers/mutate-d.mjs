// mutate-d.mjs — 9-1 首批 D 笔(结算指针模块 + deriveWinnerBet 抽取 + leaf-state rowid tiebreak)的变异对照(J2 2026-09-20)。
...
const FILES = { ptr: `${LIB}/proto-settlement-pointers.mjs`, inp: `${LIB}/proto-settlement-inputs.mjs`, leaf: `${LIB}/proto-leaf-state.mjs` };
const TESTS = { ptr: 'proto-settlement-pointers.test.mjs', inp: 'proto-settlement-inputs.test.mjs' };
...
const muts = [
  ...
  ['M-35 ▲ deriveCloseCommitInputs 不再调 deriveWinnerBet、自己重写一份(且漏掉"恰 1 条"检查)——两份逻辑分叉', 'inp', [['const { market, poolValue, winner } = deriveWinnerBet(marketId, { db, who });',
  ...
  ['M-36 ▲ deriveCloseCommitInputs 自己重写但保留全部检查, 只是 winnerBetId 取错(取第一条下注)——"选出的赢家"分叉', 'inp', [['winnerBetId: winner.id }', 'winnerBetId: deriveWinnerBet(marketId, { db }).bets[0].id }']], PI],
  ['M-37 ▲ deriveWinnerBet 胜方条数检查放宽(允许 2 条)', 'inp', [['if (winners.length !== 1) {\n    throw E(', 'if (winners.length < 1) {\n    throw E(']], PI],
  ['M-38 deriveWinnerBet 不再要求 winning_side 已写', 'inp', [["if (market.winning_side !== 0 && market.winning_side !== 1) {\n    throw E('winner_side_unset'", "if (false) {\n    throw E('winner_side_unset'"]], PI],
  ['M-39 ▲ status 闸漏进 deriveWinnerBet(claim_draw 时市场是 resolved, 会必抛)', 'inp', [["if (!market) throw E('winner_market_missing', '市场不存在');", "if (!market) throw E('winner_market_missing', '市场不存在'); if (market.status !== 'sealed') throw E('winner_status', '状态闸漏进来了');"]], PI],
  ['M-40 deriveWinnerBet 的错误不再带 .code', 'inp', [['Object.assign(new Error(`${who}: fail-closed — ${msg}`), { code })', 'new Error(`${who}: fail-closed — ${msg}`)']], PI],
...
try {
  ...
  for (const [name, fk, edits, tks] of muts) {
