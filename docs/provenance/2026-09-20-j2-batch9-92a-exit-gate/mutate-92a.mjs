...
mut('X17', sub('([0-9a-f]{64})', '([0-9a-fA-F]{64})'), '大写 hex id 放行');
mut('X21', sub('([0-9a-f]{64})', '([0-9a-f]{64}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})'), '首版假设的 UUID 形状放行(MUST-1 的反面)');
mut('X22', sub('([0-9a-f]{64})', '([0-9a-f]{32,64})'), 'id 长度放宽到 32–64');
mut('X23', sub('([0-9a-f]{64})', '([0-9a-f]{63,})'), 'id 长度不设上限');
mut('X24', sub('([0-9a-f]{64})', '([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})'), '回到首版的 UUID 形状(生产者↔出口对照应红)');
