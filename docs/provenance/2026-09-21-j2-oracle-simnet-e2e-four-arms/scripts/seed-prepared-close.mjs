...
const RUN = 'D:/kanet-tn12/scratch/_j2_e2e_run'; const WT = 'D:/kanet-tn12/scratch/_j2_wt_e2e'; const EVD = `${RUN}/evidence`;
...
if (cmd === 'fund2') {
  ...
  if (cmd === 'plan') { rec({ action: 'seed_prepared_plan', marketId, ...pre }); await done(); }
    ...
    const built = await ops.build('close_commit', { kaspa, network: 'simnet', relaySpkHex: payerSpkHex, prep: { ...prep, pointers }, chainParents, feeCandidates: [feeCand], withFeeParent, subjectId: marketId, marketId, db: sqlite });
    ...
    const key = `settle:market:${marketId}:resolve`; const now = new Date().toISOString();
