# provenance · `k_max` 绝对成本表工具（(21) v0.9 入库 durable 版）
...
| `vectors-wcap.json` / `expected-output-wcap.json` | 13 条向量 / 期望 |

```bash
node docs/provenance/2026-08-27-kmax/wcap-window.test.mjs      # 期望 13/13 PASS ((D)(E) 只在合成小 DAG 上跑)
```
