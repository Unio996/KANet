...
> **归因更正（2026-09-15，账本1430，NWT 按 rusty-kaspa v2.0.1 源码钉死，不改下方原文/原始数据）**：
> 下方"结论先行 ③"与"实测数据"表格第一行把 `~19.95 KAS` 全部归到"漏 `CovenantBinding` 声明"头上，
> **归因不准确，原始数字本身没有错**。NWT 逐 sompi 复现确认：`utxo_plurality` 有声明时 `p=2`、无声明
> 时 `p=1`，storage mass 按 `C·p²/v` 计算——**声明了永远更贵，漏声明永远低估**，方向从未反转（同
> `docs/provenance/2026-09-15-j2-market-genesis-tx-assembly-vectors/` 向量②测出的 3.01× 低估同一条
> 规律，已被逐 sompi 复现）。`~19.95 KAS` 那次实验的续约值当时是 `100,000`（U 形曲线的陡峭段）——
> **同一个 `v=100,000`，带声明反而是更贵的 `80 KAS`**，不是漏声明更贵。本文档第一版实验把"把续约值 `v`
> 调到 `20,000,000`（U 形最优点）"与"补上 `CovenantBinding` 声明"两个动作的收益合并到了一次对比里，
> 全部记到了"补声明"这一个变量头上，是**多变量没有隔离**导致的归因错误，不是数据造假。下方"实测数据"
> 表格与"结论先行"原文保留不改（历史真实产出的数字）；本笔仅追加这段更正注记。**不影响本文档任何结论
> 性建议**（"续约值须维持 ~20,000,000 sompi"、"续约输出必须声明 CovenantBinding"两条都仍然成立且更
> 加确定——只是"声明的方向效果"这一句话的因果关系需要按上面订正）。
