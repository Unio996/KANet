...
## 交付
- `kasia-console/src/db/migrate.js` v212: `proto_market_verdicts` 追加表 + `proto_markets` 加 9 列(审计 3 + 判定题 5 + 已有列不变)+ 16 个触发器(verdicts 2 + proto_markets 14: R1 13 + R4 1)。迁移幂等(列 PRAGMA 守卫、表 IF NOT EXISTS、触发器 DROP IF EXISTS 后重建)、单事务。
...
- `mutate-triggers.mjs` + `mutation-raw.txt`: 65 变异(16 整触发器删除 + 49 谓词级改写)全杀; 首轮 64/65(F12c: 只改 verdict_id 的审计不可改缺测)已补测并保留首轮输出 `mutation-raw-round1.txt`。
...
5. **verdicts.outcome 只允许 {0,1}**: ABSTAIN 不落 verdicts 行(设计里弃权=不产生判定)。若后续批要存"弃权/无效"记录, 另议。`evidence_ref` 非空白必填。
