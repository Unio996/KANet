// mutate-c.mjs — 9-1 首批 C 笔(proto-settlement-c1.mjs)的变异对照(J2 2026-09-20)。
...
const muts = [
  ...
  ['M-21 ▲ C4 每步总预算的定时器不再 reject(超预算无限等)', "timer = setTimeout(() => reject(F('facts_step_budget_exceeded', ", "timer = setTimeout(() => void (F('facts_step_budget_exceeded', "],
  ['M-22 ▲ C11 请求串行发出(4×13s 最坏 52s 的活性问题, N91-4)', "const ask = (address, payload, req) => Promise.resolve()\n    .then(() => requestFacts(address, payload))", "let _chain = Promise.resolve();\n  const ask = (address, payload, req) => (_chain = _chain.then(() => requestFacts(address, payload)))"],
  ...
  ['M-28 ▲ chainParents.hasCovenant 写死 false(mass plurality 取常量而非链上事实)', 'hasCovenant: u.covenantId !== null };', 'hasCovenant: false };'],
  ...
  // ── fee 候选(§19.3 步骤 6) ──
  ['M-33 ▲ C5 不再跳过 covenantId != null 的毒化候选', "if (u.covenantId !== null || normHex(u.scriptPublicKey.scriptHex) !== relaySpk) { skippedPoisoned++; continue; }", "if (normHex(u.scriptPublicKey.scriptHex) !== relaySpk) { skippedPoisoned++; continue; }"],
  ...
  ['M-36 ▲ C9 无干净候选一律报 none(不区分 saturated)', "(skippedPoisoned > 0 || truncated ? 'saturated' : 'none')", "'none'"],
  ...
  ['M-41 ▲ chainParents 的 fee 项 hasCovenant 写死 false(不取自形态 L 条目)', 'hasCovenant: feeCandidate.covenantId !== null }', 'hasCovenant: false }'],
  // ── 报警分级(§19.3 步骤 7) ──
  ['M-42 ▲ C10 同一 tick 内的重试也累计', 'if (tickId !== lastTick) { count += 1; lastTick = tickId; }', 'count += 1; lastTick = tickId;'],
  ['M-43 ▲ C10 成功不清零', 'onSuccess() { count = 0; lastTick = undefined; },', 'onSuccess() {},'],
  ...
  ['M-45 C10 升级阈值差一(> 而非 >=)', "level: count >= ticksToError ? 'error' : 'warn'", "level: count > ticksToError ? 'error' : 'warn'"],
