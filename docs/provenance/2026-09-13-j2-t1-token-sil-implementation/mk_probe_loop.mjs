...
const ctorArgs = JSON.parse(fs.readFileSync('ProbeBoutCombo.args.json', 'utf8')).map(a => Array.isArray(a.value) ? '0x' + Buffer.from(a.value).toString('hex') : a.value);
...
fs.writeFileSync('ProbeBoutLoop.test.json', JSON.stringify({ tests }, null, 2));
