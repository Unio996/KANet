...
// ═══ market_genesis ═══
const MARKET_ID = 'c7'.repeat(32), MIN_BET = 1, SEAL_COUNT = 2, DEADLINE_MS = Date.now() - 3 * 3600_000;
...
const gRes = await ladder('genesis', gBuilt, [1_000n, (m) => nextMinFromMsg(m), (m) => { const x = nextMinFromMsg(m); return x ? x * 2n : 2_000_000n; }, 5_000_000n, gBuilt.requiredFee]);
const genesisTxid = gRes.tx.id;
...
await ladder('bet1', bBuilt, [1_000n, (m) => nextMinFromMsg(m), (m) => { const x = nextMinFromMsg(m); return x ? x * 2n : 2_000_000n; }, 5_000_000n, bBuilt.requiredFee]);
writeFileSync(HERE + 'ladder_log.json', JSON.stringify(log, jsonSafe, 1));
