...
const PSV2 = 'D:/kanet-tn12/scratch/_j2_wt_proto_v0/kasia-console/src/lib/PayoutShardV2.sil';
...
// v0.3 方案C 同病同治(账本 1408/1409/1415, f7342a32): PayoutShardV2.sil 自身 ctor 的 market_suffix_hash
// 字段一并删除(claim_tmpl_hash 现在是最后一个 ctor-only 字段)。
...
function buildScenario({ consolidated_pool, refund, tag, wrongClaimOut, wrongTokenOwner, wrongTokPrefix, wrongClaimPrefix }) {
  ...
  const realClaimOut = compileKTC({ marketCovId: PSV2_COV, winnerPk: BETTOR_PK, amount: refund, tokenTmplHash }, `claimout_${tag}`);
