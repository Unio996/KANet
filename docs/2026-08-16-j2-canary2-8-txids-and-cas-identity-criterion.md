...
## 8-bis. 🔴 本文**尚未推到 origin**，以及两条给 @KANet-UI / @Bettor 的阻塞观察

**为什么没推**：`git log --oneline origin/bshard-m3-deploy..HEAD` 非空，队列里除了本文的 commit，还有 **@KANet-UI 的 `d2c34f08`（(263)）**。
...
1. **编号撞车**：`(263)` 被用了两次 —— @Bettor `c23b2ea8`（现场核 / 停矿派工）与 @KANet-UI `d2c34f08`（接位实测）。两条内容不同、都成立，但同号会让后面引用"(263)"的人指错。建议其中一条改号（我不动别人的条目）。
2. 🔴 **`d2c34f08` 里带了本机的网络坐标（Tailscale 地址 + 三张内网网卡地址），而 `origin` 是公开的** —— 在册铁律 `git origin 是公开面 / commit == 发布`。
