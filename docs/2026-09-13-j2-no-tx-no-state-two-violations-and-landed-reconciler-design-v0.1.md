...
### 10.6 第 5 笔（Codex 8118732e 两条 · NWT diff 审 2f9377e7 GREEN-with-ONE-BLOCKER 的唯一阻塞 = (B)）
- **(A) 同字节重播前提 · 范围写明（NWT 提醒）**：intent 机制现只用于 **KAS 转账**（escrow 锁仓 / 派彩），往返验证对象 = **relay `Generator` 实际构造的转账交易（含其真实输入 UTXO 上下文）**——`kasia-relay/src/lib/serialize-roundtrip.test.mjs` 已覆盖（Generator → sign → `serializeToSafeJSON` → `deserializeFromSafeJSON` → `finalize()` ⇒ id 三处相等；改字节 ⇒ 不 finalize 照旧 / finalize 后变）。"真实 covenant 交易"只在以后代币 `transfer` 也走同套 intent 时才需要——先以**超集**形式钉住：`kasia-relay/src/lib/covenant-roundtrip.test.mjs`（version 1、P2SH covenant 输入带 `utxo.covenantId`、no-sig scriptSig 押 redeem、续约输出 `CovenantBinding(0, covId)`、一个 P2PK fee 输入真签名）⇒ 往返 id 相等、`CovenantBinding` 与两输入 scriptSig 原样、safe JSON 带输入 utxo 上下文；重播向量：正常 / mempool 已有 ⇒ 幂等接受零提交 / 续约输出已在 P2SH UTXO 集 ⇒ 零提交 / 改字节 ⇒ `replay_txid_mismatch` / covenant 输入不在可花集 ⇒ `inputs_spent` / 反序列化失败 ⇒ `replay_bad_json` / submit 抛错 ⇒ `replay_rejected`。**往返失败 ⇒ fail-closed**：console `resolvePrepared` 对 `replay_bad_json` 与 `replay_txid_mismatch` 同样 **hold**（告警 `intent_replay_unrecoverable`，不重试不重建，行留 prepared 人工恢复；`submit-intent.test.mjs` 新向量）。**已知边界**：`replayPreparedTransactions` 的输入检查只查一个 `senderAddress` 的 UTXO 集——KAS 转账全部输入都是 relay 自己地址，成立；covenant 交易的输入分属 P2SH 地址，将来接代币 transfer 时要把 `senderAddress` 扩成地址集（v2），向量里已注明。
- **(B) `escrow_landed_at` 硬消费门（Codex HOLD）**：v203 `exchange_offers` 加 `escrow_landed_at/escrow_landed_depth`（maker 锁，两条 publish 路都写 `metadata.escrow_lock_tx`）+ `taker_escrow_landed_at/taker_escrow_landed_depth`（列 `taker_escrow_lock_tx`）。规则单源 `services/escrow-landed-gate.mjs`：带 escrow 锁的预测 offer，`escrow_landed_at IS NULL ⇒` ① **`exchange-machine.transition()` 单一所有权点拒** `matched`（无 taker 接受/无匹配）与 `verifying/awaiting_*/collecting_sigs/delivering/verified/completed`（无结算资格/无对手方价值移动/无声誉终态——`paid` 只在 completed 后）；有 taker 锁时结算态还要 `taker_escrow_landed_at`；`refunded/cancelled/expired/timed_out/disputed` 不拦（不是对手方价值移动）；② settler 每 offer 入口 `assertSettleEligible` 跳过（不白跑 oracle 轮），且 `transition('delivering')` 后**回读状态**（transition 被拦时返回原 offer 不抛——原码会照样派彩，这条本身就是绕门的洞，一并堵）；③ taker-stake 处理器 `takerAcceptGate` ⇒ 409 `escrow_not_landed`，taker 押金不发。**唯一写入方** = F3 对账器把 `submit_intents` landed 回填（`applyIntentLanded`，只在列为 NULL 时写，幂等）。三处 escrow HTTP 处理器仍异步返回 txId，**对象落库即 pending/non-consumable**。negative test `escrow-landed-gate.test.mjs`（真 `transition`）：submitted-永不落链 ⇒ matched/verifying→delivering/delivering→completed 全部不可达、taker 门拒、无结算资格；回填后可达；taker 锁未落 ⇒ 结算态不可达；普通 exchange offer 与无锁老路预测 offer 不受影响；翻转臂红。
- v202 块补 announce log（前后版本块同形，功能不变）；v203 同。
