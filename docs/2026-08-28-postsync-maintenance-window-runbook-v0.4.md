...
## §supervisor-v0.1.4(lifetime 风暴保护 + PID-aware boot grace · 与 ③ console 单体重启同一动作 · v0.1.3 GREEN 被 NWT 对抗审撤回, v0.1.4 等 NWT 重跑 5× + 复核)
> 设计 `docs/2026-08-29-j2-supervisor-lifetime-storm-guard-and-boot-grace-design.md`（v0.1.4）；产物 `docs/provenance/2026-08-29-supervisor-v01/`（**`dab53cbc`**：候选全脚本 + `index.v01.diff` + `selftest.sh` + `MANIFEST.sha256`；J2 5× exit 0、**64/0**）。🔴 **pin 只认 `dab53cbc` 的 MANIFEST**（v0.1.3 `3b10b479` 的候选对长命 console 的瞬时阻塞一击判死 = 回归, 禁部署）。v0.1.4 关键：HUNG_BOOT 对"重启后曾 ALIVE"的 console 只算软 fail(3 连)；心跳陈阈 `HEARTBEAT_STALE_SEC=20`(原 10)。根因：现役 supervisor 的风暴保护**从 8/23 起从未真正存在**（headless `:65-71` 杀掉调用者 ⇒ `record_restart` 永不达）+ boot 期无心跳被当死重启 = 05:14Z 风暴自激环。批次记 DECISIONS.md D-014 注记。
...
3. **闸**：`bash docs/provenance/2026-08-29-supervisor-v01/selftest.sh`；**退出码 = 闸**（汇总行 `64 PASS / 0 FAIL`；`smoke(not gating)` 一栏不影响退出码）。红 ⇒ 还原回滚件, 不进 ③。
...
5. **验收**（只读）：新 `logs/console-supervisor.log` 首行 `supervisor start pid=… boot_grace=300s short_lifetime=300s streak_max=3 heartbeat_stale=20s state:last_restart=…`（= `load_state` 已跑）；稳态若见 `unresponsive but was-ALIVE soft-fail #1/3` 行**是预期**（8–10.5 s 阻塞段, 见 eventloop 调查稿）, 连到 #3/3 才重启；`logs/console-boot.txt` 出现且 `<pid> <ms>` 的 pid = 新 console PID；boot 期日志 `booting: pid=… age=…s/300s — wait` 若干行后 **`boot OK: boot_ms=…`**（这个数决定下轮是否收紧 grace）；`logs/console-supervisor-state.env` 落盘（四 key）。任一缺 ⇒ 报 Bettor, 不算过。
...
v0.5.3(re-pin·NWT 对抗审撤回 v0.1.3 GREEN·J2 落): §supervisor 段 pin 改 **v0.1.4 `dab53cbc`**(64/0; HUNG_BOOT 对曾 ALIVE 的 console 只软 fail 3 连; 心跳陈阈 20 s; v0.1.3 `3b10b479` 禁部署); 验收加 `heartbeat_stale=20s` 与"稳态 soft-fail #1/3 行是预期"。程序步骤不变。
