# T+125 后预告维护窗 runbook v0.4(定稿)

> **Status**: CURRENT · KANet-UI 2026-08-28 · D-013 §3 配套 · 从 scratch 转正(Bettor 令)
...
>
> 🔴🔴 **窗内 on-chain dev-coord 频道 DOWN**(NWT 三眼): §②-bis `stopAll()` 遍历全部 `_relays` **含 comm relay 743c0360** ⇒ **步 ③–⑥ 期间 dev-coord 频道断**。**协调改走 pipe SendMessage(本地 IPC, 不经 relay)+ git**; **别假设窗内频道能用**。⑦ console boot `startAll` 后频道自动恢复。
...
| ②-bis | **quiesce money ingress + drain 在飞 handler**(NWT v0.2 改进) | J1(提权 stopRelay)/KANet-UI(只读 drain 确认) | 见 §②-bis | 花钱链出口已挡(relay 停)且**在飞已 drain 落链**后才进 ③ | 5–10 min |
| ③ | **J1 提权: console 补丁 + 单体重启(同时激活 llm-fallback)** | J1(提权) | 见 §检查③ | j1-patch `-Apply` 幂等成功；旧 console PID 停、新 PID 起、pidfile 真 PID | 5–8 min |
| ④ | **llama 按新脚本重启(loopback,256k)+a5-verify** | J1(起)/KANet-UI(验收) | `node scripts/a5-verify.ps1` | n_ctx=262144 ∧ **监听 127.0.0.1:8000** ∧ commit 合硬闸(≤80∧≥20) | 5 min |
...
|---|---|---|---|
| `api/bettor.js:1415` transfer→escrow | **HTTP**(/api 押注) | KAS(maker 质押) | :3200 已 localhost(仅本机进程/agent 能打)；+②-bis stopRelay 挡链出口 |
...
| `api/chat.js:250/806` send_broadcast | **HTTP/消息** | fee(频道广播) | ②-bis stopRelay(链出口) |
| `broker-v2/router.js:183` transferUsdt→pay_address | **用户 DM(消息)** | **真 USDT**(broker 提现) | ②-bis: 消息触发, HTTP 挡不住 ⇒ **停 relay 消息intake**(唯一无新码挡法) |
| `exchange-machine.js:213/225` transition()→auto-pay transferUsdt/发 KAS | **协议消息**(handler 内同步跑) | **真 USDT / KAS**(auto-pay/auto-deliver) | 同上: 消息触发 ⇒ ②-bis stopRelay + drain |
🔴 **关键**: HTTP 路径在 :3200(已 localhost)——网络打不到, 但**本机 agent/脚本仍能触发**; **消息触发路径(broker 提现 / exchange auto-pay)经 relay 到达, HTTP 防火墙挡不住** ⇒ 唯一无新码挡法 = **②-bis 停 relay**(relay=唯一链上出口, 停它=所有花钱在链出口被挡)。
...
### 无新码挡法(NWT 二选一/组合)
**(a) 停 relay + 防火墙(推荐·本窗可用)**:
- **`stopAll()` / 逐个 `stopRelay(id)`**(`services/relay-manager.js:150/204`, API `api/relay.js:185`)= graceful SIGTERM(3s→SIGKILL)。**relay = 唯一链上出口** ⇒ 停它 = **timer + HTTP + 消息** 三类花钱在链出口全被挡(sendCommandAsync/transfer 无 relay 可打=失败不花钱)。
  - 🔴 `stopRelay` **不 drain**(SIGTERM 直杀)⇒ **必先 drain(见下)再停**, 否则孤儿化在飞(`reference-console-restart-orphans-inflight-relay-child-log-line-lost`)。
...
- 三者齐 ⇒ `stopAll()` 停 relay ⇒ 进 ③。任一不满足 = 等(未落链绝不进③, §② 规则)。

🔵 **兜底(NWT 三眼): `stopAll` 本身是硬闸** —— 停 relay 后任何新 money 请求/消息 → `sendCommandAsync` 无 relay 可打 → **报错 fail-closed 不花钱**(NO TX NO STATE CHANGE)。⇒ drain+quiesce **只需盖住"停前那一小段在飞"**, 不需追踪未来。**时序 = drain+quiesce(稳定窗三条) → `stopAll` → ③ 重启**。

## §检查③(console 补丁+单体重启) — J1 提权
- 补丁: `j1-patch.ps1 -Apply`(NWT GREEN, %TEMP%; 改 `kasia-console/src/index.js:10-15`+`:474-475`; 幂等/两锚点全命中否则 ABORT; 备份 `.bak-j1-20260826`; `node --check`)。已打则 SKIP。
- **单体重启(不用 kanet-start 整套)**: 记旧 console PID(`netstat -ano | findstr :3200` owner)→ 通知后 `taskkill //PID <console> //F`(SYSTEM=J1 提权)→ `_launch_*` 单起或受控 supervisor 拉起。
  - 🔴 **不要**跑 `kanet-start-headless.sh` 整套(杀 sidecar+重起全栈, 漂移表 #24)。
...
| ④llama | 监听未收敛为 loopback / commit 超硬闸 | 停新 llama；查是否走了未打补丁旧脚本；按 loopback(88ab6f6f)重起 |
...
---
🔴 **relay 归位**: ②-bis `stopAll()` 停 relay 后, ③ console 重启 boot 时 `startAll`(relay-manager r281)**自动拉回 relay** ⇒ money-quiesce 窗 = stopAll→console 起好这段; 之后 relay 回、money 路径复活(窗已完)。
