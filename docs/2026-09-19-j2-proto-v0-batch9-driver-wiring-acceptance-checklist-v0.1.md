...
- 批9 接线的前提：六个 builder（批3–8）各自**先过 simnet**（D-022 生产字节闸）。当前状态：批3/4/5 已过 simnet（全链六笔 ACCEPT，见 `docs/provenance/2026-09-19-j2-fullchain-simnet/`）；
  **批6 claim_draw 已过 simnet（全链 v2 七笔 ACCEPT，`docs/provenance/2026-09-19-j2-claim-draw-simnet/`；n=1、同一把 keypair）**；批6 字节层/上游 VM 已由 NWT 验 PASS（账本1521）；批7 withdraw 已过 simnet（`docs/provenance/2026-09-19-j2-withdraw-simnet/`，n=1、测试目的地），**且按 Bettor 1520 裁定批9 排除 withdraw**（见 §3 与 §7）；批8 ticket_reclaim 已过 simnet（v2 形状 [ticket, fee]→[P2PK, 找零]，`docs/provenance/2026-09-19-j2-ticket-reclaim-v2-simnet/`，n=1；但**现有 relay 的 fee 估算器与节点最低费之间无可行 fee，走 relay 无法广播它，见 §3 ticket_reclaim 行 ⑤**）。批9 不得在批6–8 缺 simnet 证据时宣称"六步闭环"。
...
| ticket_reclaim | ① 可回收闸 `assertTicketReclaimable`：只有 closed==1 且票是输家（side≠winningSide）才构造，**`marketState` 必须由驱动从链上已验证的 RootClose/RootClaim 状态提供，不得取本地库推算**；② 签名前 `assertTicketSigningKey`（同 claim_draw）；③ C1：ticket 链上面值 == 20,000,000 且 spk == 现算（角色表 `ticket_reclaim:[ticket]`）；④ fee = 精确 mass 现算（100×max(storage,compute,transient)×3/2），**不得复用 `computeRequiredFeeSompiOrThrow`**（本地 wasm 少付 ⇒ 被拒，simnet 已实证）；`feeProfile.ticket_reclaim.cap` 为 1.0 KAS 占位（旧 2,000,000 随形状变更作废）待 NWT 推数；⑤ 🔴 **阻塞（事实，不是"relay 侧无需改动"）**：v2 形状（Bettor 1524 裁定②）已不再有空 `signInputIndices` 问题，但 relay 的 `computeRequiredFeeSompi`（kaspa-wasm 本地 mass×100，wasm 漏计 v1 交易的 computeBudget）对该 compute 占优形状估出 141,000，`validateNetLoss`/`validateImpliedMinerFee` 天花板=2×=282,000，而节点最低费=100×compute=1,528,200 ⇒ 没有任何 fee 能同时过 relay 与节点（simnet 实证 + 离线测试 ㉖ 表征）。接线前须 Bettor 裁定：修 relay fee 估算器（钱路，须批准）或 bettor 自行广播回收（不经 relay）；⑥ 收款脚本必须由 bettor 明确指定（34 字节 P2PK，无默认值）；⑦ 输入布局 [ticket, fee]，`inputHasCovenant=[false,false]`，C1 角色表 `ticket_reclaim:[ticket]`，`signInputIndices=[1]` |
