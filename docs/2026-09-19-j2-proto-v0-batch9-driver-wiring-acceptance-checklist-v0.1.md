...
- 批9 接线的前提：六个 builder（批3–8）各自**先过 simnet**（D-022 生产字节闸）。当前状态：批3/4/5 已过 simnet（全链六笔 ACCEPT，见 `docs/provenance/2026-09-19-j2-fullchain-simnet/`）；
  **批6 claim_draw 已过 simnet（全链 v2 七笔 ACCEPT，`docs/provenance/2026-09-19-j2-claim-draw-simnet/`；n=1、同一把 keypair）**；批6 字节层/上游 VM 已由 NWT 验 PASS（账本1521）；批7 withdraw 已过 simnet（`docs/provenance/2026-09-19-j2-withdraw-simnet/`，n=1、测试目的地），**且按 Bettor 1520 裁定批9 排除 withdraw**（见 §3 与 §7）；批8 ticket_reclaim 未落码。批9 不得在批6–8 缺 simnet 证据时宣称"六步闭环"。
...
| ticket_reclaim | 批8 落码后补；ticket_reclaim 的 fee 计算陷阱（计划 §2.6）与同一个 `assertTicketSigningKey` |
