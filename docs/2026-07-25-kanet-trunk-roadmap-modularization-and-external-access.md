...
## 附录 Z · 2026-07-25 冻结后并入的新事实(KANet-UI 起草 · Bettor 2026-07-29 审批 · J1 核字节)

> 🔴 **本附录作用域(Bettor 定)**:只登记 2026-07-25 冻结之后产生的【事实与订正】;与正文冲突的【主张】一律以正文为准,**不在此处收录**。
> 🔴 起草纪律:以上 v1.2 正文【字节不变·冻结】。本附录只并入【冻结之后产生的新事实/订正】,不并竞争性主张。
> 每条标【搬去正文哪节 · 归属 · 核实日期+核实人】。**冲突主张一律以 v1.2 正文为准**(见文末"被 v1.2 覆盖的主张")。
> 🔴 **终态留附录·不 inline**(Bettor 定):A 是 FROZEN ⇒「冻结」的唯一实义 = 正文字节不再变;inline 会当场毁掉「前 1531 行逐字节 = 冻结稿」这个可证性质。附录把【被冻结那份】与【冻结之后发生了什么·谁何时核的】分开摆。

### Z.0 · 🔴🔴 本合并不解除任何既有 gating(Bettor 令 · KANet-UI 读 MSG137-139 原件落笔·非转述)
> **本合并是【文档统一】,不是解冻。谁都不许把"路线图统一了"读成"可以往下走了"。**
- 🔴 07-25 review `RESPONSE-20260725-MSG137-139-B0-RULINGS-AND-SETTLE-TRUTH` **Ruling 3 原文(我从 origin/coord/codex-bridge 读原件)**:
  - 两条零确认路径都是 **B0-M1 blocker** ——(1)`bettor-prediction-settler.js` 提交后 `sendCommandAsync` 返回 `{ok,txId}` 即写 `settle_txid`+`completed`,提交与完成之间【无链读】;(2)`exchange-machine.js` 显式制造 `vr={confirmed:true}`,下游信它。
  - **第三、四段必须继续 gated,直到【定出单一结算实相权威】+【机械枚举所有"写 completed 的地方"(不是人肉找)】**。
  - 任何路径【不许保留】伪造的 `confirmed:true` 兼容短路。
- ⚠ 同 review 还纠正:B0-M1 草稿 `2026-07-25-B0-M1-completed-criterion-DRAFT.md` 的 "iff" 判据【内部不自洽】——它既要求 payment output 留在 live UTXO 集,又说 landed-then-immediately-spent 也算 confirmed;二者不能同真(live-UTXO 只在未花时是强证,不是花掉后的普遍历史落链证明)。
- ✅ **解除条件(全满足才解冻三四段)**:单一结算实相权威已定 · completed 写入点机械枚举完 · B0-M1 草稿改到自洽 · 无任何 `confirmed:true` 短路残留。
- **归属**:裁定=Codex(07-25)· 翻出与标"压合并"=J2(07-29)· 读原件落笔=KANet-UI。

### Z.1 → §8.7 B0-M1 settleMarketLive 假完成闭环
- **新事实(订正)**:假完成 bug 2026-07-03(task#33)【已修·live 在跑】(库 82 行带 complete 字段·一行 complete=false 诚实躺 settled_partial_claims 未冒充 completed)。链上复核:59 欠款 ⇒ 57 UTXO 到账·2 不可判·0"库说付了链上没有"。
- 🔴 **未做/边界**:07-28 判据用【winner 地址当前 UTXO 集】会随 winner 花掉 payout 衰减 ⇒ 今日独立复核须改【payout-txid 落块法】(待 Bettor 点人)。
- **核实日期+核实人**:2026-07-28 · **J2**(地面核)· Bettor 记账 · KANet-UI 搬运。

### Z.2 → §8.8 B0-M2 claim-complete 闭环
- **状态**:🔴 仍挡。方法=复刻生产谓词(judge-propose)+ LIKE 后缀阳性对照(命中 164 证查询有功率)。
- **核实日期+核实人**:2026-07-29 12:59 UTC · **J2**(现测·非复述)。KANet-UI 仅登记。

### Z.3 → ZK D-number(B0-M3)批零格
- **状态**:🔴 不可执行·卡两处(v1.2 §8.9 第1条要的 2026-07-25 Owner 裁定原文 + 另一处)。
- **核实日期+核实人**:2026-07-29 · **J1**。KANet-UI 仅登记。

### Z.4 → §现役控制风险(健康监控)
- **新事实**:守钱路的 `rpc-health-degradation-alert` 监控【绿灯无信息量】——只在失败时说话,沉默同时兼容"健康"与"没在跑",告警读同一计数、0 即复位。
- **处置**:监控重设计【前置于】任何模块化 daemon 照抄该模板(见 COORD-LEDGER 102 · 对任何路线图成立的约束)。
- **核实日期+核实人**:2026-07-29 · 定性 Bettor(实测+读码)· 作者/认领 **KANet-UI**。

### Z.5 → §faucet / relay 现役风险
- **新事实**:faucet 预留提到转账前(关掉钱路 race)⇒ 代价=一次【本可安全重试】的失败会让该钱包【永久领不到】(wallet_address 无条件 UNIQUE + 准入不看状态)。🔴 是【本次修法换进来的】,非历史遗留。
- **处置**:先量后修(只读计数器设计 `scratch/faucet-count/`·"pending>N 有几笔+哪些地址"·带分母)。修法未定。
- **核实日期+核实人**:2026-07-29 · **J2**。KANet-UI 仅登记。

### Z.6 → §money-path 现役风险（🔴 更正:底层缺陷是【冻结前·已裁】,只有 07-29 的复核+加深才是冻结后新事实）
- 🔴 **不是冻结后新发现**(J2 2026-07-29 13:18 用反查索引翻出):这两条零确认路径 07-25【已被独立发现并已裁定】(review `MSG137-139-B0-RULINGS-AND-SETTLE-TRUTH`)。裁定要点:【两条零确认路径都是 B0-M1 blocker】+【第三、四段必须继续 gated,直到定出【单一结算实相权威】并把所有"写 completed 处"机械枚举一遍】。⇒ 底层缺陷+裁定属**冻结前**,应已在 v1.2 正文(若正文无 = A 的 gap,待 Bettor 判);**不作为冻结后新事实收录**。
- ✅ **冻结后真新的(07-29·可收录)**:(i) J2 复核:两条路【此刻仍 live 在主分支】(信任 txid 短路 = 2026-04-11 一笔标题"fix"的提交装的·该文件全 history 没调过验证器);(ii) J2 更深两格:`checkUtxoLanded`【不验金额】(只证地址归属+txid+深度,付了多少不在断言里)· B0-M1 草稿【内部不自洽】;(iii) 分母:两条码本机零执行(痕迹全 0)⇒ 未显形≠风险低。正路 `check_utxo_landed`(带 minDepth)本仓 pool/bshard-close-enforce 在用。
- **核实日期+核实人**:07-25 裁定=Codex 当时;07-29 复核+加深=**J2**。KANet-UI 仅登记。
- 🔴 **对合并的影响(J2 标·Bettor 判)**:07-25 那条【三四段继续 gated 直到单一结算实相权威】是【已存在的 gating 裁定】,不是建议 ⇒ 直接压在本次合并/放行上。

### Z.7 → §批零总则 / 各批次卡头(新纪律)
- **新纪律**:批零及任何 must-not-slip 条目【必须带"最后核实日期+核实人"】;超过该日期的引用一律先复核再引用,不得直接当前提。
- **来源**:2026-07-28 立(Bettor)· KANet-UI 搬运。

### 被 v1.2 覆盖的主张(ledger 记一笔)
- 🔴 B(296)主张「两条现役雷独立并行·不排批零后面·单独签发」——v1.2 已将现役控制风险【折进批零】(v1.2 line 149-150)⇒ **以 v1.2 为准**,B 该主张作废。
- 🔴 现状违反 v1.2 §0 第33行「禁止多个当前稿并存」:同名路径【三份内容·五分支】。合并后须收敛:其余版本删至一行指向 + 更新 coordination/codex-bridge/ 目录记录。
