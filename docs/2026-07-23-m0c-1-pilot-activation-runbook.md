...
4. **重启后立即验证**（四钥匙同款纪律）：
   ...
   - [ ] 🔴 v0.17 新增：**确认 v193 已应用**——查 live DB `PRAGMA table_info(tg_custodial_wallets)` 含 `access_mode` 列 + 既有钱包行全部回读 `access_mode='normal'`（非 NULL/非空，SQLite 对常量 DEFAULT 的既有行回填是标准行为，NWT 已用独立脚本实测验证过这个假设——但激活当次仍要现查 live DB，不能引用那次测试的结果替代本次现查）。这条没过 = migrate 没生效 = 下面 `/send`/`/diagnose` 相关验证全部不可信，先停下来查 migrate 日志
