...
## §10 🔴 继承的信任边界与**身份抢注面**（@NWT 第二轮对抗性复核 · 我实核后全认 · 附一个他没有的数据点）

### 攻击（NWT 提出，我逐环核过代码，成立）

攻击者**不需要偷任何人的密钥**：自己完整生成一套 mnemonic / rootXpub / identityPubkey，
**自己对自己签名**，把 `s.relayId` 填成 `relay_nodes` 里**任意一个已存在且 custody=mnemonic 的 id**（如 `J2test`）。

逐关都过：
- **N4-bis** `deriveCustody(sqlite, s.relayId)` —— 只按 relayId 查该 relay 的 **custody 类型**，
  `SELECT mnemonic_encrypted, privkey_encrypted`，**只判存在性，不比对任何密钥**；
- **绑定证明** —— rootXpub → identityPubkey 的派生自洽，**攻击者自己的那套当然自洽**；
- **N8 PoP** —— 验签用的公钥是**申报的那个 identity pubkey 本身**（`u1-registration-pop.mjs:11-13`），
  攻击者用自己的私钥签自己的 pubkey ⇒ **必过**；
- ⇒ INSERT 成功，`relay_id` 是 **PRIMARY KEY** ⇒ **那个 relay 的身份槽被永久占死**。

### 🔴 为什么 PoP 挡不住 —— 它防的是**镜像方向**的攻击

`u1-registration-pop.mjs:6-7` 自己写着，它存在是为了防
「**域 X 抄域 Y 的根**、算出正确 pubkey、**用自己的 relay_id** 提交」。

**而本攻击是它的镜像**：**用自己的根 + 别人的 relay_id**。
PoP payload 里确实绑了 `relay_id`（`:44` `:55`），但那只防"把一个签名挪到另一个 relay_id 上重放"，
**不证明签名者就是那个 relay**。⇒ **两类攻击，现有防线只覆盖了第一类。**

### 🔵 我补的数据点：**便宜的修法并不存在**

直觉的修法是"拿 `relay_nodes` 里存的公钥比一下"。`relay_nodes` **确实有** `ecdsa_pubkey_xonly` 列 ——
🔴 **但实读：32 行里非空的有 0 行**，且整个 `u1-*` 注册链路**零处引用它**。

⇒ 锚定 `relay_id ↔ 密钥控制权` **不是加一行比较**，而是要让 **relay 本人证明它控制那把钥**（走 IPC 让 relay 签一个挑战），
这既跨越"私钥只在 relay、Console 不碰钥"的边界，也要先把那一列**填起来**。
**⇒ NWT 那句「整个 console API 信任模型都得动」是准的，不是保守说法。**

### 🟡 严重度校准（NWT 做的，我认同并复述以免被读大）

Console 的资金类端点同样是"loopback 即可信、零 per-resource 授权"
（`/api/relay/:id/transfer` `relay.js:511`、`/relays/:id/mnemonic` `relay.js:530`）。
⇒ 身份注册**不是新开一个洞，是继承了既有的信任模型**。

🔴 **但它的风险面比资金操作【宽】，这一点必须写下来**：
资金操作的影响半径封在"这个 operator 自己的资源"内；
而身份注册写的是一条 **relay_id 主键的永久记录**，
**而北极星是开放测试网** —— 将来外部方可能把「`relay_id=X` 的注册」当作「X 确实控制 X」来信任，
**而这个事实现在完全没有密码学锚定，只是"占位赛跑"。**

### ✅ 采纳（不在本轮修，那会撑破"零生产改动"的既定范围）

- **§0 增补**：本设计**继承** Console 现有信任模型（loopback 可信 + 零 per-resource 授权），
  与 `transfer` / `mnemonic-reveal` 同档。**这是前提，不是本设计新增的假设。**
- **§7 增补一条未解决**：**`relay_id` 无密码学锚定 = 身份抢注面** ——
  归类为**北极星（开放测试网）之前必须处理的既定风险**，**不阻当前 wiring 落地**，
  但**必须在"对外宣称注册记录含义"之前解决**，否则我们会把一个没有锚的事实卖成有锚的。

🔵 **记 @NWT 一功，并说明这轮红队为什么有效**：第一轮他只核了 §2 的死分支（他自己说"太浅"），
第二轮**去找"能不能打穿"**，读了 `u1-same-origin.mjs` + `u1-registration-pop.mjs` 全文才找到这条。
🔨 **判据：真对抗性复核与"检查有没有写错"是两件事** —— 后者查的是我说的对不对，前者查的是我**没想到**什么。
