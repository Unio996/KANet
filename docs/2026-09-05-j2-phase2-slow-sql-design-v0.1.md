# Phase-2 慢 SQL 根治设计 v0.2.4（P2-1…P2-5 + 横切 P2-0）

> **v0.2.4（2026-09-05T13:2xZ · `date -u` · C 包实现稿随行）改动一览**：(1) **C 包影子模式与 A/B 相反**（Bettor 派工 905）：主路**仍走旧查询**，新查询只在影子里跑，两边去 LIMIT 按集合比，差异 LOUD 到 `events`（`phase2_shadow_mismatch`，payload 含双方差集样本 ≤20），一周零差异后 Bettor 批切换；开关与 A 包同一个 `PHASE2_SHADOW_EVERY`（默认 0=关）。(2) **P2-2 旧 SQL 原地不动**（settler 内联文本一字节不改，只拎成常量给影子去 LIMIT 复用）；新查询里 `pbs.refund_attempted_at < datetime('now','-1 hour')` 被 lint `R-SQL-TIME-STRINGCMP` 挡（新文件 baseline 只准降），**lint 建议的 `julianday()` 形不等价**——活库实核该列 2,278 非空 = 2,273 空格形 + 0 T 形 + **5 行整数 epoch**（整数 vs TEXT 整数恒小 ⇒ 旧法判候选；`julianday(整数)` 判排除 ⇒ 翻转）；改为**参数绑定**（lint 合法形）：`< ?` 绑 `SELECT datetime('now','-1 hour')` 的 TEXT，比较规则与 TEXT 表达式完全相同，测试 V7 三种存值形逐一同判。这 5 行整数 epoch 属数据形不一致，另案记（不在本稿改）。(3) **P2-4 两条已知语义差进测试**（坏 JSON 含子串 / 大小写）：断言差异形状（旧认新不认 = `onlyLegacy`）而不是假装相等；核心集合相等；C3 前置 0 保证活库当前无此类行，影子负责以后显形。(4) **D 包（P2-0）不采**（Bettor 裁 ledger 903）：小表统计让两条钱路查询退化，且 `ENABLE_STAT4` 构建下 ANALYZE 写 stat1+stat4、只删 stat1 不是回滚；脚本 v3（四表回滚）留档。
