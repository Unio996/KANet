...
# 锚点随被测代码漂过一次(我改结算顺序时把原来那句注释换掉了, 它当场变 INERT 而不是悄悄"通过")。
# 现在锚在心跳段开头: 在心跳检查【之前】就结算并退出 = 刹车中的陈旧心跳被整段静默。
mut "结算提前到心跳之前(静音陈旧心跳)" 's|^# 心跳必须是一个|if [ -z "$miner_msg" ] \&\& [ -z "$halted_msg" ]; then exit 0; fi\n# 心跳必须是一个|'
...
# 🔴 最要紧那条: 把 halted 塞回矿机数分支里 —— 那正是 Codex 抓的状态机矛盾, 必须能红。
# 行首锚定 ⇒ 只命中【独立那条】, 不碰矿机数分支里那条(它是缩进的)。
# 这一条重现的正是 Codex 抓的状态机矛盾: halted 不再独立判 ⇒ MINER=1 时整段静默。
mut "halted 不再独立判(MINER=1 就静默)" 's/^if \[ "\$BR" = "halted" \]; then/if [ "$BR" = "__never__" ]; then/'
mut "halted 结算被跳过"             's/^if \[ -n "\$halted_msg" \]; then/if [ -n "__never__" ]; then/'
