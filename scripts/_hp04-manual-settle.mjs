...
// HP-04 manual settle broker→NWT 0.150 USDT BSC (setImmediate hook SELL path unreliable).
// Root cause Bug AQ fixed in commit ea3a46f84 (SELL path setImmediate hook unreliable — same pattern as Bug AM, explicit await).
...
import('../kasia-console/src/services/evm-transfer.js').then(async ({ transferUsdt }) => {
  ...
  console.log('--- broker → NWT BSC 0.150 USDT (HP-04 settle) ---');
  const r = await transferUsdt('bnb', w.privkey_encrypted, NWT_BSC, 0.150, 'USDT');
  console.log('result:', JSON.stringify(r));
  if (r.ok) {
    const upd = db.prepare("UPDATE user_escrow_balances SET status = 'settled', settle_tx = ?, updated_at = datetime('now') WHERE id LIKE 'f8ff127c%' AND status = 'active'").run(r.txHash);
