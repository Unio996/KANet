...
//   · ⚠ 本行原写「20 次累计过程本身 —— N1-a 测门槛之前, N1-b 测跨越那一刻, 中间那 18 次两臂都不测」。
//     **该披露已过期, 2026-08-07 NWT 独立审抓出**: N1-b 已被三方裁定摘除(:172-173), 而 N1-a 实测走的
//     **根本不是失败计数那条路, 是 QUORUM-TIMEOUT 纯计时器**(当场冻结、零累计, 从未碰过抽样路)
//     ⇒ 旧文字描述的是一个【本用例里不再存在的机制】, 会让读文件头的人以为还有个计数门槛没被覆盖。
//     ✅ 如实替换: **本用例不覆盖"失败计数累计跨越门槛"那条路** —— 它属另一条冻结路径, 不在本命题场景族。
