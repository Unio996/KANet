import sellNewbie from '../test-framework/personas/cn_newbie_sell.mjs';
import { freshTestPeer } from '../test-framework/lib/peers.mjs';

const TRADER_B_ID = '0a8e9723-f00b-4b10-8c79-1dbd4fe3cfb0';
const peer = freshTestPeer('ux-probe-sell-' + Date.now());

let state = sellNewbie.initialState();
let lastReply = null;
const turns = [];

for (let i = 1; i <= 8; i++) {
  const turn = sellNewbie.step(state, lastReply);
  state = turn.nextState || state;
  if (turn.done && !turn.message) { turns.push({ n: i, sent: '<persona done>', reply: '', latency: 0 }); break; }
  if (!turn.message) { turns.push({ n: i, sent: '<no msg>', reply: '', latency: 0 }); break; }
  const t0 = Date.now();
...
console.log('═══════ cn_newbie_sell SELL full journey ═══════\n');
