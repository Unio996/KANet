...
let phrase = null, acct = null, leaf = null, priv = null;
try { phrase = decrypt(relay.mnemonic_encrypted); } catch (e) { die(`decrypt 失败(CONSOLE_ENCRYPTION_KEY 不是 console 那把?): ${e.message}`); }
if (typeof phrase !== 'string' || phrase.trim().split(/\s+/).length < 12) die('解密结果不像助记词(词数 < 12), 拒绝继续');
try {
...
} catch (e) { die(`派生失败: ${e.message}`); }
const rootXpub = acct.toXPub().intoString('kpub');                                           // 账户层 xpub(公开), = U1_ACCOUNT_PATH
const identityPubkeyXOnly = deriveIdentityPubkey(rootXpub, 0);                               // 生产派生, 非自算
// ② 反核: 身份钥必须 == relay 现用地址钥; 不等 = 派生路径漂了或 relay 被换钥, 拒
const addrKey = kaspa.XOnlyPublicKey.fromAddress(new kaspa.Address(relay.address)).toString().toLowerCase();
const matches = addrKey === String(identityPubkeyXOnly).toLowerCase();
if (!matches) { zero(); die(`identityPubkeyXOnly(${identityPubkeyXOnly.slice(0, 12)}…) != relay 地址钥(${addrKey.slice(0, 12)}…) — 派生路径或 relay 钥不一致, 拒`); }
...
const signature = kaspa.signMessage({ message: popMessageHashHex(payload), privateKey: priv.toString() });
zero();
function zero() { phrase = null; acct = null; leaf = null; priv = null; }
// ── 密钥区结束 ───────────────────────────────────────────────────────────────
