...
async function _advanceNoOfferRefund({ orderId, order, reason }) {
  ...
  const { makeUtxoLookup } = await import('../lib/kaspa-utxo-lookup.mjs');
  const rpcUtxoLookup = await makeUtxoLookup(userKasiaAddr);
  // 前移合并(2026-09-27, 原 coord/broker-money-path 2026-08-29 写法)顺带修: 原用地址前缀推断网络
  // (R-NET-PREFIX-INFER, D-017 主网过渡态存量 kaspatest 地址会被误判 testnet-12) — 网络是整个进程配置的
  // 单一常量, 不该从某个用户地址反推; 改用 configuredNetwork()(env 单一源), 同函数内其它依赖一样动态 import。
