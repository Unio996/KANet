// proto-driver.mjs — 原型 v0 后台驱动(账本1438③, J2 buildAndBroadcast 接线 Stage 1: 先只驱动
// market_genesis; bet_mint 两步链式在 Stage 2/3 补)。
//
...
import { buildMarketGenesisAndBroadcast, shardLeafTargetAddress } from '../lib/proto-broadcast-ops.mjs';
...
/**
 * 单次 tick 的真正工作(可离线测试的纯逻辑层, 依赖全部注入)。Stage 1 只覆盖 market_genesis:
 *   ① genesis_pending/genesis_prepared 行 → driveMarketGenesis(maxAttempts=1, 一次尝试不重试, 重试
 *      交给下一次 tick, 避免单次 tick 因为某一行的 sleepMs 重试阻塞太久)。
 ...
 * 补两个参数, 不需要改其余逻辑。
 * @returns {Promise<{actioned:number, genesisAdvanced:number, genesisLandedChecked:number, genesisLanded:number, errored:number, held:number}>}
 */
export async function runProtoDriverTick({ sendCmd, relayId, kaspa, network, relayAddress, log = console, cap = DEFAULT_TICK_CAP }) {
  const out = { actioned: 0, genesisAdvanced: 0, genesisLandedChecked: 0, genesisLanded: 0, errored: 0, held: 0 };
