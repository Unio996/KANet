...
import { releaseReservationOnPrepared, deferReservationReconciliation } from '../lib/proto-fee-reservation.mjs';
...
export async function buildProductionDriver({ health, network, ops, kaspa, sendCmd, relayId, tickIntervalMs, budget }) {
  ...
  return createSettlementDriver({
    ...
    // F4: build() 选中的 fee UTXO 在这次广播尝试收到【确定回执】(成功或明确拒绝)后释放进程内预留;
    // MUST-2(NWT 账本1623/1624): sendCmd 本身抛错(IPC 超时/无响应, 结果不确定)时不在这里释放, 交
    // deferReservationReconciliation 到期先查 DB 是否已有 prepared 字节(真实 relay 在 IPC 应答前就已经
    // ingestPhase 回 console 了)再决定移交/释放。
    releaseFeeReservation: releaseReservationOnPrepared,
    deferReservationReconciliation: (feeUtxo, checkPreparedInDb) => deferReservationReconciliation(feeUtxo, checkPreparedInDb),
    checkIntentPreparedInDb: (intentKey) => { const row = SI.getSettlementIntent(intentKey); return !!(row && row.prepared_tx_json); },
  });
}
