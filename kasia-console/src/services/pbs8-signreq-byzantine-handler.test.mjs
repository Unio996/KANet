...
function seedVote(marketId, outcome) {
  sqlite.prepare(`INSERT INTO chain_events (id, txid, event_type, observed_by, observed_at, payload) VALUES (?, ?, 'pool_oracle_vote', 'test-fixture', ?, ?)`)
    .run(randomBytes(8).toString('hex'), randomBytes(8).toString('hex'), new Date().toISOString(),
      JSON.stringify({ market_id: marketId, voter_pubkey: ORACLE_PK, outcome }));
}
function seedMalformedVote(marketId) {
...
seedMarket(MARKET_MALFORMED); seedMalformedVote(MARKET_MALFORMED);
