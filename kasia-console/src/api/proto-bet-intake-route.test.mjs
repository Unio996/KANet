...
const bet = (id) => app.inject({ method: 'POST', url: `/api/proto-markets/${id}/bet`, payload: { direction: 0, amount: 5 } });
...
console.log('[test] 受理点门接线:');
{
  ...
  const noEnd = mk({ resolution_rule_spec: '{"r":1}' }); const r1 = await bet(noEnd);
  ...
  const withEnd = mk({ outcome_condition_id: '0xabc', outcome_end_ms: OE }); const r2 = await bet(withEnd);
  ...
  const sealed = mk({ status: 'sealed', resolution_rule_spec: '{"r":1}' }); const r3 = await bet(sealed);
  ok(r3.statusCode === 409 && /not accepting bets/.test(r3.body), `状态校验先于门: sealed ⇒ 既有 409 not accepting bets(实际 ${r3.statusCode})`);
  const badDir = await app.inject({ method: 'POST', url: `/api/proto-markets/${withEnd}/bet`, payload: { direction: 2, amount: 5 } });
