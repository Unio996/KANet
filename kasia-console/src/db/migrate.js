...
export function runMigrations() {
  ...
  */
  ...
  // v107 (broker hat — skipped on J1 predictions branch) — user_escrow_balances 表 belongs to Bug H
  // broker-escrow custody (Owner 5/14 12:05 钦定 candidate A v2, ship 在 NWT/J2 broker bundle).
  // J1 predictions hat 不 introduce 该 schema; 版本号 v107 reserved 给 broker hat 合并时 ship.
  ...
  // ── v217(前移自 coord/broker-money-path 原 v199, 2026-08-29 J2 原作 · 2026-09-27 Owner 令合并前移 · 账本1690):
  //   kaspa_tx_log_coverage + broker_refund_intents + idx_spc_daa_ts —— 纯新增, 零改表 ──────────
  //   🔴 编号前移说明: 原分支写的是 v199, 但主线 v199 已被 idx_kaspa_tx_log_to_addr_observed(2026-09-05) 占用
  //   (两者都是"2026-08-29 前后"独立立项, 撞了同一个号)——原意图(建表/建索引)与判据逐字未改, 只是编号改成主线
  //   现尾号(v216)之后的 v217, 内容本身跟"是不是首个 v199"无关, 平移不算新功能。
  //
  ...
  //   Bettor 裁: 拆两段 —— 本块(原 v199) = 纯新增(不破 schema); 原 v200(retail_dex_orders 整表重建加 held_for_review,
  //   Owner A/B 定案后、窗内、带回滚件) 【不在本块，未随此次合并一起前移——那部分需要独立的 Owner A/B 定案，
  //   跟本次"合并资金路径修复"的授权范围不是一回事，如实标注不是遗漏】。
  //
  ...
  //   验收: src/lib/indexer-coverage.test.mjs / src/lib/broker-refund-classify.test.mjs / src/lib/broker-escrow-check.test.mjs (真 schema, temp DB)。
  {
    ...
    `);
    console.log('[migrate] v217: kaspa_tx_log_coverage + broker_refund_intents + idx_spc_daa_ts 建表/建索引完成 (纯新增; 写入方待 wiring 阶段 2/3; 编号前移自原 coord/broker-money-path 分支的 v199, 见上方头注).');
  }
