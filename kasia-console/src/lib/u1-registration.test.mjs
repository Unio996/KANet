...
await t('V7 · privkey-only relay ⇒ 拒(fail-closed), 且理由可读', async () => {
  ...
  const id = makeIdentity();
  const ch = 'ch-v7';
  const r = await registerIdentity({ sqlite, submission: submissionFor(relayId, id, ch), now: Date.now() });
...
await t('绑定闸 · identityPubkeyXOnly 与登记根派生的不一致 ⇒ 拒 BINDING_INVALID, 且不落库', async () => {
  ...
  const r = await registerIdentity({ sqlite, submission: sub, now: Date.now() });
...
await t('入口层 · PoP 失败(挑战串没签发) ⇒ 拒, 且不落库', async () => {
  ...
  const r = await registerIdentity({ sqlite, submission: submissionFor(relayId, id, 'ch-none'), now: Date.now(), ...wire(stNone) });
  assert.strictEqual(r.ok, false);
...
await t('N3 兜底 · 同一个根注册到第二个 relay ⇒ 被【DB 约束】拒(不靠前面的检查)', async () => {
  ...
  const a = await registerIdentity({ sqlite, submission: submissionFor(r1, id, 'ch-n3-a'), now: Date.now(), ...wire(stN3) });
  assert.strictEqual(a.ok, true, `第一条应当成功: ${a.code} ${a.reason}`);
  const b = await registerIdentity({ sqlite, submission: submissionFor(r2, id, 'ch-n3-b'), now: Date.now(), ...wire(stN3) });
...
await t('(A-1) 省略 challengeStore ⇒ fail-closed 拒, 且【一个字节都不落库】', async () => {
  ...
  const st = chStore(); st.issue(ch);
  const r = await registerIdentity({
    sqlite, submission: submissionFor(relayId, id, ch), now: Date.now(),
    // ← 故意不传
  });
...
await t('(B) 挑战未签发(store 里没有)⇒ 在 PoP 层就拒, 零落库', async () => {
  ...
  const r = await registerIdentity({ sqlite, submission: submissionFor(relayId, id, ch), now: Date.now(), ...wire(st) });
...
await t('(D) 🔴 真竞态: 另一连接在【PoP 读之后、事务之前】抢先消费 ⇒ 事务内前置读必须拦住', async () => {
  ...
  let raced = false;
  const r = await registerIdentity({
    sqlite, submission: submissionFor(relayId, id, ch), now: Date.now(), ...wire(st),
...
await t('(C) 成功边界【之后】拿同一挑战重放 ⇒ 必拒(一次性的全部含义)', async () => {
  ...
  const first = await registerIdentity({
    sqlite, submission: submissionFor(relay1, id1, ch), now: Date.now(), ...wire(st),
  });
  ...
  const replay = await registerIdentity({
    sqlite, submission: submissionFor(relay2, id2, ch), now: Date.now(), ...wire(st),
  });
