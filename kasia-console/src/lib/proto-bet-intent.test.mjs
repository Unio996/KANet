// proto-bet-intent.test.mjs — 下注(register_append 单笔交易, D-020账本1446/1448)状态机离线向量
// (J2 2026-09-14, 设计 v0.1 §2.3.1, NWT MUST-FIX 1336)。
// 真 migration 临时库(DB_PATH) + 假 sendCmd(脚本化 relay), 零链零 IPC。同 submit-intent.test.mjs 手法。
// 🔴 D-020: 原两步(A铸stake筹码+B register_append)链式依赖(dependsOn/checkDependencyLanded)已随
// 步骤A取消一起删除, 原③④⑤(依赖闸/mint landed后append放行/ensureBetIntent拒绝缺dependsOn的append)
// 三组向量随之删除——单步设计下这套机制结构性不适用, 不是"删漏了"。
...
const {
  ...
  getBetIntent, ensureBetIntent, markBetIntent, betIntentKeyFor,
} = BI;
...
console.log('[test] ① 下注唯一步骤(append): pending → 首次 build 成功 → submitted:');
{
  const R = makeRelay();
  seedBet('bet1');
  const res = await driveBetIntent({
    sendCmd: R.sendCmd, relayId: 'relay-A', betId: 'bet1', step: 'append',
    targetAddress: 'kaspatest:append-addr', buildAndBroadcast: makeBuildAndBroadcast(R), log: quiet,
  });
  ok(!!res.txId, 'append 拿到 txId');
  const intent = getBetIntent(betIntentKeyFor('bet1', 'append'));
  ok(intent.status === 'submitted', `append intent status=submitted (实际 ${intent.status})`);
}
...
console.log('[test] ② 幂等: 再调一次 driveBetIntent(同 betId/step) → reused, 不重新构造:');
{
  ...
  const res = await driveBetIntent({
    sendCmd: R.sendCmd, relayId: 'relay-A', betId: 'bet1', step: 'append',
    targetAddress: 'kaspatest:append-addr', buildAndBroadcast: wrapped, log: quiet,
  });
  ok(res.reused === true, 'append 第二次调用 reused=true');
  ok(buildCalls === 0, `buildAndBroadcast 未被再次调用(实际调用 ${buildCalls} 次)`);
}

console.log('[test] ③ markBetIntent 单调性: 不能从 landed 退回 pending:');
{
  const R = makeRelay();
  const intent = getBetIntent(betIntentKeyFor('bet1', 'append'));
  R.landed.set(intent.submitted_txid, 10); // 模拟 10 confirmation depth
  const landedRes = await checkBetIntentLanded({ sendCmd: R.sendCmd, relayId: 'relay-A', intent, targetAddress: 'kaspatest:append-addr', minDepth: 5 });
  ok(landedRes.landed === true, 'checkBetIntentLanded 判定已 landed');
  const after1 = getBetIntent(intent.intent_key);
  ok(after1.status === 'landed', `intent 行更新为 landed(实际 ${after1.status})`);
  const after2 = markBetIntent(intent.intent_key, { status: 'pending' });
...
console.log('[test] ④ resolvePrepared: prepared 行且 txid 已在 mempool → submitted, 不重新构造:');
{
  seedBet('bet3');
  const R = makeRelay();
  const key = betIntentKeyFor('bet3', 'append');
  ensureBetIntent({ betId: 'bet3', step: 'append' });
  ...
  let buildCalls = 0;
  const res = await driveBetIntent({
    sendCmd: R.sendCmd, relayId: 'relay-A', betId: 'bet3', step: 'append',
    targetAddress: 'kaspatest:append-addr3', buildAndBroadcast: async () => { buildCalls++; return { txId: 'wrong' }; }, log: quiet,
  });
...
console.log('[test] ⑤ resumeStaleBetIntents: 扫到陈旧 prepared 行并恢复:');
{
  seedBet('bet4');
  const R = makeRelay();
  const key = betIntentKeyFor('bet4', 'append');
  ensureBetIntent({ betId: 'bet4', step: 'append' });
  ...
  const out = await resumeStaleBetIntents({
    sendCmd: R.sendCmd, targetAddressFor: () => 'kaspatest:append-addr4', relayIdFor: () => 'relay-A',
...
console.log('[test] ⑥a resolvePrepared: 同字节重播遇 inputs_spent 但 kaspa_tx_log 有正向落地证据 → submitted(不重建):');
{
  seedBet('bet5');
  const R = makeRelay();
  const key = betIntentKeyFor('bet5', 'append');
  ensureBetIntent({ betId: 'bet5', step: 'append' });
  ...
  try {
    res = await driveBetIntent({
      sendCmd: R.sendCmd, relayId: 'relay-A', betId: 'bet5', step: 'append',
      targetAddress: 'kaspatest:append-addr5', buildAndBroadcast: async () => { throw new Error('should-not-be-called: prepared 态走重播不走重建'); }, log: quiet,
    });
  } catch (e) { threw = e; }
...
console.log('[test] ⑥b resolvePrepared: inputs_spent 且 kaspa_tx_log 查无正向证据 → ambiguous 终态, HOLD 不重建不放弃:');
{
  seedBet('bet6');
  const R = makeRelay();
  const key = betIntentKeyFor('bet6', 'append');
  ensureBetIntent({ betId: 'bet6', step: 'append' });
  ...
  try {
    await driveBetIntent({
      sendCmd: R.sendCmd, relayId: 'relay-A', betId: 'bet6', step: 'append',
      targetAddress: 'kaspatest:append-addr6', buildAndBroadcast: async () => { buildCalls++; return { txId: 'should-not-happen' }; }, log: quiet,
    });
  } catch (e) { threw = e; }
...
console.log(fails === 0
  ? '\n✅✅ ALL PASS — proto-bet-intent 单步状态机(幂等/单调/恢复/inputs_spent 歧义终态) 全绿'
