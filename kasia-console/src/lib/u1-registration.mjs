...
//
// 🔴🔴 **(374) 第二张表: 生产路径上对【调用方给的对象】的每一处解引用**(@Bettor 要求; 起因是
//    今晚被逐级挖的三个层次 —— 参数 → 绑定元数据 → **对象方法**。枚举参数不够, 还要枚举【解引用】。)
//    🔨 **判据: "我绑了哪个对象"不等于"我绑了这个对象会做什么"。**
//       `caller_obj.foo()` 这种写法, authority 落在**调用那一刻对象上挂着的那个函数**, 不在绑定检查上。
//
