...
const SPEC = JSON.stringify({ side_map: { yes: 1, no: 0 } });   // 批 B C1: 判定题受理门要读 side_map
const BR = { direction: 1, sideLabel: 'yes' };                   // 与 SPEC 一致的合法下注请求
const T_NOW = OE - 1_000_000;                                     // 注入的"墙钟"(早于 outcome_end 很多): 不依赖真实时钟
const chk = (o) => checkBetIntake({ nowMs: () => T_NOW, network: 'simnet', betRequest: BR, ...o });   // 批 B: 判定题受理门要 network(B5 谓词); 这些老用例都在非主网上
...
await t('X3 ▲ N4 判定题受理时 pmt 无效 / 读失败 / readPmt 抛 ⇒ 503 fail-closed(读了 1 次); 有效且 pmt < outcome_end ⇒ 受理; pmt = outcome_end−1 受理 / = outcome_end 拒 / 超过拒(409)', async () => {
  const id = mk({ resolution_rule_spec: SPEC, outcome_end_ms: OE });
...
await t('X6 ▲ M2 取 max(墙钟, pmt): pmt = oe−140s ∧ 墙钟 = oe+1ms ⇒ 必拒(409)且【不再读 pmt】; 墙钟 < oe ∧ pmt 落后 ⇒ 收(读 1 次 pmt); 墙钟 < oe ∧ pmt ≥ oe ⇒ 拒; 墙钟恰 = oe ⇒ 拒', async () => {
  ...
  let p = probe(lag); let r = await checkBetIntake({ network: 'simnet', betRequest: BR, db: sqlite, marketId: id, readPmt: p.readPmt, nowMs: () => OE + 1 });
  assert.deepEqual([r.accept, r.code, r.http], [false, 'outcome_end_passed', 409]); assert.equal(p.calls.n, 0, '墙钟已过 ⇒ 不必读 pmt');
  p = probe(lag); r = await checkBetIntake({ network: 'simnet', betRequest: BR, db: sqlite, marketId: id, readPmt: p.readPmt, nowMs: () => OE }); assert.equal(r.accept, false); assert.equal(p.calls.n, 0);
  p = probe(lag); r = await checkBetIntake({ network: 'simnet', betRequest: BR, db: sqlite, marketId: id, readPmt: p.readPmt, nowMs: () => OE - 1 }); assert.deepEqual(r, { accept: true, code: 'ok' }); assert.equal(p.calls.n, 1);
  p = probe({ valid: true, pmtMs: OE }); r = await checkBetIntake({ network: 'simnet', betRequest: BR, db: sqlite, marketId: id, readPmt: p.readPmt, nowMs: () => OE - 1 }); assert.deepEqual([r.accept, r.code], [false, 'outcome_end_passed']);
  p = probe({ valid: false, reason: 'not_synced' }); r = await checkBetIntake({ network: 'simnet', betRequest: BR, db: sqlite, marketId: id, readPmt: p.readPmt, nowMs: () => OE + 3 }); assert.deepEqual([r.code, r.http], ['outcome_end_passed', 409], '墙钟已过 ⇒ 409, 即使 pmt 本来也读不到');
  p = probe(lag); r = await checkBetIntake({ network: 'simnet', betRequest: BR, db: sqlite, marketId: mk({ outcome_condition_id: 'c' }), readPmt: p.readPmt, nowMs: () => OE + 3 }); assert.equal(r.code, 'outcome_end_missing', '缺 outcome_end 仍先于墙钟判断');
  const plainId = mk(); p = probe(lag); r = await checkBetIntake({ network: 'simnet', betRequest: BR, db: sqlite, marketId: plainId, readPmt: p.readPmt, nowMs: () => OE + 999_999 }); assert.equal(r.accept, true); assert.equal(p.calls.n, 0, '无判定题: 墙钟再晚也豁免且不读 pmt');
});
...
await t('X7 ▲ C1 判定题必须带 side_label(库层 fail-closed): 缺 betRequest / 缺 label / 非 yes|no / 与 side_map 不一致 ⇒ 400 且【不读 pmt】; side_map 缺失或非法 ⇒ 500; 一致 ⇒ 放行; 非判定题不看 betRequest', async () => {
  const id = mk({ resolution_rule_spec: SPEC, outcome_end_ms: OE });
  const run = (betRequest) => { const p = probe({ valid: true, pmtMs: OE - 1 }); return checkBetIntake({ nowMs: () => T_NOW, network: 'simnet', db: sqlite, marketId: id, readPmt: p.readPmt, betRequest }).then((r) => ({ r, n: p.calls.n })); };
  for (const [name, br, code] of [['缺 betRequest', undefined, 'side_label_required'], ['betRequest=null', null, 'side_label_required'], ['缺 label', { direction: 1 }, 'side_label_required'], ['label 非法', { direction: 1, sideLabel: 'YES' }, 'side_label_required'], ['yes 配 direction 0', { direction: 0, sideLabel: 'yes' }, 'side_label_mismatch'], ['no 配 direction 1', { direction: 1, sideLabel: 'no' }, 'side_label_mismatch']]) { const { r, n } = await run(br); assert.deepEqual([r.accept, r.code, r.http], [false, code, 400], name); assert.equal(n, 0, name + ': 坏请求不占 IPC(不读 pmt)'); }
  for (const br of [{ direction: 1, sideLabel: 'yes' }, { direction: 0, sideLabel: 'no' }]) { const { r } = await run(br); assert.deepEqual(r, { accept: true, code: 'ok' }, JSON.stringify(br)); }
  const bad = mk({ resolution_rule_spec: 'not-json', outcome_end_ms: OE }); const p = probe({ valid: true, pmtMs: 1 });
  const r500 = await checkBetIntake({ nowMs: () => T_NOW, network: 'simnet', db: sqlite, marketId: bad, readPmt: p.readPmt, betRequest: BR }); assert.deepEqual([r500.accept, r500.code, r500.http], [false, 'side_map_invalid', 500]); assert.equal(p.calls.n, 0);
  const plain = mk(); const rp = await checkBetIntake({ nowMs: () => T_NOW, network: 'simnet', db: sqlite, marketId: plain, readPmt: probe(null).readPmt, betRequest: { direction: 0 } }); assert.deepEqual(rp, { accept: true, code: 'not_judged_exempt' });
});
await t('X8 ▲ B5/N5b 站点②: 主网 + 非白名单代币的判定题 ⇒ 403 judged_market_not_allowed_here(先于 outcome_end / side_label / pmt, 不读 pmt); 主网 + 白名单 ⇒ 走原有门; 网络未配置(null)⇒ fail-closed 403; 非判定题不受影响(主网也豁免); 非主网不受影响', async () => {
  const id = mk({ resolution_rule_spec: SPEC, outcome_end_ms: OE }); const W = { PROTO_ORACLE_VALUELESS_TOKEN_IDS: 'tok1' };
  const go = (o) => { const p = probe({ valid: true, pmtMs: OE - 1 }); return checkBetIntake({ nowMs: () => T_NOW, db: sqlite, marketId: id, readPmt: p.readPmt, betRequest: BR, ...o }).then((r) => ({ r, n: p.calls.n })); };
  for (const [name, o] of [['主网 + 空白名单', { network: 'mainnet', env: {} }], ['主网 + 别的代币白名单', { network: 'mainnet', env: { PROTO_ORACLE_VALUELESS_TOKEN_IDS: 'other' } }], ['网络未配置', { network: null, env: W }]]) { const { r, n } = await go(o); assert.deepEqual([r.accept, r.code, r.http], [false, 'judged_market_not_allowed_here', 403], name); assert.equal(n, 0, name + ': 不读 pmt'); }
  const noEnd = mk({ resolution_rule_spec: SPEC }); const rNoEnd = await checkBetIntake({ nowMs: () => T_NOW, db: sqlite, marketId: noEnd, readPmt: probe(null).readPmt, betRequest: BR, network: 'mainnet', env: {} }); assert.equal(rNoEnd.code, 'judged_market_not_allowed_here', '主网非白名单先于 outcome_end 检查');
  assert.deepEqual((await go({ network: 'mainnet', env: W })).r, { accept: true, code: 'ok' }); assert.deepEqual((await go({ network: 'testnet-12', env: {} })).r, { accept: true, code: 'ok' });
  const plain = mk(); assert.deepEqual(await checkBetIntake({ nowMs: () => T_NOW, db: sqlite, marketId: plain, readPmt: probe(null).readPmt, network: 'mainnet', env: {} }), { accept: true, code: 'not_judged_exempt' });
});
