...
await t('X1 无判定题(operator 市场)豁免: 放行且【不读 pmt】(即使 outcome_end 已写且已过 / pmt 会无效)', async () => {
  for (const extra of [{}, { outcome_end_ms: OE - 1 }, { outcome_end_ms: null }]) { const id = mk(extra); const p = probe({ valid: false, reason: 'not_synced' }); const r = await checkBetIntake({ db: sqlite, marketId: id, readPmt: p.readPmt }); assert.deepEqual(r, { accept: true, code: 'not_judged_exempt' }); assert.equal(p.calls.n, 0, '不得为无判定题市场读 pmt(不给 operator 路径引入新依赖)'); }
});
await t('X2 ▲ N3 判定题 ∧ outcome_end 空 ⇒ 409 outcome_end_missing(不读 pmt); 四个判定题列各自单独即算判定题', async () => {
  ...
  for (const c of JUDGED_COLUMNS) { const id = mk({ [c]: vals[c] }); const p = probe({ valid: true, pmtMs: 1 }); const r = await checkBetIntake({ db: sqlite, marketId: id, readPmt: p.readPmt }); assert.deepEqual([r.accept, r.code, r.http], [false, 'outcome_end_missing', 409], c); assert.equal(p.calls.n, 0, c); }
});
await t('X3 ▲ N4 判定题受理时 pmt 无效 / 读失败 / readPmt 抛 ⇒ 503 fail-closed(读了 1 次); 有效且 pmt < outcome_end ⇒ 受理; pmt = outcome_end−1 受理 / = outcome_end 拒 / 超过拒(409)', async () => {
  ...
  for (const res of [{ valid: false, reason: 'not_synced' }, { valid: false, reason: 'read_failed' }, null, new Error('ipc down')]) { const p = probe(res); const r = await checkBetIntake({ db: sqlite, marketId: id, readPmt: p.readPmt }); assert.deepEqual([r.accept, r.code, r.http], [false, 'pmt_unavailable_fail_closed', 503], String(res && res.message || JSON.stringify(res))); assert.equal(p.calls.n, 1); }
  const run = (pmtMs) => checkBetIntake({ db: sqlite, marketId: id, readPmt: async () => ({ valid: true, pmtMs }) });
...
await t('X4 市场不存在 ⇒ 404(不读 pmt); 参数校验: db / readPmt 必填', async () => {
  const p = probe({ valid: true, pmtMs: 1 }); const r = await checkBetIntake({ db: sqlite, marketId: 'nope', readPmt: p.readPmt }); assert.deepEqual([r.accept, r.http], [false, 404]); assert.equal(p.calls.n, 0);
...
await t('X5 ▲ N4 门只在受理点: checkBetIntake 只读——不写任何表; 已受理注(proto_bets / proto_bet_intents 已存在)的 append 由驱动推进, 与本函数无关(它不读 bets / intents)', async () => {
  ...
  await checkBetIntake({ db: sqlite, marketId: id, readPmt: async () => ({ valid: true, pmtMs: OE + 1 }) });
