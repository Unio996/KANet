// proto-oracle-spec.test.mjs — D-032 v0.2.4(单口径)判定题创建入口校验 + 下注 side_label 校验 + 公开读呈现。纯函数, 无 DB / 无网络(finder 用真 findExtractor: 纯 URL 解析)。
// Run: cd kasia-console && node src/lib/proto-oracle-spec.test.mjs
import assert from 'node:assert/strict';
import { findUnrecognizedJudgedShapedKey, findDualJudgeKeyInBody, validateJudgedMarketInput, dryRunPredicate, checkSideLabel, presentProtoMarket, findRelayKeyInBody, hasJudgedInput, parseStoredSpec, JUDGED_PRESENTATION_COLS, ALLOWED_SPEC_KEYS } from './proto-oracle-spec.mjs';
...
const H = 3_600_000, NOW = 1_800_000_000_000, OE = NOW + 24 * H, TICK = 300_000;
const MIN_DL = OE + cfg.graceMs + cfg.closePipelineMarginMs + 2 * TICK;   // D-032: 去 UMA 定稿窗项
const spec = (over = {}) => ({ data_source_canonical: 'https://site.api.espn.com/apis/site/v2/sports/basketball/nba/summary?event=401', secondary_sources: ['https://www.espn.com/nba/game/_/gameId/401'], ambiguity_handler: 'abstain', dispute_keywords: ['postponed'], edge_case_examples: [], resolution_predicate: { metric: 'winner', op: '==', operand: 'LAL' }, side_map: { yes: 1, no: 0 }, title: 'LAL beat BOS?', ...over });
const call = (over = {}) => validateJudgedMarketInput({ title: 'q?', deadlineMs: MIN_DL, resolutionRuleSpec: spec(), outcomeEndMs: OE, budgetCfg: cfg, adapterTickMs: TICK, nowMs: NOW, ...over });
...
await t('S1 ▲ 合法输入 ⇒ ok, 规范化列(D-032 单口径): spec 串 / outcome_market_source=kanet_native / outcome_condition_id=null / relay ids 恒 [] / outcome_end_ms 整数; spec 也可传 JSON 串; outcomeEnd 可传数字串', () => {
  const r = call(); assert.equal(r.ok, true); const n = r.normalized;
  assert.equal(n.outcome_market_source, 'kanet_native'); assert.equal(n.outcome_condition_id, null); assert.equal(n.outcome_oracle_relay_ids, '[]'); assert.equal(n.outcome_end_ms, OE); assert.equal(n.minDeadlineMs, MIN_DL); assert.deepEqual(parseStoredSpec(n.resolution_rule_spec), spec());
  assert.equal(call({ resolutionRuleSpec: JSON.stringify(spec()), outcomeEndMs: String(OE) }).ok, true);
});
await t('S2 ▲ 半套判定题输入(缺任一 / 空串 / null)⇒ judged_input_incomplete(路由映射 400); D-032 只剩两个字段(去 outcomeConditionId); 都缺由 hasJudgedInput=false ⇒ 走旧流程', () => {
  rejects({ resolutionRuleSpec: undefined }, 'judged_input_incomplete'); rejects({ resolutionRuleSpec: null }, 'judged_input_incomplete');
  for (const v of [undefined, null, '']) rejects({ outcomeEndMs: v }, 'judged_input_incomplete');
  ...
  for (const k of ['resolutionRuleSpec', 'outcomeEnd']) assert.equal(hasJudgedInput({ [k]: 1 }), true, k); assert.equal(hasJudgedInput({ outcomeEnd: null }), true, 'null 也算"出现"(≠ undefined)⇒ 不能绕过半套检查');
  assert.equal(hasJudgedInput({ outcomeConditionId: 1 }), false, 'D-032: outcomeConditionId 不再是判定题触发字段(不在 JUDGED_BODY_KEYS)——单独字段出现时既不触发全套校验, 也会被 findDualJudgeKeyInBody 拒');
});
...
await t('S4 spec 结构: 非对象 / 非 JSON / 未知字段(输入面白名单, 含 D-032 已移除的 polymarket_outcome_side)/ 缺 5 必填任一 / 类型错 / 超长 / 超上限', () => {
  ...
  rejects({ resolutionRuleSpec: spec({ polymarket_outcome_side: 'YES' }) }, 'spec_unknown_field', 'D-032 §2.1: polymarket_outcome_side 已从 ALLOWED_SPEC_KEYS 去掉, 出现即未知字段拒(不再是必填, 是不认识)');
  assert.ok(!ALLOWED_SPEC_KEYS.includes('polymarket_outcome_side'), 'polymarket_outcome_side 不在白名单里');
...
await t('S6 ▲ B3 side_map(D-032 去极性字段, 只剩双射校验): 缺 / 非双射(同值 / 三键 / 值非 0-1 / 数组 / 字符串值)⇒ side_map_invalid; 两个合法排列均过, 不再要求 polymarket_outcome_side', () => {
  ...
  for (const sm of [{ yes: 1, no: 0 }, { yes: 0, no: 1 }]) assert.equal(call({ resolutionRuleSpec: spec({ side_map: sm }) }).ok, true, JSON.stringify(sm));
});
await t('S7 ▲ D-032 §2.1 单口径: findDualJudgeKeyInBody 拒 outcomeConditionId / polymarket* 形状键(第二裁判输入); 无关键与已识别键放行', () => {
  for (const k of ['outcomeConditionId', 'polymarketOutcomeSide', 'polymarket_condition_id', 'POLYMARKET_X']) assert.equal(findDualJudgeKeyInBody({ [k]: 1, title: 'x' }), k, k);
  assert.equal(findDualJudgeKeyInBody({ title: 'x', resolutionRuleSpec: {}, outcomeEnd: 1 }), null); assert.equal(findDualJudgeKeyInBody(null), null); assert.equal(findDualJudgeKeyInBody([]), null);
  // call() 本身不再接受 outcomeConditionId 参数(已从函数签名去掉)——即便调用方手误传入也不会进入规范化输出, 证明输入面已经不认这个字段了
  assert.equal(call({ outcomeConditionId: '0x' + 'a'.repeat(64) }).ok, true, '传入也被忽略(未解构进函数签名), 不影响校验通过'); assert.equal(call({ outcomeConditionId: '0x' + 'a'.repeat(64) }).normalized.outcome_condition_id, null);
});
await t('S8 ▲ B6(c) 时间预算(D-032 去 UMA 定稿窗项): deadline ≥ outcomeEnd + 宽限窗 + 余量 + 2×adapter tick 恰过, 少 1ms 拒; 各项都真的进预算(把任一项改大则边界同步移动); outcomeEnd 已过 / 非整数 / 非正 ⇒ 拒', () => {
...
await t('S9b ▲ SHOULD② 未识别的 resolution* / outcome* 形键 ⇒ 命中; D-032: outcomeConditionId 现在也是"未识别"(不再是"识别但丢弃"——那是 findDualJudgeKeyInBody 的活, 两个函数各管各的, 路由里 dual-judge 检查在前, 消息更精确); 真正已识别键(resolutionRuleSpec/outcomeEnd/resolutionNote/attestStatement)与无关键 ⇒ 不命中', () => {
  for (const k of ['resolution_rule_spec', 'outcome_end', 'outcome_condition_id', 'outcomeMarketSource', 'resolutionPredicate', 'Resolution_Rule_Spec', 'OUTCOME_END', 'outcomeoracleRelayIds', 'resolution', 'outcome', 'resolutionrulespec', 'outcomeConditionId']) assert.equal(findUnrecognizedJudgedShapedKey({ title: 'x', [k]: 1 }), k, k);
  for (const k of ['resolutionRuleSpec', 'outcomeEnd', 'resolutionNote', 'attestStatement', 'title', 'deadline', 'tokenId', 'myoutcome', 'x_resolution']) assert.equal(findUnrecognizedJudgedShapedKey({ [k]: 1 }), null, k);
...
await t('S11 ▲ C1 公开读呈现(D-032 §2.4 单口径改版): 判定题 ⇒ 附 judged{side_map, outcome_end_ms, data_source_canonical, judge:{kind,predicate,tie_rule,value_time}, human_metadata_only:{...}}(不再含 polymarket_outcome_side / outcome_market_source / outcome_condition_id)且内部列不外露; 非判定题 ⇒ 字节与今天完全相同', () => {
  ...
  const j = { ...base, outcome_end_ms: OE, resolution_rule_spec: JSON.stringify(spec()), outcome_market_source: 'kanet_native', outcome_condition_id: null, outcome_oracle_relay_ids: '[]' };
