...
//   - pool-market-seeder.js  (polymarket mirror → category; classifies on the QUESTION TITLE only,
//      NOT the long description — descriptions mention player nationalities / boilerplate that
//      false-match country keywords, e.g. a tennis match w/ a Russian player → 'politics'. r256 regress.)
...
//
// ORDER MATTERS: politics is checked LAST. Its keyword set includes country/geopolitics names
// (iran/russia/...) which are the weakest signal — a sports/economy/crypto question that merely
// mentions a country (e.g. "Will Russia win the World Cup") must classify by its strong signal
// (sports) first, falling to politics only when nothing else matches (genuine geopolitics).
//
...
const RULES = [
  ...
  ['sports',
    /\b(nba|nfl|mlb|nhl|world cup|premier league|champions league|super bowl|finals|playoff|playoffs|championship|celtics|lakers|world series|fifa|olympics|grand slam|formula 1|tennis|atp|wta|roland garros|wimbledon|us open|australian open|la liga|bundesliga|serie a|ufc|pga)\b/i,
    /(世界杯|总冠军|季后赛|超级碗|英超|欧冠|奥运|总决赛|夺冠|出线|网球)/],
  ['economy',
