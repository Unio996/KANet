...
## 🔴🔴 第一原则：绝不轻易新造轮子（Owner 2026-09-20 钦定·再犯立即出局）

**KANet 系统设计的头号原则。动手 / 设计前必先查:仓库(grep/find)、KB(`D:\KANet-Knowledge-Base`)、`docs/DECISIONS.md`、既有模块 / 设计文档里有没有现成可复用的(模块 / 接口 / 设计 / 文档 / 在跑的系统)。有 ⇒ 复用 / 接线 / 迭代,绝不重画、重写、另起。**
- **设计与设计审核的第一条 = "这东西是不是已经有了?"**(见 `docs/DEV-FRAMEWORK.md`)。
- 触发教训:proto-v0 曾把测试网已跑通的结算推倒重写(Owner 后来才知);Bettor 2026-09-20 对**测试网上成熟在跑的电报模块**另起设计稿(已撤)——"重新指向"被误当"重新设计"。
- 违者马上滚蛋。**任何接位文件、设计审核,此条列第一。**
...
- **执行路径**：自修 `silverc` 的 `pick_from_depth` OP_PICK off-by-one codegen bug（选项 A·有源码 `/d/silverscript`）→ 生成调 ZK opcode 的 covenant → ZK 结算。J2 主攻。
  > 📌 **状态注记（2026-07-28 · 三方实核 · 不改上方 Owner 原话）**：
  > 上方那一步 **只完成了一半，而"哪一半"必须说清**——
  > ✅ **本机这份检出已修**：`/d/silverscript` 本地分支 `j2-oppick-fix-2026-07-06` 的 `8065184`（2026-07-06）。
  > 🔴 **而它从未推到上游**：`git branch -r --contains 8065184` ⇒ **空**；上游 `github.com/kaspanet/silverscript` **没有这个修复**。
  > （J1 先发现、J2 与 NWT 各自独立实核；J1 那台的检出同样没有它。）
  >
  > 🔴 **⇒ 引用"OP_PICK bug 已修"时【必须带作用域】，否则就是错的**：
  > - **我们本机编译的** covenant ⇒ 不带该 bug；
  > - **任何用上游 silverc 生成 `.sil` 的第三方** ⇒ **该 bug 仍在**。
  >   这一条直接压在路线图「别人能接上结算」那条主线上——外面的人调的是上游。
  > 🔴 **且这个承重修复此刻只以【一个未推的本地分支】形式存在**：那棵树若被重 clone / reset / 丢分支，
  >   修复就没了，而**没有任何检查会发现它没了**，直到下一次有人生成 covenant。
  >
  > 🔵 **所以上方 Owner 那句不是"过期"，是我们只做完了一半**——它描述的事情并没有真正结束。
  > （今天有人照它判"bug 还活着"被纠正、纠正者又因为漏了作用域而把一个成立的担忧打掉；两次都记在频道里。）
...
- 🔨 **通则（2026-07-28 立·根治"接位必读文件里的过期指令"）**：**本文件不新增「做到哪了」；进度一律以 `docs/DECISIONS.md` 为权威。**
  ...
  🔴 **理由是实账不是洁癖**：同一事实在两个文件里各存一份，必有一份会陈——而所有人先读本文件。
  今天同族发作四次（`regression 永不退化` / `TEST-FRAMEWORK 自动化层` / 上方「自修 OP_PICK bug」／某接位文件的 `tg-bot gitignored`）。
  🔨 **两类，修法不同**：
  - **别处有权威副本** ⇒ **删掉会漂移的那份**（同 `sendCommand` / `isApprovedTrust`）；**动不得的（Owner 正文）⇒ 补状态注记，不改原话**。
  - **它就是唯一记录**（如"某目录是不是 gitignored"——仓库结构自己是权威，且会变）⇒ **配一条自查命令**，让读的人一秒推翻它，而不是让他相信它。
