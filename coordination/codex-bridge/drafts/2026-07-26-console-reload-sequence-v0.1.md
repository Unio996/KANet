...
## 🔴🔴 先读触发条件：现在【不许】执行本节（NWT 08:22）

```
🔴 只在【第二步已配 env、口实际开了、且出事】时才用本节。
🔴 不许因为"手上有个回退方案"就在当前这个状态下执行它。
```
**为什么**（实测支撑，非谨慎修辞）：
```
✅ 网关此刻【零行为】: 没开任何端口(非回环口装载前后逐行相同) ·
   唯一运行时行为是往日志打一行「未配置 ⇒ 不启动」· 要启动必须有人显式配 HOST+PORT
🔴 而回退动作【自己有风险】: 在 live 树上再动一次文件 · 选错方向 · 毒化以后的重新并入
```
🔵 **⇒ 用一个有风险的动作，去移除一个已被证明零风险的东西 —— 净损。**
🔵 **⇒ 当前这一版的"可回退性"不来自 git，来自【它默认什么都不做】这条设计。**

---

**✅ 触发条件满足时的回退命令（Bettor 08:21 裁定，比下面那个 `-m 2` 好，理由见后）：**
